...
void EUTelTrackAnalysis::plotResidualVsPosition(EUTelTrack track){
  streamlog_out(DEBUG2) << " EUTelTrackAnalysis::plotResidualVsPosition------------------------------BEGIN"<< std::endl;
	...
	for(int i=0; i<states.size();++i){
		...
		double statePosition[3];
		//TO DO:Fix master2Localtwo so that is is the only one. This is currently a hack
		const double referencePoint[]	= {state.getPosition()[0], state.getPosition()[1],state.getPosition()[2]};//Need this since geometry works with const doubles not floats 
		geo::gGeometry().master2Localtwo( state.getLocation(), referencePoint, statePosition );
		float residual[2];
