		...
		else 
		{
			...
			while( !millepede.eof() )
			{
				...
				if( goodLine )
				{
					...
					geo::gGeometry().setPlaneXPosition(sensorID, newOffset[0]+xOff);
					geo::gGeometry().setPlaneYPosition(sensorID, newOffset[1]+yOff);
					geo::gGeometry().setPlaneZPosition(sensorID, newOffset[2]+zOff);
