...
EUTelPreAlign::EUTelPreAlign () :Processor("EUTelPreAlign") {
  ...
  registerOptionalParameter("AddDut","Hack: Add 0 for dut!", thomashack, bool (false));

  registerOptionalParameter("ZeroPlanes","Hack: SensorID of plane(s) to be set to zero!", _zeroplane, std::vector< int > ());
...
//
  ...
  if(_fillHistos) {
    for(unsigned int i = 1; i < _sensorIDVecZOrder.size(); i++)
    {
      ...
      AIDA::IHistogram1D * histo1Da = AIDAProcessor::histogramFactory(this)->createHistogram1D( (basePath + tempHistoName).c_str(), 400 , -20., 20.);
      _hitXCorr.insert( make_pair( sensorID, histo1Da) );
 
      tempHistoName = "hitYCorr_0_to_" + to_string( sensorID) ;
      AIDA::IHistogram1D * histo1Db = AIDAProcessor::histogramFactory(this)->createHistogram1D( (basePath + tempHistoName).c_str(), 400 , -20., 20.) ;
...
int EUTelPreAlign::guessSensorID(const double * hit ) 
{
  ...
  if( _referenceHitVec == 0 || _useReferenceHitCollection == false )
  {
    ...
    if ( minDistance > 30  ) {
      ...
      streamlog_out( DEBUG3 ) << "A hit was found " << minDistance << " mm far from the nearest plane\n"
...
void EUTelPreAlign::end() {
  ...
  if (thomashack == true)
  {
      ...
      constant->setXOffset( 0.0 );
      constant->setYOffset( 0.0 );
      constant->setSensorID( 6 );
      constantsCollection->push_back( constant );
  streamlog_out (ERROR1) << "HACK: added plane 6!" << endl;
  }
  ...
  for(size_t ii = 0 ; ii < _preAligners.size(); ii++){
   ...
   if( abs( _preAligners.at(ii).getPeakY() ) <1000. && fabs(Bx) < 1e-6 && fabs(By) < 1e-6 && fabs(Bz) < 1e-6)
    ...
    for (size_t ij =0; ij < _zeroplane.size(); ij++)
    {
	if (sensorID == _zeroplane.at(ij))
	{
	    constant->setXOffset ( 0.0 );
	    constant->setYOffset ( 0.0 );
	}
