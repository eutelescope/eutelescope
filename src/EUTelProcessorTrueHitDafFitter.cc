// based on EUTelDafBase.cc and EUTelDafFitter.cc

// eutelescope includes ".h"
#include "EUTelProcessorTrueHitDafFitter.h"
...
EUTelProcessorTrueHitDafFitter::EUTelProcessorTrueHitDafFitter() : marlin::Processor(EUTelProcessorTrueHitDafFitter) {
  ...
  // input collection
	...
	//registerProcessorParameter ("mccollections", "List of hit collections. First one is INPUT collection, every subsequent corresponds to applying alignment collection",_mcCollectionStr, _mcCollectionExample );
  ...
  //Track quality parameters
	...
	//parameters from EUTelDafFitter.cc
...
bool EUTelProcessorTrueHitDafFitter::defineSystemFromData(){
	//Find three measurements per plane, use these three to define the plane as a point and a normal vector
	bool gotIt = true;
	for(size_t plane = 0; plane < _trueHitSystem.planes.size(); plane++) {

		daffitter::FitPlane<float>& pl = _trueHitSystem.planes.at(plane);
		bool gotPlane = false;

		//If plane is not DUT and not Tel, it does not have hits, and can not be initialized
		if( find(_telPlanes.begin(), _telPlanes.end(), pl.getSensorID()) == _telPlanes.end() and find(_dutPlanes.begin(), _dutPlanes.end(), pl.getSensorID()) == _dutPlanes.end()) continue;
	...
	for(size_t plane = 0; plane < _reconstructedHitSystem.planes.size(); plane++) {

		daffitter::FitPlane<float>& pl = _reconstructedHitSystem.planes.at(plane);
		bool gotPlane = false;
...
void EUTelProcessorTrueHitDafFitter::gearRotate(size_t gearIndex, daffitter::FitPlane<float>& pl){
...
/*Eigen::Vector3f EUTelProcessorTrueHitDafBase::applyAlignment(EUTelAlignmentConstant* alignment, Eigen::Vector3f point){
...
/*void EUTelProcessorTrueHitDafBase::alignRotate(std::string collectionName, LCEvent* event) {
...
void EUTelProcessorTrueHitDafFitter::getPlaneNorm(daffitter::FitPlane<float>& pl){
...
void EUTelProcessorTrueHitDafFitter::init() {
  printParameters ();
  ...
  for( ; zit != _zSort.end(); index++, zit++){
    ...
    _trueHitSystem.addPlane(sensorID, zPos , errX, errY, scatter, excluded);
    gearRotate((*zit).second, _trueHitSystem.planes.at(index));
    _reconstructedHitSystem.addPlane(sensorID, zPos , errX, errY, scatter, excluded);
    gearRotate((*zit).second, _reconstructedHitSystem.planes.at(index));
  }
 
  //Prepare track finder
  switch( _trackFinderType ) {
	case combinatorialKF: _trueHitSystem.setCKFChi2Cut(_normalizedRadius*_normalizedRadius); _reconstructedHitSystem.setCKFChi2Cut(_normalizedRadius*_normalizedRadius); break;
	case simpleCluster: _trueHitSystem.setClusterRadius(_normalizedRadius); _reconstructedHitSystem.setClusterRadius(_normalizedRadius); break;
  }
  _trueHitSystem.setNominalXdz(_nXdz); //What is the tangent angle of the beam? (Probably zero)
  _trueHitSystem.setNominalYdz(_nYdz);
  _trueHitSystem.setXdzMaxDeviance(_nXdzMaxDeviance); //How far og the nominal angle can the first two measurements be?
  _trueHitSystem.setYdzMaxDeviance(_nYdzMaxDeviance);

  //Prepare and preallocate memory for track fitter
  _reconstructedHitSystem.setChi2OverNdofCut(_maxChi2);
  _reconstructedHitSystem.setDAFChi2Cut(_chi2cutoff);
  _reconstructedHitSystem.init();

  _reconstructedHitSystem.setNominalXdz(_nXdz); //What is the tangent angle of the beam? (Probably zero)
  _reconstructedHitSystem.setNominalYdz(_nYdz);
  _reconstructedHitSystem.setXdzMaxDeviance(_nXdzMaxDeviance); //How far og the nominal angle can the first two measurements be?
  _reconstructedHitSystem.setYdzMaxDeviance(_nYdzMaxDeviance);
  ...
  if( _ndofMin < 0.5) {
	...
	if(_fitDuts){
		...
		for (size_t i = 0; i < _trueHitSystem.planes.size(); i++){

			if( find(_dutPlanes.begin(), _dutPlanes.end(), _trueHitSystem.planes.at(i).getSensorID()) != _dutPlanes.end()){ 

			_trueHitSystem.planes.at(i).include(); 
			}
		}
...
void EUTelProcessorTrueHitDafFitter::processRunHeader (LCRunHeader * rdr) {
...
float EUTelProcessorTrueHitDafFitter::getScatterThetaVar( float radLength ){
  //From pdg live
  float scatterTheta = 0.0136f/_eBeam * sqrt( radLength ) *  (1.0f + 0.038f * std::log(radLength) );
  return(scatterTheta * scatterTheta);
}

size_t EUTelProcessorTrueHitDafFitter::getPlaneIndex(float zPos) {
...
void EUTelProcessorTrueHitDafFitter::readHitCollection(LCEvent* event){
	...
	for (int i = 0; i < _trueHitCollectionVec->getNumberOfElements(); i++) {
		...
		if(planeIndex >=0 ){
			...
			_trueHitSystem.addMeasurement( planeIndex, static_cast<float>(pos[0])*1000.0f, static_cast<float>(pos[1])*1000.0f, static_cast<float>(pos[2])*1000.0f, region, i);
		}
    }
    
	for ( int i = 0; i < _hitCollection->getNumberOfElements(); i++ ) {

		TrackerHitImpl* reconstructedHit = static_cast<TrackerHitImpl*>(_reconstructedHitCollectionVec->getElementAt(i));
	...
	streamlog_out ( DEBUG5 ) << " SIM: simhit="<< ( simhit != 0 ) <<" add point [" << planeIndex << "] "<< 
	  static_cast< float >(pos[0]) * 1000.0f << " " << static_cast< float >(pos[1]) * 1000.0f << " " <<  static_cast< float >(pos[2]) * 1000.0f << endl;
		...
		if (reconstructedHit != 0 ){

			const double * hitpos = reconstructedHit->getPosition();
			pos[0] = hitpos[0];
			pos[1] = hitpos[1];
			pos[2] = hitpos[2];
			//UTIL::CellIDDecoder<TrackerHitImpl> hitDecoder ( EUTELESCOPE::HITENCODING );
			int planeID  = static_cast<int>(reconstructedHitDecoder(reconstructedHit)["sensorID"]);
			planeIndex = _indexIDMap[planeID];
			streamlog_out ( DEBUG5 ) << " REAL: add point [" << planeIndex << "] "<< static_cast<float>(pos[0])*1000.0f << " " << static_cast<float>(pos[1])*1000.0f << " " <<  static_cast<float>(pos[2])*1000.0f << endl;
    	  }
		...
		if(planeIndex >=0 ){
			...
			streamlog_out ( DEBUG5 ) << " add point [" << planeIndex << "] "<< static_cast<float>(pos[0])*1000.0f << " " << static_cast<float>(pos[1])*1000.0f << " " <<  static_cast<float>(pos[2])*1000.0f << endl;
			_reconstructedHitSystem.addMeasurement( planeIndex, static_cast<float>(pos[0])*1000.0f, static_cast<float>(pos[1])*1000.0f, static_cast<float>(pos[2])*1000.0f, region, i);
		}
    }
}

int EUTelProcessorTrueHitDafFitter::checkInTime(daffitter::TrackCandidate<float, 4>& track, daffitter::TrackerSystem<float, 4>& const system){
  size_t nMatches(0);
  for( size_t ii = 0; ii < system.planes.size() ; ii++){
    daffitter::FitPlane<float>& plane = system.planes.at(ii);
...
void EUTelDafFitter::addToLCIO(daffitter::TrackCandidate<float, 4>& track, dafffitter::TrackerSystem<float, 4>& const system, LCCollectionVec* fitPointVec, LCCollectionVec* trackVec) {
	...
	CellIDEncoder<TrackerHitImpl> idHitEncoder(EUTELESCOPE::HITENCODING, fitPointVec);

	float refpoint[3];
  
	for(size_t plane = 0; plane < _system.planes.size(); plane++){

		daffitter::FitPlane<float>& pl = system.planes.at(plane);
		...
		int sensorID =  system.planes.at(plane).getSensorID();
...
void EUTelProcessorTrueHitDafFitter::processEvent(LCEvent * event){
  ...
  if( isFirstEvent() ){
    ...
    if(nResolutions > _trueHitSystem.planes.size()){
      streamlog_out ( MESSAGE ) << "More resolutions than planes, check config." << endl;
      nResolutions = _trueHitSystem.planes.size();
    }
    if(nResolutions > _reconstructedHitSystem.planes.size()){
      streamlog_out ( MESSAGE ) << "More resolutions than planes, check config." << endl;
      nResolutions = _reconstructedHitSystem.planes.size();
    }
    for(size_t ii = 0; ii < nResolutions; ii++){
      _trueHitSystem.planes.at(ii).setSigmas( _sigmaX.at(ii), _sigmaY.at(ii));
      _reconstructedHitSystem.planes.at(ii).setSigmas( _sigmaX.at(ii), _sigmaY.at(ii));
    }
  }
  ...
  //Prepare tracker system for new data, new tracks
	_trueHitSystem.clear();
	_reconstructedHitSystem.clear();
  ...
  if( not _initializedSystem ){
    ...
    for(size_t i = 0; i < _reconstructedHitSystem.planes.size(); i++){
      _reconstructedHitSystem.planes.at(i).print();
    }
  }
  
  //Run track finder
  switch( _trackFinderType ) {
	case combinatorialKF: _trueHitSystem.combinatorialKF(); _reconstructedHitSystem.combinatorialKF(); break;
	case simpleCluster: _trueHitSystem.clusterTracker(); _reconstructedHitSystem.clusterTracker(); break;
  }
 
	//Child specific actions: dafEvent from EUTelDafFitter.cc
	//Prepare track collection
	// Define output track and hit collections
	_trueTrackVec = new LCCollectionVec(LCIO::TRACK);
	_trueFitPointVec = new LCCollectionVec(LCIO::TRACKERHIT);
	_reconstructedTrackVec = new LCCollectionVec(LCIO::TRACK);
	_reconstructedFitPointVec = new LCCollectionVec(LCIO::TRACKERHIT);
	// Set flag for storing track hits in track collection
	LCFlagImpl trueTrackFlag(_trueTrackVec->getFlag());
	trueTrackFlag.setBit( LCIO::TRBIT_HITS );
	_trueTrackVec->setFlag(trueTrackFlag.getFlag());
	LCFlagImpl reconstructedTrackFlag(_reconstructedTrackVec->getFlag());
	trueTrackFlag.setBit( LCIO::TRBIT_HITS );
	_reconstructedTrackVec->setFlag(reconstructedTrackFlag.getFlag());
  
	//Check found tracks
	for(size_t i = 0; i < _trueHitSystem.getNtracks(); i++ ){
		//run track fitter
		_nTrueCandidates++;
		//Prepare track for DAF fit
		_trueHitSystem.fitPlanesInfoDaf(_trueHitSystem.tracks.at(i));
		//Check resids, intime, angles
		if(not checkTrack( _trueHitSystem.tracks.at(i))) continue;
		int inTimeHits = checkInTime(_trueHitSystem.tracks.at(i), _trueHitSystem);
		if( inTimeHits < _nDutHits) continue;

		//Dump to LCIO
		addTrueTrackToLCIO(_trueHitSystem.tracks.at(i), _trueHitSystem, _trueFitPointVec, _trueTrackVec);
		_nTrueTracks++;
 
		//Fill plots
		//fillTrueHitPlots(_trueHitSystem.tracks.at(i)); 
		//fillDetailTrueHitPlots(_trueHitSystem.tracks.at(i));
		//fillComparaisonPlots(_reconstructedHitSystem.tracks.at(i));
	}

	for(size_t i = 0; i < _reconstructedHitSystem.getNtracks(); i++ ){
		//run track fitter
		_nReconstructedCandidates++;
		//Prepare track for DAF fit
		_reconstructedHitSystem.fitPlanesInfoDaf(_reconstructedHitSystem.tracks.at(i));
		//Check resids, intime, angles
		if(not checkTrack( _reconstructedHitSystem.tracks.at(i))) continue;
		int inTimeHits = checkInTime(_reconstructedHitSystem.tracks.at(i), _reconstructedHitSystem);
		if( inTimeHits < _nDutHits) continue;

		//Dump to LCIO
		addReconstructedTrackToLCIO(_reconstructedHitSystem.tracks.at(i), _reconstructedHitSystem, _reconstructedFitPointVec, _reconstructedTrackVec);
		_nReconstructedTracks++;
	}
	...
	event->addCollection(_trueTrackVec,_trueTrackCollectionName);
	event->addCollection(_reconstructedTrackVec,_reconstructedTrackCollectionName); 
	...
	event->addCollection(_trueFitPointVec, sfitpoints);
	event->addCollection(_reconstructedFitPointVec, sfitpoints);
...
bool EUTelProcessorTrueHitDafFitter::checkTrack(daffitter::TrackCandidate<float,4>& track){
...
void EUTelProcessorTrueHitDafFitter::fillHistos() {
}

void EUTelProcessorTrueHitDafFitter::fillPlots(daffitter::TrackCandidate<float,4>& track){
...
void EUTelProcessorTrueHitDafFitter::fillDetailPlots(daffitter::TrackCandidate<float,4>& track){
  for( size_t ii = 0; ii < _system.planes.size() ; ii++){
    daffitter::FitPlane<float>& plane = _system.planes.at(ii);
...
void EUTelProcessorTrueHitDafFitter::bookHistos(){
...
void EUTelProcessorTrueHitDafFitter::bookDetailedHistos(){
  for( size_t ii = 0; ii < _system.planes.size() ; ii++) {
    daffitter::FitPlane<float>& plane = _system.planes.at(ii);
    char iden[4];
    sprintf(iden, "%d", plane.getSensorID());
...
void EUTelProcessorTrueHitDafFitter::end() {
  dafEnd();
