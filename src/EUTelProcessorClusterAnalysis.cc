...
if (_clusterAvailable)
			...
			//the next cluster will be the next thing written to the file, so include the cluster seperator			
			...
			float clusterCenterOfGravityX;
			float clusterCenterOfGravityY;
			cluster.getCenterOfGravity(clusterCenterOfGravityX,clusterCenterOfGravityY);
			...
			if (_hotpixelAvailable)
			{
			  ...
			  for ( unsigned int iPixel = 0; iPixel < sparseData->size(); iPixel++ )
			  {
			    auto sparsePixel = std::make_unique<EUTelGenericSparsePixel>();
			    sparseData->getSparsePixelAt( iPixel, sparsePixel.get() );
			    if (abs(clusterCenterOfGravityX-(sparsePixel->getXCoord())) < _limit && abs(clusterCenterOfGravityY-(sparsePixel->getYCoord())) < _limit)
			    {
			      _nHotpixelClusters++;
			      goto nextCluster; 
			      //hotpixel = true; 
			...
			if (_deadColumnAvailable)
			{
			  ...
			  for ( unsigned int iPixel = 0; iPixel < sparseData->size(); iPixel++ )
			  {
			    auto sparsePixel = std::make_unique<EUTelGenericSparsePixel>();
			    sparseData->getSparsePixelAt( iPixel, sparsePixel.get() );
			    if (abs(clusterCenterOfGravityX-(sparsePixel->getXCoord())) < _limit)
			    {
...
{
  streamlog_out ( MESSAGE4 ) << "The amount of processed events was " << _nEvent << endl;
