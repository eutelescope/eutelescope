...
EUTelMille::EUTelMille() : Processor("EUTelMille") {
    ...
    registerOptionalParameter("DistanceMaxVec", "Maximal allowed distance between hits entering the fit per 10 cm space between the planes. "
            "One value for each neighbor planes. DistanceMax will be used for each pair if this vector is empty.",
    ...
    registerOptionalParameter("ResidualsXMin", "Minimal values of the hit residuals in the X direction for a track. "
            "Note: these numbers are ordered according to the z position of the sensors and NOT according to the sensor id.", _residualsXMin, MinimalResidualsX);

    registerOptionalParameter("ResidualsYMin", "Minimal values of the hit residuals in the Y direction for a track. "
            "Note: these numbers are ordered according to the z position of the sensors and NOT according to the sensor id.", _residualsYMin, MinimalResidualsY);



    registerOptionalParameter("ResidualsXMax", "Maximal values of the hit residuals in the X direction for a track. "
            "Note: these numbers are ordered according to the z position of the sensors and NOT according to the sensor id.", _residualsXMax, MaximalResidualsX);

    registerOptionalParameter("ResidualsYMax", "Maximal values of the hit residuals in the Y direction for a track. "
            "Note: these numbers are ordered according to the z position of the sensors and NOT according to the sensor id.", _residualsYMax, MaximalResidualsY);



    registerOptionalParameter("ResolutionX", "X resolution parameter for each plane. Note: these numbers are ordered according to the z position of the sensors and NOT according to the sensor id.",
            _resolutionX, std::vector<float> (static_cast<int> (6), 10.));

    registerOptionalParameter("ResolutionY", "Y resolution parameter for each plane. Note: these numbers are ordered according to the z position of the sensors and NOT according to the sensor id.",
            _resolutionY, std::vector<float> (static_cast<int> (6), 10.));

    registerOptionalParameter("ResolutionZ", "Z resolution parameter for each plane. Note: these numbers are ordered according to the z position of the sensors and NOT according to the sensor id.",
    ...
    registerOptionalParameter("ApplyToReferenceCollection", "Do you want the reference hit collection to be corrected by the shifts and tilts from the alignment collection?",
            _applyToReferenceHitCollection, static_cast<bool> (false));

    registerOptionalParameter("FixParameter", "Fixes the given alignment parameters in the fit if alignMode==3 is used. "
            "For each sensor an integer must be specified (If no value is given, then all parameters will be free). "
            "bit 0 = x shift, bit 1 = y shift, bit 2 = z shift, bit 3 = alpha, bit 4 = beta, bit 5 = gamma. "
            "Note: these numbers are ordered according to the z position of the sensors and NOT according to the sensor id.",
            _FixParameter, std::vector<int> (static_cast<int> (6), 24));
    ...
    registerOptionalParameter("UseSensorRectangular", "Do not use all pixels for alignment, only these in the rectangular (A|B) e.g. (0,0) and (C|D) e.g. (100|100) of sensor S. "
            "Type in the way S1 A1 B1 C1 D1 S2 A2 B2 C2 D2 ...",
...
void EUTelMille::init() {
    ...
    for (int iPlane = 0; iPlane < _siPlanesLayerLayout->getNLayers(); iPlane++) {
        ...
        streamlog_out(MESSAGE4) << "iPlane " << iPlane << " sensor_#_along_Z_axis " << _sensors_to_the_left << " [z=  " << fixed << setprecision(3) << keepZPosition[ iPlane ] << " ] [sensorID " << sensorID << " ] " << endl;
    ...
    for (size_t i = 0; i < _excludePlanes_sensorIDs.size(); i++) {
        ...
        while (iter != sensorIDMap.end()) {
            if (iter->second == _excludePlanes_sensorIDs[i]) {
                streamlog_out(MESSAGE4) << "excludePlanes_sensorID " << setw(2) << int(i) << " of " << setw(2)
                        << static_cast<int> (_excludePlanes_sensorIDs.size()) << " (" << setw(2) << counter << ")" << endl;
    ...
    while (iter != sensorIDMap.end()) {
        ...
        for (size_t i = 0; i < _excludePlanes.size(); i++) {
            if (_excludePlanes[i] == counter) {
                streamlog_out(MESSAGE4) << "excludePlanes " << setw(2) << int(i) << " of " << static_cast<int> (_excludePlanes_sensorIDs.size()) << " (" << setw(2) << counter << ")" << endl;
                excluded = true;
                break;
            }
        }
 ...
 * 
 * Track search is done using an algorithm of ...
 * 
 * @param missinghits
 * @param indexarray
 * @param vec
...
void EUTelMille::findtracks2(
    ...
    for (size_t j = 0; j < _allHitsArray[i].size(); j++) {
        ...
        int ihit = (int) j;
        streamlog_out(DEBUG) <<  "plane " << setw(2) << i << " of " << setw(2) << (int)(_allHitsArray.size())-1 << " ; hit " << setw(2) << ihit << endl;
...
void EUTelMille::FitTrack(unsigned int nPlanesFitter, double xPosFitter[], double yPosFitter[], double zPosFitter[], double xResFitter[], double yResFitter[], double chi2Fit[2], double residXFit[], double residYFit[], double angleFit[2]) 
{
...
void EUTelMille::end() {
    ...
    CDashMeasurement meas_ntracks("ntracks", _nMilleTracks);
    cout << meas_ntracks;
