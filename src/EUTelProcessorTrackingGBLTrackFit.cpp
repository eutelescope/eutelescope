...
void EUTelProcessorTrackingGBLTrackFit::processEvent(LCEvent * evt) {
    ...
    if (col != NULL) {
        if (_nProcessedEvents % 1000 == 1) streamlog_out(MESSAGE5) << "EUTelProcessorTrackingGBLTrackFit" << endl;

        for ( int iCol = 0; iCol < col->getNumberOfElements() ; iCol++ ) {
            EUTelLCObjectTrackCandidate* lcTrackCandidates = static_cast<EUTelLCObjectTrackCandidate*> ( col->getElementAt(iCol) );
            ...
            vector < EVENT::TrackerHitVec > trackCandidates = lcTrackCandidates->getTrackCandates();
