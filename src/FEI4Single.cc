													...
													93.660734 )				//rad length					
{
	...
	//And place them to make a singlechip
        float xoffset = 0.125;
        float yoffset = 0.025;
	plane->AddNode(centreregion, 1, new TGeoTranslation( 0.00 + xoffset, 0 + yoffset, 0) );
	plane->AddNode(edgeregion,   2, new TGeoTranslation(-9.95 + xoffset, 0 + yoffset, 0) );
	plane->AddNode(edgeregion,   3, new TGeoTranslation( 9.95 + xoffset, 0 + yoffset, 0) );
}
