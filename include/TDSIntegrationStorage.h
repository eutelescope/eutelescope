...
class TDSIntegrationStorage {
 ...
 public:
  ...
  { 
    ...
    if (integPixelSegmentsAlongL >= 2000)
      {
	cout << "Too many pixel segments along length (for integration storage)!" << endl;
	exit(1);
      }
    integPixelSegmentsAlongW = (val_integPixelSegmentsAlongW/2)*2;
    if (integPixelSegmentsAlongW >= 2000)
      {
	cout << "Too many pixel segments along width (for integration storage)!" << endl;
	exit(1);
      }
    integPixelSegmentsAlongH = val_integPixelSegmentsAlongH;
    if (integPixelSegmentsAlongH >= 1000)
      {
	cout << "Too many pixel segments along height (for integration storage)!" << endl;
	exit(1);
      }
 ...
 private:
  ...
  map<unsigned long long int, double> integResultsForSegments;
