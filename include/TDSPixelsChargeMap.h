...
typedef map<unsigned long long int, double> type_PixelsChargeMap;
 ...
 *  @version $Id: TDSPixelsChargeMap.h,v 1.1 2008-11-11 19:10:15 zarnecki Exp $
 *
 */
...
class TDSPixelsChargeMap {
 ...
 public:
  ...
  void print(string filename = "pixelsChargeMap.out");
   ...
   */
  vector<TDSPixel> getVectorOfPixels();
 ...
 private:
  ...
  static double funChargeDistribution (double *k, size_t dim, void * p)
  {
    ...
    double A  = exp( - r / gp->lambda ) * abs(gp->H) / (r2*r) / (4*M_PI);
	...
	A += gp->reflectedContribution * exp( - rimage / gp->lambda ) * abs(2*gp->height - gp->H) / (rimage2*rimage) / (4*M_PI);
      };
