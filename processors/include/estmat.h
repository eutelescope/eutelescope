...
private:
  ...
  void vectToEst(gsl_vector *v1, std::vector<int> &indexVector,
                 vector<FITTERTYPE> &dataVector, size_t &param);
  void multiVectToEst(gsl_vector *v1, std::vector<int> &indexVector,
                      vector<FITTERTYPE> &dataVector, size_t &param);
  // newtons method
  FITTERTYPE stepVector(gsl_vector *vc, size_t index, FITTERTYPE value,
...
public:
  int fitCount;
  // parameters
  std::vector<FITTERTYPE> resX, resY, radLengths, zPos, xShift, yShift, xScale,
      yScale, zRot;
  // single parameter iteration indexes
  std::vector<int> resXIndex, resYIndex, resXYIndex, radLengthsIndex, zPosIndex,
      xShiftIndex, yShiftIndex, xScaleIndex, yScaleIndex, zRotIndex;
  ...
  void printParams(std::string name, std::vector<FITTERTYPE> &params, bool plot,
...
public:
  ...
  SDR(bool SDR1, bool SDR2, bool cholDec, EstMat &mat)
      : Minimizer(mat), SDR1(SDR1), SDR2(SDR2), cholDec(cholDec) {
    ;
  }
  virtual void operator()(size_t offset, size_t stride);
};
