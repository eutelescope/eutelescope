...
namespace eutelescope {
  ...
  public:
    ...
    void print() {
      streamlog_out(MESSAGE4) << "Sensor: " << sensor << ": (" << A << "|" << B
                              << ") to (" << C << "|" << D << ")" << std::endl;
    }
  };
  ...
  public:
    ...
    public:
      hit() {}
      hit(double tx, double ty, double tz, double rx, double ry, double rz,
          int i) {
        x = tx;
        y = ty;
        z = tz;
    ...
    public:
      ...
      double fit(double *x) {
        ...
        for (size_t i = 0; i < n; i++) {
          ...
          const double t[n] = {b0 + c0 * coeff - p0, b1 + c1 * coeff - p1,
                               b2 + c2 * coeff - p2};

          // sum of distances divided by resolution^2
          chi2 += t[0] * t[0] / pow(resol_x, 2) +
                  t[1] * t[1] / pow(resol_y, 2) + t[2] * t[2] / pow(resol_z, 2);
        }
    ...
    virtual void FitTrack(unsigned int nPlanesFitter, double xPosFitter[],
                          double yPosFitter[], double zPosFitter[],
                          double xResFit[], double yResFit[], double chi2Fit[2],
                          double residXFit[], double residYFit[],
                          double angleFit[2]);
    ...
    virtual void findtracks2(
        ...
        std::vector<std::vector<EUTelMille::HitsInPlane>>
            &_hitsArray, // contains all hits for each plane
     ...
     */
    virtual Processor *newProcessor() { return new EUTelMille; }
    ...
    //! Track search method, fills class members _xPos and _trackResidX (Y,Z as
    //! well)
    /** */
    ...
    TVector3 Line2Plane(int iplane, const TVector3 &lpoint,
  ...
  protected:
    ...
    StringVec _pedeSteerAddCmds; // allows user-added commands in the pede
                                 // steering file
