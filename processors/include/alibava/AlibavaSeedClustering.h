...
namespace alibava {
  ...
  public:
     ...
     */
    virtual Processor *newProcessor() { return new AlibavaSeedClustering; }
    ...
    // SNR Cuts
    // The signal/noise ratio that channels have to pass to be considered as
    // seed channel
    float _seedCut;

    // The signal/noise ratio that neigbour channels have to pass to be added to
    // the cluster
    float _neighCut;

    // Sensitive Axis
    // The sensitive axis of the strip sensor(s) according to telescope. It has
    // to set to either "X" ot "Y" any other value will be disregarded and
    // sensitive axis will assumed to be "X"
    int _sensitiveAxisX;

    // Signal Polarity
    // Polarity of the signal. Set this parameter to -1 for negative signals,
    // any other value will be disregarded and the signal will be assumed to be
    // positive
    int _signalPolarity;
  ...
  protected:
    ...
    //	void convertAlibavaCluster(AlibavaCluster alibavaCluster,
    //LCCollectionVec * clusterColVec, LCCollectionVec * sparseClusterColVec);
