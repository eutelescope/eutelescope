...
namespace alibava {
  ...
  public:
    ...
    std::string _variableName; // Hasto be one of these: xBin, xMin, xMax, yBin,
                               // yMin, yMax
  ...
  public:
    ...
    std::string _topTag;
    // The tag (inside the top tag) that should be processed by this histogram
    // maker
    ...
    // yMax
    // so variableName has to be one of these: xBin, xMin, xMax, yBin, yMin,
    // yMax (case sensitive)
    // for xBin and yBin newValue will changed to integer
    void changeXMLVariable(std::string histoName, std::string variableName,
                           float newValue);
    ...
    // whichSide defines which side of the title you want to add the
    // "stringToAdd",
    ...
    void addToXMLTitle(std::string histoName, std::string titleName,
                       std::string whichSide, std::string stringToAdd);
  ...
  protected:
    ...
    void updateXMLVariables(std::string histoName, int *xBin, float *xMin,
                            float *xMax, int *yBin, float *yMin, float *yMax,
                            std::string *title, std::string *labelX,
                            std::string *labelY);
  };
}
