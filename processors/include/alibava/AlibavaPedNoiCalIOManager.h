...
namespace alibava {
  ...
  public:
    ...
    void addToFile(std::string filename, std::string collectionName,
                   int chipnum, lcio::FloatVec datavec);

    lcio::FloatVec getPedNoiCalForChip(std::string filename,
  ...
  private:
    ...
    // returns the element number of the TrackerData with the chosen chipnum in
    // collection col.
