 ...
 *  based on EUTELESCOPE.h written by Antonio Bulgheroni, INFN
...
namespace alibava {
  ...
  public:
    ...
    // System Parameters //
    ///////////////////////
    // If you do change these, you might need to change things in
    // AlibavaConverter processor
    //! Parameter to store/recall number of chips in Alibava readout system
    static const int NOOFCHIPS = 2;
    //! Parameter to store/recall number of channels in a chip in Alibava
    //! readout system
    static const int NOOFCHANNELS = 128;
    //! Parameter to store/recall length of header in a chip in Alibava readout
    //! system
    ...
    //! Parameter key to store/recall injected charge value (e) in Charge
    //! calibration run
    ...
    ////////////////////////////////
    //! Parameter key to store/recall the CellIDEncoding string for all
    //! TrackerData before clustering
    static const char *ALIBAVADATA_ENCODE;
    //! Parameter key to store/recall the part of CellIDEncode that returns chip
    //! number
    ...
    //! Parameter key to store/recall the CellIDEncoding string for
    //! AlibavaCluster
    static const char *ALIBAVACLUSTER_ENCODE;
    //! Parameter key to store/recall the part of CellIDEncode that returns
    //! cluster ID
    static const char *ALIBAVACLUSTER_ENCODE_CLUSTERID;
    //! Parameter key to store/recall the part of CellIDEncode that returns if X
    //! axis is sensitive
    static const char *ALIBAVACLUSTER_ENCODE_ISSENSITIVEAXISX;
    //! Parameter key to store/recall the part of CellIDEncode that returns if
    //! signal is negative
    static const char *ALIBAVACLUSTER_ENCODE_ISSIGNALNEGATIVE;
    //! Parameter key to store/recall the part of CellIDEncode that returns chip
    //! number
    static const char *ALIBAVACLUSTER_ENCODE_CHIPNUM;
    //! Parameter key to store/recall the part of CellIDEncode that returns seed
    //! channel number
...
std::string getSubStringUpToChar(std::string search_string,
