...
namespace alibava {
  ...
  public:
    ...
    // Note that chip selection, pedestalfile name and pedestal and noise
    // collection names have to be set before calling this method.
    void setPedestals();

    // Checks the size and existance of the pedestal and noise values of each
    // selected chips
    void checkPedestals();
    ...
    // Note that chip selection, calibrationFile name and charge calibration
    // collection names have to be set before calling this method.
    void setCalibration();

    // Checks the size and existance of the charge calibration values of each
    // selected chips
    ...
    void decodeMaskingString(std::string istring, int *onchip, int *fromchannel,
                             int *tochannel);
    ...
    // returns chip number of the TrackerDataImpl using
    // ALIBAVA::ALIBAVADATA_ENCODE and ALIBAVA::ALIBAVADATA_ENCODE_CHIPNUM
    ...
    /*! The format of _channelsToBeUsed std::string parameter shoul be like
     * $ChipNumber:StartChannel-EndChannel$
     *   ex. $0:5-20$ $0:30-100$ $1:50-70$ means from chip 0 channels between
     * 5-20 and 30-100, from chip 1 channels between 50-70
