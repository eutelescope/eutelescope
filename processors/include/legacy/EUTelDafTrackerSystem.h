...
namespace daffitter {
  template <typename T, size_t N> class TrackEstimate {
  public:
  ...
  template <typename T, size_t N> class TrackCandidate {
  public:
  ...
  template <typename T> class FitPlane {
  ...
  public:
    ...
    FitPlane(int sensorID, T zPos, T sigmaX, T sigmaY, T scatterVariance,
             bool excluded);
    ...
    void addMeasurement(T x, T y, T z, bool goodRegion, size_t measIden) {
      Measurement<T> a(x, y, z, goodRegion, measIden);
      meas.push_back(a);
    }
  ...
  template <typename T> class PlaneHit {
  private:
    Eigen::Matrix<T, 2, 1> xy;
    int plane, index;

  public:
    PlaneHit(T x, T y, int plane, int index) : plane(plane), index(index) {
      xy(0) = x;
      xy(1) = y;
    }
    PlaneHit(Eigen::Matrix<T, 2, 1> xy, int plane, int index)
        : xy(xy), plane(plane), index(index) {}
  ...
  template <typename T, size_t N> class EigenFitter {
  ...
  public:
    ...
    void
    calculateWeights(std::vector<FitPlane<T>> &pl, T chi2cut,
                     std::vector<Eigen::Matrix<T, Eigen::Dynamic, 1>> &weights);
    void calculatePlaneWeight(FitPlane<T> &pl, TrackEstimate<T, N> &e,
                              T chi2cutoff,
                              Eigen::Matrix<T, Eigen::Dynamic, 1> &weights);

    // Information filter
    void predictInfo(const FitPlane<T> &prev, const FitPlane<T> &cur,
                     TrackEstimate<T, N> &e);
    void addScatteringInfo(const FitPlane<T> &pl, TrackEstimate<T, N> &e);
    void updateInfo(const FitPlane<T> &pl, const int index,
                    TrackEstimate<T, N> &e);
    void updateInfoDaf(const FitPlane<T> &pl, TrackEstimate<T, N> &e,
                       Eigen::Matrix<T, Eigen::Dynamic, 1> &weights);
    void getAvgInfo(TrackEstimate<T, N> &e1, TrackEstimate<T, N> &e2,
                    TrackEstimate<T, N> &result);
    void smoothInfo();
    // Standard formulation
    void predict(const FitPlane<T> &prev, const FitPlane<T> &cur,
                 TrackEstimate<T, N> &e);
    // smoother
    void smooth();
    void getAvg(TrackEstimate<T, N> &f, TrackEstimate<T, N> &b,
                TrackEstimate<T, N> &result);
    // kf
    void kfUpdate(const FitPlane<T> &cur, int index, TrackEstimate<T, N> &e);
    void getKFGain(const FitPlane<T> &p1, TrackEstimate<T, N> &e);
    void predictB(const FitPlane<T> &prev, const FitPlane<T> &cur,
                  TrackEstimate<T, N> &e);
  };

  template <typename T, size_t N> class TrackerSystem {
    bool m_inited;
    ...
    int addNeighbors(std::vector<PlaneHit<T>> &candidate,
                     std::list<PlaneHit<T>> &hits);
  ...
  public:
    ...
    void addPlane(int sensorID, T zPos, T sigmaX, T sigmaY, T scatterVariance,
                  bool excluded);
    void addMeasurement(size_t planeIndex, T x, T y, T z, bool goodRegion,
                        size_t iden);
    ...
    Eigen::Matrix<T, 2, 1> getBiasedResidualErrors(FitPlane<T> &pl,
                                                   TrackEstimate<T, N> &estim);
    Eigen::Matrix<T, 2, 1>
    getUnBiasedResidualErrors(FitPlane<T> &pl, TrackEstimate<T, N> &estim);
    Eigen::Matrix<T, 2, 1> getResiduals(Measurement<T> &meas,
                                        TrackEstimate<T, N> &estim);
  ...
  template <typename T>
  inline void partialFastInvert(Eigen::Matrix<T, 4, 4> &cov, size_t p1,
                                size_t p2) {
    // Matrix is sparce, if state vector was [x,dx/dz, y,dy/dz] it would be
    // block diagonal
    // Invert as if it was
  ...
  template <typename T> inline void fastInvert(Eigen::Matrix<T, 4, 4> &cov) {
    // Matrix is sparce, if state vector was [x,dx/dz, y,dy/dz] it would be
    // block diagonal
    // Invert as if it was
    // "Block" [x, dx]
