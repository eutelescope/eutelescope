...
void EUTelDafMaterial::dafParams() {
  _description = "This processor preforms track reconstruction. The tracks are "
                 "used to estimate material thicknesses and measurement "
                 "resolutions.";
  ...
  registerOptionalParameter("RadLengthIndex",
...
int EUTelDafMaterial::checkDutResids(
    daffitter::TrackCandidate<float, 4> &track) {
  int nHits(0);
  for (size_t ii = 0; ii < _system.planes.size(); ii++) {
...
void EUTelDafMaterial::dafEvent(LCEvent * /*event*/) {
  ...
  for (size_t pl = 0; pl < _system.planes.size(); pl++) {
    if (find(_dutPlanes.begin(), _dutPlanes.end(),
             _system.planes.at(pl).getSensorID()) == _dutPlanes.end()) {
      continue;
    }
    if (_system.planes.at(pl).getSensorID() == 12) {
      continue;
    }
    if (_system.planes.at(pl).meas.size() > 1) {
      shower = true;
    }
  }
  if (shower) {
  ...
  for (size_t ii = 0; ii < _system.getNtracks(); ii++) {
    ...
    if (inTimeHits >= _nDutHits) {
      ...
      for (size_t plane = 0; plane < _system.tracks.at(ii).indexes.size();
           plane++) {
        int hitIndex = _system.tracks.at(ii).indexes.at(plane);
        if (hitIndex >= 0) {
          Measurement<float> m = _system.planes.at(plane).meas.at(hitIndex);
          // if(m.goodRegion()){ track.push_back( Measurement<float>(m.getX(),
          // m.getY(), _system.planes.at(plane).getMeasZ(), true, plane)); }
          if (m.goodRegion()) {
            track.push_back(m);
          }
          // track.push_back( m );
...
void EUTelDafMaterial::dafEnd() {
  ...
  for (size_t ii = 0; ii < _system.planes.size(); ii++) {
    ...
    _matest.system.addPlane(_system.planes.at(ii).getSensorID(),
                            _system.planes.at(ii).getZpos(), plane.getSigmaX(),
                            plane.getSigmaY(), plane.getScatterThetaSqr(),
                            plane.isExcluded());
  }
  ...
  // Initialize EstMat from configuration vectors. These are the alignment
  // parameters
  // for(size_t ii = 0; ii < _xShift.size(); ii++){ _matest.xShift.at(ii) =
  // _xShift.at(ii); }
  // for(size_t ii = 0; ii < _yShift.size(); ii++){ _matest.yShift.at(ii) =
  // _yShift.at(ii); }
  // for(size_t ii = 0; ii < _xScale.size(); ii++){ _matest.xScale.at(ii) =
  // _xScale.at(ii); }
  // for(size_t ii = 0; ii < _yScale.size(); ii++){ _matest.yScale.at(ii) =
  // _yScale.at(ii); }
  // for(size_t ii = 0; ii < _zRot.size(); ii++){ _matest.zRot.at(ii) =
  // _zRot.at(ii); }
  // for(size_t ii = 0; ii < _system.planes.size(); ii++){ _matest.zPos.at(ii) =
  // _system.planes.at(ii).getZpos(); }
