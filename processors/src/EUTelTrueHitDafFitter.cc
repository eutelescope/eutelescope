...
EUTelTrueHitDafFitter::EUTelTrueHitDafFitter()
    : Processor("EUTelTrueHitDafFitter") {
  ...
  registerInputCollection(LCIO::TRACKERHIT, "TrueHitCollectionName",
                          ...
                          string("true_hits"));
  registerOutputCollection(LCIO::TRACKERHIT, "FitpointCollectionName",
                           "Collection name for fitpoints of true track",
                           _fitpointCollectionName,
                           string("true_track_fitpoints"));
  registerOutputCollection(LCIO::TRACK, "TrackCollectionName",
                           "Collection name for fitted true tracks",
                           _trackCollectionName, string("true_track"));
...
void EUTelTrueHitDafFitter::gearRotate(size_t index, size_t gearIndex) {
  ...
  gRotation[0] =
      _siPlanesLayerLayout->getLayerRotationXY(gearIndex); // Euler alpha
  gRotation[1] =
      _siPlanesLayerLayout->getLayerRotationZX(gearIndex); // Euler beta
  gRotation[2] =
      _siPlanesLayerLayout->getLayerRotationZY(gearIndex); // Euler gamma
...
void EUTelTrueHitDafFitter::init() {
  ...
  for (; zit != _zSort.end(); index++, zit++) {
    ...
    float zPos =
        _siPlanesLayerLayout->getSensitivePositionZ((*zit).second) * 1000.0;
    ...
    float radLength = _siPlanesLayerLayout->getLayerThickness((*zit).second) /
  ...
  if (_fitDuts) {
    ...
    for (size_t i = 0; i < _system.planes.size(); i++) {
      ...
      if (find(_dutPlanes.begin(), _dutPlanes.end(),
               _system.planes.at(i).getSensorID()) != _dutPlanes.end()) {
...
void EUTelTrueHitDafFitter::readHitCollection(LCEvent *event) {
  ...
  // Add all true hits in collection that are part of the main track to
  // corresponding plane
  // true hits are part of the main track of the event if their trackID is 1
  // the trackID of the true hits is stored in the quality parameter of the
  // TrackerHit
  ...
  for (int i = 0; i < _trueHitCollectionVec->getNumberOfElements(); i++) {
    ...
    TrackerHitImpl *trueHit =
        static_cast<TrackerHitImpl *>(_trueHitCollectionVec->getElementAt(i));
    ...
    if (planeIndex >= 0) {
      ...
      _system.addMeasurement(planeIndex, static_cast<float>(pos[0]) * 1000.0f,
                             static_cast<float>(pos[1]) * 1000.0f,
                             static_cast<float>(pos[2]) * 1000.0f, region, i);
    }
  }
}

int EUTelTrueHitDafFitter::checkInTime(
...
void EUTelTrueHitDafFitter::processEvent(LCEvent *event) {
  ...
  for (size_t i = 0; i < _system.getNtracks(); i++) {
    ...
    if (not checkTrack(_system.tracks.at(i)))
      continue;
...
bool EUTelTrueHitDafFitter::checkTrack(
    daffitter::TrackCandidate<float, 4> &track) {
...
void EUTelTrueHitDafFitter::fillPlots(
    daffitter::TrackCandidate<float, 4> &track) {
  ...
  for (size_t i = 0; i < _system.planes.size(); i++) {
    ...
    for (size_t w = 0; w < plane.meas.size(); w++) {
      ...
      // 2D Resids
      _2DResiduals[plane.getSensorID()]->fill((estim.getX() - meas.getX()),
                                              (estim.getY() - meas.getY()), 1);
...
void EUTelTrueHitDafFitter::fillDetailPlots(
    daffitter::TrackCandidate<float, 4> &track) {
...
void EUTelTrueHitDafFitter::bookHistos() {
  ...
  for (size_t i = 0; i < _system.planes.size(); i++) {
    ...
    _2DResiduals[plane.getSensorID()] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residual2D", 600, residminX, residmaxX, 600, residminX,
            residmaxX);
...
void EUTelTrueHitDafFitter::addToLCIO(
    daffitter::TrackCandidate<float, 4> &track, LCCollectionVec *lcvec) {
  ...
  for (size_t plane = 0; plane < _system.planes.size(); plane++) {
    ...
    for (size_t mm = 0; mm < pl.meas.size(); mm++) {
      ...
      TrackerHitImpl *meashit = static_cast<TrackerHitImpl *>(
          _trueHitCollectionVec->getElementAt(pl.meas.at(mm).getIden()));
