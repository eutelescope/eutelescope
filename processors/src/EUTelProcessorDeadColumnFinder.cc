...
EUTelProcessorDeadColumnFinder::EUTelProcessorDeadColumnFinder()
    : Processor("EUTelProcessorDeadColumnFinder"), _zsDataCollectionName(""),
      _fillHistos(false), _nLayer(0), _xPixel(), _yPixel(), _nEvent(0),
      isDead(0) {
  ...
  for (size_t iDetector = 0; iDetector < zsInputDataCollectionVec->size();
    ...
    for (size_t iPixel = 0; iPixel < sparseData.size(); iPixel++) {
      ...
      if (iPixel != sparseData.size() - 1) {
        ...
        if (sparsePixel.getXCoord() == sparsePixel2.getXCoord() &&
            sparsePixel.getYCoord() == sparsePixel2.getYCoord()) {
          isDead[iDetector][sparsePixel.getXCoord()] = true;
          if (sparsePixel.getXCoord() % 2 == 0)
            isDead[iDetector][sparsePixel.getXCoord() + 1] = true;
          else
            isDead[iDetector][sparsePixel.getXCoord() - 1] = true;
        }
        //          cerr << "Same pixel (" << sparsePixel->getXCoord() << ", "
        //          << sparsePixel->getYCoord() << ") appearing twice in event "
...
void EUTelProcessorDeadColumnFinder::bookHistos() {
  for (int iLayer = 0; iLayer < _nLayer; iLayer++) {
    hitMap[iLayer] =
        new TH2I(Form("hitMap_%d", iLayer), Form("Hit map of layer %d", iLayer),
                 _xPixel[iLayer], 0, _xPixel[iLayer], _yPixel[iLayer] - 1, 0,
                 _yPixel[iLayer] - 1);
  }
}
...
void EUTelProcessorDeadColumnFinder::end() {
  ...
  LCCollectionVec *deadColumnCollection =
  ...
  for (int iLayer = 0; iLayer < _nLayer; iLayer++) {
    CellIDEncoder<TrackerDataImpl> deadColumnEncoder(
        eutelescope::EUTELESCOPE::ZSDATADEFAULTENCODING, deadColumnCollection);
    deadColumnEncoder["sensorID"] = iLayer;
    deadColumnEncoder["sparsePixelType"] =
    ...
    auto sparseFrame =
        std::make_unique<eutelescope::EUTelTrackerDataInterfacerImpl<
            eutelescope::EUTelGenericSparsePixel>>(currentFrame.get());
    ...
    for (int x = 1; x < hitMap[iLayer]->GetNbinsX() + 1; x++) {
      ...
      for (int y = 1; y < hitMap[iLayer]->GetNbinsY() + 1; y++) {
        if (hitMap[iLayer]->GetBinContent(x, y) != 0) {
          //          if ( x == 1) cerr << iLayer << "\t" << y << "\t" <<
          //          hitMap[iLayer]->GetBinContent(x,y) << endl;
          hitPixels[x - 1]++;
...
for (int x = 0; x < _xPixel[iLayer]; x++) {
  if (isDead[iLayer][x]) {
    ...
    streamlog_out(MESSAGE5) << "Dead double column found in layer " << iLayer
                            << " at X=" << x << endl;
  }
}
  }
