...
Cluster::Cluster() : size(0), x(0), y(0) {}
...
void Cluster::NeighbourPixels(int x, int y, vector<int> xOriginal,
                              vector<int> yOriginal, vector<int> &xNeighbour,
...
std::map<int, int> Cluster::SymmetryPairs(vector<Cluster> clusterVec,
                                          const char *type) {
...
vector<vector<int>> Cluster::sameShape(vector<Cluster> clusterVec) {
  ...
  for (size_t i = 0; i < clusterVec.size(); i++) {
    ...
    for (int mir = 0; mir < 2; mir++) {
      if (mir == 0)
        cluster2 = clusterOriginal;
      else
        cluster2 = clusterOriginal.mirrorX();
