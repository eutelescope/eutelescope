...
EUTelProcessorSpuriousClusterFinder::EUTelProcessorSpuriousClusterFinder()
    : Processor("EUTelProcessorSpuriousClusterFinder"),
      _trueHitCollectionName(""), _reconstructedHitCollectionName(""), _iRun(0),
      _iEvt(0), _sensorIDVec(), _trueHitCollectionVec(nullptr),
      _reconstructedHitCollectionVec(nullptr), _1DHistos(), _2DHistos() {

  _description = "EUTelProcessorSpuriousClusterFinder finds and anlalyses "
                 "spurious clusters (x cluster size 2, y cluster size 3, with "
                 "a particular cluster shape)";

  registerInputCollection(LCIO::TRACKERHIT, "TrueHitCollectionName",
                          "Input of True Hit data", _trueHitCollectionName,
                          std::string("true_hits"));

  registerInputCollection(LCIO::TRACKERHIT, "ReconstructedHitCollectionName",
                          "Input of Reconstructed Hit data",
                          _reconstructedHitCollectionName, std::string("hit"));
}
...
void EUTelProcessorSpuriousClusterFinder::readCollections(LCEvent *event) {
  ...
  try {
    ...
    _trueHitCollectionVec = dynamic_cast<LCCollectionVec *>(
        event->getCollection(_trueHitCollectionName));
    streamlog_out(DEBUG4) << "_trueHitCollectionVec: "
                          << _trueHitCollectionName.c_str() << " found\n";
  } catch (lcio::DataNotAvailableException &e) {

    streamlog_out(DEBUG4) << "_trueHitCollectionVec: "
                          << _trueHitCollectionName.c_str()
  ...
  try {
    ...
    _reconstructedHitCollectionVec = dynamic_cast<LCCollectionVec *>(
        event->getCollection(_reconstructedHitCollectionName));
    streamlog_out(DEBUG4) << "_reconstructedHitCollectionVec: "
                          << _reconstructedHitCollectionName.c_str()
                          << "found\n";
  } catch (lcio::DataNotAvailableException &e) {
...
void EUTelProcessorSpuriousClusterFinder::processEvent(LCEvent *event) {
  ...
  if (eutelEvent->getEventType() == kEORE) {
    ...
    streamlog_out(WARNING2) << "Event number " << eutelEvent->getEventNumber()
...
int EUTelProcessorSpuriousClusterFinder::findPairIndex(
    double const *a, std::vector<double const *> vec) {
  ...
  if (vec.size() == 1)
    ...
    double min_distance =
        sqrt(pow(a[0] - vec[0][0], 2) + pow(a[1] - vec[0][1], 2));

    for (size_t i = 1; i < vec.size(); i++) {

      double current_distance =
          sqrt(pow(a[0] - vec[i][0], 2) + pow(a[1] - vec[i][1], 2));
...
void EUTelProcessorSpuriousClusterFinder::fillHistos(LCEvent *event) {
  ...
  try {
    ...
    LCCollectionVec *_trueHitCollectionVec = dynamic_cast<LCCollectionVec *>(
        event->getCollection(_trueHitCollectionName));
    ...
    LCCollectionVec *_reconstructedHitCollectionVec =
        dynamic_cast<LCCollectionVec *>(
            event->getCollection(_reconstructedHitCollectionName));
    CellIDDecoder<TrackerHitImpl> reconstructedHitDecoder(
        _reconstructedHitCollectionVec);
    CellIDDecoder<TrackerDataImpl> rawDataDecoder(
        "sensorID:7,sparsePixelType:5");
    ...
    for (size_t i = 0; i < _sensorIDVec.size(); i++) {
      ...
      trueHitMap.insert(
          std::make_pair(_sensorIDVec[i], std::vector<double const *>()));
      trueHitEDepMap.insert(
          std::make_pair(_sensorIDVec[i], std::vector<double>()));
    }

    // fill the map with the true hits
    for (int i = 0; i < _trueHitCollectionVec->getNumberOfElements(); i++) {

      TrackerHitImpl *trueHit = dynamic_cast<TrackerHitImpl *>(
          _trueHitCollectionVec->getElementAt(i));
    ...
    for (int i = 0; i < _reconstructedHitCollectionVec->getNumberOfElements();
         i++) {

      TrackerHitImpl *reconstructedHit = dynamic_cast<TrackerHitImpl *>(
          _reconstructedHitCollectionVec->getElementAt(i));
      int detectorID = static_cast<int>(
          reconstructedHitDecoder(reconstructedHit)["sensorID"]);
      if (trueHitMap.at(detectorID).size() == 0) {

        streamlog_out(WARNING2) << "found an unpaired hit at event "
                                << event->getEventNumber() << std::endl;
        continue;
      }

      TrackerDataImpl *zsData = dynamic_cast<TrackerDataImpl *>(
          reconstructedHit->getRawHits().front());

      int pixelType =
          static_cast<int>(rawDataDecoder(zsData)["sparsePixelType"]);
      int pixelEntries;
      if (pixelType == 1)
        pixelEntries = 3; // EUTelSimpleSparsePixel
      else if (pixelType == 2)
        pixelEntries = 4; // EUTelGenericSparsePixel
      else if (pixelType == 3)
        pixelEntries = 8; // EUTelGeometricPixel
      else if (pixelType == 4)
        pixelEntries = 7; // EUTelMuPixel
      else {

        streamlog_out(ERROR3)
            << "Unrecognized pixel type in reconstructed hit raw data at event "
            << event->getEventNumber() << std::endl;
        return;
      }
      ...
      for (size_t j = pixelEntries; j < chargeValues.size();
           j += pixelEntries) {

        // add the hit pixel index to the appropriate vector if it is not yet
        // stored there
        if (std::find(XHitPixels.begin(), XHitPixels.end(), chargeValues[j]) ==
            XHitPixels.end())
          XHitPixels.push_back(chargeValues[j]);
        if (std::find(YHitPixels.begin(), YHitPixels.end(),
                      chargeValues[j + 1]) == YHitPixels.end())
      ...
      double edepTotal = (static_cast<std::vector<double>>(
          trueHitEDepMap.at(detectorID)))[pairIndex];

      double diff_x = (hitPos[0] - pair[0]) * 1000;
      double diff_y = (hitPos[1] - pair[1]) * 1000;

      if ((clusterSizeX == 2) &&
          (clusterSizeY == 3)) { // potentially a spurious cluster
        ...
        for (size_t j = pixelEntries; j < chargeValues.size();
          ...
          shape.push_back(
              static_cast<int>(chargeValues[j + 1] - chargeValues[1]));
        }

        bool isSpuriousCluster = false;
        if ((static_cast<int>(shape.size()) == 8) && (shape[2] == -1) &&
            (shape[3] == 1) && (shape[4] == 0) && (shape[5] == 1) &&
            (shape[6] == -1) && (shape[7] && 2))
          isSpuriousCluster = true;
      ...
      if ((clusterSizeX == 2) && (clusterSizeY == 2)) {
        ...
        if (static_cast<int>(shape.size()) == 8)
          is2x2Cluster = true;
      ...
      trueHitMap.at(detectorID)
          .erase(trueHitMap.at(detectorID).begin() + pairIndex);
    }
  } catch (lcio::DataNotAvailableException &e) {
...
void EUTelProcessorSpuriousClusterFinder::bookHistos() {
  ...
  for (size_t i = 0; i < _sensorIDVec.size(); i++) {
    ...
    histoTitle = "difference in x position between true simulated hits and "
                 "reconstructed hits for spurious clusters (x cluster size 2, "
                 "y cluster size 3);#Deltax /#mum;Entries";
    _1DHistos[0].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, xPosDiffMin,
                      xPosDiffMax)));
    ...
    histoTitle = "difference in y position between true simulated hits and "
                 "reconstructed hits for spurious clusters (x cluster size 2, "
                 "y cluster size 3);#Deltay /#mum;Entries";
    _1DHistos[1].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, yPosDiffMin,
                      yPosDiffMax)));
    ...
    histoTitle = "difference in x position between true simulated hits and "
                 "reconstructed hits for spurious clusters (x cluster size 2, "
                 "y cluster size 3) with y difference offset by 9.2 "
                 "#mum;#Deltax /#mum;Entries";
    _1DHistos[13].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, xPosDiffMin,
                      xPosDiffMax)));
    ...
    histoTitle = "difference in y position between true simulated hits and "
                 "reconstructed hits for spurious clusters (x cluster size 2, "
                 "y cluster size 3) with y difference offset by 9.2 "
                 "#mum;#Deltay /#mum;Entries";
    _1DHistos[14].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, yPosDiffMin,
                      yPosDiffMax)));
    ...
    histoTitle = "x position of reconstructed hits for spurious clusters (x "
                 "cluster size 2, y cluster size 3);#Deltay /#mum;Entries";
    _1DHistos[2].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, xHitPosMin,
                      xHitPosMax)));
    ...
    histoTitle = "y position of reconstructed hits for spurious clusters (x "
                 "cluster size 2, y cluster size 3);#Deltay /#mum;Entries";
    _1DHistos[3].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, yHitPosMin,
                      yHitPosMax)));
    ...
    histoTitle = "x position of true simulated hits for spurious clusters (x "
                 "cluster size 2, y cluster size 3);#Deltay /#mum;Entries";
    _1DHistos[4].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, xHitPosMin,
                      xHitPosMax)));
    ...
    histoTitle = "y position for true simulated hits for spurious clusters (x "
                 "cluster size 2, y cluster size 3);#Deltay /#mum;Entries";
    _1DHistos[5].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, yHitPosMin,
                      yHitPosMax)));
    ...
    histoTitle = "total deposition energy for true simulated hits for spurious "
                 "clusters (x cluster size 2, y cluster size 3);Total "
                 "deposition energy;Entries";
    _1DHistos[6].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), 160, 0, 80)));
    ...
    histoTitle = "difference in x position between true simulated hits and "
                 "reconstructed hits for non spurious clusters;#Deltax "
                 "/#mum;Entries";
    _1DHistos[7].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, xPosDiffMin,
                      xPosDiffMax)));
    ...
    histoTitle = "difference in y position between true simulated hits and "
                 "reconstructed hits for non spurious clusters;#Deltay "
                 "/#mum;Entries";
    _1DHistos[8].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, yPosDiffMin,
                      yPosDiffMax)));
    ...
    histoTitle = "x position of reconstructed hits for non spurious "
                 "clusters;#Deltay /#mum;Entries";
    _1DHistos[9].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, xHitPosMin,
                      xHitPosMax)));
    ...
    histoTitle = "y position of reconstructed hits for non spurious "
                 "clusters;#Deltay /#mum;Entries";
    _1DHistos[10].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, yHitPosMin,
                      yHitPosMax)));
    ...
    histoTitle = "x position of true simulated hits for non spurious "
                 "clusters;#Deltay /#mum;Entries";
    _1DHistos[11].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, xHitPosMin,
                      xHitPosMax)));
    ...
    histoTitle = "y position for true simulated hits for non spurious "
                 "clusters;#Deltay /#mum;Entries";
    _1DHistos[12].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, yHitPosMin,
                      yHitPosMax)));
    _1DHistos[12].at(sensorID)->setTitle(histoTitle.c_str());

    // set up 1D histograms for x cluster size 2 y cluster size 2 non 2x2
    // clusters
    // set up x position difference histogram
    histoName = "xPositionDifference_Non2x2_d" + to_string(sensorID);
    histoTitle = "difference in x position between true simulated hits and "
                 "reconstructed hits for x cluster size 2 y cluster size 2 non "
                 "2x2;#Deltax /#mum;Entries";
    _1DHistos[19].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, xPosDiffMin,
                      xPosDiffMax)));
    ...
    histoTitle = "difference in y position between true simulated hits and "
                 "reconstructed hits for x cluster size 2 y cluster size 2 non "
                 "2x2;#Deltay /#mum;Entries";
    _1DHistos[20].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, yPosDiffMin,
                      yPosDiffMax)));
    ...
    histoTitle = "difference in x position between true simulated hits and "
                 "reconstructed hits for 2x2 clusters;#Deltax /#mum;Entries";
    _1DHistos[15].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, xPosDiffMin,
                      xPosDiffMax)));
    ...
    histoTitle = "difference in y position between true simulated hits and "
                 "reconstructed hits for 2x2 clusters;#Deltay /#mum;Entries";
    _1DHistos[16].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, yPosDiffMin,
                      yPosDiffMax)));
    ...
    histoTitle = "difference in x position between true simulated hits and "
                 "reconstructed hits for 2x2 and spurious clusters;#Deltax "
                 "/#mum;Entries";
    _1DHistos[17].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, xPosDiffMin,
                      xPosDiffMax)));
    ...
    histoTitle = "difference in y position between true simulated hits and "
                 "reconstructed hits for 2x2 and spurious clusters;#Deltay "
                 "/#mum;Entries";
    _1DHistos[18].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + histoName).c_str(), histo1DNBin, yPosDiffMin,
                      yPosDiffMax)));
    ...
    histoTitle = "difference in position between true simulated hits and "
                 "reconstructed hits for spurious clusters (x cluster size 2, "
                 "y cluster size 3);#Deltax /#mum;#Deltay /#mum;Entries";
    _2DHistos[0].insert(std::make_pair(
        sensorID,
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + histoName).c_str(), histo2DXNBin, xPosDiff2DMin,
            xPosDiff2DMax, histo2DYNBin, yPosDiff2DMin, yPosDiff2DMax)));
    ...
    histoTitle = "difference in position between true simulated hits and "
                 "reconstructed hits for spurious clusters (x cluster size 2, "
                 "y cluster size 3) with y difference offset by 9.2 "
                 "#mum;#Deltax /#mum;#Deltay /#mum;Entries";
    _2DHistos[9].insert(std::make_pair(
        sensorID,
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + histoName).c_str(), histo2DXNBin, xPosDiff2DMin,
            xPosDiff2DMax, histo2DYNBin, yPosDiff2DMin, yPosDiff2DMax)));
    ...
    histoTitle = "position of reconstructed hits for spurious clusters (x "
                 "cluster size 2, y cluster size 3);#Deltax /#mum;#Deltay "
                 "/#mum;Entries";
    _2DHistos[1].insert(std::make_pair(
        sensorID,
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + histoName).c_str(), histo2DXNBin, xHitPos2DMin,
            xHitPos2DMax, histo2DYNBin, yHitPos2DMin, yHitPos2DMax)));
    ...
    histoTitle = "position of true simulated hits for spurious clusters (x "
                 "cluster size 2, y cluster size 3);#Deltax /#mum;#Deltay "
                 "/#mum;Entries";
    _2DHistos[2].insert(std::make_pair(
        sensorID,
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + histoName).c_str(), histo2DXNBin, xHitPos2DMin,
            xHitPos2DMax, histo2DYNBin, yHitPos2DMin, yHitPos2DMax)));
    ...
    histoTitle = "shape of clusters for spurious clusters (x cluster size 2, y "
                 "cluster size 3);#Deltax /#mum;#Deltay /#mum;Entries";

    _2DHistos[3].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram2D(
                      (basePath + histoName).c_str(), shapeXNBin, shapeXMin,
                      shapeXMax, shapeYNBin, shapeYMin, shapeYMax)));
    _2DHistos[3].at(sensorID)->setTitle(histoTitle.c_str());

    // set up shape histogram for all clusters with x cluster size 2 and y
    // cluster size 3
    histoName = "2DClusterShape_d" + to_string(sensorID);
    histoTitle = "shape of all clusters of x cluster size 2 and y cluster size "
                 "3;#Deltax /#mum;#Deltay /#mum;Entries";
    _2DHistos[4].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram2D(
                      (basePath + histoName).c_str(), shapeXNBin, shapeXMin,
                      shapeXMax, shapeYNBin, shapeYMin, shapeYMax)));
    ...
    histoTitle = "difference in position between true simulated hits and "
                 "reconstructed hits for non spurious clusters;#Deltax "
                 "/#mum;#Deltay /#mum;Entries";
    _2DHistos[5].insert(std::make_pair(
        sensorID,
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + histoName).c_str(), histo2DXNBin, xPosDiff2DMin,
            xPosDiff2DMax, histo2DYNBin, yPosDiff2DMin, yPosDiff2DMax)));
    ...
    // set up reconstructed hit position histogram
    histoName =
        "2DReconstructedHitPosition_NonSpurious_d" + to_string(sensorID);
    histoTitle = "position of reconstructed hits for non spurious "
                 "clusters;#Deltax /#mum;#Deltay /#mum;Entries";
    _2DHistos[6].insert(std::make_pair(
        sensorID,
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + histoName).c_str(), histo2DXNBin, xHitPos2DMin,
            xHitPos2DMax, histo2DYNBin, yHitPos2DMin, yHitPos2DMax)));
    ...
    histoTitle = "position of true simulated hits for non spurious "
                 "clusters;#Deltax /#mum;#Deltay /#mum;Entries";
    _2DHistos[7].insert(std::make_pair(
        sensorID,
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + histoName).c_str(), histo2DXNBin, xHitPos2DMin,
            xHitPos2DMax, histo2DYNBin, yHitPos2DMin, yHitPos2DMax)));
    ...
    histoTitle = "shape of clusters for non spurious clusters;#Deltax "
                 "/#mum;#Deltay /#mum;Entries";
    _2DHistos[8].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram2D(
                      (basePath + histoName).c_str(), shapeXNBin, shapeXMin,
                      shapeXMax, shapeYNBin, shapeYMin, shapeYMax)));
    _2DHistos[8].at(sensorID)->setTitle(histoTitle.c_str());

    // set up 2D histograms for x cluster size 2 y cluster size 2 non 2x2
    // clusters
    // set up position difference histogram
    histoName = "2DPositionDifference_Non2x2_d" + to_string(sensorID);
    histoTitle = "difference in position between true simulated hits and "
                 "reconstructed hits for x cluster size 2 y cluster size 2 non "
                 "2x2;#Deltax /#mum;#Deltay /#mum;Entries";
    _2DHistos[13].insert(std::make_pair(
        sensorID,
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + histoName).c_str(), histo2DXNBin, xPosDiff2DMin,
            xPosDiff2DMax, histo2DYNBin, yPosDiff2DMin, yPosDiff2DMax)));
    ...
    histoTitle = "shape of clusters for x cluster size 2 y cluster size 2 non "
                 "2x2 clusters;#Deltax /#mum;#Deltay /#mum;Entries";
    _2DHistos[14].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram2D(
                      (basePath + histoName).c_str(), shapeXNBin, shapeXMin,
                      shapeXMax, shapeYNBin, shapeYMin, shapeYMax)));
    ...
    histoTitle = "difference in position between true simulated hits and "
                 "reconstructed hits for 2x2 clusters;#Deltax /#mum;#Deltay "
                 "/#mum;Entries";
    _2DHistos[10].insert(std::make_pair(
        sensorID,
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + histoName).c_str(), histo2DXNBin, xPosDiff2DMin,
            xPosDiff2DMax, histo2DYNBin, yPosDiff2DMin, yPosDiff2DMax)));
    ...
    histoTitle = "shape of clusters for 2x2 clusters;#Deltax /#mum;#Deltay "
                 "/#mum;Entries";
    _2DHistos[11].insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram2D(
                      (basePath + histoName).c_str(), shapeXNBin, shapeXMin,
                      shapeXMax, shapeYNBin, shapeYMin, shapeYMax)));
    ...
    histoTitle = "difference in position between true simulated hits and "
                 "reconstructed hits for 2x2 and spurious clusters;#Deltax "
                 "/#mum;#Deltay /#mum;Entries";
    _2DHistos[12].insert(std::make_pair(
        sensorID,
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + histoName).c_str(), histo2DXNBin, xPosDiff2DMin,
            xPosDiff2DMax, histo2DYNBin, yPosDiff2DMin, yPosDiff2DMax)));
