...
EUTelHitMaker::EUTelHitMaker ():Processor ("EUTelHitMaker"), _pulseCollectionName (),
_hitCollectionName (), _switchLocalCoordinates (false),
_histogramSwitch (true), _iRun (0), _iEvt (0), _alreadyBookedSensorID ()
{
...
void
EUTelHitMaker::processRunHeader (LCRunHeader * rdr)
{
...
void
EUTelHitMaker::processEvent (LCEvent * event)
{
  ...
  try
  {
    pulseCollection =
      static_cast <
      LCCollectionVec * >(event->getCollection (_pulseCollectionName));
  }
  ...
  try
  {
    hitCollection =
      static_cast <
      LCCollectionVec * >(event->getCollection (_hitCollectionName));
  }
  catch ( ...)
  {
  ...
  CellIDDecoder < TrackerDataImpl >
    cellDecoder (EUTELESCOPE::ZSDATADEFAULTENCODING);
  ...
  for (int iCluster = 0; iCluster < pulseCollection->getNumberOfElements ();
	...
	dynamic_cast <
	TrackerPulseImpl * >(pulseCollection->getElementAt (iCluster));
      ...
      if (clusterType == kEUTelGenericSparseClusterImpl)
	{
	  ...
	  if (pixelType == kEUTelGenericSparsePixel)
	    {
	      EUTelGenericSparseClusterImpl < EUTelGenericSparsePixel >
		cluster (trackerData);
	    ...
	    {
	      streamlog_out (ERROR4) << "We do not support pixel type: " <<
		pixelType << " for kEUTelGenericSparseClusterImpl" << std::
		endl;
	      throw
      ...
      else
	{
	  ...
	  if (clusterType == kEUTelBrickedClusterImpl)
	      ...
	      if (p_tmpBrickedCluster == nullptr)
		{
		  streamlog_out (ERROR4) <<
		    " .COULD NOT CREATE EUTelBrickedClusterImpl* !!!" << std::
		    endl;
		  throw
		    UnknownDataTypeException
	  ...
	  streamlog_out (DEBUG1)
	    << "cluster[" << setw (4) << iCluster << "] on sensor[" <<
	    setw (3) << sensorID << "] at [" << setw (8) << setprecision (3)
	    << xCoG << ":" << setw (8) << setprecision (3) << yCoG << "]" <<
	    " ->  [" << setw (8) << setprecision (3) << xDet << ":" <<
	    setw (8) << setprecision (3) << yDet << "]" << std::endl;
      ...
      if (_histogramSwitch)
	{
	  AIDA::IHistogram2D * histo_loc =
	    dynamic_cast < AIDA::IHistogram2D * >(_hitLocalHistos[sensorID]);
	  if (histo_loc)
	    {
		...
		<< "Not able to retrieve histogram pointer for hitLocal_det"
      ...
      if (!_switchLocalCoordinates)
	{
      ...
      if (_histogramSwitch)
	{
	  AIDA::IHistogram2D * histo_tel =
	    dynamic_cast <
	    AIDA::IHistogram2D * >(_hitTelescopeHistos[sensorID]);
	  if (histo_tel)
	    {
	  ...
	  else
	    {
	      streamlog_out (ERROR1)
		<<
		"Not able to retrieve histogram pointer for hitTelescope_det"
		<< sensorID << ".\nDisabling histogramming from now on " <<
		std::endl;
	      _histogramSwitch = false;
	    }
...
void
EUTelHitMaker::bookHistos (int sensorID)
{
  ...
  if (hist2D_hitLocal)
    {
      hist2D_hitLocal->
	setTitle
	("Hit map (detector local frame); x position [mm]; y position [mm]");
	...
	"off histogramming and continue w/o" << std::endl;
      _histogramSwitch = false;
    }

  //create 2D histogram: hit map telescope
  std::string histName_hitTelescope =
    "hitTelescope_det" + std::to_string (sensorID);
  ...
  if (hist2D_hitTelescope)
    {
      hist2D_hitTelescope->
	setTitle
	("Hit map (telescope frame); x position [mm]; y position [mm]");
      _hitTelescopeHistos.
	insert (std::make_pair (sensorID, hist2D_hitTelescope));
    }
  else
    {
      streamlog_out (ERROR1) << "Problem booking the "
