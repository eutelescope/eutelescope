...
_telescopecoordinate()
{
	...
	_description =	"AlibavaClustering does lots of things. It clusters non-zero supressed strip sensor data, can perform FIR filtering beforehand and gets the missing coordinate for the strip sensor from a telescope cluster file.";
	...
	registerOptionalParameter ("TelescopeCollectionName","Telescope cluster collection name we want to get the unsensitive axis positions from", _telescopeCollectionName, string("cluster_m26"));
	...
	registerOptionalParameter ("SearchNearest", "Take the missing coordinate from a mean of all telescope seeds (false)? Or search for the nearest one based on the relative position (true).", _searchnearest, bool(true) );
...
void AlibavaClustering::init () {
	...
	// Thomas assumes this is good for about 3m of flatband ribbon between alibava daughterboard and alibava motherboard in the DESY testbeam
...
void AlibavaClustering::processRunHeader (LCRunHeader * rdr) {
	...
	if (_telescopePlane != -1)
	{
		...
		for (int i=0;i<_maxcount;i++)
		{
			...
			if ( i % 10000 == 0 )
				streamlog_out (MESSAGE4) << "Storing mean telescope coordinate in event " << i << " as " << _telescopecoordinate[i].mean << endl;
		}
		lcReader->close() ;
		delete lcReader ;
...
void AlibavaClustering::processEvent (LCEvent * anEvent) {
	...
	try
		...
		int noOfDetector = inputCollectionVec->getNumberOfElements();
			...
			teleinfo othercoordinate;
			othercoordinate.mean = 0.0;

			// if we are using the telescope for this value, the coordinate will be the same for all clusters in this event to keep sync.
			if (_telescopePlane != -1)
			{

				streamlog_out ( DEBUG4 )<< "Getting telescope in Event " << alibavaEvent->getEventNumber() << endl;
				if (((unsigned int)(anEvent->getEventNumber())) < _telescopecoordinate.size())
				{
						othercoordinate.mean = _telescopecoordinate[anEvent->getEventNumber()].mean;
						othercoordinate.pixelx = _telescopecoordinate[anEvent->getEventNumber()].pixelx;
						othercoordinate.pixely = _telescopecoordinate[anEvent->getEventNumber()].pixely;
				}
				else
				{
...
void AlibavaClustering::findSeedClusters(TrackerDataImpl * trkdata, LCCollectionVec * clusterCollection, LCCollectionVec * sparseClusterCollectionVec, AlibavaEventImpl * alibavaEvent, teleinfo othercoordinate)
{
	...
	for (size_t ichan=0; ichan<datavec.size();ichan++)
	{
		...
		if ((datavec[ichan] * _polarity) >= seedlevel && ( isMasked(chipnum,ichan) == false) && ( isMasked(chipnum,ichan+1) == false) && ( isMasked(chipnum,ichan-1) == false))
		{
			...
			if ((clusize >= _clusterminsize) && (clusize <= _clustermaxsize))
			{
				...
				streamlog_out (DEBUG9) << "Found a cluster in event: " << alibavaEvent->getEventNumber() << endl;
	...
	for ( unsigned int iPixel = 0; iPixel < datavec.size(); iPixel++ )
	{
		...
		float missingvalue = othercoordinate.mean;

		// Now we have the actual pixel coordinate. Depending on the search window settings, we must look for the missing coordinate.
		// The mean of the missing coordinate is passed in the struct, as well as the actual pixel seed coordinates from the selected sensor plane in both x and y
		if (_searchnearest == true)
		{
			streamlog_out (DEBUG4) << "Searching suitable missing pixel coordinate!" << endl;

			//This is the position of our local cluster element relative to the channelcount
			float relativeposition = iPixel/datavec.size();

			// The largest possible distance
			float distance = 1.0;

			// If our SENSITIVE axis is Y, we compare y to get a fake x and vice-versa
			if (_nonsensitiveaxis == "x")
			{
				for (size_t i=0; i<othercoordinate.pixely.size(); i++)
				{
					// The relative position in y:
					float newdistance = fabs(othercoordinate.pixely[i]/576.0 - relativeposition);

					// If the cluster is nearer than the mean or previous loop, go for it
					if (newdistance < distance)
					{
						distance = newdistance;

						// Remember: We compared our y against telescope y to get an x!
						missingvalue = othercoordinate.pixelx[i];
						streamlog_out (DEBUG0) << "Distance at pixel " << i << " is " << distance << "!" << endl;
					}
				}
			}
			// If our SENSITIVE axis is X, we compare x to get a fake y and vice-versa
			if (_nonsensitiveaxis == "y")
			{
				for (size_t i=0; i<othercoordinate.pixelx.size(); i++)
				{
					//The relative position in x:
					float newdistance = fabs(othercoordinate.pixelx[i]/1152.0 - relativeposition);
					...
					if (newdistance < distance)
					{
						...
						// Remember: We compared our x against telescope x to get a y!
						missingvalue = othercoordinate.pixely[i];
						streamlog_out (DEBUG0) << "Distance at pixel " << i << " is " << distance << "!" << endl;
					}
				}
			}

			if (missingvalue != othercoordinate.mean)
			{
				// Output that stuff has been done
				streamlog_out(DEBUG1) << "Moved position from " << othercoordinate.mean << " to " << missingvalue << endl;
				streamlog_out(DEBUG1) << "The relative distance is " << distance << " %!." << endl;
				fillFakeHisto(missingvalue,othercoordinate.mean);
			}
		}
	...
	for (it = clusterNumber.begin(); it != clusterNumber.end(); ++it)
	{
		...
		for (unsigned int i=0;i< datavec.size();i++)
		{
			if((clusterNumber.at(i)== *it) && (clusterNumber.at(i) > 0))
			{
				...
				// hack pixelCluster->addSparsePixel(PixelVec.at(i));
				auto temppix = PixelVec.at(i);
				//sparseCluster->push_back(PixelVec.at(i));
	...
	for (std::vector<EUTelGenericSparsePixel>::iterator pixit = PixelVec.begin(); pixit < PixelVec.end();pixit++)
	{
		//delete pixit;
	}
...
void AlibavaClustering::fillFakeHisto(float missingvalue, float mean)
{
	if ( TH1D * histo = dynamic_cast<TH1D*> (_rootObjectMap["FakeDistance"]) )
	{
		histo->Fill(missingvalue - mean);
	}
}
...
void AlibavaClustering::end()
{
	...
	if (_writecoefficients == true)
	{
		...
		/*if (fabs(filtercoefficient2)>fabs(filtercoefficient1))
		{
...
void AlibavaClustering::bookHistos()
{
	...
	// a histogram showing the distance between mean and estimated fake coordinates
	TH1D * fakedistance = new TH1D ("FakeDistance","",2000,-1000,1000);
	_rootObjectMap.insert(make_pair("FakeDistance", fakedistance));
	fakedistance->SetTitle("Fake Distance;Mean Pixel - Estimated Pixel;Number of Entries");
...
teleinfo AlibavaClustering::getTelescope()
{
	// the output struct
	teleinfo output;
	...
	telescopeCollectionVec = dynamic_cast< LCCollectionVec * > ( evt->getCollection( _telescopeCollectionName ) ) ;
		...
		output.mean = 0.0;
		output.pixelx.push_back(0);
		output.pixely.push_back(0);
		return(output);
        }
	...
	{ 
		...
		lcio::TrackerDataImpl * input2 = dynamic_cast< lcio::TrackerDataImpl * > ( input->getTrackerData() );
		...
		float xPos = 0;
		float yPos = 0;
		EUTelGenericSparseClusterImpl<EUTelGenericSparsePixel> cluster (input2);
		cluster.getCenterOfGravity(xPos, yPos);

		// We only want one plane
		if (sensorID == _telescopePlane)
		{
			streamlog_out (DEBUG0) << "Telescope x read: " << xPos << endl;
			streamlog_out (DEBUG0) << "Telescope y read: " << yPos << endl;
			telescope_x += xPos;
			telescope_y += yPos;
			planecount++;
			output.pixelx.push_back(xPos);
			output.pixely.push_back(yPos);
		}
	}
