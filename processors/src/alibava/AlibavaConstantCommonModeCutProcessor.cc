...
AlibavaConstantCommonModeCutProcessor::AlibavaConstantCommonModeCutProcessor()
    : AlibavaBaseProcessor("AlibavaConstantCommonModeCutProcessor"),
      _commonModeColName(ALIBAVA::NOTSET), _commonModeCutMin(-10.0),
      _commonModeCutMax(10.0), _maskIfAnyChipsCommonModeIsNotInRange(true),
      _numberOfMaskedEvents(0), _totalNumberOfMaskedEvents(0),
  ...
  _description = "AlibavaConstantCommonModeCutProcessor masks the events if "
                 "their common mode correction value is not in the range "
                 "specified by CommonModeCutMin and CommonModeCutMax.";

  // first of register the input collection
  registerInputCollection(LCIO::TRACKERDATA, "InputCollectionName",
                          "Input collection name that will be masked if common "
                          "mode noise is not in the range",
  ...
  registerProcessorParameter(
      ...
      "event will be masked if all chips selected has common mode noise not in "
      "the range.",
...
void AlibavaConstantCommonModeCutProcessor::processEvent(LCEvent *anEvent) {
  ...
  try {
    dataColVec = dynamic_cast<LCCollectionVec *>(
        alibavaEvent->getCollection(getInputCollectionName()));
    cmmdColVec = dynamic_cast<LCCollectionVec *>(
        alibavaEvent->getCollection(_commonModeColName));

    // check these collections have same number of elements
    if ((dataColVec->getNumberOfElements()) !=
        (cmmdColVec->getNumberOfElements())) {
      streamlog_out(ERROR5)
          << "Number of elements in collections are not equal!" << endl;
      streamlog_out(ERROR5) << getInputCollectionName() << " has "
                            << dataColVec->getNumberOfElements()
                            << " elements while " << _commonModeColName
                            << " has " << cmmdColVec->getNumberOfElements()
                            << endl;
    }
    ...
    for (size_t i = 0; i < noOfChips; ++i) {
      ...
      TrackerDataImpl *dataImpl =
          dynamic_cast<TrackerDataImpl *>(dataColVec->getElementAt(i));
      TrackerDataImpl *cmmdImpl =
          dynamic_cast<TrackerDataImpl *>(cmmdColVec->getElementAt(i));

      // check that they belong to same chip
      if ((getChipNum(dataImpl)) != (getChipNum(cmmdImpl))) {
        streamlog_out(ERROR5)
            << "The chip numbers in the collections is not same! " << endl;
      }
      ...
      if (int(datavec.size()) != ALIBAVA::NOOFCHANNELS)
        streamlog_out(ERROR5) << "Number of channels in input data is not "
                                 "equal to ALIBAVA::NOOFCHANNELS! "
                              << endl;
      if (int(cmmdvec.size()) != ALIBAVA::NOOFCHANNELS)
        streamlog_out(ERROR5) << "Number of channels in common mode data is "
                                 "not equal to ALIBAVA::NOOFCHANNELS! "
                              << endl;

      // now find common mode value, it should be constant so we can use any not
      // masked channel
    ...
    // now mask the event according to the rule specified by
    // MaskIfAnyChipsCommonModeIsNotInRange
    if ((_maskIfAnyChipsCommonModeIsNotInRange && Nchipsfailed > 0) ||
        ((!_maskIfAnyChipsCommonModeIsNotInRange) &&
         Nchipsfailed == noOfChips)) {
...
void AlibavaConstantCommonModeCutProcessor::end() {
  ...
  streamlog_out(MESSAGE5) << _numberOfMaskedEvents
                          << " event masked using common mode cut "
                          << getCommonModeCutMin() << " - "
                          << getCommonModeCutMax() << endl;

  streamlog_out(MESSAGE5) << "In total " << _totalNumberOfMaskedEvents
                          << " event masked out of " << _totalNumberOfEvents
                          << endl;

  streamlog_out(MESSAGE4) << "Successfully finished" << endl;
}

void AlibavaConstantCommonModeCutProcessor::fillHistos(
    AlibavaEventImpl *anAlibavaEvent) {

  int eventnum = anAlibavaEvent->getEventNumber();

  if (TH1D *histo =
          dynamic_cast<TH1D *>(_rootObjectMap[_hMaskedEventsNumberName]))
    histo->Fill(eventnum);
}
...
void AlibavaConstantCommonModeCutProcessor::bookHistos() {
  ...
  TH1D *_hMaskedEventsNumber = new TH1D(_hMaskedEventsNumberName.c_str(), "",
                                        10000, 0, _totalNumberOfEvents);
  _rootObjectMap.insert(
      make_pair(_hMaskedEventsNumberName, _hMaskedEventsNumber));
  _hMaskedEventsNumber->SetTitle("Event Number of Masked Events By "
                                 "ConstantCommonModeCutProcessor; Number of "
                                 "Entries;Event Number");
