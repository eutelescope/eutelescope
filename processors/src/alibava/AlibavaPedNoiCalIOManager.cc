...
EVENT::FloatVec AlibavaPedNoiCalIOManager::getDataFromEventForChip(
    LCEvent *evt, string collectionName, unsigned int chipnum) {

  EVENT::FloatVec tmp_vec;
  tmp_vec.clear();

  LCCollectionVec *col =
      dynamic_cast<LCCollectionVec *>(evt->getCollection(collectionName));

  int ielement = getElementNumberOfChip(col, chipnum);
  if (ielement != -1) {
    TrackerDataImpl *trkdata =
        dynamic_cast<TrackerDataImpl *>(col->getElementAt(ielement));
...
int AlibavaPedNoiCalIOManager::getElementNumberOfChip(LCCollectionVec *col,
                                                      int chipnum) {
  int ielement = -1;
  ...
  for (size_t i = 0; i < noOfChips; ++i) {
    ...
    TrackerDataImpl *trkdata =
        dynamic_cast<TrackerDataImpl *>(col->getElementAt(i));
    // check chip number
    const int ichip = static_cast<int>(
...
EVENT::FloatVec AlibavaPedNoiCalIOManager::getPedNoiCalForChip(
    string filename, string collectionName, unsigned int chipnum) {
  ...
  try {
    ...
    if (lcReader->getNumberOfRuns() != 1)
      streamlog_out(ERROR5)
          << " There are more than one run in AlibavaPedNoiCalFile: "
          << filename << endl;
    if (lcReader->getNumberOfEvents() != 1)
      streamlog_out(ERROR5)
          << " There are more than one event in AlibavaPedNoiCalFile: "
          << filename << endl;
    ...
    // if datavec is empty
    if (tmp_vec.size() == 0)
      streamlog_out(ERROR5) << "Trying to access" << collectionName
                            << " for non existing chip (" << chipnum << ")."
                            << endl;

    lcReader->close();
  } catch (IOException &e) {
    streamlog_out(ERROR5) << " Unable to read the AlibavaPedNoiCal file - "
...
void AlibavaPedNoiCalIOManager::addToFile(string filename,
                                          string collectionName, int chipnum,
                                          EVENT::FloatVec datavec) {

  // if file doesn't exist
  if (!doesFileExist(filename)) {
    streamlog_out(WARNING5) << " The AlibavaPedNoiCalFile: " << filename
                            << " doesn't exist. " << endl;
...
bool AlibavaPedNoiCalIOManager::doesCollectionExist(LCEvent *evt,
                                                    string collectionName) {

  bool it_exists = false;
  //	vector<string> * colnames = (vector<string> *)
  //evt->getCollectionNames();
  StringVec *colnames = const_cast<StringVec *>(evt->getCollectionNames());

  for (unsigned int i = 0; i < colnames->size(); i++)
    if (colnames->at(i) == collectionName)
      it_exists = true;
...
void AlibavaPedNoiCalIOManager::createFile(string filename) {
  ...
  streamlog_out(WARNING5)
      << " New AlibavaPedNoiCalFile is created with empty header. File is: "
      << filename << endl;
}
    ...
    if ((lcReader->getNumberOfRuns()) > 0) {
      if ((lcReader->getNumberOfRuns()) > 1) {
        streamlog_out(ERROR5)
            << " There are more than one run in AlibavaPedNoiCalFile: "
            << filename << endl;
        streamlog_out(ERROR5) << " Using only one of them (probably first "
                                 "one)! Might cause problems!"
                              << endl;
      }
      newRunHeader = dynamic_cast<LCRunHeaderImpl *>(
          lcReader->readNextRunHeader(LCIO::UPDATE));
    } else if ((lcReader->getNumberOfRuns()) == 0) {
      streamlog_out(ERROR5)
          << " There is no run header in AlibavaPedNoiCalFile: " << filename
          << endl;
    }
...
LCEventImpl *AlibavaPedNoiCalIOManager::getEvent(string filename) {
  ...
  try {
    ...
    if (lcReader->getNumberOfEvents() > 0) {
      ...
      newEvent =
          dynamic_cast<LCEventImpl *>(lcReader->readNextEvent(LCIO::UPDATE));
    } else if (lcReader->getNumberOfEvents() == 0) {
      streamlog_out(MESSAGE5)
          << " New event created in AlibavaPedNoiCalFile: " << filename << endl;
    }
