...
AlibavaPedestalNoiseProcessor::AlibavaPedestalNoiseProcessor()
    : AlibavaBaseProcessor("AlibavaPedestalNoiseProcessor"),
      ...
      _temperatureHistoName("htemperature"), _chanDataHistoName("Data_chan"),
      _chanDataFitName("Fit_chan") {

  // modify processor description
  _description = "AlibavaPedestalNoiseProcessor computes the pedestal and "
                 "noise values of each channel";
  ...
  registerProcessorParameter(
      "PedestalOutputFile",
      "The filename to store the pedestal and noise values", _pedestalFile,
      string("outputped.slcio"));
...
void AlibavaPedestalNoiseProcessor::processEvent(LCEvent *anEvent) {
  ...
  try {
    ...
    TH1D *temperatureHisto =
        dynamic_cast<TH1D *>(_rootObjectMap[_temperatureHistoName]);
...
void AlibavaPedestalNoiseProcessor::calculatePedestalNoise() {
  ...
  for (unsigned int i = 0; i < chipSelection.size(); i++) {
    ...
    TH1D *hped =
        dynamic_cast<TH1D *>(_rootObjectMap[getPedestalHistoName(ichip)]);
...
string AlibavaPedestalNoiseProcessor::getChanDataHistoName(unsigned int ichip,
                                                           unsigned int ichan) {
  stringstream s;
  s << _chanDataHistoName << "_chip" << ichip << "_chan" << ichan;
  return s.str();
}
string AlibavaPedestalNoiseProcessor::getChanDataFitName(unsigned int ichip,
                                                         unsigned int ichan) {
  stringstream s;
...
void AlibavaPedestalNoiseProcessor::bookHistos() {
  ...
  TH1D *temperatureHisto =
      new TH1D(_temperatureHistoName.c_str(), "Temperature", 1000, -50, 50);
  ...
  for (unsigned int i = 0; i < chipSelection.size(); i++) {
    ...
    TH1D *pedestalHisto =
        new TH1D(getPedestalHistoName(ichip).c_str(), "", ALIBAVA::NOOFCHANNELS,
  ...
  // here are the histograms used to calculate pedestal and noise for each
  // channel
  string tempHistoName, tempFitName;
  for (unsigned int i = 0; i < chipSelection.size(); i++) {
    unsigned int ichip = chipSelection[i];
