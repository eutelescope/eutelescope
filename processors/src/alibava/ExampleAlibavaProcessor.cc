...
ExampleAlibavaProcessor::ExampleAlibavaProcessor()
  ...
  _description = "ExampleAlibavaProcessor does whatever it does :) ";
...
void ExampleAlibavaProcessor::processEvent(LCEvent *anEvent) {
  ...
  if (anEvent->getEventNumber() % 1000 == 0)
    streamlog_out(MESSAGE4) << "Looping events " << anEvent->getEventNumber()
                            << endl;
  ...
  try {
    ...
    for (size_t i = 0; i < noOfChip; ++i) {
      ...
      /*
       Example:
       TrackerDataImpl * trkdata = dynamic_cast< TrackerDataImpl * > (
       collectionVec->getElementAt( i ) ) ;
       fillHistos(trkdata);
       */
    }
...
void ExampleAlibavaProcessor::fillHistos(TrackerDataImpl * /* trkdata */) {
  ...
  for (size_t ichan=0; ichan<datavec.size();ichan++) {
          ...
          if ( TH1D * histo = dynamic_cast<TH1D*>
  (_rootObjectMap[tempHistoName]) )
...
void ExampleAlibavaProcessor::bookHistos() {
  ...
  TH1D * pedestalHisto = new TH1D
  (_pedestalHistoName.c_str(),"",nChips*ALIBAVA::NOOFCHANNELS, -0.5,
  nChips*ALIBAVA::NOOFCHANNELS-0.5);
  ...
  if (!(getNumberOfChips()>0)) {
          streamlog_out ( ERROR5 )<<"Number of chips in the collection
  ("<<getInputCollectionName()<<") is = "<< nChips<<endl;
  }
