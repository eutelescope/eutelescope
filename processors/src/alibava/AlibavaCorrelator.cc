...
AlibavaCorrelator::AlibavaCorrelator()
    : AlibavaBaseHistogramMaker("AlibavaCorrelator"),
      ...
      _hHitPosX("hHitPosX"), _hHitPosY("hHitPosY"), _hCorX("hCorX"),
      _hCorY("hCorY"), _hSyncX("hSyncX"), _hSyncY("hSyncY"), _detectorIDs() {
  ...
  registerProcessorParameter("TagToProcess",
                             "The tag in TopTagInXMLFile. This processor will "
                             "only consider the histogram definitions inside "
                             "this tag. This tag should be inside "
                             "<TopTagInXMLFile> ... <TopTagInXMLFile/>",
                             _tagToProcess, string("myAlibavaCorrelator"));

  registerProcessorParameter("DetectorIDs", "The list of detector IDs",
                             _detectorIDs, IntVec());
}
...
void AlibavaCorrelator::createRulesToChangeXMLValues() {
  ...
  //  events
  //	changeXMLVariable(_someOtherHistoName, "xMax",
  ...
  //		addToXMLTitle(_someHistoName, "labelY", "left",
  //signalMultipliedby);
  // Again it is usually bad idea but if you want to replace the X label here
  // how you can do
  //		addToXMLTitle(_someOtherHistoName, "labelX", "all", string("Event
  //Number"));
  // And here how you add string to right of title
  //		addToXMLTitle(_someHistoName, "title", "right", string("Some
  //thing"));
}
...
std::string AlibavaCorrelator::getHistoNameForDetector(std::string name,
                                                       int detID) {
  stringstream s;
  s << name << "_d" << detID;
  return s.str();
...
void AlibavaCorrelator::createClones_hCor(string histoName) {
  ...
  for (unsigned int idet = 0; idet < _detectorIDs.size(); idet++) {
    int detID = _detectorIDs[idet];
    for (unsigned int iCorDet = idet + 1; iCorDet < _detectorIDs.size();
         iCorDet++) {
...
void AlibavaCorrelator::createClones_hSync(string histoName) {
  ...
  for (unsigned int idet = 0; idet < _detectorIDs.size(); idet++) {
    ...
    for (unsigned int iCorDet = idet + 1; iCorDet < _detectorIDs.size();
      ...
      title = title + string(" d") + to_string(detID) + string(" - d") +
              to_string(corDetID);
...
void AlibavaCorrelator::processEvent(LCEvent *anEvent) {
...

  {
    ...
    streamlog_out(MESSAGE2)
        << "No input collection " << getInputCollectionName()
        << " found on event " << alibavaEvent->getEventNumber()
  ...
  try {
    CellIDDecoder<TrackerHitImpl> hitDecoder(
        eutelescope::EUTELESCOPE::HITENCODING);

    noOfHits = collectionVec->getNumberOfElements();

    for (size_t ihit = 0; ihit < noOfHits; ++ihit) {
      TrackerHitImpl *ahit =
          dynamic_cast<TrackerHitImpl *>(collectionVec->getElementAt(ihit));
      ...
      for (size_t i = 0; i < noOfHits; ++i) {
        TrackerHitImpl *anotherHit =
            dynamic_cast<TrackerHitImpl *>(collectionVec->getElementAt(i));
