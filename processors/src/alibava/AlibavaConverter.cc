...
AlibavaConverter::AlibavaConverter ( ) : DataSourceProcessor ( "AlibavaConverter" ),
...
_storeHeaderPedestalNoise ( false )
{
    ...
    _description = "Reads data streams produced by an ALiBaVa and produces the corresponding LCIO output";
    ...
    registerOutputCollection ( LCIO::TRACKERDATA, "RawDataCollectionName", "Name of the collection", _rawDataCollectionName, string ( "rawdata" ) );

    registerOutputCollection ( LCIO::TRACKERDATA, "RawChipHeaderCollectionName", "Name of the collection", _rawChipHeaderCollectionName, string ( "chipheader" ) );
...
{
    // this event counter is used to stop the processing when it is greater than numEvents.
    int eventCounter = 0;
    ...
    {
	streamlog_out ( MESSAGE4 ) << "Input file " << _fileName << " is opened!" << endl;
    }
	...
	{
	    streamlog_out ( ERROR5 ) << "Unexpected data type found (type = " << userEventTypeCode << "). Data is not saved!" << endl;
	    return;
	}
	...
	// firmware v3 introduces the clock to the header
	if ( version == 3 )
	{
...
double AlibavaConverter::tdc_time ( unsigned int tdcTime )
{
    ...
    double tt = 100.0 * ( ipart + ( fpart / 65535.0 ) );
    return tt;
}
