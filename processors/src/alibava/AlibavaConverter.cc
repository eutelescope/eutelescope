...
AlibavaConverter::AlibavaConverter()
    : DataSourceProcessor("AlibavaConverter"), _fileName(ALIBAVA::NOTSET),
      _geoID(0), _formattedRunNumber("0"), _runNumber(-1),
      _rawDataCollectionName("rawdata"),
      _rawChipHeaderCollectionName("chipheader"), _chipSelection(),
      _startEventNum(-1), _stopEventNum(-1), _storeHeaderPedestalNoise(false) {

  // initialize few variables

  _description = "Reads data streams produced by Alibava and produces the "
  ...
  registerOutputCollection(LCIO::TRACKERDATA, "RawDataCollectionName",
                           "Name of the collection", _rawDataCollectionName,
                           string("rawdata"));
  ...
  registerOptionalParameter("ChipSelection",
                            "Selection of chip that you want to store data "
                            "from. Chip numbers start from 0. If not set, all "
                            "data (i.e. chip 0 and 1) will be stored",
                            _chipSelection, EVENT::IntVec());

  registerOptionalParameter("StartEventNum",
                            "The event number that AlibavaConverter should "
                            "start storing. Default value is -1, in this case "
                            "it will store every event",
                            _startEventNum, int(-1));

  registerOptionalParameter("StopEventNum",
                            "The event number that AlibavaConverter should "
                            "stop storing. Default value is -1, in this case "
                            "it will store every event",
                            _stopEventNum, int(-1));
  registerOptionalParameter(
      "StoreHeaderPedestalNoise",
      "Alibava stores a pedestal and noise set in the run header. These values "
      "are not used in te rest of the analysis, so it is optional to store it. "
      "By default it will not be stored, but it you want you can set this "
      "variable to true to store it in the header of slcio file",
...
void AlibavaConverter::init() {
  ...
  if (_startEventNum != -1 && _stopEventNum == -1) {
    streamlog_out(WARNING5) << "First " << _startEventNum << " will be skipped!"
                            << endl;
  } else if (_startEventNum != -1 && _stopEventNum != -1) {
    streamlog_out(WARNING5) << "Only events from " << _startEventNum << " to "
                            << _stopEventNum << " will be stored! " << endl;
  } else if (_startEventNum == -1 && _stopEventNum != -1) {
    streamlog_out(WARNING5) << "Events after event number: " << _stopEventNum
...
void AlibavaConverter::readDataSource(int /* numEvents */) {
  ...
  if (!infile.is_open()) {
    streamlog_out(ERROR5) << "AlibavaConverter could not read the file "
                          << _fileName << " correctly. Please check the path "
                                          "and file names that have been input"
                          << endl;
    exit(-1);
  } else
    streamlog_out(MESSAGE4) << "Input file " << _fileName << " is opened!"
                            << endl;
  ...
  header.clear();
  infile.read(reinterpret_cast<char *>(&lheader),
              sizeof(unsigned int)); // length of header
  ...
  // Alibava stores a pedestal and noise set in the run header. These values are
  // not used in te rest of the analysis, so it is optional to store it. By
  // default it will not be stored, but it you want you can set
  // _storeHeaderPedestalNoise variable to true.
  float tmp_float;
  FloatVec headerPedestal;
  FloatVec headerNoise;

  // first pedestal
  for (int ichan = 0; ichan < ALIBAVA::NOOFCHIPS * ALIBAVA::NOOFCHANNELS;
       ichan++) {
  ...
  if (version < 2) {
    ...
    streamlog_out(ERROR5) << " Unexpected data version found (version="
                          << version << "<2). Data is not saved" << endl;
    return;
  }
  ...
  do {
    ...
    if (eventCounter % 1000 == 0)
      streamlog_out(MESSAGE4) << "Processing event " << eventCounter
                              << " in run " << _runNumber << endl;

    unsigned int headerCode, eventSize, userEventTypeCode = 0,
                                        eventTypeCode = 0;
    do {
    ...
    if (userEventTypeCode) {
      streamlog_out(ERROR5)
          << " Unexpected data type found (type= User type). Data is not saved"
          << endl;
      return;
    }
    ...
    for (int ichip = 0; ichip < ALIBAVA::NOOFCHIPS; ichip++) {
      infile.read(reinterpret_cast<char *>(chipHeader[ichip]),
                  ALIBAVA::CHIPHEADERLENGTH * sizeof(unsigned short));
      ...
      for (int ichan = 0; ichan < ALIBAVA::NOOFCHANNELS; ichan++) {
        infile.read(reinterpret_cast<char *>(&tmp_short),
    ...
    CellIDEncoder<TrackerDataImpl> chipIDEncoder(ALIBAVA::ALIBAVADATA_ENCODE,
                                                 rawDataCollection);

    // creating LCCollection for raw chip header
    LCCollectionVec *rawChipHeaderCollection =
        new LCCollectionVec(LCIO::TRACKERDATA);
    CellIDEncoder<TrackerDataImpl> chipIDEncoder2(ALIBAVA::ALIBAVADATA_ENCODE,
                                                  rawChipHeaderCollection);

    // for this to work the _chipselection has to be sorted in ascending
    // order!!!
    for (unsigned int ichip = 0; ichip < _chipSelection.size(); ichip++) {
      ...
      chipdata.insert(chipdata.end(),
                      all_data.begin() +
                          _chipSelection[ichip] * ALIBAVA::NOOFCHANNELS,
                      all_data.begin() +
                          (_chipSelection[ichip] + 1) * ALIBAVA::NOOFCHANNELS);
      TrackerDataImpl *arawdata = new TrackerDataImpl();
      arawdata->setChargeValues(chipdata);
      chipIDEncoder[ALIBAVA::ALIBAVADATA_ENCODE_CHIPNUM] =
          _chipSelection[ichip];
      ...
      chipHeader_vec.insert(
          chipHeader_vec.end(),
          all_chipheaders.begin() +
              _chipSelection[ichip] * ALIBAVA::CHIPHEADERLENGTH,
          all_chipheaders.begin() +
              (_chipSelection[ichip] + 1) * ALIBAVA::CHIPHEADERLENGTH);
      TrackerDataImpl *achipheader = new TrackerDataImpl();
      achipheader->setChargeValues(chipHeader_vec);
      chipIDEncoder2[ALIBAVA::ALIBAVADATA_ENCODE_CHIPNUM] =
          _chipSelection[ichip];
    ...
    anEvent->addCollection(rawChipHeaderCollection,
                           _rawChipHeaderCollectionName);

    if (_startEventNum != -1 && eventCounter < _startEventNum) {
      streamlog_out(MESSAGE5) << " Skipping event " << eventCounter
                              << ". StartEventNum is set to " << _startEventNum
                              << endl;
      eventCounter++;
      continue;
    }
    ...
    if (_stopEventNum != -1 && eventCounter > _stopEventNum) {
      streamlog_out(MESSAGE5) << " Reached StopEventNum: " << _stopEventNum
                              << ". Last saved event number is " << eventCounter
                              << endl;
      break;
    }
  ...
  if (_stopEventNum != -1 && eventCounter < _stopEventNum)
    streamlog_out(MESSAGE5)
        << " Stooped before reaching StopEventNum: " << _stopEventNum
        << ". The file has " << eventCounter << " events." << endl;
}
...
void AlibavaConverter::checkIfChipSelectionIsValid() {
  ...
  // check if there is chip selection or if there is chip selection but not
  // valid
  if (_chipSelection.size() == 0) {
    ...
    streamlog_out(WARNING5)
        << "The number of chips you selected ( " << _chipSelection.size()
        << " ) is more than an alibava daughter board can have ( "
        << ALIBAVA::NOOFCHIPS << " )" << endl;
    resetChipSelection = true;
  } else {
    ...
    for (int ichip = 0; ichip < int(_chipSelection.size()); ichip++) {
      ...
      if (_chipSelection[ichip] < 0) {
        streamlog_out(ERROR5) << "Selected chip cannot have negative value. "
                              << endl;
        del_this_chip = true;
  ...
  streamlog_out(WARNING5) << "Only data coming from these chips will be stored"
                          << endl;
}
