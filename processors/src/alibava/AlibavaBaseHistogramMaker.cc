...
AlibavaBaseHistogramMaker::AlibavaBaseHistogramMaker ( std::string processorName ) : AlibavaBaseProcessor ( processorName ),
...
_replaceXMLValues ( )
{
    ...
    _description = "AlibavaBaseHistogramMaker is the base processor to produce histograms ";
...
{
    // Checks if all the histograms needed by this processor are defined in _histoXMLFileName
    // Unfortunately histogram names are hard coded, we are checking if all histo names exist in the _rootObjectMap

    // Now check if all histos exist
	...
	{
	    streamlog_out ( ERROR5 ) << "Histogram " << histoName << " doesn't exist!" << endl;
