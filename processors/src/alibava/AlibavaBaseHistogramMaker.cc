...
AlibavaBaseHistogramMaker::AlibavaBaseHistogramMaker(std::string processorName)
    : AlibavaBaseProcessor(processorName),
      _histoXMLFileName("AlibavaHistoList.xml"), _topTag("AlibavaHistoList"),
      _tagToProcess("myAlibavaBaseHistogramMaker"),
      _plotPedestalAndNoise(false), _plotEvents(), _plotXPercentOfEvents(0),
      _pedestalHistoName("hPedestal"), _noiseHistoName("hNoise"),
      _pedestalAndNoiseHistoDir("PedestalAndNoise"),
      // Event plots
      _eventHistoName("hEvent"), _eventHistoDir("Events"),
      _maskedEventsHistoName("hMaskedEvents"),
      // Calibration
      _calChargeHistoName("hCalibrationChargeValues"),
      _delayHistoName("hDelayValues"), _histoList(),
      // Others
      _multiplySignalby(1.0), _totalNumberOfEvents(0), _insertToXMLTitle(),
      _replaceXMLValues() {
...
void AlibavaBaseHistogramMaker::processHistoXMLFile() {
  ...
  streamlog_out(MESSAGE1) << "Reading HistoXMLFile : " << _histoXMLFileName
                          << endl;
  ...
  if (histoXMLDoc->LoadFile(_histoXMLFileName)) {
    ...
    if (!topTagElement) {
      ...
      throw ParseException(
          string("AlibavaBaseHistogramMaker::bookHistos no <") + _topTag +
          string("> ... </") + _topTag + string("> block found in ") +
          _histoXMLFileName);
    ...
    if (!histosBlockNode) {
      delete histoXMLDoc;
      throw ParseException(
          string("AlibavaBaseHistogramMaker::bookHistos no <") + _tagToProcess +
          string("> ... </") + _tagToProcess + string("> block found in ") +
          _histoXMLFileName);
    }
    ...
    while (histoNode) {
      ...
      updateXMLVariables(histoName, &xBin, &xMin, &xMax, &yBin, &yMin, &yMax,
                         &title, &labelX, &labelY);
      ...
      if (perEachChip == string("true") || perEachChip == string("True")) {
        for (unsigned int i = 0; i < chipSelection.size(); i++) {
          ...
          titleString =
              title + string(" (chip") + to_string(ichip) + string(")");
          ...
          if (histoType == string("TH1D")) {
            ...
            histo2D = new TH2D(newHistoName.c_str(), "", xBin, xMin, xMax, yBin,
                               yMin, yMax);
            histo2D->SetTitle(titleString.c_str());
            _rootObjectMap.insert(make_pair(newHistoName, histo2D));
          } else if (histoType == string("TProfile")) {
            profile = new TProfile(newHistoName.c_str(), "", xBin, xMin, xMax,
                                   yMin, yMax);
            profile->SetTitle(titleString.c_str());
            _rootObjectMap.insert(make_pair(newHistoName, profile));
          } else
            streamlog_out(ERROR5) << "Histogram type of " << newHistoName
                                  << " is not valid!" << endl;
          streamlog_out(DEBUG1) << "Histogram  " << newHistoName << " created!"
                                << endl;
        }
        ...
        if (histoType == string("TH1D")) {
          ...
          histo2D = new TH2D(histoName.c_str(), "", xBin, xMin, xMax, yBin,
                             yMin, yMax);
          histo2D->SetTitle(titleString.c_str());
          _rootObjectMap.insert(make_pair(histoName, histo2D));
        } else if (histoType == string("TProfile")) {
          profile =
              new TProfile(histoName.c_str(), "", xBin, xMin, xMax, yMin, yMax);
          profile->SetTitle(titleString.c_str());
          _rootObjectMap.insert(make_pair(histoName, profile));
        } else
          streamlog_out(ERROR5) << "Histogram type of " << histoName
                                << " is not valid!" << endl;

        streamlog_out(DEBUG1) << "Histogram  " << histoName << " created!"
                              << endl;
      }
  ...
  if (checkListOfHistosCreatedByXMLFile()) {
    streamlog_out(MESSAGE1) << "All histograms needed are defined in "
                            << _histoXMLFileName << endl;
  }
}
...
bool AlibavaBaseHistogramMaker::checkListOfHistosCreatedByXMLFile() {
  ...
  // _histoXMLFileName
  // Unfortunately histogram names are hard coded,  we are checking if all histo
  // names exists in the _rootObjectMap
  ...
  for (unsigned int ihisto = 0; ihisto < _histoList.size(); ihisto++) {
    ...
    if (!doesRootObjectExists(histoName)) {
      streamlog_out(ERROR5) << "Histogram " << histoName << " doesn't exists"
                            << endl;
      streamlog_out(ERROR5) << "Define " << _histoList[ihisto] << " in "
                            << _histoXMLFileName << endl;
      return false;
    } // end of if
  }
...
void AlibavaBaseHistogramMaker::plotPedestalAndNoise() {
  ...
  for (unsigned int i = 0; i < chipSelection.size(); i++) {
    ...
    if (_pedestalCollectionName != string(ALIBAVA::NOTSET)) {
      TH1D *pedestalHisto =
          new TH1D(getPedestalHistoName(ichip).c_str(), "",
      ...
      _rootObjectMap.insert(
          make_pair(getPedestalHistoName(ichip), pedestalHisto));
    }

    // noise
    if (_noiseCollectionName != string(ALIBAVA::NOTSET)) {
      TH1D *noiseHisto =
          new TH1D(getNoiseHistoName(ichip).c_str(), "", ALIBAVA::NOOFCHANNELS,
...
string AlibavaBaseHistogramMaker::getHistoNameForChip(string histoName,
                                                      int ichip) {
  stringstream s;
...
// changeXMLVariable method is used to replace xBin, xMin, xMax, yBin, yMin,
// yMax
// so variableName has to be one of these: xBin, xMin, xMax, yBin, yMin, yMax
// (case sensitive)
// for xBin and yBin newValue will changed to integer
void AlibavaBaseHistogramMaker::changeXMLVariable(string histoName,
                                                  string variableName,
                                                  float newValue) {
...
void AlibavaBaseHistogramMaker::addToXMLTitle(string histoName,
                                              string titleName,
                                              string whichSide,
                                              string stringToAdd) {
...
void AlibavaBaseHistogramMaker::updateXMLVariables(
    string histoName, int *xBin, float *xMin, float *xMax, int *yBin,
    float *yMin, float *yMax, string *title, string *labelX, string *labelY) {
...
ReplaceXMLValues::ReplaceXMLValues()
    : _histoName(), _variableName(), _newValue(0) {}

InsertToXMLTitle::InsertToXMLTitle()
    : _histoName(), _titleName(), _whichSide(), _stringToAdd() {}
