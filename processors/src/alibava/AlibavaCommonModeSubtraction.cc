...
AlibavaCommonModeSubtraction::AlibavaCommonModeSubtraction()
  ...
  _description = "AlibavaCommonModeSubtraction subtracts the provided common "
                 "mode values from the input reco (pedestal subtracted) data. ";
...
void AlibavaCommonModeSubtraction::processEvent(LCEvent *anEvent) {
  ...
  try {
    ...
    cmmdColVec = dynamic_cast<LCCollectionVec *>(
        alibavaEvent->getCollection(_commonmodeCollectionName));
    ...
    if ((dataColVec->getNumberOfElements()) !=
      ...
      streamlog_out(ERROR5) << getInputCollectionName() << " has "
                            ...
                            << " elements while " << _commonmodeCollectionName
...
string AlibavaCommonModeSubtraction::getChanDataHistoName(int chipnum,
                                                          int ichan) {
  stringstream s;
...
void AlibavaCommonModeSubtraction::bookHistos() {
  ...
  TH1D *signalHisto = new TH1D(tempHistoName.c_str(), "", 2000, -1000, 1000);
