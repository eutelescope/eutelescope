...
AlibavaCommonModeSubtraction::AlibavaCommonModeSubtraction ( ) : AlibavaBaseProcessor ( "AlibavaCommonModeSubtraction" ),
...
_chanDataHistoName ( "Common_and_Pedestal_subtracted_data_channel" )
{
    ...
    _description = "AlibavaCommonModeSubtraction subtracts the provided common mode values from the input reco (pedestal subtracted) data. ";

    // first register the input collection
    registerInputCollection ( LCIO::TRACKERDATA, "InputCollectionName", "Input reco data collection name", _inputCollectionName, string ( "recodata" ) );

    registerOutputCollection ( LCIO::TRACKERDATA, "OutputCollectionName", "Output data collection name", _outputCollectionName, string ( "recodata_cmmd" ) );

    // now the optional parameters
    registerProcessorParameter ( "CommonModeCollectionName", "Common mode collection name, better not to change", _commonmodeCollectionName, string ( "commonmode" ) );

    registerProcessorParameter ( "CommonModeErrorCollectionName", "Common mode error collection name, better not to change", _commonmodeerrorCollectionName, string ( "commonmodeerror" ) );
}
...
void AlibavaCommonModeSubtraction::bookHistos ( )
{
	...
	for ( int ichan = 0; ichan < ALIBAVA::NOOFCHANNELS; ichan++ )
	{
	    ...
	    TH1D * chanDataHisto = new TH1D ( tempHistoName.c_str ( ), "", 2000, -1000, 1000 );
