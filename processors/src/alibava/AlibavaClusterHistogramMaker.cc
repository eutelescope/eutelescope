...
AlibavaClusterHistogramMaker::AlibavaClusterHistogramMaker()
    : AlibavaBaseHistogramMaker("AlibavaClusterHistogramMaker"),
      ...
      _etaHistoName("hEta"), _etaHistoNameCS2("hEtaCS2"),
      _etaHistoNameCS3("hEtaCS3"), _etaHistoNameCS4("hEtaCS4"),
      _etaHistoNameCS5("hEtaCS5"), _etaHistoNameCSgt5("hEtaCSgt5"),
      _clusterSizeHistoName("hClusterSize"),
      _hitAmplitudeHistoName("hHitAmplitude"), _etaVSCoG("hEta_vs_CoG"),
      _etaVSClusterSize("hEta_vs_ClusterSize") {

  // modify processor description
  _description = "AlibavaClusterHistogramMaker takes some type of Alibava data "
  ...
  registerOptionalParameter("PlotNoise", "Choose if noise should be plotted. "
                                         "If you want to plot only noise set "
                                         "this variable true and set the noise "
                                         "collection name.",
...
void AlibavaClusterHistogramMaker::createRulesToChangeXMLValues() {
  ...
  changeXMLVariable(_maskedEventsHistoName, "xMax",
                    float(_totalNumberOfEvents));

  // if MultiplySignalBy is set, it is a good idea to add it to the label of the
  // "signal" axis
...
void AlibavaClusterHistogramMaker::processEvent(
    LCEvent *anEvent) { // HERE look for it
...
void AlibavaClusterHistogramMaker::printAlibavaCluster(
    TrackerDataImpl *trkdata) {
...
void AlibavaClusterHistogramMaker::fillHistos(TrackerDataImpl *trkdata) {
  ...
  histo = dynamic_cast<TH1D *>(
      _rootObjectMap[getHistoNameForChip(_hitAmplitudeHistoName, ichip)]);
  ...
  histo = dynamic_cast<TH1D *>(
      _rootObjectMap[getHistoNameForChip(_clusterSizeHistoName, ichip)]);
  histo->Fill(clusterSize);

  // Then fill eta histograms
  float eta = anAlibavaCluster.getEta();
  histo = dynamic_cast<TH1D *>(
      _rootObjectMap[getHistoNameForChip(_etaHistoName, ichip)]);
  histo->Fill(eta);

  // center of gravity
  float CoG = anAlibavaCluster.getCenterOfGravity();
  profile = dynamic_cast<TProfile *>(
      _rootObjectMap[getHistoNameForChip(_etaVSCoG, ichip)]);
  profile->Fill(CoG, eta);

  // center of gravity
  profile = dynamic_cast<TProfile *>(
      _rootObjectMap[getHistoNameForChip(_etaVSClusterSize, ichip)]);
  ...
  if (clusterSize == 2) {
    histo = dynamic_cast<TH1D *>(
        _rootObjectMap[getHistoNameForChip(_etaHistoNameCS2, ichip)]);
    histo->Fill(eta);
  } else if (clusterSize == 3) {
    histo = dynamic_cast<TH1D *>(
        _rootObjectMap[getHistoNameForChip(_etaHistoNameCS3, ichip)]);
    histo->Fill(eta);
  } else if (clusterSize == 4) {
    histo = dynamic_cast<TH1D *>(
        _rootObjectMap[getHistoNameForChip(_etaHistoNameCS4, ichip)]);
    histo->Fill(eta);
  } else if (clusterSize == 5) {
    histo = dynamic_cast<TH1D *>(
        _rootObjectMap[getHistoNameForChip(_etaHistoNameCS5, ichip)]);
    histo->Fill(eta);
  } else if (clusterSize > 5) {
    histo = dynamic_cast<TH1D *>(
        _rootObjectMap[getHistoNameForChip(_etaHistoNameCSgt5, ichip)]);
    histo->Fill(eta);
  }
}
...
void AlibavaClusterHistogramMaker::bookEventHisto(int eventnum) {
  ...
  for (unsigned int i = 0; i < chipSelection.size(); i++) {
    ...
    // to be sure that we won't create more than one histogram per event per
    // chip
    if (!doesRootObjectExists(histoName)) {
      TH1D *eventHisto = new TH1D(histoName.c_str(), "", ALIBAVA::NOOFCHANNELS,
                                  -0.5, ALIBAVA::NOOFCHANNELS - 0.5);
...
void AlibavaClusterHistogramMaker::fillEventHisto(int eventnum,
                                                  TrackerDataImpl *trkdata) {
  ...
  string histoName = getEventHistoName(eventnum, ichip);
  if (!doesRootObjectExists(histoName)) {
    streamlog_out(ERROR3) << "Root object for Event " << eventnum
                          << " doesn't exists!" << endl;
    return;
  }
