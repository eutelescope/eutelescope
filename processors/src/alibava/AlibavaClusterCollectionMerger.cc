...
AlibavaClusterCollectionMerger::AlibavaClusterCollectionMerger()
  ...
  registerOutputCollection(
      LCIO::TRACKERPULSE, "OutputClusterPulseCollectionName",
      "Name of the merged/output cluster pulse collection",
      _outputPulseCollectionName, string("merged_cluster_pulse"));

  // sparse collection
  registerOutputCollection(
      LCIO::TRACKERDATA, "OutputSparseClusterCollectionName",
      "Name of the merged/output sparse cluster collection. DO NOT Change "
      "this. This is hard coded in other  ",
...
void AlibavaClusterCollectionMerger::init() { printParameters(); }
...
void AlibavaClusterCollectionMerger::readDataSource(int /* numEvents */) {
  ...
  try {
    ...
    streamlog_out(ERROR1) << "Can't open the telescope file: " << e.what()
                          << endl;
  }
  ...
  try {
    ...
    streamlog_out(ERROR1) << "Can't open the alibava file: " << e.what()
                          << endl;
  }
  ...
  try {
    ...
    streamlog_out(ERROR1) << "Can't access run header of the alibava file: "
                          << e.what() << endl;
  }
  ...
  if (_eventIDDiff < 0) {
    for (int i = 0; i < abs(_eventIDDiff); i++)
      telescopeEvent =
          static_cast<EUTelEventImpl *>(telescope_lcReader->readNextEvent());
  } else if (_eventIDDiff > 0) {
    for (int i = 0; i < _eventIDDiff; i++)
      alibavaEvent =
          static_cast<AlibavaEventImpl *>(alibava_lcReader->readNextEvent());
  }

  bool noCollectionFound = false;
  while (((telescopeEvent = static_cast<EUTelEventImpl *>(
               telescope_lcReader->readNextEvent())) != 0) &&
         ((alibavaEvent = static_cast<AlibavaEventImpl *>(
               alibava_lcReader->readNextEvent())) != 0)) {
    ...
    if (eventCounter % 1000 == 0)
      streamlog_out(MESSAGE4) << "Looping events "
                              << alibavaEvent->getEventNumber() << endl;
    ...
    try {
      ...
      alibavaPulseColVec = dynamic_cast<LCCollectionVec *>(
          alibavaEvent->getCollection(_alibavaPulseCollectionName));
      alibavaSparseColVec = dynamic_cast<LCCollectionVec *>(
          alibavaEvent->getCollection(_alibavaSparseCollectionName));

    } catch (DataNotAvailableException &e) {
      noCollectionFound = true;
      streamlog_out(WARNING5)
          << "No input collection " << _alibavaPulseCollectionName << " or "
          << _alibavaSparseCollectionName << " found on alibava event "
          << alibavaEvent->getEventNumber() << endl;
    }

    try {
      // get telescope collections
      telescopePulseColVec = dynamic_cast<LCCollectionVec *>(
          telescopeEvent->getCollection(_telescopePulseCollectionName));
      telescopeSparseColVec = dynamic_cast<LCCollectionVec *>(
          telescopeEvent->getCollection(_telescopeSparseCollectionName));

    } catch (DataNotAvailableException &e) {
      noCollectionFound = true;
      streamlog_out(WARNING5)
          << "No input collection " << _telescopePulseCollectionName << " or "
          << _telescopeSparseCollectionName << " found on telescope event "
          << telescopeEvent->getEventNumber() << endl;
    }

    // create output collections
    LCCollectionVec *outputPulseColVec =
        new LCCollectionVec(LCIO::TRACKERPULSE);
    LCCollectionVec *outputSparseColVec =
        new LCCollectionVec(LCIO::TRACKERDATA);

    if (!noCollectionFound) {
      // copy telescope clusters
      copyClustersInCollection(outputPulseColVec, outputSparseColVec,
                               telescopePulseColVec, telescopeSparseColVec);
      // copy alibava cluster
      copyClustersInCollection(outputPulseColVec, outputSparseColVec,
                               alibavaPulseColVec, alibavaSparseColVec);
    }
    try {
      ...
      if (!noCollectionFound) {
        ...
        outputEvent->addCollection(outputPulseColVec,
                                   _outputPulseCollectionName);
        outputEvent->addCollection(outputSparseColVec,
                                   _outputSparseCollectionName);
      }
      ProcessorMgr::instance()->processEvent(
          static_cast<LCEventImpl *>(outputEvent));
      // delete outputEvent;
      //	streamlog_out ( MESSAGE1 ) << "Successfully copied Alibava
      //collections to output event" << endl ;
...
void AlibavaClusterCollectionMerger::copyClustersInCollection(
    LCCollectionVec *outputPulseColVec, LCCollectionVec *outputSparseColVec,
  ...
  CellIDEncoder<TrackerDataImpl> outputSparseColEncoder(
      eutelescope::EUTELESCOPE::ZSCLUSTERDEFAULTENCODING, outputSparseColVec);
  // for pulseFrame
  CellIDEncoder<TrackerPulseImpl> outputPulseColEncoder(
      eutelescope::EUTELESCOPE::PULSEDEFAULTENCODING, outputPulseColVec);
  ...
  for (size_t i = 0; i < noOfClusters; ++i) {
    ...
    TrackerPulseImpl *inputPulseFrame =
        dynamic_cast<TrackerPulseImpl *>(inputPulseColVec->getElementAt(i));
    TrackerDataImpl *inputSparseFrame =
        dynamic_cast<TrackerDataImpl *>(inputPulseFrame->getTrackerData());

    // set Cell ID for sparse collection
    outputSparseColEncoder["sensorID"] =
        static_cast<int>(inputSparseColDecoder(inputSparseFrame)["sensorID"]);
    outputSparseColEncoder["sparsePixelType"] = static_cast<int>(
        inputSparseColDecoder(inputSparseFrame)["sparsePixelType"]);
    outputSparseColEncoder["quality"] =
        static_cast<int>(inputSparseColDecoder(inputSparseFrame)["quality"]);
    ...
    outputPulseColEncoder["sensorID"] =
        static_cast<int>(inputPulseColDecoder(inputPulseFrame)["sensorID"]);
    outputPulseColEncoder["type"] =
        static_cast<int>(inputPulseColDecoder(inputPulseFrame)["type"]);
...
void AlibavaClusterCollectionMerger::end() {
  ...
  streamlog_out(MESSAGE5)
      << "AlibavaClusterCollectionMerger Successfully finished" << endl;
}
