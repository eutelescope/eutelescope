...
ExampleAlibavaHistogramMaker::ExampleAlibavaHistogramMaker()
    : AlibavaBaseHistogramMaker("ExampleAlibavaHistogramMaker"),
      ...
      _someHistoName("hSomeHisto"), _someOtherHistoName("hSomeOtherHisto") {

  // modify processor description
  _description = "ExampleAlibavaHistogramMaker takes some type of Alibava data "
  ...
  registerProcessorParameter(
      "HistoXMLFile", "The path of XML file where the histograms are defined",
      _histoXMLFileName, string("AlibavaHistoList.xml"));

  registerProcessorParameter("TopTagInXMLFile", "The top tag in HistoXMLFile",
                             _topTag, string("AlibavaHistoList"));

  registerProcessorParameter(
      "TagToProcess", "The tag in TopTagInXMLFile. This processor will only "
                      "consider the histogram definitions inside this tag. "
                      "This tag should be inside <TopTagInXMLFile> ... "
                      "<TopTagInXMLFile/>",
      _tagToProcess, string("myExampleAlibavaHistogramMaker"));

  // optional parameters, these parameters are defined in
  // AlibavaBaseHistogramMaker, but they are optional
  registerOptionalParameter(
      "PedestalInputFile",
      "The filename where the pedestal and noise values stored", _pedestalFile,
      string(ALIBAVA::NOTSET));
  ...
  registerOptionalParameter(
      "ChargeCalibrationCollectionName",
  ...
  registerOptionalParameter("PlotPedestalAndNoise",
                            ...
                            "If you want to plot only noise or pedestal set "
                            "this variable true and only set the noise or "
                            "pedeatal collection name you want to be plotted.",
  ...
  registerOptionalParameter("MultiplySignalBy", "In case this variable is set, "
                                                "all signals will be multipled "
                                                "by this value.",
                            _multiplySignalby, float(1.0));

  registerOptionalParameter("PlotSomePercentOfEvents",
                            "In case this variable is set (say x), x percent "
                            "of total events will be plotted randomly. The "
...
void ExampleAlibavaHistogramMaker::init() {
  ...
  /* To set of channels to be used
   ...
   means from chip 0 channels between 5-20 and 30-100, from chip 1 channels
   between 50-70 will be used (all numbers included). the rest will be masked
   and not used
   Note that the numbers should be in ascending order and there should be no
   space between two $ character
   */
  if (Global::parameters->isParameterSet(ALIBAVA::CHANNELSTOBEUSED))
    Global::parameters->getStringVals(ALIBAVA::CHANNELSTOBEUSED,
                                      _channelsToBeUsed);
  else {
    streamlog_out(MESSAGE4) << "The Global Parameter "
                            << ALIBAVA::CHANNELSTOBEUSED << " is not set!"
                            << endl;
  }
  ...
  if (Global::parameters->isParameterSet(ALIBAVA::SKIPMASKEDEVENTS))
    _skipMaskedEvents =
        bool(Global::parameters->getIntVal(ALIBAVA::SKIPMASKEDEVENTS));
  else {
    streamlog_out(MESSAGE4)
        << "The Global Parameter " << ALIBAVA::SKIPMASKEDEVENTS
        << " is not set! Masked events will be used!" << endl;
  }

  // if you want to change any variable defined in HistoXMLFile use this
  // function
...
void ExampleAlibavaHistogramMaker::createRulesToChangeXMLValues() {
  ...
  /*! If you want to replace one of these variables: xBin, xMin, xMax, yBin,
   * yMin, yMax
   *  We will use the function
   *      		void changeXMLVariable(string histoName, string variableName,
   * float newValue);
   *          defined in AlibavaBaseHistogramMaker
   *          Note that variableNames are case sensitive!
   */

  //  For example usually it is good idea to replace maximum event number for
  //  example
  //  Lets say _someOtherHistoName histograms X axis should be total number of
  //  events
  changeXMLVariable(_someOtherHistoName, "xMax", float(_totalNumberOfEvents));
  // And _someOtherHistoName histograms Y axis should be 1000 since it is the
  // max value it can get
  changeXMLVariable(_someHistoName, "yMax", 1000.0);
  // It is really bad idea, but for some reason if you want to hard code the
  // binning of X axis, here how you can do. Note that float number will be
  // changed to integer in AlibavaBaseHistogramMaker::updateXMLVariables()
  // method but here we should give it as float
  changeXMLVariable(_someOtherHistoName, "xBin", float(100));

  // HERE
  /*! If you want to replace or modify one of these variables: title, labelX and
   * labelY
   *  We will use the function
   *      		void addToXMLTitle(string histoName, string titleName, string
   * whichSide, string stringToAdd);
   *          defined in AlibavaBaseHistogramMaker
   *          Note that titleName and whichSide are case sensitive!
   */

  // For example if MultiplySignalBy is set, it is a good idea to add it to the
  ...
  if (_multiplySignalby != 1.0) {
    ...
    // Assuming that "Signal" is written in HistoXMLFile as this histograms
    // labelY, to get "(_multiplySignalby) Signal" you should add
    // signalMultipliedby string to the left
    addToXMLTitle(_someHistoName, "labelY", "left", signalMultipliedby);
    // Again it is usually bad idea but if you want to replace the X label here
    // how you can do
...
void ExampleAlibavaHistogramMaker::fillListOfHistos() {
  // Checks if all the histograms needed by this processor is defined in
  // _histoXMLFileName
  // Unfortunately histogram names are hard coded, so we are checking if all
  // histo names exists in the _rootObjectMap
...
void ExampleAlibavaHistogramMaker::processEvent(LCEvent *anEvent) {
  ...
  if (anEvent->getEventNumber() % 1000 == 0)
    streamlog_out(DEBUG1) << "Looping events " << anEvent->getEventNumber()
                          << endl;
  ...
  try {
    collectionVec = dynamic_cast<LCCollectionVec *>(
        alibavaEvent->getCollection(getInputCollectionName()));
    noOfChip = collectionVec->getNumberOfElements();

    for (size_t i = 0; i < noOfChip; ++i) {
      TrackerDataImpl *trkdata =
          dynamic_cast<TrackerDataImpl *>(collectionVec->getElementAt(i));
    ...
    // do nothing again
    streamlog_out(ERROR5) << "Collection (" << getInputCollectionName()
                          << ") not found! " << endl;
  }
}
...
void ExampleAlibavaHistogramMaker::fillHistos(TrackerDataImpl *trkdata) {
  ...
  // Remember we had _someHistoName for each chip, they are booked in
  // AlibavaBaseHistogramMaker::processHistoXMLFile()
  TH2D *histo = dynamic_cast<TH2D *>(
      _rootObjectMap[getHistoNameForChip(_someHistoName, ichip)]);
  // OK it is again stupid but just as an example, fill x and y with signals in
  // trkdata
...
void ExampleAlibavaHistogramMaker::end() {
  ...
  if (_numberOfSkippedEvents > 0)
    streamlog_out(MESSAGE5) << _numberOfSkippedEvents
                            << " events skipped since they are masked" << endl;
...
void ExampleAlibavaHistogramMaker::bookEventHisto(int eventnum) {
  ...
  for (unsigned int i = 0; i < chipSelection.size(); i++) {
    ...
    TH1D *eventHisto =
        new TH1D(getEventHistoName(eventnum, ichip).c_str(), "",
                 ALIBAVA::NOOFCHANNELS, -0.5, ALIBAVA::NOOFCHANNELS - 0.5);

    stringstream sp; // title string for histogram
    sp << "Event " << eventnum << " (chip " << ichip
       << ");Channel Number;Signal (ADCs)";
    eventHisto->SetTitle((sp.str()).c_str());

    _rootObjectMap.insert(
        make_pair(getEventHistoName(eventnum, ichip), eventHisto));

  } // end of loop over selected chips
}

// You have to define what fillEventHisto will do
void ExampleAlibavaHistogramMaker::fillEventHisto(int eventnum,
                                                  TrackerDataImpl *trkdata) {
  ...
  TH1D *histo =
      dynamic_cast<TH1D *>(_rootObjectMap[getEventHistoName(eventnum, ichip)]);
