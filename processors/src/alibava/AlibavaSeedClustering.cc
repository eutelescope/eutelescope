...
AlibavaSeedClustering::AlibavaSeedClustering()
    : AlibavaBaseProcessor("AlibavaSeedClustering"), _seedCut(3), _neighCut(2),
      _sensitiveAxisX(1), _signalPolarity(-1), _etaHistoName("hEta"),
      _clusterSizeHistoName("hClusterSize"), _isSensitiveAxisX(true) {
  ...
  registerInputCollection(
      LCIO::TRACKERDATA, "InputCollectionName",
  ...
  registerOutputCollection(LCIO::TRACKERDATA, "OutputCollectionName",
                           "Output data collection name", _outputCollectionName,
                           string("alibava_clusters"));
  ...
  registerProcessorParameter("SeedSNRCut", "The signal/noise ratio that "
                                           "channels have to pass to be "
                                           "considered as seed channel",
                             _seedCut, float(3));

  registerProcessorParameter("NeighbourSNRCut",
                             "The signal/noise ratio that neigbour channels "
                             "have to pass to be added to the cluster",
                             _neighCut, float(2));

  registerProcessorParameter("IsSensitiveAxisX",
                             "The default sensitive axis of the strip "
                             "sensor(s) according to telescope is X. If "
                             "sensitive axis is Y then set this parameter to "
                             "zero (0). Any other value will be disregarded "
                             "and sensitive axis will assumed to be \"X\" ",
                             _sensitiveAxisX, int(1));

  registerProcessorParameter(
      "SignalPolarity", "Polarity of the signal. Set this parameter to -1 for "
                        "negative signals, any other value will be disregarded "
                        "and the signal will be assumed to be positive ",
...
void AlibavaSeedClustering::processEvent(LCEvent *anEvent) {
  ...
  CellIDEncoder<TrackerDataImpl> clusterIDEncoder(
      ALIBAVA::ALIBAVACLUSTER_ENCODE, clusterColVec);

  unsigned int noOfChip;
  try {
    inputColVec = dynamic_cast<LCCollectionVec *>(
    ...
    for (size_t i = 0; i < noOfChip; ++i) {
      ...
      TrackerDataImpl *trkdata =
          dynamic_cast<TrackerDataImpl *>(inputColVec->getElementAt(i));
      ...
      for (unsigned int icluster = 0; icluster < clusters.size(); icluster++) {
        ...
        // now store chip number, seed channel, cluster ID, cluster size,
        // sensitive axis and signal polarity in CellIDEncode

        // set cluster ID
        clusterIDEncoder[ALIBAVA::ALIBAVACLUSTER_ENCODE_CLUSTERID] =
        ...
        // set chip number
        clusterIDEncoder[ALIBAVA::ALIBAVACLUSTER_ENCODE_CHIPNUM] =
            acluster.getChipNum();
        // set cluster size
        clusterIDEncoder[ALIBAVA::ALIBAVACLUSTER_ENCODE_CLUSTERSIZE] =
            acluster.getClusterSize();
        // set seed channel number
        clusterIDEncoder[ALIBAVA::ALIBAVACLUSTER_ENCODE_SEED] =
            acluster.getSeedChanNum();
...
vector<AlibavaCluster>
AlibavaSeedClustering::findClusters(TrackerDataImpl *trkdata) {
  ...
  for (unsigned int i = 0; i + 1 < seedCandidates.size(); i++) {
    ...
    float next_snr =
        (_signalPolarity * dataVec[next_chan]) / noiseVec[next_chan];
  ...
  for (unsigned int iseed = 0; iseed < seedCandidates.size(); iseed++) {
    ...
    // We will check if one of the neighbours not bonded. If it is not this is
    // not a good cluster we will not use this
    ...
    while (continueSearch) {
      ...
      if (isMasked(chipnum, ichan) == true) {
        ...
        // the members on the right that should be in this cluster, should not
        // be used by some other cluster.
...
float AlibavaSeedClustering::calculateEta(TrackerDataImpl *trkdata,
                                          int seedChan) {
  ...
  // we will multiply all signal values by _signalPolarity to work on positive
  // signal always
  ...
  if (rightChan < int(dataVec.size()) &&
      isMasked(chipnum, rightChan) == false) {
  ...
  // if one of the channel is masked, since it will be set to unrealisticSignal
  // other signal will always be higher then the masked one.
  ...
  // Eta calculation: chargeOnLeftChannel / (chargeOnLeftChannel +
  // chargeOnRightChannel)
...
void AlibavaSeedClustering::bookHistos() {
  ...
  for (unsigned int i = 0; i < chipSelection.size(); i++) {
    ...
    title = string("Cluster Size (chip ") + to_string(ichip) +
            string(");Number of Entries;Cluster Size");
    ...
    title = string("Eta distribution ClusterSize > 1 (chip ") +
            to_string(ichip) + string(");Number of Entries;Eta");
