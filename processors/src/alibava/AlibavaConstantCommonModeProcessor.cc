...
AlibavaConstantCommonModeProcessor::AlibavaConstantCommonModeProcessor()
    : AlibavaBaseProcessor("AlibavaConstantCommonModeProcessor"),
      ...
      _commonmodeerrorCollectionName(ALIBAVA::NOTSET), _Niteration(3),
      _NoiseDeviation(2.5), _commonmodeHistoName("hcommonmode"),
      _commonmodeerrorHistoName("hcommonmodeerror"), _commonmode(),
      _commonmodeerror() {

  // modify processor description
  _description = "AlibavaConstantCommonModeProcessor computes the common mode "
                 "values of each chip and their errors";
  ...
  registerOptionalParameter(
      "CommonModeErrorCollectionName",
      "Common mode error collection name, better not to change",
      _commonmodeerrorCollectionName, string("commonmodeerror"));

  registerOptionalParameter(
      "CommonModeErrorCalculationIteration",
      "The number of iteration that should be used in common mode calculation",
      _Niteration, int(3));

  registerOptionalParameter(
      "NoiseDeviation", "The limit to the deviation of noise. The data exceeds "
                        "this deviation will be considered as signal and not "
                        "be included in common mode error calculation",
...
void AlibavaConstantCommonModeProcessor::init() {
  ...
  if (Global::parameters->isParameterSet(ALIBAVA::CHANNELSTOBEUSED))
    ...
    streamlog_out(MESSAGE4)
        << "The Global Parameter " << ALIBAVA::CHANNELSTOBEUSED
        << " is not set! All channels will be used!" << endl;
  }
...
void AlibavaConstantCommonModeProcessor::processEvent(LCEvent *anEvent) {
  ...
  try {
    ...
    CellIDEncoder<TrackerDataImpl> commonCol_CellIDEncode(
        ALIBAVA::ALIBAVADATA_ENCODE, commonCollection);
    CellIDEncoder<TrackerDataImpl> commerrCol_CellIDEncode(
        ALIBAVA::ALIBAVADATA_ENCODE, commerrCollection);
    ...
    alibavaEvent->addCollection(commonCollection,
                                getCommonModeCollectionName());
    alibavaEvent->addCollection(commerrCollection,
                                getCommonModeErrorCollectionName());
...
void AlibavaConstantCommonModeProcessor::calculateConstantCommonMode(
    TrackerDataImpl *trkdata) {
  ...
  streamlog_out(DEBUG0) << "Chip " << chipnum << " of " << getNumberOfChips()
                        << ", now iterating..." << endl;
  ...
  for (int i = 0; i < _Niteration; i++) {
    ...
    if (nchan > 0) {
      ...
      sigma_mean_signal =
          sqrt(total_signal_square / nchan - mean_signal * mean_signal);
    }
  ...
  streamlog_out(DEBUG0) << "==================================================="
                           "============================"
                        << endl;
  streamlog_out(DEBUG0) << "Chip " << chipnum
                        << " : CommonModeCorrection = " << mean_signal
                        << ", CommonModeCorrectionError = " << sigma_mean_signal
                        << endl;
  ...
  // Now reloop over all channels and create the output vector. This will be the
...
void AlibavaConstantCommonModeProcessor::fillHistos(TrackerDataImpl *trkdata,
                                                    int event) {
  ...
  for (size_t ichan = 0; ichan < datavec.size(); ichan++) {
    ...
    if (TH2D *histo = dynamic_cast<TH2D *>(
            _rootObjectMap["Common Mode Correction Values over Events"]))
      histo->Fill(event, datavec[ichan]);
  }
}

// getter and setter for _commonmodeCollectionName
void AlibavaConstantCommonModeProcessor::setCommonModeCollectionName(
    std::string commonmodeCollectionName) {
...
void AlibavaConstantCommonModeProcessor::setCommonModeErrorCollectionName(
    std::string commonmodeerrorCollectionName) {
  _commonmodeerrorCollectionName = commonmodeerrorCollectionName;
}
std::string
AlibavaConstantCommonModeProcessor::getCommonModeErrorCollectionName() {
  return _commonmodeerrorCollectionName;
}

// getter and setter for _commonmode
void AlibavaConstantCommonModeProcessor::setCommonModeVec(
    EVENT::FloatVec common) {
  if (common.size() != unsigned(ALIBAVA::NOOFCHANNELS)) {
    streamlog_out(ERROR5) << "Size of common mode vector set doesn't match "
                             "with number of channels in the data stream."
                          << endl;
  }
  _commonmode = common;
}
...
void AlibavaConstantCommonModeProcessor::setCommonModeErrorVec(
    EVENT::FloatVec commonerror) {
  if (commonerror.size() != unsigned(ALIBAVA::NOOFCHANNELS)) {
    streamlog_out(ERROR5) << "Size of common mode error vector set doesn't "
                             "match with number of channels in the data stream."
                          << endl;
  }
...
void AlibavaConstantCommonModeProcessor::bookHistos() {
  ...
  tempHistoTitle2 << "Common Mode Correction Values over Events"
                  << ";ADCs;NumberofEntries";

  TH2D *signalHisto2 = new TH2D("Common Mode Correction Values over Events", "",
                                5000, 0, 500000, 1000, -500, 500);
  _rootObjectMap.insert(
      make_pair("Common Mode Correction Values over Events", signalHisto2));
