...
AlibavaCluster::AlibavaCluster()
    : _channums(), _signals(), _eta(-1), _chipNum(-1), _seedChanNum(-1),
      _clusterID(-1), _isSensitiveAxisX(true), _signalPolarity(-1) {
  // does nothing
}
...
AlibavaCluster::AlibavaCluster(TrackerDataImpl *trkdata)
    : _channums(), _signals(), _eta(-1), _chipNum(-1), _seedChanNum(-1),
      _clusterID(-1), _isSensitiveAxisX(true), _signalPolarity(-1) {
  CellIDDecoder<TrackerDataImpl> clusterIDDecoder(
      ALIBAVA::ALIBAVACLUSTER_ENCODE);
  _clusterID = static_cast<int>(
      clusterIDDecoder(trkdata)[ALIBAVA::ALIBAVACLUSTER_ENCODE_CLUSTERID]);
  _seedChanNum = static_cast<int>(
      clusterIDDecoder(trkdata)[ALIBAVA::ALIBAVACLUSTER_ENCODE_SEED]);
  _chipNum = static_cast<int>(
      clusterIDDecoder(trkdata)[ALIBAVA::ALIBAVACLUSTER_ENCODE_CHIPNUM]);

  int sensitiveAxisX = static_cast<int>(clusterIDDecoder(
      trkdata)[ALIBAVA::ALIBAVACLUSTER_ENCODE_ISSENSITIVEAXISX]);
  if (sensitiveAxisX == 0)
    _isSensitiveAxisX = false;
  else
    _isSensitiveAxisX = true;

  int negSignal = static_cast<int>(clusterIDDecoder(
      trkdata)[ALIBAVA::ALIBAVACLUSTER_ENCODE_ISSIGNALNEGATIVE]);
  if (negSignal == 0)
    _signalPolarity = 1;
  else
    _signalPolarity = -1;
  ...
  if (data.size() % 2 != 1)
    streamlog_out(ERROR5) << "Size in TrackerData that stores cluster "
                             "information is not even! Either channel number "
                             "or signal information is missing!"
                          << endl;
...
float AlibavaCluster::getEta() { return _eta; }

void AlibavaCluster::setEta(float eta) { _eta = eta; }
  ...
  for (int imember = 0; imember < getClusterSize(); imember++) {
    ...
    if (noiseVec[channel] == 0) {
      streamlog_out(ERROR5)
          << "Channels noise is zero! Check this cluster! Returning zero!"
          << endl;
      return 0;
    }
...
int AlibavaCluster::getClusterSize() {
  if (_channums.size() != _signals.size()) {
    streamlog_out(ERROR5) << "There is something wrong with this "
                             "AlibavaCluster, number of channels is not "
                             "consistent with number of signals, registered in "
                             "this cluster!"
                          << endl;
  }
...
int AlibavaCluster::getChipNum() { return _chipNum; }
void AlibavaCluster::setChipNum(int chipnum) { _chipNum = chipnum; }
...
void AlibavaCluster::setClusterID(int clusterID) { _clusterID = clusterID; }
...
bool AlibavaCluster::getIsSensitiveAxisX() { return _isSensitiveAxisX; }
/*
...
int AlibavaCluster::getSignalPolarity() { return _signalPolarity; }
...
void AlibavaCluster::print() {
  ...
  streamlog_out(MESSAGE1) << "ClusterID: " << getClusterID()
                          << " ChipNum: " << getChipNum() << endl;
  ...
  if (getIsSensitiveAxisX())
    ...
    axis = string("Y");
  streamlog_out(MESSAGE1) << "SensitiveAxis: " << axis
                          << " SignalPolarity: " << getSignalPolarity() << endl;

  streamlog_out(MESSAGE1) << "SeedChannel: " << getSeedChanNum()
