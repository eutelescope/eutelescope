...
AlibavaDataHistogramMaker::AlibavaDataHistogramMaker()
    : AlibavaBaseHistogramMaker("AlibavaDataHistogramMaker"),
      // Signal
      _signalHistoName("hSignal"), _signalVsTimeHistoName("hSignal_vs_Time"),
      _signalVsTempHistoName("hSignal_vs_Temperature"),
      // SNR
      _snrHistoName("hSNR"), _snrVsTimeHistoName("hSNR_vs_Time"),
  ...
  _description = "AlibavaDataHistogramMaker takes TrackerData of Alibava data "
...
void AlibavaDataHistogramMaker::processEvent(LCEvent *anEvent) {
  ...
  profile =
      dynamic_cast<TProfile *>(_rootObjectMap[_temperatureVsEventNumHistoName]);
...
void AlibavaDataHistogramMaker::fillHistos(TrackerDataImpl * /* trkdata */) {
  // nothing to do here, see
  // AlibavaDataHistogramMaker::fillOtherHistos(TrackerDataImpl * trkdata, float
  // tdctime, float temperature)
}

void AlibavaDataHistogramMaker::fillOtherHistos(TrackerDataImpl *trkdata,
                                                float tdctime,
                                                float temperature) {
  ...
  TH1D *histoSignal = dynamic_cast<TH1D *>(
      _rootObjectMap[getHistoNameForChip(_signalHistoName, ichip)]);
  TH2D *histoSignalVsTime = dynamic_cast<TH2D *>(
      _rootObjectMap[getHistoNameForChip(_signalVsTimeHistoName, ichip)]);
  TH2D *histoSignalVsTemp = dynamic_cast<TH2D *>(
      _rootObjectMap[getHistoNameForChip(_signalVsTempHistoName, ichip)]);

  TH1D *histoSNR = dynamic_cast<TH1D *>(
      _rootObjectMap[getHistoNameForChip(_snrHistoName, ichip)]);
  TH2D *histoSNRVsTime = dynamic_cast<TH2D *>(
      _rootObjectMap[getHistoNameForChip(_snrVsTimeHistoName, ichip)]);
  TH2D *histoSNRVsTemp = dynamic_cast<TH2D *>(
      _rootObjectMap[getHistoNameForChip(_snrVsTempHistoName, ichip)]);
...
void AlibavaDataHistogramMaker::fillEventHisto(int eventnum,
                                               TrackerDataImpl *trkdata) {
...
void AlibavaDataHistogramMaker::createRulesToChangeXMLValues() {
  ...
  changeXMLVariable(_timeVsEventNumHistoName, "xMax",
                    float(_totalNumberOfEvents));
  changeXMLVariable(_temperatureVsEventNumHistoName, "xMax",
                    float(_totalNumberOfEvents));
