...
AlibavaBaseProcessor::AlibavaBaseProcessor(std::string processorName)
    : Processor(processorName), _rootObjectMap(),
      _inputCollectionName(ALIBAVA::NOTSET),
      _outputCollectionName(ALIBAVA::NOTSET), _pedestalFile(ALIBAVA::NOTSET),
      _calibrationFile(ALIBAVA::NOTSET),
      _pedestalCollectionName(ALIBAVA::NOTSET),
      _noiseCollectionName(ALIBAVA::NOTSET),
      _chargeCalCollectionName(ALIBAVA::NOTSET), _channelsToBeUsed(),
      _skipMaskedEvents(false), _numberOfSkippedEvents(0), _chipSelection(),
      _pedestalMap(), _noiseMap(), _chargeCalMap(), _isPedestalValid(false),
      _isNoiseValid(false), _isCalibrationValid(false) {
...
void AlibavaBaseProcessor::setInputCollectionName(
...
void AlibavaBaseProcessor::setOutputCollectionName(
...
void AlibavaBaseProcessor::setPedestals() {
  ...
  if (selectedchips.size() == 0)
    streamlog_out(ERROR5)
        << "No selected chips found! Couldn't set the pedestal, noise values!"
        << endl;
  ...
  for (unsigned int ichip = 0; ichip < selectedchips.size(); ichip++) {
    ...
    if (getPedestalCollectionName() != string(ALIBAVA::NOTSET)) {
      ...
      vec_float.clear();
      vec_float = man.getPedNoiCalForChip(_pedestalFile,
                                          _pedestalCollectionName, chipnum);
      _pedestalMap.insert(make_pair(chipnum, vec_float));
    } else {
      streamlog_out(DEBUG5)
          << "The pedestal values for chip " << chipnum
          << " is not set, since pedestalCollectionName is not set!" << endl;
    }
    ...
    if (getNoiseCollectionName() != string(ALIBAVA::NOTSET)) {
      ...
      streamlog_out(DEBUG5)
          << "The noise values for chip " << chipnum
          << " is not set, since noiseCollectionName is not set!" << endl;
    }
  }
  checkPedestals();
}
...
void AlibavaBaseProcessor::checkPedestals() {
  ...
  for (unsigned int ichip = 0; ichip < selectedchips.size(); ichip++) {
    ...
    if (getPedestalCollectionName() != string(ALIBAVA::NOTSET)) {
      ...
      if (int(vec_float.size()) != ALIBAVA::NOOFCHANNELS) {
        streamlog_out(ERROR5) << "The pedestal values for chip " << chipnum
                              << " is not set properly!" << endl;
      } else
    ...
    if (getNoiseCollectionName() != string(ALIBAVA::NOTSET)) {
      ...
      if (int(vec_float.size()) != ALIBAVA::NOOFCHANNELS) {
        streamlog_out(ERROR5) << "The noise values for chip " << chipnum
                              << " is not set properly!" << endl;
      } else
        _isNoiseValid = true;
    }
  ...
  if (_isPedestalValid) {
    streamlog_out(MESSAGE5)
        << "The pedestal values for all selected chips are valid!" << endl;
  } else {
    streamlog_out(WARNING5)
        << "The pedestal values for all selected chips are not set properly!"
        << endl;
  }

  if (_isNoiseValid) {
    streamlog_out(MESSAGE5)
        << "The noise values for all selected chips are valid!" << endl;
  } else {
    streamlog_out(WARNING5)
        << "The noise values for all selected chips are not set properly!"
        << endl;
  }
}
...
void AlibavaBaseProcessor::setPedestalCollectionName(
    std::string pedestalCollectionName) {
...
bool AlibavaBaseProcessor::isPedestalValid() { return _isPedestalValid; }

///////////////////////////
// Noise
///////////////////////////
// getter and setter for _noiseCollectionName
void AlibavaBaseProcessor::setNoiseCollectionName(
    std::string noiseCollectionName) {
...
float AlibavaBaseProcessor::getNoiseAtChannel(int chipnum, int channum) {
  if (isNoiseValid()) {
    ...
    // streamlog_out(ERROR5)<< "The noise values for chip "<<chipnum<<" is not
    // set properly!"<<endl;
    return 0;
  }
}

bool AlibavaBaseProcessor::isNoiseValid() { return _isNoiseValid; }
...
void AlibavaBaseProcessor::setCalibration() {
  ...
  if (selectedchips.size() == 0)
    streamlog_out(ERROR5)
        << "No selected chips found! Couldn't set calibration values!" << endl;
  ...
  for (unsigned int ichip = 0; ichip < selectedchips.size(); ichip++) {
    ...
    vec_float.clear();
    vec_float = man.getPedNoiCalForChip(_calibrationFile,
                                        _chargeCalCollectionName, chipnum);
...
void AlibavaBaseProcessor::checkCalibration() {
  ...
  for (unsigned int ichip = 0; ichip < selectedchips.size(); ichip++) {
    ...
    if (int(vec_float.size()) != ALIBAVA::NOOFCHANNELS) {
      streamlog_out(ERROR5) << "The charge calibration values for chip "
                            << chipnum << " is not set properly!" << endl;
      _isCalibrationValid = false;
    }
  }
  ...
  if (_isCalibrationValid)
    streamlog_out(MESSAGE5)
        << "The calibration values for all selected chips are valid!" << endl;
}

// getter and setter for _chargeCalCollectionName
void AlibavaBaseProcessor::setChargeCalCollectionName(
    std::string chargeCalCollectionName) {
...
bool AlibavaBaseProcessor::isChipValid(int ichip) {
  for (unsigned int i = 0; i < _chipSelection.size(); i++)
    if (_chipSelection[i] == ichip)
      return true;

  // if it reaches here it means that the chip number is not in the selected
  // chip numbers, then return false
  return false;
}
...
bool AlibavaBaseProcessor::isMasked(int ichip, int ichan) {
  ...
  if (_channelsToBeUsed.size() == 0)
    ...
    streamlog_out(ERROR5) << "Trying to access mask value of non existing "
                             "chip/channel. Returning zero."
                          << endl;
    return 0;
  }
}
...
void AlibavaBaseProcessor::setChannelsToBeUsed() {
   ...
   *   $0:5-20$ $0:30-100$ $1:50-70$ means from chip 0 channels between 5-20 and
   * 30-100, from chip 1 channels between 50-70
  ...
  for (unsigned int istr = 0; istr < _channelsToBeUsed.size(); istr++) {
    ...
    streamlog_out(DEBUG5) << "Processing channel selection: " << istring
                          << " ... un-masking channels from " << fromchannel
                          << " to " << tochannel << " on chip " << onchip
                          << endl;
...
void AlibavaBaseProcessor::decodeMaskingString(string istring, int *onchip,
                                               int *fromchannel,
                                               int *tochannel) {
...
bool AlibavaBaseProcessor::isMaskingValid(int onchip, int fromchannel,
                                          int tochannel) {
  ...
  if (!isChipValid(onchip)) {
    streamlog_out(ERROR5) << " Chip selection on channel masking is not valid!"
                          << endl;
    isValid = false;
  }
  if (!isChannelValid(fromchannel) || !isChannelValid(tochannel)) {
    streamlog_out(ERROR5)
        << " Channel selection on channel masking is not valid!" << endl;
    isValid = false;
  }
  // start channel cannot be bigger than end chan
  if (fromchannel > tochannel) {
    streamlog_out(ERROR5) << " Channel range on channel masking is not valid! "
                             "Channels should be in ascending order!"
                          << endl;
    isValid = false;
  }
  ...
  for (unsigned int i = 0; i < _chipSelection.size(); i++) {
    ...
    for (int ichan = 0; ichan < ALIBAVA::NOOFCHANNELS;) {
      if (ichan % channelprintnum == 0) {
        streamlog_out(MESSAGE5) << endl;
        streamlog_out(MESSAGE5) << " Channels " << ichan << " - "
                                << ichan + channelprintnum - 1 << " : ";
      }
...
int AlibavaBaseProcessor::getChipNum(TrackerDataImpl *trkdata) {
  ...
  int chipnum = static_cast<int>(
      chipIDDecoder(trkdata)[ALIBAVA::ALIBAVADATA_ENCODE_CHIPNUM]);
  return chipnum;
}
