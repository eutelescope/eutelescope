...
EUTelFitTupleAlibava::EUTelFitTupleAlibava() : Processor("EUTelFitTupleAlibava")
{
	...
	registerProcessorParameter ("MissingValue", "Value written to the ntuple for missing measurements.", _missingValue, static_cast < double > (-100.));
...
void EUTelFitTupleAlibava::getReference (LCEvent * event)
{
	...
	for(size_t i = 0 ; i <  static_cast< size_t >(referenceHitVec->getNumberOfElements()); i++)
	{
		...
		EUTelAlignmentConstant * refhit = static_cast< EUTelAlignmentConstant*> ( referenceHitVec->getElementAt(i) );
		if( _manualDUTid == refhit->getSensorID() )
		{
			_x_refhit = refhit->getXOffset();
			_y_refhit = refhit->getYOffset();
			_z_refhit = refhit->getZOffset();
			_a_refhit = refhit->getAlpha();
			_b_refhit = refhit->getBeta();
			_c_refhit = refhit->getGamma();
		}
	}
	...
	for( int itrack = 0; itrack < maxTracks ; itrack++ )
	{
		...
		if (_iDUT != -1 )
		{
			...
			if (_useTrackFit == true && _useTriplet == false)
			{
				...
				streamlog_out ( DEBUG6 ) << "Using DAF track fit!" << endl;
			}
...
void EUTelFitTupleAlibava::end()
{
	...
	if (_iDUT != -1)
	{
		...
		// FIXME until now only gamma (XY) rotations, as this is the easiest to align with a DUT
		// not sure if other angles possible
...
void EUTelFitTupleAlibava::fillEtaHisto(float left, float right, float left1, float right1, float left2, float right2, float local, int event, float tdctime)
{
	...
	if (fractpart == 0.5)
	{
		signal = ((left2/2)+left1+left+right+right1+(right2/2)) * _polarity;
		seedsignal = ( (left + right) / 2 ) * _polarity;
		rightneighbour = ( ( right + right1) / 2 ) * _polarity;
		leftneighbour = ( ( left1 + left ) / 2 ) * _polarity;
	}
	if (fractpart > 0.5)
	{
...
void EUTelFitTupleAlibava::fillEtaDebug(float left, float right, float left1, float right1, float left2, float right2, float local, float event)
{
	if ( TH1D * eta = dynamic_cast<TH1D*> (_rootObjectMap["TrackEtaCentral_debug"]) )
	{
		eta->Fill(left/(left+right));
	}
	if ( TH1D * eta = dynamic_cast<TH1D*> (_rootObjectMap["TrackEtaLeft_debug"]) )
	{
		eta->Fill(left1/(left1+left));
	}
	if ( TH1D * eta = dynamic_cast<TH1D*> (_rootObjectMap["TrackEtaRight_debug"]) )
	{
		eta->Fill(right/(right+right1));
	}
	...
	if (fractpart < 0.25 )
	{
		if ( TH1D * eta = dynamic_cast<TH1D*> (_rootObjectMap["TrackEtaCentral2_debug"]) )
		{
			eta->Fill(left1/(left1+right));
		}
		if ( TH1D * eta = dynamic_cast<TH1D*> (_rootObjectMap["TrackEtaLeft2_debug"]) )
		{
			eta->Fill(left2/(left2+left));
		}
		if ( TH1D * eta = dynamic_cast<TH1D*> (_rootObjectMap["TrackEtaRight2_debug"]) )
		{
			eta->Fill(left/(left+right1));
		}
	}
	...
	if (fractpart > 0.75 )
	{
		...
		if ( TH1D * eta = dynamic_cast<TH1D*> (_rootObjectMap["TrackEtaRight2_debug"]) )
		{
			eta->Fill(right/(right+right2));
		}
	}

	if ( TH2D * eta = dynamic_cast<TH2D*> (_rootObjectMap["2DEta_debug"]) )
	{
		eta->Fill(left/(left+right),fractpart);
	}

	if ( TH2D * etat = dynamic_cast<TH2D*> (_rootObjectMap["TrackEtaCentralTime_debug"]) )
	{
		etat->Fill(left/(left+right),event);
	}
}
...
void EUTelFitTupleAlibava::bookHistos()
{
	...
	if (_iDUT != -1)
	{
		...
		if (_alibava == true)
		{
			...
			TH1D * etac_debug = new TH1D ("TrackEtaCentral_debug","",120,-1,2);
			_rootObjectMap.insert(make_pair("TrackEtaCentral_debug", etac_debug));
			etac_debug->SetTitle("Track Eta Central;Eta unit;Events");

			TH1D * etal_debug = new TH1D ("TrackEtaLeft_debug","",120,-1,2);
			_rootObjectMap.insert(make_pair("TrackEtaLeft_debug", etal_debug));
			etal_debug->SetTitle("Track Eta Left;Eta unit;Events");

			TH1D * etar_debug = new TH1D ("TrackEtaRight_debug","",120,-1,2);
			_rootObjectMap.insert(make_pair("TrackEtaRight_debug", etar_debug));
			etar_debug->SetTitle("Track Eta Right;Eta unit;Events");

			TH2D * eta2d_debug = new TH2D ("2DEta_debug","",120,-1,2,100,-0.5,1.5);
			_rootObjectMap.insert(make_pair("2DEta_debug", eta2d_debug));
			eta2d_debug->SetTitle("Eta 2D;Eta;Y mod pitch");

			TH1D * etac2_debug = new TH1D ("TrackEtaCentral2_debug","",120,-2,3);
			_rootObjectMap.insert(make_pair("TrackEtaCentral2_debug", etac2_debug));
			etac2_debug->SetTitle("Track Eta Central;Eta unit;Events");

			TH1D * etal2_debug = new TH1D ("TrackEtaLeft2_debug","",120,-2,3);
			_rootObjectMap.insert(make_pair("TrackEtaLeft2_debug", etal2_debug));
			etal2_debug->SetTitle("Track Eta Left;Eta unit;Events");

			TH1D * etar2_debug = new TH1D ("TrackEtaRight2_debug","",120,-2,3);
			_rootObjectMap.insert(make_pair("TrackEtaRight2_debug", etar2_debug));
			etar2_debug->SetTitle("Track Eta Right;Eta unit;Events");

			TH2D * etat_debug = new TH2D ("TrackEtaCentralTime_debug","",120,-1,2,1000,-10,10);
			_rootObjectMap.insert(make_pair("TrackEtaCentralTime_debug", etat_debug));
			etat_debug->SetTitle("Track Eta Central vs. Time;Eta unit;Eventnr.");

			TH1D * etaintegral_debug = new TH1D ("TrackEtaIntegral_debug","",120,-1,2);
			_rootObjectMap.insert(make_pair("TrackEtaIntegral_debug", etaintegral_debug));
			etaintegral_debug->SetTitle("Track Eta Integral;Eta unit;Events");
