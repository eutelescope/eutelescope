...
AlibavaTimeCutProcessor::AlibavaTimeCutProcessor ( ) : AlibavaBaseProcessor ( "AlibavaTimeCutProcessor" ),
_timecutmin ( 0.0 ),
_timecutmax ( 100.0 ),
...
_hMaskedEventsNumberName ( "EventNumberOfMaskedEvents" )
{
    ...
    _description = "AlibavaTimeCutProcessor masks events if their TDC time value is not in the range specified by TimeCutMin and TimeCutMax ";

    registerProcessorParameter ( "TimeCutMin", "The minimum tdc time that is acceptable to use in an Event", _timecutmin, float ( 0.0 ) );

    registerProcessorParameter ( "TimeCutMax", "The maximum tdc time that is acceptable to use in an Event", _timecutmax, float ( 100.0 ) );
