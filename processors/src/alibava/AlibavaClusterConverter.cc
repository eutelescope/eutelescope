...
AlibavaClusterConverter::AlibavaClusterConverter()
    : AlibavaBaseProcessor("AlibavaClusterConverter"),
      ...
      _sparseCollectionName(ALIBAVA::NOTSET), _sensorIDStartsFrom(0),
      _missingCorrdinateValue(0) {

  // modify processor description
  _description = "AlibavaClusterConverter converts AlibavaClusters to "
                 "EUTelSparseCluster and  :) ";
  ...
  registerProcessorParameter("OutputEUTelClusterPulseCollectionName",
                             "The collection name of cluster pulse.  This "
                             "might be hardcoded in EUTelescope framework",
                             _pulseCollectionName, string("clustercollection"));

  registerProcessorParameter("OutputEUTelSparseClusterCollectionName",
                             "The collection name of sparse cluster.  This "
                             "might be hardcoded in EUTelescope framework",
                             _sparseCollectionName, string("original_zsdata"));

  registerProcessorParameter("SensorIDStartsFrom",
                             "The sensor ID for the data. The actual sensorID "
                             "will be stored as SensorIDStartsFrom + "
                             "ChipNumber ",
                             _sensorIDStartsFrom, int(6));

  // now the optional parameters
  registerOptionalParameter("MissingCoordinateValue",
                            "The value that should be stored in missing "
                            "coordinate. This number has to be integer since "
                            "it will be used as channel number of the missing "
                            "coordinate",
...
void AlibavaClusterConverter::processEvent(LCEvent *anEvent) {
  ...
  CellIDEncoder<TrackerDataImpl> sparseColEncoder(
      EUTELESCOPE::ZSCLUSTERDEFAULTENCODING, sparseColVec);
  // for pulseFrame
  CellIDEncoder<TrackerPulseImpl> pulseColEncoder(
      EUTELESCOPE::PULSEDEFAULTENCODING, pulseColVec);

  // HERE

  unsigned int noOfClusters;
  try {
    alibavaCluColVec = dynamic_cast<LCCollectionVec *>(
    ...
    for (size_t i = 0; i < noOfClusters; ++i) {
      ...
      TrackerDataImpl *alibavaClu =
          dynamic_cast<TrackerDataImpl *>(alibavaCluColVec->getElementAt(i));
      ...
      EUTelSparseClusterImpl<EUTelGenericSparsePixel> *eutelPixelCluster =
          new EUTelSparseClusterImpl<eutelescope::EUTelGenericSparsePixel>(
              sparseFrame);

      // for each member (channel) in the alibava cluster create a
      ...
      for (int imember = 0; imember < clusterSize; imember++) {
        ...
        float memberSignal = anAlibavaCluster.getSignal(imember) *
                             anAlibavaCluster.getSignalPolarity();
        ...
        eutelPixel.setSignal(
            memberSignal); // EUTelGenericSparsePixel::setSignal() gets float
      ...
      float totalSignal = anAlibavaCluster.getTotalSignal() *
                          anAlibavaCluster.getSignalPolarity();

      // set the ID for this zsCluster
      sparseColEncoder["sensorID"] = _sensorIDStartsFrom + chipnum;
      sparseColEncoder["sparsePixelType"] =
          static_cast<int>(kEUTelGenericSparsePixel);
