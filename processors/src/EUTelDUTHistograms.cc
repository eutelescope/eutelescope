...
EUTelDUTHistograms::EUTelDUTHistograms()
    : Processor("EUTelDUTHistograms"), 
      ...
      _trackhitposY(), _trackhitsizeX(), _trackhitsizeY(), _trackhitsubM(),
      _trackhitsensorID(), _cluSizeXCut(0), _cluSizeYCut(0), _trackNCluXCut(0),
      _trackNCluYCut(0), _measuredX(), _measuredY(), _bgmeasuredX(),
      _bgmeasuredY(), _localX(), _localY(), _fittedX(), _fittedY(),
      _bgfittedX(), _bgfittedY(), _DUTalign(), _ClusterSizeHistos(),
      _ShiftHistos(), _MeasuredHistos(), _MatchedHistos(), _UnMatchedHistos(),
      _FittedHistos(), _EfficiencyHistos(), _BgEfficiencyHistos(),
      _NoiseHistos(), _BgShiftHistos(), _ShiftXvsYHisto(), _ShiftYvsXHisto(),
      _ShiftXvsXHisto(), _ShiftYvsYHisto(), _ShiftXvsY2DHisto(),
      _ShiftYvsX2DHisto(), _ShiftXvsX2DHisto(), _ShiftYvsY2DHisto(),
      _EtaXHisto(), _EtaYHisto(), _EtaX2DHisto(), _EtaY2DHisto(),
      _EtaX3DHisto(), _EtaY3DHisto(), _PixelEfficiencyHisto(),
      _PixelResolutionXHisto(), _PixelResolutionYHisto(),
...

{
  ...
  registerInputCollection(LCIO::TRACK, "InputTrackCollectionName",
                          ...
                          _inputTrackColName, std::string("testfittracks"));

  registerInputCollection(LCIO::TRACKERHIT, "InputHitCollectionName",
                          "Name of the input DUT hit collection",
                          _inputHitColName, std::string("hit"));

  registerInputCollection(LCIO::TRACKERHIT, "InputRecHitCollectionName",
                          "Name of the input DUT hit collection",
                          _inputRecHitColName, std::string("localHit"));

  registerInputCollection(LCIO::TRACKERHIT, "InputFitHitCollectionName",
                          "Name of the input DUT hit collection",
                          _inputFitHitColName, std::string("dummy"));
  ...
  registerProcessorParameter("DUTalignment", "Alignment corrections for DUT: "
                                             "shift (in mm) in X, Y and "
                                             "rotation around Z",
                             _DUTalign, initAlign);
...
void EUTelDUTHistograms::processRunHeader(LCRunHeader *runHeader) {
  ...
  message<DEBUG5>(log() << "Processing run header " << _nRun << ", run nr "
                        << runNr);
...
void EUTelDUTHistograms::processEvent(LCEvent *event) {
  ...
  if (_inputFitHitColName != "dummy") {
    message<DEBUG5>(log() << "inputFitHitColName = " << _inputFitHitColName
                          << " (not dummy)" << endl);
  ...
  if (streamlog_level(DEBUG5)) {
    ...
    for (int itrack = 0; itrack < _maptrackid; itrack++) {
      message<DEBUG5>(log() << " track " << itrack << " has "
                            << _fittedX[itrack].size()
                            << " fitted positions at DUT ");
    }
  }
  ...
  for (int itrack = 0; itrack < _maptrackid; itrack++) {
    for (int ifit = 0; ifit < static_cast<int>(_fittedX[itrack].size());
         ifit++) {
      (dynamic_cast<AIDA::IHistogram1D *>(_FittedHistos.at(projX)))
          ->fill(_fittedX[itrack][ifit]);

      (dynamic_cast<AIDA::IHistogram1D *>(_FittedHistos.at(projY)))
          ->fill(_fittedY[itrack][ifit]);
      (dynamic_cast<AIDA::IHistogram2D *>(_FittedHistos.at(projXY)))
          ->fill(_fittedX[itrack][ifit], _fittedY[itrack][ifit]);
      if (streamlog_level(DEBUG5)) {
  ...
  for (int ihit = 0; ihit < static_cast<int>(_measuredX.size()); ihit++) {
    (dynamic_cast<AIDA::IHistogram1D *>(_MeasuredHistos.at(projX)))
        ->fill(_measuredX[ihit]);
    (dynamic_cast<AIDA::IHistogram1D *>(_MeasuredHistos.at(projY)))
        ->fill(_measuredY[ihit]);
    (dynamic_cast<AIDA::IHistogram2D *>(_MeasuredHistos.at(projXY)))
        ->fill(_measuredX[ihit], _measuredY[ihit]);
    if (streamlog_level(DEBUG5)) {
      message<DEBUG5>(log() << "Hit " << ihit << "   X = " << _measuredX[ihit]
                            << "   Y = " << _measuredY[ihit]);
    }
  }
#endif
  ...
  for (int itrack = 0; itrack < _maptrackid; itrack++) {
    ...
    if (static_cast<int>(_fittedX[itrack].size()) < 1)
      continue;
    ...
    for (int ifit = 0; ifit < static_cast<int>(_fittedX[itrack].size());
      ...
      for (int ihit = 0; ihit < static_cast<int>(_measuredX.size()); ihit++) {
        ...
        double dist2rd = (_measuredX[ihit] - _fittedX[itrack][ifit]) *
                             (_measuredX[ihit] - _fittedX[itrack][ifit]) +
                         (_measuredY[ihit] - _fittedY[itrack][ifit]) *
                             (_measuredY[ihit] - _fittedY[itrack][ifit]);

        if (streamlog_level(DEBUG5)) {
          message<DEBUG5>(log() << "Fit [" << itrack << ":" << _maptrackid
                                << "], ifit= " << ifit << " ["
                                << _fittedX[itrack][ifit] << ":"
                                << _fittedY[itrack][ifit] << "]" << endl);
          message<DEBUG5>(log() << "rec " << ihit << " [" << _measuredX[ihit]
                                << ":" << _measuredY[ihit] << "]" << endl);
          message<DEBUG5>(log() << "distance : " << TMath::Sqrt(dist2rd)
                                << endl);
        }
        if (dist2rd < distmin) {
          distmin = dist2rd;
...
// Matched hits positions
      ...
      (dynamic_cast<AIDA::IHistogram1D *>(
           _ClusterSizeHistos.at(projX).at(FullDetector)))
          ->fill(_clusterSizeX[besthit] + 0.0);
      (dynamic_cast<AIDA::IHistogram1D *>(
           _ClusterSizeHistos.at(projY).at(FullDetector)))
          ->fill(_clusterSizeY[besthit] + 0.0);
      (dynamic_cast<AIDA::IHistogram2D *>(
           _ClusterSizeHistos.at(projXY).at(FullDetector)))
          ->fill(_clusterSizeX[besthit] + 0.0, _clusterSizeY[besthit] + 0.0);

      // .. and once for the submatrix (identified by the index)
      (dynamic_cast<AIDA::IHistogram1D *>(_ClusterSizeHistos.at(projX).at(
           static_cast<detMatrix>(_subMatrix[besthit]))))
          ->fill(_clusterSizeX[besthit] + 0.0);
      (dynamic_cast<AIDA::IHistogram1D *>(_ClusterSizeHistos.at(projY).at(
           static_cast<detMatrix>(_subMatrix[besthit]))))
          ->fill(_clusterSizeY[besthit] + 0.0);
      (dynamic_cast<AIDA::IHistogram2D *>(_ClusterSizeHistos.at(projXY).at(
           static_cast<detMatrix>(_subMatrix[besthit]))))
          ->fill(_clusterSizeX[besthit] + 0.0, _clusterSizeY[besthit] + 0.0);

      (dynamic_cast<AIDA::IHistogram1D *>(_MatchedHistos.at(projX)))
          ->fill(_measuredX[besthit]);
      (dynamic_cast<AIDA::IHistogram1D *>(_MatchedHistos.at(projY)))
          ->fill(_measuredY[besthit]);
      (dynamic_cast<AIDA::IHistogram2D *>(_MatchedHistos.at(projXY)))
          ->fill(_measuredX[besthit], _measuredY[besthit]);
      ...
      // fill global: any matrix, any cluster size (cluster size 0 -> any
      // cluster size)
      (dynamic_cast<AIDA::IHistogram1D *>(
           _ShiftHistos.at(projX).at(FullDetector).at(0)))
          ->fill(shiftX);
      (dynamic_cast<AIDA::IHistogram1D *>(
           _ShiftHistos.at(projY).at(FullDetector).at(0)))
          ->fill(shiftY);
      (dynamic_cast<AIDA::IHistogram2D *>(
           _ShiftHistos.at(projXY).at(FullDetector).at(0)))
          ->fill(shiftX, shiftY);

      // fill for submatrix and any cluster size
      (dynamic_cast<AIDA::IHistogram1D *>(
           _ShiftHistos.at(projX)
               .at(static_cast<detMatrix>(_subMatrix[besthit]))
               .at(0)))
          ->fill(shiftX);
      (dynamic_cast<AIDA::IHistogram1D *>(
           _ShiftHistos.at(projY)
               .at(static_cast<detMatrix>(_subMatrix[besthit]))
               .at(0)))
          ->fill(shiftY);
      (dynamic_cast<AIDA::IHistogram2D *>(
           _ShiftHistos.at(projXY)
               .at(static_cast<detMatrix>(_subMatrix[besthit]))
               .at(0)))
          ->fill(shiftX, shiftY);

      // check that the cluster size is within the limits of our multi diff.
      // binning
      if (_clusterSizeX[besthit] <= HistoMaxClusterSize &&
          _clusterSizeY[besthit] <= HistoMaxClusterSize) {
        // fill for any matrix
        (dynamic_cast<AIDA::IHistogram1D *>(_ShiftHistos.at(projX)
                                                .at(FullDetector)
                                                .at(_clusterSizeX[besthit])))
            ->fill(shiftX);
        (dynamic_cast<AIDA::IHistogram1D *>(_ShiftHistos.at(projY)
                                                .at(FullDetector)
                                                .at(_clusterSizeY[besthit])))
            ->fill(shiftY);
        // for XY: only if cluster size identical in both x and y
        if (_clusterSizeX[besthit] == _clusterSizeY[besthit]) {
          (dynamic_cast<AIDA::IHistogram2D *>(_ShiftHistos.at(projXY)
                                                  .at(FullDetector)
                                                  .at(_clusterSizeX[besthit])))
              ->fill(shiftX, shiftY);
        }
      ...
      if (_clusterSizeX[besthit] == 1 && _clusterSizeY[besthit] == 1) {
        _PixelEfficiencyHisto->fill(_localX[itrack][bestfit] * 1000.,
                                    _localY[itrack][bestfit] * 1000., 1.);
        _PixelResolutionXHisto->fill(
            _localX[itrack][bestfit] * 1000., _localY[itrack][bestfit] * 1000.,
            _measuredX[besthit] - _fittedX[itrack][bestfit]);
        _PixelResolutionYHisto->fill(
            _localX[itrack][bestfit] * 1000., _localY[itrack][bestfit] * 1000.,
            _measuredY[besthit] - _fittedY[itrack][bestfit]);
      }

      _ShiftXvsYHisto->fill(_fittedY[itrack][bestfit],
                            _measuredX[besthit] - _fittedX[itrack][bestfit]);
      _ShiftYvsXHisto->fill(_fittedX[itrack][bestfit],
                            _measuredY[besthit] - _fittedY[itrack][bestfit]);
      _ShiftXvsX2DHisto->fill(_fittedX[itrack][bestfit],
                              _measuredX[besthit] - _fittedX[itrack][bestfit]);
      _ShiftXvsXHisto->fill(_fittedX[itrack][bestfit],
                            _measuredX[besthit] - _fittedX[itrack][bestfit]);

      _ShiftYvsY2DHisto->fill(_fittedY[itrack][bestfit],
                              _measuredY[besthit] - _fittedY[itrack][bestfit]);

      _ShiftYvsYHisto->fill(_fittedY[itrack][bestfit],
                            _measuredY[besthit] - _fittedY[itrack][bestfit]);

      _ShiftXvsY2DHisto->fill(_fittedY[itrack][bestfit],
                              _measuredX[besthit] - _fittedX[itrack][bestfit]);

      _ShiftYvsX2DHisto->fill(_fittedX[itrack][bestfit],
                              _measuredY[besthit] - _fittedY[itrack][bestfit]);

      // Eta function check plots
      if (_clusterSizeX[besthit] == 1 && _clusterSizeY[besthit] == 1) {
        _EtaXHisto->fill(_localX[itrack][bestfit],
                         _measuredX[besthit] - _fittedX[itrack][bestfit]);
        _EtaYHisto->fill(_localY[itrack][bestfit],
                         _measuredY[besthit] - _fittedY[itrack][bestfit]);
        _EtaX2DHisto->fill(_localX[itrack][bestfit],
                           _measuredX[besthit] - _fittedX[itrack][bestfit]);
        _EtaY2DHisto->fill(_localY[itrack][bestfit],
                           _measuredY[besthit] - _fittedY[itrack][bestfit]);
        _EtaX3DHisto->fill(_localX[itrack][bestfit], _localY[itrack][bestfit],
                           _measuredX[besthit] - _fittedX[itrack][bestfit]);
        _EtaY3DHisto->fill(_localX[itrack][bestfit], _localY[itrack][bestfit],
                           _measuredY[besthit] - _fittedY[itrack][bestfit]);
      }
// extend Eta histograms to 2 pitch range
      ...
      // Efficiency plots
      (dynamic_cast<AIDA::IProfile1D *>(_EfficiencyHistos.at(projX)))
          ->fill(_fittedX[itrack][bestfit], 1.);
      (dynamic_cast<AIDA::IProfile1D *>(_EfficiencyHistos.at(projY)))
          ->fill(_fittedY[itrack][bestfit], 1.);
      (dynamic_cast<AIDA::IProfile2D *>(_EfficiencyHistos.at(projXY)))
          ->fill(_fittedX[itrack][bestfit], _fittedY[itrack][bestfit], 1.);

      // Noise plots
      (dynamic_cast<AIDA::IProfile1D *>(_NoiseHistos.at(projX)))
          ->fill(_measuredX[besthit], 0.);
      (dynamic_cast<AIDA::IProfile1D *>(_NoiseHistos.at(projY)))
          ->fill(_measuredY[besthit], 0.);
      (dynamic_cast<AIDA::IProfile2D *>(_NoiseHistos.at(projXY)))
          ->fill(_measuredX[besthit], _measuredY[besthit], 0.);
    ...
    if (streamlog_level(DEBUG5)) {
      ...
      message<DEBUG5>(log() << _measuredX.size()
                            << " DUT hits not matched to any track ");
      message<DEBUG5>(
          log() << "track " << itrack << " has " << _fittedX[itrack].size()
                << " _fittedX[itrack].size() not matched to any DUT hit ");
    }
...
// Efficiency plots - unmatched tracks
    ...
    for (int ifit = 0; ifit < static_cast<int>(_localX[itrack].size());
         ifit++) {
      _PixelEfficiencyHisto->fill(_localX[itrack][ifit] * 1000.,
                                  _localY[itrack][ifit] * 1000., 0.);
    }

    for (int ifit = 0; ifit < static_cast<int>(_fittedX[itrack].size());
         ifit++) {
      (dynamic_cast<AIDA::IProfile1D *>(_EfficiencyHistos.at(projX)))
          ->fill(_fittedX[itrack][ifit], 0.);
      (dynamic_cast<AIDA::IProfile1D *>(_EfficiencyHistos.at(projY)))
          ->fill(_fittedY[itrack][ifit], 0.);
      (dynamic_cast<AIDA::IProfile2D *>(_EfficiencyHistos.at(projXY)))
          ->fill(_fittedX[itrack][ifit], _fittedY[itrack][ifit], 0.);
    }
#endif
  }
  ...
  for (int ihit = 0; ihit < static_cast<int>(_measuredX.size()); ihit++) {
    (dynamic_cast<AIDA::IProfile1D *>(_NoiseHistos.at(projX)))
        ->fill(_measuredX[ihit], 1.);
    (dynamic_cast<AIDA::IProfile1D *>(_NoiseHistos.at(projY)))
        ->fill(_measuredY[ihit], 1.);
    (dynamic_cast<AIDA::IProfile2D *>(_NoiseHistos.at(projXY)))
        ->fill(_measuredX[ihit], _measuredY[ihit], 1.);

    // Unmatched hit positions
    (dynamic_cast<AIDA::IHistogram1D *>(_UnMatchedHistos.at(projX)))
        ->fill(_measuredX[ihit]);
    (dynamic_cast<AIDA::IHistogram1D *>(_UnMatchedHistos.at(projY)))
        ->fill(_measuredY[ihit]);
    (dynamic_cast<AIDA::IHistogram2D *>(_UnMatchedHistos.at(projXY)))
        ->fill(_measuredX[ihit], _measuredY[ihit]);
  }
...
void EUTelDUTHistograms::end() {
  ...
  // fill global: any matrix, any cluster size (cluster size 0 -> any cluster
  // size)
...
void EUTelDUTHistograms::bookHistos() {
  ...
  std::string _ClusterSizeHistoBaseName =
      "clusterSize"; // append [X, Y, XY][submatrix][A-D]
  ...
  std::string _ShiftHistoBaseName =
      "DUTshift"; // append [X, Y, XY][A-D][1-HistoMaxClusterSize]
  ...
  message<MESSAGE5>(log() << "Histogram information searched in "
                          << _histoInfoFileName);
  ...
  for (int thisProjection = 0; thisProjection <= 2; thisProjection++) {
    ...
    std::map<detMatrix, std::string> detMatrixDescription; // maps enumerator
                                                           // value to
                                                           // description e.g.
                                                           // for histo title
    ...
    std::map<detMatrix, std::string> detMatrixName; // maps enumerator value to
                                                    // name element e.g. for
                                                    // histo names
    ...
    for (int thisMatrix = 0; thisMatrix < 5; thisMatrix++) {

      {
        ...
        if (static_cast<detMatrix>(thisMatrix) != FullDetector)
          thisHistoName =
              _ClusterSizeHistoBaseName +
              projectionName.at(static_cast<projAxis>(thisProjection)) +
              "submatrix" +
        ...
        // set default title
        std::string Title =
            "Fitted cluster size in " +
            projectionName.at(static_cast<projAxis>(thisProjection)) + " " +
            detMatrixDescription.at(static_cast<detMatrix>(thisMatrix));
        ...
        if (isHistoManagerAvailable) {
          ...
          if (histoInfo) {
            ...
            if (histoInfo->_title != "")
              Title = histoInfo->_title;
          }
        }

        // create 'outer' map if not existing
        _ClusterSizeHistos.insert(
            make_pair(static_cast<projAxis>(thisProjection),
                      std::map<detMatrix, AIDA::IBaseHistogram *>()));
        ...
        if (static_cast<projAxis>(thisProjection) == projXY)
          thisHisto = dynamic_cast<AIDA::IBaseHistogram *>(
              AIDAProcessor::histogramFactory(this)->createHistogram2D(
                  thisHistoName.c_str(), NBinX, MinX, MaxX, NBinY, MinY, MaxY));
        else
          thisHisto = dynamic_cast<AIDA::IBaseHistogram *>(
              AIDAProcessor::histogramFactory(this)->createHistogram1D(
                  thisHistoName.c_str(), NBinX, MinX, MaxX));

        thisHisto->setTitle(Title);
        _ClusterSizeHistos.at(static_cast<projAxis>(thisProjection))
            .insert(make_pair(static_cast<detMatrix>(thisMatrix), thisHisto));
      } // cluster size histos

      {
        ...
        for (int thisClusterSize = 0; thisClusterSize <= HistoMaxClusterSize;
             thisClusterSize++) {
          ...
          if (thisClusterSize == 0) {
            ...
            thisHistoName =
                ...
                projectionName.at(static_cast<projAxis>(thisProjection)) +
                detMatrixName.at(static_cast<detMatrix>(thisMatrix)) +
                to_string(thisClusterSize);
          }

          // set default title
          std::string Title = "";
          if (thisClusterSize == 0) {
            Title = " Measured - fitted " +
                    projectionName.at(static_cast<projAxis>(thisProjection)) +
                    " position " +
                    detMatrixDescription.at(static_cast<detMatrix>(thisMatrix));
          } else {
            Title =
                ...
                " position " +
                detMatrixDescription.at(static_cast<detMatrix>(thisMatrix)) +
                "cl.sz. " + to_string(thisClusterSize);
          }
          ...
          _ShiftHistos.insert(make_pair(
              static_cast<projAxis>(thisProjection),
              std::map<detMatrix, std::map<int, AIDA::IBaseHistogram *>>()));
          _ShiftHistos.at(static_cast<projAxis>(thisProjection))
              .insert(make_pair(static_cast<detMatrix>(thisMatrix),
                                std::map<int, AIDA::IBaseHistogram *>()));

          AIDA::IBaseHistogram *thisHisto;
          if (static_cast<projAxis>(thisProjection) == projXY)
            thisHisto = dynamic_cast<AIDA::IBaseHistogram *>(
                AIDAProcessor::histogramFactory(this)->createHistogram2D(
                    thisHistoName.c_str(), NBinX, MinX, MaxX, NBinY, MinY,
                    MaxY));
          else
            thisHisto = dynamic_cast<AIDA::IBaseHistogram *>(
                AIDAProcessor::histogramFactory(this)->createHistogram1D(
          ...
          thisHisto->setTitle(Title);
          _ShiftHistos.at(static_cast<projAxis>(thisProjection))
              .at(static_cast<detMatrix>(thisMatrix))
              .insert(make_pair(thisClusterSize, thisHisto));
        } // cluster size
      }   // shift histos
...

    {
      ...
      // append [X, Y, XY]
      std::string thisHistoName = "";
      thisHistoName = _BgShiftHistoBaseName +
                      projectionName.at(static_cast<projAxis>(thisProjection));
      ...
      thisHisto->setTitle(Title);
      _BgShiftHistos.insert(
          make_pair(static_cast<projAxis>(thisProjection), thisHisto));
    ...
    {                     // Measured position
      ...
      // construct the name of the histogram we are looking for:
      std::string thisHistoName = "";
      thisHistoName = _MeasuredHistoBaseName +
                      projectionName.at(static_cast<projAxis>(thisProjection));

      // set default title
      std::string Title =
          "Measured particle position in " +
          projectionName.at(static_cast<projAxis>(thisProjection)) + "; " +
          projectionName.at(static_cast<projAxis>(thisProjection)) +
          " [mm];# of hits";
    ...
    {                     // matched position
      ...
      // construct the name of the histogram we are looking for:
      std::string thisHistoName = "";
      thisHistoName = _MatchedHistoBaseName +
                      projectionName.at(static_cast<projAxis>(thisProjection));

      // set default title
      std::string Title =
          "Matched particle position in " +
          projectionName.at(static_cast<projAxis>(thisProjection));
    ...
    {                     // unmatched position
      ...
      // construct the name of the histogram we are looking for:
      std::string thisHistoName = "";
      thisHistoName = _UnMatchedHistoBaseName +
                      projectionName.at(static_cast<projAxis>(thisProjection));

      // set default title
      std::string Title =
          "Unmatched particle position in " +
          projectionName.at(static_cast<projAxis>(thisProjection));
    ...
    {                     // fitted position
      ...
      // construct the name of the histogram we are looking for:
      std::string thisHistoName = "";
      thisHistoName = _FittedHistoBaseName +
                      projectionName.at(static_cast<projAxis>(thisProjection));

      // set default title
      std::string Title =
          "Fitted particle position in " +
          projectionName.at(static_cast<projAxis>(thisProjection));
    ...
    {                     // efficiency as fcn of fitted position
      ...
      // construct the name of the histogram we are looking for:
      std::string thisHistoName = "";
      thisHistoName = _EfficiencyHistoBaseName +
                      projectionName.at(static_cast<projAxis>(thisProjection));

      // set default title
      std::string Title =
          "Efficiency vs particle position in " +
          projectionName.at(static_cast<projAxis>(thisProjection));
    ...
    {                     // background match prob. as fcn of fitted position
      ...
      // construct the name of the histogram we are looking for:
      std::string thisHistoName = "";
      thisHistoName = _BgEfficiencyBaseHistoName +
                      projectionName.at(static_cast<projAxis>(thisProjection));

      // set default title
      std::string Title =
          "Bckgrd match prob vs part pos in " +
          projectionName.at(static_cast<projAxis>(thisProjection));
    ...
    {                     //  Noise as a function of the measured position
      ...
      // construct the name of the histogram we are looking for:
      std::string thisHistoName = "";
      thisHistoName = _NoiseHistoBaseName +
                      projectionName.at(static_cast<projAxis>(thisProjection));

      // set default title
      std::string Title =
          "Noise vs part pos in " +
          projectionName.at(static_cast<projAxis>(thisProjection));
  ...
  std::string shiftXTitle =
      "Measured - fitted X position vs Y;Y; #Delta X [mm]";
  ...
  if (isHistoManagerAvailable) {
    ...
    if (histoInfo) {
      ...
      if (histoInfo->_title != "")
        shiftXTitle = histoInfo->_title;
    }
  }

  _ShiftXvsYHisto = AIDAProcessor::histogramFactory(this)->createProfile1D(
      _ShiftXvsYHistoName.c_str(), shiftXNBin, shiftXMin, shiftXMax, shiftVMin,
      shiftVMax);
  ...
  _ShiftXvsXHisto = AIDAProcessor::histogramFactory(this)->createProfile1D(
      _ShiftXvsXHistoName.c_str(), shiftXNBin, shiftXMin, shiftXMax, shiftVMin,
      shiftVMax);
  ...
  shiftVMin = -1.0;
  shiftVMax = 1.0;
  std::string shiftYTitle =
      "Measured - fitted Y position vs X; X [mm]#Delta Y [mm];";
  ...
  if (isHistoManagerAvailable) {
    ...
    if (histoInfo) {
      ...
      shiftVMax = histoInfo->_yMax;
      if (histoInfo->_title != "")
        shiftYTitle = histoInfo->_title;
    }
  }

  _ShiftYvsXHisto = AIDAProcessor::histogramFactory(this)->createProfile1D(
      _ShiftYvsXHistoName.c_str(), shiftYNBin, shiftYMin, shiftYMax, shiftVMin,
      shiftVMax);
  ...
  _ShiftYvsYHisto = AIDAProcessor::histogramFactory(this)->createProfile1D(
      _ShiftYvsYHistoName.c_str(), shiftYNBin, shiftYMin, shiftYMax, shiftVMin,
      shiftVMax);
  ...
  _ShiftXvsY2DHisto = AIDAProcessor::histogramFactory(this)->createHistogram2D(
      _ShiftXvsY2DHistoName.c_str(), shiftXNBin, shiftXMin, shiftXMax,
      shiftVNBin, shiftVMin, shiftVMax);
  ...
  _ShiftXvsX2DHisto = AIDAProcessor::histogramFactory(this)->createHistogram2D(
      _ShiftXvsX2DHistoName.c_str(), shiftXNBin, shiftXMin, shiftXMax,
      shiftVNBin, shiftVMin, shiftVMax);
  ...
  _ShiftYvsX2DHisto = AIDAProcessor::histogramFactory(this)->createHistogram2D(
      _ShiftYvsX2DHistoName.c_str(), shiftYNBin, shiftYMin, shiftYMax,
      shiftVNBin, shiftVMin, shiftVMax);
  ...
  _ShiftYvsY2DHisto = AIDAProcessor::histogramFactory(this)->createHistogram2D(
      _ShiftYvsY2DHistoName.c_str(), shiftYNBin, shiftYMin, shiftYMax,
      shiftVNBin, shiftVMin, shiftVMax);
  ...
  double etaVMax = 0.03;
  string etaXTitle =
      "Measured - fitted X position vs local X; local X [mm]; #Delta X";
  ...
  if (isHistoManagerAvailable) {
    ...
    if (histoInfo) {
      ...
      if (histoInfo->_title != "")
        etaXTitle = histoInfo->_title;
    }
  }

  _EtaXHisto = AIDAProcessor::histogramFactory(this)->createProfile1D(
      _EtaXHistoName.c_str(), etaXNBin, etaXMin, etaXMax, etaVMin, etaVMax);
  ...
  etaVMax = 0.03;
  string etaYTitle =
      "Measured - fitted Y position vs local Y;  local Y [mm];#Delta Y [mm]";
  ...
  if (isHistoManagerAvailable) {
    ...
    if (histoInfo) {
      ...
      etaVMax = histoInfo->_yMax;
      if (histoInfo->_title != "")
        etaYTitle = histoInfo->_title;
    }
  }

  _EtaYHisto = AIDAProcessor::histogramFactory(this)->createProfile1D(
      _EtaYHistoName.c_str(), etaYNBin, etaYMin, etaYMax, etaVMin, etaVMax);
  ...
  etaVMax = 0.03;
  etaXTitle =
      "Measured - fitted X position vs local X; local x [mm]; #Delta X [mm]";
  ...
  _EtaX2DHisto = AIDAProcessor::histogramFactory(this)->createHistogram2D(
      _EtaX2DHistoName.c_str(), etaXNBin, etaXMin, etaXMax, etaVNBin, etaVMin,
      etaVMax);
  ...
  _EtaY2DHisto = AIDAProcessor::histogramFactory(this)->createHistogram2D(
      _EtaY2DHistoName.c_str(), etaYNBin, etaYMin, etaYMax, etaVNBin, etaVMin,
      etaVMax);
  ...
  etaVMax = 0.03;
  etaXTitle =
      "Measured - fitted X position vs local X-Y; XY [mm];#Delta X [mm]";
  ...
  _EtaX3DHisto = AIDAProcessor::histogramFactory(this)->createProfile2D(
      _EtaX3DHistoName.c_str(), etaXNBin, etaXMin, etaXMax, etaYNBin, etaYMin,
      etaYMax, etaVMin, etaVMax);
  ...
  etaVMax = 0.03;
  etaYTitle =
      "Measured - fitted Y position vs local X-Y; XY [mm];#Delta Y [mm]";
  ...
  _EtaY3DHisto = AIDAProcessor::histogramFactory(this)->createProfile2D(
      _EtaY3DHistoName.c_str(), etaXNBin, etaXMin, etaXMax, etaYNBin, etaYMin,
      etaYMax, etaVMin, etaVMax);
  ...
  if (isHistoManagerAvailable) {
    ...
    if (histoInfo) {
      ...
      if (histoInfo->_title != "")
        pixTitle = histoInfo->_title;
    }
  }

  _PixelEfficiencyHisto =
      AIDAProcessor::histogramFactory(this)->createProfile2D(
          _PixelEfficiencyHistoName.c_str(), pixYNBin, pixYMin, pixYMax,
          pixXNBin, pixXMin, pixXMax, pixVMin, pixVMax);
  ...
  _PixelResolutionXHisto =
      AIDAProcessor::histogramFactory(this)->createProfile2D(
          _PixelResolutionXHistoName.c_str(), pixYNBin, pixYMin, pixYMax,
          pixXNBin, pixXMin, pixXMax, pixVMin, pixVMax);
  ...
  _PixelResolutionYHisto =
      AIDAProcessor::histogramFactory(this)->createProfile2D(
          _PixelResolutionYHistoName.c_str(), pixYNBin, pixYMin, pixYMax,
          pixXNBin, pixXMin, pixXMax, pixVMin, pixVMax);
  ...
  _PixelChargeSharingHisto =
      AIDAProcessor::histogramFactory(this)->createProfile2D(
          _PixelChargeSharingHistoName.c_str(), pixYNBin, pixYMin, pixYMax,
          pixXNBin, pixXMin, pixXMax, pixVMin, pixVMax);
...
#else
  message<MESSAGE5>(
      log() << "No histogram produced because Marlin doesn't use AIDA");
#endif
...
int EUTelDUTHistograms::getClusterSize(int sensorID, TrackerHit *hit,
  ...
  try {
    ...
    if (hit->getType() == kEUTelBrickedClusterImpl) {
      ...
      cluster =
          new EUTelSparseClusterImpl<EUTelGenericSparsePixel>(clusterVector);
      ...
      SparsePixelType type = static_cast<SparsePixelType>(
          static_cast<int>(cellDecoder(clusterVector)["sparsePixelType"]));

      if (type == kEUTelGenericSparsePixel) {
        cluster = new EUTelGenericSparseClusterImpl<EUTelGenericSparsePixel>(
            clusterVector);
      } else if (type == kEUTelGeometricPixel) {
        cluster = new EUTelGenericSparseClusterImpl<EUTelGeometricPixel>(
            clusterVector);
      }
    }
...
int EUTelDUTHistograms::read_track_from_collections(LCEvent *event) {
  ...
  try {
    ...
    streamlog_out(DEBUG5) << "Not able to get collection " << _inputTrackColName
  ...
  message<DEBUG5>(log() << "Total of " << nTrack
                        << " tracks in input collection ");
  ...
  try {
    ...
    streamlog_out(DEBUG5) << "Not able to get collection "
                          << _inputFitHitColName << " from event "
  ...
  try {
    ...
    streamlog_out(DEBUG5) << "Not able to get collection "
                          << _inputRecHitColName << " from event "
                          << event->getEventNumber() << " in run "
                          << event->getRunNumber() << endl;
    return 2;
  }
  ...
  if (streamlog_level(DEBUG5)) {
    message<DEBUG5>(log() << "\n tracks " << nTracks << " \n hits " << nRecHits
                          << endl);
    message<DEBUG5>(log() << "Total of " << nTracks
                          << " (fit) hits in input collection ");
    message<DEBUG5>(log() << "Total of " << nRecHits
                          << " (rec) hits  in input collection ");
  }
  ...
  for (int itrack = 0; itrack < nTracks; itrack++) {
    ...
    TrackerHit *fithit =
        dynamic_cast<TrackerHit *>(fit__col->getElementAt(itrack));
...

    {
      ...
      if (hsensorID == _iDUT) // get all fitted hits on board
      {
        ...
        if (streamlog_level(DEBUG5)) {
          message<DEBUG5>(
              log() << "_fittedX element [" << _fittedX[_maptrackid].size() - 1
                    << "]" << _fittedX[_maptrackid][_fittedX.size() - 1] << " "
                    << _fittedY[_maptrackid][_fittedX.size() - 1] << " for DUT "
                    << hsensorID << endl);
        }
      }
    }
  ...
  if (streamlog_level(DEBUG5)) {
    for (int ii = 0; ii < _maptrackid; ii++) {
      message<MESSAGE5>(log() << "for _maptrackid=" << ii << " found fithits "
                              << _fittedX[ii].size() << endl);
      for (unsigned int jj = 0; jj < _fittedX[ii].size(); jj++) {
        message<MESSAGE5>(
            log() << "fit hits [" << jj << " of " << _fittedX[ii].size() << "] "
                  << _fittedX[ii][jj] << " " << _fittedY[ii][jj] << endl);
      }
    }
  }
  ...
  for (int ihit = 0; ihit < nRecHits; ihit++) {
    ...
    TrackerHit *meshit =
        dynamic_cast<TrackerHit *>(rec__col->getElementAt(ihit));
    ...
    if (hsensorID == _iDUT) {
      ...
      if (streamlog_level(DEBUG5)) {
        ...
        message<DEBUG5>(log() << "_measured element [" << _measuredX.size() - 1
                              << "]" << _measuredX[_measuredX.size() - 1] << " "
                              << _measuredY[_measuredX.size() - 1]
                              << " for DUT " << hsensorID << endl);
      }
    }
  }

  message<DEBUG5>(log() << "Total of " << _measuredX.size() << " hits at DUT "
                        << _iDUT << endl);
...
int EUTelDUTHistograms::read_track(LCEvent *event) {
...

    {
      ...
      for (int ihit = 0; ihit < nHit; ihit++) {
        ...
        if (((hitCellDecoder(meshit)["properties"] & kFittedHit) > 0) &&
            hsensorID == _iDUT) // get all fitted hits on board
        {
      ...
      for (int ihit = 0; ihit < nHit; ihit++) {
        ...
        if ((hitCellDecoder(meshit)["properties"] & kFittedHit) == 0) {
          ...
          int sizeY = -1;
          int subMatrix = -1;
          getClusterSize(hsensorID, static_cast<TrackerHitImpl *>(meshit),
                         sizeX, sizeY, subMatrix);
  ...
  try {
    ...
    streamlog_out(DEBUG5) << "Not able to get collection " << _inputHitColName
                          << " from event " << event->getEventNumber()
                          << " in run " << event->getRunNumber() << endl;
  }
  ...
  message<DEBUG5>(log() << "Total of " << nHit
                        << " tracker hits in input collection ");
  ...
  for (int ihit = 0; ihit < nHit; ihit++) {
    ...
    if (sensorID == _iDUT) // measured info only for DUT plane
    {
      ...
      int sizeY = -1;
      int subMatrix = -1;
      getClusterSize(sensorID, static_cast<TrackerHitImpl *>(meshit), sizeX,
                     sizeY, subMatrix);
      _clusterSizeX.push_back(sizeX);
      _clusterSizeY.push_back(sizeY);
      _subMatrix.push_back(subMatrix);
      ...
      double corrX = pos[0] * cos(_DUTalign.at(2)) +
                     pos[1] * sin(_DUTalign.at(2)) + _DUTalign.at(0);

      double corrY = pos[1] * cos(_DUTalign.at(2)) -
                     pos[0] * sin(_DUTalign.at(2)) + _DUTalign.at(1);
