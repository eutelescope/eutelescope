...
EUTelAutoPedestalNoiseProcessor::EUTelAutoPedestalNoiseProcessor()
  ...
  _description = "EUTelAutoPedestalNoiseProcessor produces initial pedestal / "
  ...
  registerOptionalParameter(
      "MinXVector",
      "The minimum pixel along x (default 0, one value per detector)", _minX,
      minXExample);

  IntVec maxXExample(nDetectorExample, 255);
  registerOptionalParameter(
      "MaxXVector",
      "The maximum pixel along x (default 255, one value per detector)", _maxX,
      maxXExample);

  IntVec minYExample(nDetectorExample, 0);
  registerOptionalParameter(
      "MinYVector",
      "The minimum pixel along y (default 0, one value per detector)", _minY,
      minYExample);

  IntVec maxYExample(nDetectorExample, 255);
  registerOptionalParameter(
      "MaxYVector",
      "The maximum pixel along y (default 255, one value per detector)", _maxY,
      maxYExample);
  ...
  registerOptionalParameter(
      "InitPedestalValue",
      "The initial value of pedestal (one value for detector)", _initPedestal,
      initPedeExample);

  FloatVec initNoiseExample(nDetectorExample, 1.);

  registerOptionalParameter(
      "InitNoiseValue", "The initial value of noise (one value for detector)",
...
void EUTelAutoPedestalNoiseProcessor::processEvent(LCEvent *event) {
  ...
  if (isFirstEvent()) {
    ...
    for (unsigned int iDetector = 0; iDetector < _initPedestal.size();
      ...
      CellIDEncoder<TrackerRawDataImpl> statusEncoder(
      ...
      CellIDEncoder<TrackerDataImpl> pedestalEncoder(
      ...
      CellIDEncoder<TrackerDataImpl> noiseEncoder(
