...
EUTelSparseClustering::EUTelSparseClustering ():Processor ("EUTelSparseClustering"), _zsDataCollectionName (""),
...
void
EUTelSparseClustering::processRunHeader (LCRunHeader * rdr)
{
...
void
EUTelSparseClustering::initializeGeometry (LCEvent * event)
{
  ...
  try
  {
    ...
    for (size_t icoll = 0; icoll < _zsInputDataCollectionVec->size ();
	 ++icoll)
      {
	auto data =
	  dynamic_cast <
	  TrackerDataImpl *
	  >(_zsInputDataCollectionVec->getElementAt (icoll));
...
void
EUTelSparseClustering::readCollections (LCEvent * event)
{
...
void
EUTelSparseClustering::processEvent (LCEvent * event)
{
...
void
EUTelSparseClustering::sparseClustering (LCEvent * evt,
					 LCCollectionVec * pulseCollection)
{
  ...
  for (size_t iDetector = 0; iDetector < _zsInputDataCollectionVec->size ();
       iDetector++)
    {
	...
	static_cast < SparsePixelType > (static_cast <
      ...
      for (size_t iExcluded = 0; iExcluded < _excludedPlanes.size ();
	   ++iExcluded)
	{
	...
	EUTelBaseSparsePixel const >>newlyAdded;
      ...
      while (!hitPixelVec.empty ())
	{
	  ...
	  auto sparseCluster =
	    Utility::getClusterData (zsCluster.get (), type);
	  ...
	  while (!newlyAdded.empty ())
	    {
	      ...
	      for (auto hitVec = hitPixelVec.begin ();
...
void
EUTelSparseClustering::fillHistos (LCEvent * evt)
{
...
void
EUTelSparseClustering::bookHistos ()
{
  ...
  for (size_t iDetector = 0; iDetector < _sensorIDVec.size (); iDetector++)
	...
	(geo::gGeometry ().getPixGeoDescr (sensorID));
      ...
      std::string histName_clusterSizeTotal =
	basePath + "/totalClusterSize_det" + std::to_string (sensorID);
      AIDA::IHistogram1D * hist1D_clusterSizeTotal =
	AIDAProcessor::
	histogramFactory (this)->createHistogram1D (histName_clusterSizeTotal,
						    61, -0.5, 60.5);
      hist1D_clusterSizeTotal->
	setTitle
	("Total cluster size (in hit pixels); #cluster pixel; count");
      _clusterSizeTotalHistos.
	insert (std::make_pair (sensorID, hist1D_clusterSizeTotal));

      //create 1D histogram: cluster signal
      std::string histName_clusterSignal =
	basePath + "/clusterSignal_det" + std::to_string (sensorID);
      AIDA::IHistogram1D * hist1D_clusterSignal =
	AIDAProcessor::
	histogramFactory (this)->createHistogram1D (histName_clusterSignal,
						    61, -0.5, 60.5);
      hist1D_clusterSignal->
	setTitle
	("Total signal per cluster (in detector specific charge unit); charge; count");
      _clusterSignalHistos.
	insert (std::make_pair (sensorID, hist1D_clusterSignal));

      //create 1D histogram: cluster size along X
      std::string histName_clusterSizeX =
	basePath + "/clusterSizeX_det" + std::to_string (sensorID);
      AIDA::IHistogram1D * hist1D_clusterSizeX =
	AIDAProcessor::
	histogramFactory (this)->createHistogram1D (histName_clusterSizeX, 21,
						    -0.5, 20.5);
      hist1D_clusterSizeX->
	setTitle
	("Cluster size in x-direction (in pixels); size [# of pixels]; count");
      _clusterSizeXHistos.
	insert (std::make_pair (sensorID, hist1D_clusterSizeX));

      //create 1D histogram: cluster size along Y
      std::string histName_clusterSizeY =
	basePath + "/clusterSizeY_det" + std::to_string (sensorID);
      AIDA::IHistogram1D * hist1D_clusterSizeY =
	AIDAProcessor::
	histogramFactory (this)->createHistogram1D (histName_clusterSizeY, 21,
						    -0.5, 20.5);
      hist1D_clusterSizeY->
	setTitle
	("Cluster size in y-direction (in pixels); size [# of pixels]; count");
      _clusterSizeYHistos.
	insert (std::make_pair (sensorID, hist1D_clusterSizeY));

      //create 2D map: hit map
      std::string histName_hitMap =
	basePath + "/hitMap_det" + std::to_string (sensorID);
      ...
      AIDA::IHistogram2D * hist2D_hitMap =
	AIDAProcessor::
	histogramFactory (this)->createHistogram2D (histName_hitMap, xBin,
						    xMin, xMax, yBin, yMin,
						    yMax);
      hist2D_hitMap->
	setTitle ("Pixel index hit map; x index; y index; count");
      ...
      std::string histName_eventMultiplicity =
	basePath + "/eventMultiplicity_det" + std::to_string (sensorID);
      AIDA::IHistogram1D * hist1D_eventMultiplicity =
	AIDAProcessor::
	histogramFactory (this)->createHistogram1D
	(histName_eventMultiplicity, 15, -0.5, 14.5);
      hist1D_eventMultiplicity->
	setTitle ("Event multiplicity; multiplicity; count");
      _eventMultiplicityHistos.
	insert (std::make_pair (sensorID, hist1D_eventMultiplicity));
    }
