...
EUTelEventViewer::EUTelEventViewer()
    : Processor("EUTelEventViewer"), _trackerHitCollectionNameVec(),
      _trackCollectionNameVec(), _alignmentCollectionName(""),
      _layerTrackerHit(-1), _layerTrack(-1), _waitForKeyboard(true),
      _autoForwardDelay(4.0), _detModel(99999) {
  ...
  registerInputCollections(
      LCIO::TRACKERHIT, "TrackerHitCollections", "Tracker hit collection names",
      _trackerHitCollectionNameVec, trackerHitCollectionNameVecExample);
  ...
  registerProcessorParameter("LayerTrackerHit", "Layer for Tracker Hits",
                             _layerTrackerHit, static_cast<int>(-1));

  registerProcessorParameter("LayerTracks", "Layer for Tracks", _layerTrack,
                             static_cast<int>(-1));
...
void EUTelEventViewer::processEvent(LCEvent *evt) {
  ...
  try {
    ...
    if (_detModel == -1) {
      ...
      streamlog_out(WARNING2) << "No alignment condition found, using default "
                                 "ideal geometry model (99999)"
                              << endl;
      tempDetModel = 99999;
    }
  }
  ...
  if (tempDetModel == -1) {
    ...
    const gear::SiPlanesParameters &siPlanesParameters =
        Global::GEAR->getSiPlanesParameters();
    const gear::SiPlanesLayerLayout &siPlanesLayerLayout =
        siPlanesParameters.getSiPlanesLayerLayout();
  ...
  if (_layerTrackerHit > 0) {
    for (unsigned int iCollection = 0;
         iCollection < _trackerHitCollectionNameVec.size(); iCollection++) {
      try {
        LCCollection *collection = evt->getCollection(
            _trackerHitCollectionNameVec[iCollection].c_str());
        for (int iHit = 0; iHit < collection->getNumberOfElements(); iHit++) {
          TrackerHitImpl *hit =
              dynamic_cast<TrackerHitImpl *>(collection->getElementAt(iHit));
        ...
        streamlog_out(WARNING2) << "No input collection ("
                                << _trackerHitCollectionNameVec[iCollection]
                                << " found on " << event->getEventNumber()
  ...
  if (_layerTrack >= 0) {
    for (unsigned int iCollection = 0;
         iCollection < _trackCollectionNameVec.size(); iCollection++) {
      try {
        LCCollection *collection =
            evt->getCollection(_trackCollectionNameVec[iCollection].c_str());
        for (int iTrack = 0; iTrack < collection->getNumberOfElements();
             iTrack++) {
          TrackImpl *track =
              dynamic_cast<TrackImpl *>(collection->getElementAt(iTrack));
            ...
            if ((hit = dynamic_cast<TrackerHitImpl *>(hitvec[iHit])) != 0x0) {
              ...
              if ((hitCellDecoder(hit)["properties"] & kFittedHit) > 0) {
                if (first) {
                  ...
                  ced_hit(xPrev, yPrev, zPrev,
                          _layerTrackerHit << CED_LAYER_SHIFT, 2, color);
                  ...
                  ced_hit(xNext, yNext, zNext,
                          _layerTrackerHit << CED_LAYER_SHIFT, 2, color);

                  ced_line(xPrev, yPrev, zPrev, xNext, yNext, zNext,
                           _layerTrack << CED_LAYER_SHIFT, 2, color);
