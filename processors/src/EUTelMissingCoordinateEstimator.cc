...
EUTelMissingCoordinateEstimator::EUTelMissingCoordinateEstimator()
    : Processor("EUTelMissingCoordinateEstimator"), _inputHitCollectionName(),
      _outputHitCollectionName(), _referencePlanes(), _dutPlanes(),
      _missingCoordinate(), _maxResidual(0), _iRun(0), _iEvt(0),
      _missingHitPos(0), _knownHitPos(0), _nDutHits(0), _nDutHitsCreated(0),
      _maxExpectedCreatedHitPerDUTHit(10), _numberOfCreatedHitPerDUTHit() {
  // modify processor description
  _description = "EUTelMissingCoordinateEstimator As the name suggest this "
                 "processor is finds the position of the missing coordinate on "
                 "your How it works is simple, it gets the hits from specified "
                 "two finds the closest hit pairs, make a straight line out of "
                 "it and the estimated position in one axis on your sensor you "
                 "want. No promises that this will work with tilted sensors "
                 "and/or with magnetic field. One needs to used this with "
                 "merged hits and after pre-alignment";

  registerInputCollection(LCIO::TRACKERHIT, "InputHitCollectionName",
                          "Input hit collection name. Hits should be in global "
                          "coordinates and pre-aligned",
                          _inputHitCollectionName, std::string(""));

  registerOutputCollection(LCIO::TRACKERHIT, "OutputHitCollectionName",
                           "Output hit collection name",
                           _outputHitCollectionName, std::string(""));

  registerProcessorParameter(
      "ReferencePlanes",
      "This is the list of sensorIDs that their hits will be used to estimate "
      "the missing coordinate on your DUT. You have to give exactly 2 "
      "sensorIDs. For better results use the ones that are closest to your DUT",
      _referencePlanes, EVENT::IntVec());

  registerProcessorParameter(
      "DUTPlanes", "This is the list of sensorIDs that missing coordinate of "
                   "their hits needs to be found. Notice that if the specified "
                   "coordinate already exists it will be overwritten",
      _dutPlanes, EVENT::IntVec());

  registerProcessorParameter("MissingCoordinate",
                             "The coordinate axis that needs to be estimated. "
                             "You have to set this to either X or Y.",
                             _missingCoordinate, string("X"));

  registerProcessorParameter(
      "MaxResidual", "This processor will look for a closest hits (in known "
                     "coordinate) to determine if the hits are correlated. The "
                     "hits will be considered as correlated if the residual is "
                     "smaller than MaxResidual",
      _maxResidual, float(0));
}
...
void EUTelMissingCoordinateEstimator::init() {
  ...
  if (_referencePlanes.size() != 2) {
    streamlog_out(ERROR4)
        << "ReferencePlanes has to contain exactly 2 sensorIDs!" << std::endl;
    exit(-1);
  }
  ...
  if (_missingCoordinate == string("X") || _missingCoordinate == string("Y")) {
    streamlog_out(DEBUG4) << "MissingCoordinate value set as: "
                          << _missingCoordinate << std::endl;
  } else {
    streamlog_out(ERROR4) << "MissingCoordinate value (" << _missingCoordinate
...
void EUTelMissingCoordinateEstimator::processEvent(LCEvent *event) {
  ...
  try {
    inputHitCollection = static_cast<LCCollectionVec *>(
  ...
  try {
    outputHitCollection = static_cast<LCCollectionVec *>(
  ...
  CellIDEncoder<TrackerHitImpl> outputCellIDEncoder(EUTELESCOPE::HITENCODING,
                                                    outputHitCollection);
  ...
  for (int iInputHits = 0;
       iInputHits < inputHitCollection->getNumberOfElements(); iInputHits++) {
    TrackerHitImpl *inputHit = dynamic_cast<TrackerHitImpl *>(
        inputHitCollection->getElementAt(iInputHits));
    ...
    if (sensorID == _referencePlanes[0])
      referencePlaneHits1.push_back(iInputHits);
    if (sensorID == _referencePlanes[1])
      referencePlaneHits2.push_back(iInputHits);
    ...
    // Store all telescope hits in the new collection, we will store DUT hits
    // after updating its position
    if (!isDUTHit) {
  ...
  // with countCreatedDutHits vector we will count how many hits we create out
  // of one DUT hit
  ...
  for (unsigned int iHitRefPlane1 = 0;
       iHitRefPlane1 < referencePlaneHits1.size(); iHitRefPlane1++) {
    TrackerHitImpl *refHit1 = dynamic_cast<TrackerHitImpl *>(
        inputHitCollection->getElementAt(referencePlaneHits1[iHitRefPlane1]));
    const double *refHit1Pos = refHit1->getPosition();

    // loop over second reference plane hits
    for (unsigned int iHitRefPlane2 = 0;
         iHitRefPlane2 < referencePlaneHits2.size(); iHitRefPlane2++) {
      TrackerHitImpl *refHit2 = dynamic_cast<TrackerHitImpl *>(
          inputHitCollection->getElementAt(referencePlaneHits2[iHitRefPlane2]));
      const double *refHit2Pos = refHit2->getPosition();

      // loop over second dut plane hits
      for (unsigned int iDutHit = 0; iDutHit < dutPlaneHits.size(); iDutHit++) {
        TrackerHitImpl *dutHit = dynamic_cast<TrackerHitImpl *>(
            inputHitCollection->getElementAt(dutPlaneHits[iDutHit]));
        ...
        // t = (z-z1)/(z2-z1)
        double t =
            (dutHitPos[2] - refHit1Pos[2]) / (refHit2Pos[2] - refHit1Pos[2]);

        // find the known coordinate value correcponds to that z on the line
        double knownHitPosOnLine =
            refHit1Pos[_knownHitPos] +
            (refHit2Pos[_knownHitPos] - refHit1Pos[_knownHitPos]) * t;
        ...
        if (fabs(knownHitPosOnLine - dutHitPos[_knownHitPos]) < _maxResidual) {
          ...
          double estimatedHitPos =
              refHit1Pos[_missingHitPos] +
              (refHit2Pos[_missingHitPos] - refHit1Pos[_missingHitPos]) * t;

          newDutHitPos[_missingHitPos] = estimatedHitPos;

          // now store new hit position in the TrackerHit, copy and store in the
          // collection
...
TrackerHitImpl *
EUTelMissingCoordinateEstimator::cloneHit(TrackerHitImpl *inputHit) {
...
void EUTelMissingCoordinateEstimator::end() {
  ...
  streamlog_out(MESSAGE4) << "Number of hits you had from all DUTs "
                          << _nDutHits << endl;
  streamlog_out(MESSAGE4)
      << "Number of hits created with the estimated missing coordinate "
      << _nDutHitsCreated << endl;
  for (unsigned int i = 0; i < _numberOfCreatedHitPerDUTHit.size(); i++) {
    ...
    streamlog_out(MESSAGE4) << " hits per DUT hit "
                            << _numberOfCreatedHitPerDUTHit[i] << " many times"
                            << endl;
  }
