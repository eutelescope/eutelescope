...
EUTelPreAligner::EUTelPreAligner ():Processor ("EUTelPreAligner")
{
  ...
  registerProcessorParameter ("RequiredEvents",
			      ...
			      _requiredEvents, 50000);

  registerOptionalParameter ("FixedPlane",
			     "SensorID of fixed plane", _fixedID, 0);
...
EUTelPreAligner::init ()
{
  ...
  for (size_t index = 0; index < _sensorIDVec.size (); index++)
	...
	insert (std::make_pair (sensorID, static_cast < int >(index)));

      if (sensorID != _fixedID)
	{
	  _preAligners.
	    push_back (PreAligner
...
void
EUTelPreAligner::processRunHeader (LCRunHeader * rdr)
{
...
void
EUTelPreAligner::processEvent (LCEvent * event)
{
  ...
  try
  {
    LCCollectionVec *inputCollectionVec =
      dynamic_cast <
      LCCollectionVec * >(evt->getCollection (_inputHitCollectionName));
	...
	TrackerHitImpl *refHit =
	  dynamic_cast <
	  TrackerHitImpl * >(inputCollectionVec->getElementAt (ref));
	...
	for (size_t iHit = 0; iHit < inputCollectionVec->size (); iHit++)
	  {
	    ...
	    TrackerHitImpl *hit =
	      dynamic_cast <
	      TrackerHitImpl * >(inputCollectionVec->getElementAt (iHit));
	    ...
	    if (!gotIt)
	      {
		streamlog_out (ERROR5) << "Mismatched hit at " << pos[2] <<
		  endl;
	      }
	  }			//[END] loop over other hits

	if (prealign.size () > static_cast <
	    unsigned int >(_minNumberOfCorrelatedHits)
  ...
  catch (DataNotAvailableException & e)
  {
    streamlog_out (WARNING2) << "No input collection " <<
      ...
      getEventNumber () << " in run " << event->getRunNumber () << std::endl;
  }
...
EUTelPreAligner::end ()
{
  ...
  for (size_t ii = 0; ii < _preAligners.size (); ii++)
	...
	find (_excludedPlanes.begin (), _excludedPlanes.end (), sensorID);
      std::vector < int >::iterator itXCoord =
	find (_excludedPlanesXCoord.begin (), _excludedPlanesXCoord.end (),
	      sensorID);
      std::vector < int >::iterator itYCoord =
	find (_excludedPlanesYCoord.begin (), _excludedPlanesYCoord.end (),
	      sensorID);
      ...
      if (it == _excludedPlanes.end ())
	{
	  if (itXCoord == _excludedPlanesXCoord.end ())
	    {
	      updatedXOff =
		geo::gGeometry ().getPlaneXPosition (sensorID) +
  ...
  //write updated GEAR file
    ...
    {
      marlin::StringParameters * MarlinStringParams =
	marlin::Global::parameters;
      std::string outputFilename =
	(MarlinStringParams->getStringVal ("GearXMLFile")).substr (0,
								   (MarlinStringParams->
								    getStringVal
								    ("GearXMLFile")).
								   size () -
	...
	<< outputFilename + "_pre.xml" << std::endl;
      geo::gGeometry ().writeGEARFile (outputFilename + _GEARFileSuffix +
				       ".xml");
...
EUTelPreAligner::bookHistos ()
{
  ...
  for (size_t i = 0; i < _sensorIDVec.size (); i++)
      ...
      if (sensorID == _fixedID)
	{
      ...
      std::vector < int >::iterator it =
	std::find (_excludedPlanes.begin (), _excludedPlanes.end (),
		   sensorID);
      if (it != _excludedPlanes.end ())
	{
	  continue;
      ...
      std::string histName_hitXCorr =
	"hitXCorr_det" + std::to_string (sensorID);
      AIDA::IHistogram1D * hist1D_hitXCorr =
	marlin::AIDAProcessor::histogramFactory (this)->
	createHistogram1D ((basePath + histName_hitXCorr).c_str (), 400, -20.,
			   20.);
      hist1D_hitXCorr->setTitle ("Hit correlation X (fixed to det " +
				 std::to_string (sensorID) +
				 "); X position [mm]; count");
      _hitXCorr.insert (std::make_pair (sensorID, hist1D_hitXCorr));

      //create 1D histogram: hit correlation Y
      std::string histName_hitYCorr =
	"hitYCorr_det" + std::to_string (sensorID);
      AIDA::IHistogram1D * hist1D_hitYCorr =
	marlin::AIDAProcessor::histogramFactory (this)->
	createHistogram1D ((basePath + histName_hitYCorr).c_str (), 400, -20.,
			   20.);
      hist1D_hitYCorr->setTitle ("Hit correlation Y (fixed to det " +
				 std::to_string (sensorID) +
				 "); Y position [mm]; count");
