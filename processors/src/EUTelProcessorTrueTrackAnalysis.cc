...
EUTelProcessorTrueTrackAnalysis::EUTelProcessorTrueTrackAnalysis()
    : Processor("EUTelProcessorTrueTrackAnalysis"), _trueHitCollectionName(""),
  ...
  registerInputCollection(LCIO::TRACKERHIT, "TrueTrackFitpointCollectionName",
                          "Input of true track fitpoints",
                          _trueFitpointCollectionName,
                          std::string("true_track_fitpoints"));

  registerInputCollection(
      LCIO::TRACKERHIT, "ReconstructedTrackFitpointCollectionName",
      "Input of reconstructed track fitpoints", _recoFitpointCollectionName,
...
void EUTelProcessorTrueTrackAnalysis::init() {
  ...
  for (size_t i = 0; i < _sensorIDVec.size(); i++) {
    ...
    _trueHitMap.insert(
        std::make_pair(_sensorIDVec[i], std::vector<const double *>()));
  }
...
void EUTelProcessorTrueTrackAnalysis::readCollections(LCEvent *event) {
  ...
  try {
    ...
    LCCollectionVec *trueHitCollectionVec = dynamic_cast<LCCollectionVec *>(
    ...
    for (int i = 0; i < trueHitCollectionVec->getNumberOfElements(); i++) {
      ...
      TrackerHitImpl *trueHit =
          dynamic_cast<TrackerHitImpl *>(trueHitCollectionVec->getElementAt(i));
  ...
  try {
    ...
    _trueFitpointCollectionVec = dynamic_cast<LCCollectionVec *>(
        event->getCollection(_trueFitpointCollectionName));
    streamlog_out(DEBUG4) << "_trueFitpointCollectionVec: "
                          << _trueFitpointCollectionName.c_str() << "found\n";
  } catch (lcio::DataNotAvailableException &e) {

    streamlog_out(WARNING4)
        << "_trueFitpointCollectionVec: " << _trueFitpointCollectionName.c_str()
        << " not found in event " << event->getEventNumber() << std::endl;
  ...
  try {
    ...
    _recoFitpointCollectionVec = dynamic_cast<LCCollectionVec *>(
        event->getCollection(_recoFitpointCollectionName));
    streamlog_out(DEBUG4) << "_recoFitpointCollectionVec: "
                          << _recoFitpointCollectionName.c_str() << "found\n";
  } catch (lcio::DataNotAvailableException &e) {

    streamlog_out(WARNING4)
        << "_recoFitpointCollectionVec: " << _recoFitpointCollectionName.c_str()
        << " not found in event " << event->getEventNumber() << std::endl;
...
int EUTelProcessorTrueTrackAnalysis::findPairIndex(
    const double *a, std::vector<const double *> vec) {
...
void EUTelProcessorTrueTrackAnalysis::fillHistos(LCEvent *event) {
  ...
  for (int i = 0; i < _recoFitpointCollectionVec->getNumberOfElements(); i++) {
    ...
    TrackerHitImpl *recoFitpoint = dynamic_cast<TrackerHitImpl *>(
        _recoFitpointCollectionVec->getElementAt(i));
  ...
  for (size_t i = 0; i < _sensorIDVec.size(); i++) {
    ...
    trueFitpointMap.insert(
        std::make_pair(_sensorIDVec[i], std::vector<const double *>()));
  }

  for (int i = 0; i < _trueFitpointCollectionVec->getNumberOfElements(); i++) {

    TrackerHitImpl *trueFitpoint = dynamic_cast<TrackerHitImpl *>(
        _trueFitpointCollectionVec->getElementAt(i));
  ...
  for (int i = 0; i < _recoFitpointCollectionVec->getNumberOfElements(); i++) {
    ...
    trueFitpointMap.at(sensorID).erase(trueFitpointMap.at(sensorID).begin() +
                                       pairIndex);
  }
  ...
  for (size_t i = 0; i < _sensorIDVec.size(); i++) {
    ...
    for (size_t j = 0; j < _1DHistos.size() / 2; j++) {
      ...
      std::string histoTitle = "difference in " + coordinate +
                               ...
                               "track;#Delta" +
                               coordinate + " /#mum;Entries";

      _1DHistos[j].insert(std::make_pair(
          sensorID,
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              (basePath + histoName).c_str(), histoNBin, histoMin, histoMax)));
    ...
    for (size_t j = 0; j < _2DHistos.size() / 2; j++) {
      ...
      double histoYMin = -50;
      double histoYMax = 50;
      std::string histoTitle = "difference in position between true simulated "
                               "hits and reconstructed fitpoints from "
                               "reconstructed tracks;#Deltax /#mum;#Deltay "
                               "/#mum;Entries";

      _2DHistos[j].insert(std::make_pair(
          sensorID, AIDAProcessor::histogramFactory(this)->createHistogram2D(
                        (basePath + histoName).c_str(), histoXNBin, histoXMin,
                        histoXMax, histoYNBin, histoYMin, histoYMax)));
    ...
    for (size_t j = 0; j < _1DProfileHistos.size() / 2; j++) {
      ...
      std::string histoName = "TrueHitProfile_" + coordinate_1 + "Resid_vs_" +
                              coordinate_2 + "Pos_d" + to_string(sensorID);
      ...
      _1DProfileHistos[j].insert(std::make_pair(
          sensorID, AIDAProcessor::histogramFactory(this)->createProfile1D(
                        (basePath + histoName).c_str(), profileXNBin,
                        profileXMin, profileXMax, profileYMin, profileYMax)));
    ...
    for (size_t j = _2DHistos.size() / 2; j < _2DHistos.size(); j++) {
      ...
      double histoYMin = -16;
      double histoYMax = 16;
      std::string histoTitle = "difference in position between true fitpoints "
    ...
    // set up 1D profile histograms
    for (size_t j = _1DProfileHistos.size() / 2; j < _1DProfileHistos.size();
         j++) {
      ...
      std::string histoName = "FitpointProfile_" + coordinate_1 + "Resid_vs_" +
                              coordinate_2 + "Pos_d" + to_string(sensorID);
