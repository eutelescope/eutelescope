...
EUTelPedeGEARGBL::EUTelPedeGEARGBL() : Processor("EUTelPedeGEARGBL") {

  // modify processor description
  _description = "EUTelPedeGEARGBL calls PEDE to process a MILLE binary file and "
...
void EUTelPedeGEARGBL::processRunHeader(LCRunHeader *rdr) {
...
void EUTelPedeGEARGBL::processEvent(LCEvent * /*event*/) { /*NOP NOP NOP*/ }

void EUTelPedeGEARGBL::end() {
    ...
    if (millepede.bad() || !millepede.is_open()) {
      ...
      while (!millepede.eof()) {
        ...
        if (goodLine) {
		  ...
		  Eigen::Vector3d offsetRotCorr = rotAlign*oldOffset;

          // Eigen::Vector3d newOffset = rotAlign*oldOffset;

		  std::cout << "offsetRotCorr: " << offsetRotCorr << std::endl;

          geo::gGeometry().alignGlobalPos(sensorID, offsetRotCorr[0] - xOff,
                                          			offsetRotCorr[1] - yOff ,
                                          			offsetRotCorr[2] - zOff);
