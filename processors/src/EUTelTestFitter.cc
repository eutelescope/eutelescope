...
EUTelTestFitter::EUTelTestFitter()
    : Processor("EUTelTestFitter"), _isFirstEvent(false),
      ...
      _OutputHitsInTrack(false), _SkipLayerIDs(), _PassiveLayerIDs(),
      _AlignLayerIDs(), _AlignLayerShiftX(), _AlignLayerShiftY(),
      ...
      _indexDUTneighbour(0), _zDUTneighbour(0.0), _siPlaneCenter(),
      _siPlaneNormal() {
  ...
  registerInputCollection(LCIO::TRACKERHIT, "InputCollectionName",
                          ...
                          _inputColName, std::string("meshit"));
  ...
  registerOutputCollection(
      LCIO::TRACKERHIT, "OutputHitCollectionName",
      "Collection name for fitted particle hits (positions)", _outputHitColName,
      string("testfithits"));
  ...
  registerProcessorParameter("alignmentCollectionNames",
                             "List of alignment collections which are neede to "
                             "get track position on a Sensor surface ",
                             _alignmentCollectionNames,
                             _alignmentCollectionExample);
  ...
  registerOptionalParameter("UseSlope", "Use expected track direction to "
                                        "constraint number of considered hit "
                                        "combinations (track preselection).",
                            _UseSlope, true);
  ...
  registerOptionalParameter(
      "SkipLayerIDs", "Ids of layers which should NOT be included in the fit",
  ...
  registerOptionalParameter(
      "AlignLayerIDs",
      "Ids of layers for which alignment corrections are given", _AlignLayerIDs,
      initLayerIDs);
  ...
  registerOptionalParameter("WindowMinX", "Lower window edge in X", _WindowMinX,
                            initLayerShift);

  registerOptionalParameter("WindowMaxX", "Upper window edge in X", _WindowMaxX,
                            initLayerShift);

  registerOptionalParameter("WindowMinY", "Lower window edge in Y", _WindowMinY,
                            initLayerShift);

  registerOptionalParameter("WindowMaxY", "Upper window edge in Y", _WindowMaxY,
                            initLayerShift);

  registerOptionalParameter(
      "MaskLayerIDs", "Ids of layers for which position masks are applied",
      _MaskLayerIDs, initLayerIDs);

  registerOptionalParameter("MaskMinX", "Lower mask edge in X", _MaskMinX,
                            initLayerShift);

  registerOptionalParameter("MaskMaxX", "Upper mask edge in X", _MaskMaxX,
                            initLayerShift);

  registerOptionalParameter("MaskMinY", "Lower mask edge in Y", _MaskMinY,
                            initLayerShift);

  registerOptionalParameter("MaskMaxY", "Upper mask edge in Y", _MaskMaxY,
                            initLayerShift);
...
// check if Marlin was built with GEAR support or not
  ...
  streamlog_out(ERROR2) << "Marlin was not built with GEAR support.\n"
                           "You need to install GEAR and recompile Marlin with "
                           "-DUSE_GEAR before continue."
                        << endl;
  ...
  streamlog_out(MESSAGE0) << "Reading telescope geometry description from GEAR "
                          << endl;
  ...
  if (!_SkipLayerIDs.empty()) {
    streamlog_out(MESSAGE0) << _SkipLayerIDs.size()
                            << " layers should be skipped" << endl;
  }

  if (!_PassiveLayerIDs.empty()) {
    streamlog_out(MESSAGE0) << _PassiveLayerIDs.size()
                            << " layers should be considered passive" << endl;
  }
  ...
  if (_siPlanesParameters->getSiPlanesType() ==
      _siPlanesParameters->TelescopeWithDUT) {
    _iDUT = _nTelPlanes;
    _nTelPlanes++;
  } else {
    _iDUT = -1;
  ...
  for (int iz = 0; iz < _nTelPlanes; iz++) {
    ...
    if (ipl != _iDUT) {
      ...
      _planeThickness[iz] = _siPlanesLayerLayout->getLayerThickness(ipl) +
                            _siPlanesLayerLayout->getSensitiveThickness(ipl);
  ...
  streamlog_out(MESSAGE2) << "Telescope configuration with " << _nTelPlanes
                          << " planes" << endl;
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    ...
    if (ipl == _iDUT) {
      ...
      if (_isActive[ipl]) {
        ...
        for (int wpl = 0; wpl < static_cast<int>(_planeWindowIDs[ipl].size());
             wpl++) {
          int iwin = _planeWindowIDs[ipl].at(wpl);
          ss << "\n accepted window: X = " << _WindowMinX.at(iwin) << " to "
             << _WindowMaxX.at(iwin) << " Y = " << _WindowMinY.at(iwin)
             << " to " << _WindowMaxY.at(iwin);
        }

        for (int mpl = 0; mpl < static_cast<int>(_planeMaskIDs[ipl].size());
             mpl++) {
          int imsk = _planeMaskIDs[ipl].at(mpl);
          ss << "\n imposed mask: X = " << _MaskMinX.at(imsk) << " to "
             << _MaskMaxX.at(imsk) << " Y = " << _MaskMinY.at(imsk) << " to "
             << _MaskMaxY.at(imsk);
        }
      }
  ...
  streamlog_out(MESSAGE2) << "Total of " << _nActivePlanes
                          << " active sensor planes " << endl;
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (ipl > 0) {
      _planeDist[ipl - 1] =
          1. / (_planePosition[ipl] - _planePosition[ipl - 1]);
    }

    _planeScatAngle[ipl] =
        0.0136 / _eBeam * sqrt(_planeThickness[ipl] / _planeX0[ipl]) *
    ...
    if (streamlog_level(DEBUG5)) {
      streamlog_out(DEBUG5) << "Scattering angle in plane " << ipl << ": "
                            << _planeScatAngle[ipl] << endl;
    }
  ...
  if (status) {
    streamlog_out(ERROR2) << "\n Fit in X with nominal geometry failed !?!"
                          << endl;
  }
  // Store fit matrix
  ...
  ss << endl
     << "Expected total scattering angle [mrad]: " << totalScatAngle * 1000.;
  ...
  if (status) {
    streamlog_out(ERROR2) << "\n Fit in Y with nominal geometry failed !?!"
                          << endl;
  }
  // Store fit matrix
  ...
  if (_UseSlope && _SlopeXLimit < 5. * totalScatAngle)
    streamlog_out(ERROR2)
        << "SlopeXLimit cut probably too tight! Check parameters!" << endl;

  if (_UseSlope && _SlopeYLimit < 5. * totalScatAngle)
    streamlog_out(ERROR2)
        << "SlopeXLimit cut probably too tight! Check parameters!" << endl;
  ...
  totalScatAngle =
      sqrt(totalScatAngle * totalScatAngle + _beamSpread * _beamSpread);

  if (_UseSlope &&
      _SlopeDistanceMax <
          5. * totalScatAngle *
              (_planePosition[_nTelPlanes - 1] - _planePosition[0]))
    streamlog_out(ERROR2)
        << "SlopeDistanceMax cut probably too tight! Check parameters!" << endl;
...
void EUTelTestFitter::processRunHeader(LCRunHeader *runHeader) {
  ...
  streamlog_out(MESSAGE4) << "Processing run header " << _nRun << ", run nr "
                          << runNr << endl;
  ...
  streamlog_out(MESSAGE0) << detectorName << " : " << detectorDescription
                          << endl;
...
void EUTelTestFitter::processEvent(LCEvent *event) {
...
#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
    (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[_nTrackHistoName]))
        ->fill(0);
#endif
  ...
  if (_isFirstEvent) {
    ...
    for (int iplane = 0; iplane < _siPlanesLayerLayout->getNLayers();
      ...
      try {
        ...
        gRotation[0] =
            _siPlanesLayerLayout->getLayerRotationXY(iplane); // Euler alpha ;
        gRotation[1] =
            _siPlanesLayerLayout->getLayerRotationZX(iplane); // Euler alpha ;
        gRotation[2] =
            _siPlanesLayerLayout->getLayerRotationZY(iplane); // Euler alpha ;
          ...
          try {
            ...
            for (size_t iPos = 0; iPos < alignmentCollectionVec->size();
              ...
              c = static_cast<EUTelAlignmentConstant *>(
                  alignmentCollectionVec->getElementAt(iPos));
            ...
            if (c->getSensorID() == sensorID) {
              ...
              _planePosition[sensorID] -=
                  c->getZOffset(); // this subtraction makes things inconsistent
                                   // with the previous planePosition, but
                                   // consistent with the hits, which are
                                   // aligned
              ...
              try {
                ...
                streamlog_out(MESSAGE5) << " no sensor rotation is given in "
                                           "the GEAR steering file, assume NONE"
                                        << endl;
              }
              ...
              streamlog_out(DEBUG5)
                  << "Wrong telescope plane, not applying offsets." << endl;
            }
          } catch (...) {
            streamlog_out(WARNING2) << "Collection "
  ...
  if (streamlog_level(DEBUG5)) {
    streamlog_out(DEBUG5) << "Total of " << nHit
                          << " tracker hits in input collection " << endl;
  }

#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
  (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[_nAllHitHistoName]))
      ->fill(nHit);
#endif

  if (nHit + _allowMissingHits < _nActivePlanes) {
    streamlog_out(DEBUG5) << "Not enough hits to perform the fit, exiting... "
                          << endl;
  ...
  for (int ihit = 0; ihit < nHit; ihit++) {
    ...
    SimTrackerHitImpl *simhit =
        dynamic_cast<SimTrackerHitImpl *>(col->getElementAt(ihit));
    ...
    //
    double distMin =
        10.; // was 1., changed to 10., to take rotated planes into account
    hitPlane[ihit] = -1;

    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      double dist = hitZ[ihit] - _planePosition[ipl];
      if (dist < 0)
        dist = -dist; // always positive defined !
    ...
    if (hitPlane[ihit] < 0) {
      if (_isActive[hitPlane[ihit]]) {
        streamlog_out(WARNING0)
            << "Reconstructed hit outside sensor planes z [mm] = " << hitZ[ihit]
            << endl;
      }
      continue;
    }
    ...
    for (int wpl = 0;
         wpl < static_cast<int>(_planeWindowIDs[hitPlane[ihit]].size());
         wpl++) {
      int iwin = _planeWindowIDs[hitPlane[ihit]].at(wpl);

      if (hitX[ihit] < _WindowMinX.at(iwin))
        hitcut = true;
      if (hitX[ihit] > _WindowMaxX.at(iwin))
        hitcut = true;
      if (hitY[ihit] < _WindowMinY.at(iwin))
        hitcut = true;
      if (hitY[ihit] > _WindowMaxY.at(iwin))
        hitcut = true;
    }
    ...
    for (int mpl = 0;
         mpl < static_cast<int>(_planeMaskIDs[hitPlane[ihit]].size()); mpl++) {
    ...
    if (streamlog_level(DEBUG5)) {
      streamlog_out(DEBUG5) << "Hit " << ihit << "   X = " << hitX[ihit]
                            << " +/- " << hitEx[ihit] << "   Y = " << hitY[ihit]
                            << " +/- " << hitEy[ihit] << "   Z = " << hitZ[ihit]
                            << " (plane " << hitPlane[ihit] << ")" << endl;
    }
  }

#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
  (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[_nAccHitHistoName]))
      ->fill(nGoodHit);
#endif
  ...
  CellIDEncoder<TrackerHitImpl> fitHitEncoder(EUTELESCOPE::HITENCODING,
                                              fitpointvec);
  CellIDEncoder<TrackerHitImpl> corrHitEncoder(EUTELESCOPE::HITENCODING,
                                               corrpointvec);
  ...
  if (streamlog_level(DEBUG5)) {
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl]) {
        stringstream ss;
        ss << "Plane " << ipl << "  " << _planeHits[ipl]
           << " hit(s), hit IDs :";

        for (int ihit = 0; ihit < static_cast<int>(planeHitID[ipl].size());
             ihit++) {
  ...
  if (nFiredPlanes + _allowMissingHits < _nActivePlanes) {
    if (streamlog_level(DEBUG5)) {
      streamlog_out(DEBUG5) << "Not enough planes hit to perform the fit "
                            << endl;
    }
  ...
  if (streamlog_level(DEBUG5)) {
    streamlog_out(DEBUG5) << nFiredPlanes
                          << " active sensor planes hit, checking " << nChoice
                          << " fit possibilities " << endl;
  }
  ...
  for (type_fitcount ichoice = nChoice - _planeMod[istart] - 1; ichoice >= 0;
       ichoice--) {
    int nChoiceFired = 0;
    double choiceChi2 = -1.;
    double trackChi2 = -1.;
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      ...
      if (_isActive[ipl]) {
        ...
        if (ihit < _planeHits[ipl]) {
          ...
          _planeEx[ipl] =
              (_useNominalResolution) ? _planeResolution[ipl] : hitEx[jhit];
          _planeEy[ipl] =
              (_useNominalResolution) ? _planeResolution[ipl] : hitEy[jhit];
          ...
          if (_UseSlope && ifirst >= 0 && firstHitMissed == 0) {
            double expX =
                _planeX[ifirst] +
                expTrackSlopeX * (_planePosition[ipl] - _planePosition[ifirst]);
            double expY =
                _planeY[ifirst] +
                expTrackSlopeY * (_planePosition[ipl] - _planePosition[ifirst]);
          ...
          if (_UseSlope && ifirst >= 0 && firstTrackSlope == 0) {
            ...
            if (ilast > ifirst && (abs(slopeX - lastSlopeX) > _SlopeXLimit ||
    ...
    if (_useNominalResolution && (nChoiceFired == _nActivePlanes)) {
      ...
      if (_useNominalResolution && _beamSlopeX == _beamSlopeY)
    ...
    if (choiceChi2 < 0.) {
      streamlog_out(WARNING2)
          << "Fit to " << nChoiceFired << " planes failed for event "
          << event->getEventNumber() << " in run " << event->getRunNumber()
          << endl;
    ...
    if (nChoiceFired + _allowMissingHits >= _nActivePlanes &&
        nChoiceFired + _allowSkipHits >= nFiredPlanes && trackChi2 < chi2min) {
      chi2min = trackChi2;
    }
    ...
    if (trackChi2 < _chi2Max && trackChi2 > _chi2Min) {
      ...
      for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
        ...
        if (jhit >= 0) {
          ...
          // Resids
          _aidaHistoMap2D[bname + "residualXdX"]->fill(_fitX[ipl],
                                                       _fitX[ipl] - hitX[jhit]);
          _aidaHistoMap2D[bname + "residualYdX"]->fill(_fitX[ipl],
                                                       _fitY[ipl] - hitY[jhit]);
          _aidaHistoMap2D[bname + "residualXdY"]->fill(_fitY[ipl],
                                                       _fitX[ipl] - hitX[jhit]);
          _aidaHistoMap2D[bname + "residualYdY"]->fill(_fitY[ipl],
                                                       _fitY[ipl] - hitY[jhit]);
          // Hit Maps
...
// End of loop over track possibilities
...
#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
  (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[_firstChi2HistoName]))
      ->fill(log10(chi2min));
#endif

  if (nFittedTracks == 0) {
    if (streamlog_level(DEBUG5)) {
      streamlog_out(DEBUG5) << "No track fulfilling search criteria found ! "
                            << endl;
    }
  ...
  for (fitIterator = fittedChi2.begin(); fitIterator != fittedChi2.end();
       fitIterator++) {
    ...
    if (itrk == 0 && _searchMultipleTracks) {
      (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[_bestChi2HistoName]))
          ->fill(log10(choiceChi2));
      (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[_nBestHistoName]))
          ->fill(nChoiceFired);
    }
#endif
    ...
    for (fitIterator2 = fittedChi2.begin(); fitIterator2 != fitIterator;
         fitIterator2++) {
      int ifit2 = fitIterator2->second;
      int nAmbiguous = 0;

      for (int ipl = 0; ipl < _nTelPlanes; ipl++)
        if (fittedHits[_nTelPlanes * ifit + ipl] ==
                fittedHits[_nTelPlanes * ifit2 + ipl] &&
            fittedHits[_nTelPlanes * ifit + ipl] >= 0)
          nAmbiguous++;

      if ((_allowAmbiguousHits && nAmbiguous > _maximumAmbiguousHits) ||
    ...
    if (streamlog_level(DEBUG5)) {
      streamlog_out(DEBUG5) << "Track reconstructed from " << nChoiceFired
                            << " hits: " << endl;
      ...
      for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
        int jhit = fittedHits[_nTelPlanes * ifit + ipl];
        if (jhit >= 0)
          streamlog_out(DEBUG5) << "Hit " << jhit << "   X = " << hitX[jhit]
                                << "   Y = " << hitY[jhit]
                                << "   Z = " << hitZ[jhit] << " (plane"
                                << hitPlane[jhit] << ")" << endl;
      }
      ...
      streamlog_out(DEBUG5) << " Fit chi2 = " << choiceChi2
                            << " including penalties of " << penalty << endl;
    }

#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
    // Fill Chi2 histograms

    (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[_linChi2HistoName]))
        ->fill(choiceChi2);

    (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[_logChi2HistoName]))
        ->fill(log10(choiceChi2));

    if (_allowMissingHits && nChoiceFired == _nActivePlanes) {
      (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[_fullChi2HistoName]))
          ->fill(log10(choiceChi2));
    }

    // Fill hit histograms

    (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[_nHitHistoName]))
        ->fill(nChoiceFired);
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      ...
      /*if(!_isActive[ipl]){
              ...
              slopevec[0] =
      fittedX[_nTelPlanes*ifit+(ipl+1)]-fittedX[_nTelPlanes*ifit+(ipl-1)];
              slopevec[1] =
      fittedY[_nTelPlanes*ifit+(ipl+1)]-fittedY[_nTelPlanes*ifit+(ipl-1)];
              slopevec[2] = _siPlaneCenter[ipl+1][2]-_siPlaneCenter[ipl-1][2];
      }//this gives the possibility of inserting a proper slope*/

      getImpactPoint(pos[0], pos[1], pos[2], slopevec[0], slopevec[1],
                     slopevec[2]);
      // getFastTrackImpactPoint(pos[0], pos[1], pos[2], 0, 0); // old function,
      // does the same as new one, new one is numerically more stable
      ...
      cov[0] = fittedEx[_nTelPlanes * ifit + ipl] *
               fittedEx[_nTelPlanes * ifit + ipl];
      cov[1] = 0.;
      cov[2] = fittedEy[_nTelPlanes * ifit + ipl] *
               fittedEy[_nTelPlanes * ifit + ipl];
      cov[3] = cov[4] = 0.;
      ...
      if (_InputHitsInTrack && _isActive[ipl]) {
        ...
        if (jhit >= 0) {
          TrackerHitImpl *meshit =
              dynamic_cast<TrackerHitImpl *>(col->getElementAt(jhit));
          SimTrackerHitImpl *simhit =
              dynamic_cast<SimTrackerHitImpl *>(col->getElementAt(jhit));
          ...
          if (meshit != nullptr) {
            ...
            // set the local/global and "fittedhit" bit flag properties for the
            // hit
    ...
    for (int ihit = 0; ihit < nHit; ihit++) {
      if (_isActive[hitPlane[ihit]]) {
        ...
        (dynamic_cast<AIDA::IHistogram1D *>(
             _aidaHistoMap[_hitAmbiguityHistoName]))
            ->fill(hitFits[ihit]);
#endif
      }
    }
...
void EUTelTestFitter::end() {
  ...
  // Print the summary
  streamlog_out(MESSAGE5) << "Total number of processed events:     "
                          << setw(10) << setiosflags(ios::right) << _nEvt
                          << resetiosflags(ios::right) << endl
                          << "Total number of events w/o input hit: "
                          << setw(10) << setiosflags(ios::right)
                          << _noOfEventWOInputHit << resetiosflags(ios::right)
                          << endl
                          << "Total number of events w/o track:     "
                          << setw(10) << setiosflags(ios::right)
                          << _noOfEventWOTrack << resetiosflags(ios::right)
                          << endl
                          << "Total number of reconstructed tracks  "
                          << setw(10) << setiosflags(ios::right) << _noOfTracks
                          << resetiosflags(ios::right) << endl;
...
void EUTelTestFitter::bookHistos() {
  ...
  streamlog_out(MESSAGE2) << "Histogram information searched in "
                          << _histoInfoFileName << endl;
  ...
  AIDA::IHistogram1D *linChi2Histo =
      AIDAProcessor::histogramFactory(this)->createHistogram1D(
          _linChi2HistoName.c_str(), chi2NBin, chi2Min, chi2Max);
  ...
  AIDA::IHistogram1D *logChi2Histo =
      AIDAProcessor::histogramFactory(this)->createHistogram1D(
          _logChi2HistoName.c_str(), chi2NBin, chi2Min, chi2Max);
  ...
  AIDA::IHistogram1D *firstChi2Histo =
      AIDAProcessor::histogramFactory(this)->createHistogram1D(
          _firstChi2HistoName.c_str(), chi2NBin, chi2Min, chi2Max);
  ...
  for (int iz = 0; iz < _nTelPlanes; iz++) {
    ...
    float limitZr = 50.0;
    // Resids
    _aidaHistoMap1D[bname + "fitX"] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            bname + "fitX", limitXN, -limitX, limitX);
    _aidaHistoMap1D[bname + "fitY"] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            bname + "fitY", limitYN, -limitY, limitY);
    _aidaHistoMap1D[bname + "hitX"] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            bname + "hitX", limitXN, -limitX, limitX);
    _aidaHistoMap1D[bname + "hitY"] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            bname + "hitY", limitYN, -limitY, limitY);
    _aidaHistoMap1D[bname + "residualX"] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            bname + "residualX", limitXN, -limitXr, limitXr);
    _aidaHistoMap1D[bname + "residualY"] =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            bname + "residualY", limitYN, -limitYr, limitYr);
    // Resids 2D
    _aidaHistoMap2D[bname + "residualXdX"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residualXdX", limitXN, -limitX, limitX, limitXN, -limitXr,
            limitXr);
    _aidaHistoMap2D[bname + "residualYdX"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residualYdX", limitXN, -limitX, limitX, limitYN, -limitYr,
            limitYr);
    _aidaHistoMap2D[bname + "residualXdY"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residualXdY", limitYN, -limitY, limitY, limitXN, -limitXr,
            limitXr);
    _aidaHistoMap2D[bname + "residualYdY"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residualYdY", limitYN, -limitY, limitY, limitYN, -limitYr,
            limitYr);

    _aidaHistoMap2D[bname + "residualdZvsX"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residualdZvsX", limitXN, -limitX, limitX, limitZN,
            -limitZr, limitZr);
    _aidaHistoMap2D[bname + "residualdZvsY"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residualdZvsY", limitYN, -limitY, limitY, limitZN,
            -limitZr, limitZr);
    _aidaHistoMap2D[bname + "residualmeasZvsmeasX"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residualmeasZvsmeasX", limitXN, -limitX, limitX, limitZN,
            -limitZr, limitZr);
    _aidaHistoMap2D[bname + "residualmeasZvsmeasY"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residualmeasZvsmeasY", limitYN, -limitY, limitY, limitZN,
            -limitZr, limitZr);
    _aidaHistoMap2D[bname + "residualfitZvsmeasX"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residualfitZvsmeasX", limitXN, -limitX, limitX, limitZN,
            -limitZr, limitZr);
    _aidaHistoMap2D[bname + "residualfitZvsmeasY"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "residualfitZvsmeasY", limitYN, -limitY, limitY, limitZN,
            -limitZr, limitZr);
    _aidaHistoMap2D[bname + "hitMapHITS"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "hitMapHITS", 100, -17., 17., 100, -10., 10.);
    _aidaHistoMap2D[bname + "hitMapTRACKS"] =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            bname + "hitMapTRACKS", 100, -17., 17., 100, -10., 10.);
  }
  ...
  if (_searchMultipleTracks) {
    ...
    AIDA::IHistogram1D *bestChi2Histo =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            _bestChi2HistoName.c_str(), chi2NBin, chi2Min, chi2Max);
  ...
  if (_allowMissingHits) {
    ...
    AIDA::IHistogram1D *fullChi2Histo =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            _fullChi2HistoName.c_str(), chi2NBin, chi2Min, chi2Max);
  ...
  AIDA::IHistogram1D *nTrackHisto =
      AIDAProcessor::histogramFactory(this)->createHistogram1D(
          _nTrackHistoName.c_str(), trkNBin, trkMin, trkMax);
  ...
  AIDA::IHistogram1D *nAllHitHisto =
      AIDAProcessor::histogramFactory(this)->createHistogram1D(
          _nAllHitHistoName.c_str(), hitNBin, hitMin, hitMax);
  ...
  AIDA::IHistogram1D *nAccHitHisto =
      AIDAProcessor::histogramFactory(this)->createHistogram1D(
          _nAccHitHistoName.c_str(), hitNBin, hitMin, hitMax);
  ...
  AIDA::IHistogram1D *nHitHisto =
      AIDAProcessor::histogramFactory(this)->createHistogram1D(
          _nHitHistoName.c_str(), hitNBin, hitMin, hitMax);
  nHitHisto->setTitle(hitTitle.c_str());
  _aidaHistoMap.insert(make_pair(_nHitHistoName, nHitHisto));

  // Additional hit number histogram for best tracks in an event - use same
  // binning
  if (_searchMultipleTracks) {
    string bestTitle = hitTitle + ", best fit";
    AIDA::IHistogram1D *BestHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            _nBestHistoName.c_str(), hitNBin, hitMin, hitMax);
    BestHisto->setTitle(bestTitle.c_str());
    _aidaHistoMap.insert(make_pair(_nBestHistoName, BestHisto));
  }

  // Additional histogram for number of tracks fitted to given hit - use same
  // binning
  if (_allowAmbiguousHits) {
    string ambigTitle = "Number of tracks containing given hit (ambiguity)";
    AIDA::IHistogram1D *AmbigHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            _hitAmbiguityHistoName.c_str(), hitNBin, hitMin, hitMax);
  ...
  for (mapIter = _aidaHistoMap.begin(); mapIter != _aidaHistoMap.end();
       mapIter++)
    streamlog_out(DEBUG5) << mapIter->first << " : "
                          << (mapIter->second)->title() << endl;
  ...
  streamlog_out(MESSAGE4)
      << "No histogram produced because Marlin doesn't use AIDA" << endl;
...
double EUTelTestFitter::SingleFit() {
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    ...
    for (int jpl = 0; jpl < _nTelPlanes; jpl++)
      if (_planeEy[jpl] > 0.)
        _fitY[ipl] += _fitArray[ipl + jpl * _nTelPlanes] * _planeY[jpl] /
                      _planeEy[jpl] / _planeEy[jpl];
  }
...
double EUTelTestFitter::NominalFit() {
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    ...
    for (int jpl = 0; jpl < _nTelPlanes; jpl++) {
      if (_planeEx[jpl] > 0.)
        _fitX[ipl] += _nominalFitArrayX[ipl + jpl * _nTelPlanes] *
                      _planeX[jpl] / _planeEx[jpl] / _planeEx[jpl];
      if (_planeEy[jpl] > 0.)
        _fitY[ipl] += _nominalFitArrayY[ipl + jpl * _nTelPlanes] *
                      _planeY[jpl] / _planeEy[jpl] / _planeEy[jpl];
    }
    ...
    if (_useBeamConstraint && _beamSlopeX != 0.) {
      _fitX[ipl] -=
          _nominalFitArrayX[ipl] * _beamSlopeX * _planeDist[0] * _planeScat[0];
      _fitX[ipl] += _nominalFitArrayX[ipl + _nTelPlanes] * _beamSlopeX *
                    _planeDist[0] * _planeScat[0];
    }

    if (_useBeamConstraint && _beamSlopeY != 0.) {
      _fitY[ipl] -=
          _nominalFitArrayY[ipl] * _beamSlopeY * _planeDist[0] * _planeScat[0];
      _fitY[ipl] += _nominalFitArrayY[ipl + _nTelPlanes] * _beamSlopeY *
                    _planeDist[0] * _planeScat[0];
    }
  }
...
int EUTelTestFitter::DoAnalFit(double *pos, double *err, double slope) {
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    for (int jpl = 0; jpl < _nTelPlanes; jpl++) {
      ...
      if (jpl == ipl - 2)
        _fitArray[imx] +=
            _planeDist[ipl - 2] * _planeDist[ipl - 1] * _planeScat[ipl - 1];

      if (jpl == ipl + 2)
        _fitArray[imx] +=
            _planeDist[ipl] * _planeDist[ipl + 1] * _planeScat[ipl + 1];

      if (jpl == ipl - 1) {
        if (ipl > 0 && ipl < _nTelPlanes - 1)
          _fitArray[imx] -= _planeDist[ipl - 1] *
                            (_planeDist[ipl] + _planeDist[ipl - 1]) *
                            _planeScat[ipl];
        if (ipl > 1)
          _fitArray[imx] -= _planeDist[ipl - 1] *
                            (_planeDist[ipl - 1] + _planeDist[ipl - 2]) *
                            _planeScat[ipl - 1];
      }

      if (jpl == ipl + 1) {
        if (ipl > 0 && ipl < _nTelPlanes - 1)
          _fitArray[imx] -= _planeDist[ipl] *
                            (_planeDist[ipl] + _planeDist[ipl - 1]) *
                            _planeScat[ipl];
        if (ipl < _nTelPlanes - 2)
          _fitArray[imx] -= _planeDist[ipl] *
                            (_planeDist[ipl + 1] + _planeDist[ipl]) *
                            _planeScat[ipl + 1];
      }
      ...
      if (jpl == ipl) {
        ...
        if (ipl > 0 && ipl < _nTelPlanes - 1)
          _fitArray[imx] += _planeScat[ipl] *
                            (_planeDist[ipl] + _planeDist[ipl - 1]) *
                            (_planeDist[ipl] + _planeDist[ipl - 1]);

        if (ipl > 1)
          _fitArray[imx] +=
              _planeScat[ipl - 1] * _planeDist[ipl - 1] * _planeDist[ipl - 1];

        if (ipl < _nTelPlanes - 2)
          _fitArray[imx] +=
              _planeScat[ipl + 1] * _planeDist[ipl] * _planeDist[ipl];
      }
...
double EUTelTestFitter::GetFitChi2() {
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++)
    if (_isActive[ipl]) {
      if (_planeEx[ipl] > 0.)
        chi2 += (_fitX[ipl] - _planeX[ipl]) * (_fitX[ipl] - _planeX[ipl]) /
                _planeEx[ipl] / _planeEx[ipl];

      if (_planeEy[ipl] > 0.)
        chi2 += (_fitY[ipl] - _planeY[ipl]) * (_fitY[ipl] - _planeY[ipl]) /
                _planeEy[ipl] / _planeEy[ipl];
    }
...
void EUTelTestFitter::getImpactPoint(double &x, double &y, double &z,
  ...
  //
  // working principle: 1.) take the impact point extrapolated to the
  // perpendicular plane
  // 			  2.) build a line, starting from the extrapolated impact point
  // going in the direction of the slope (in the simplest case, there is no
  // slope, i.e. slope=z-direction
  // 			  3.) calculate the intersection point of the line and the rotated
  // plane, by using the normal form of the plane and by using scalar products
  //
  // mathematically: plane: x.n=d, where n - normal vector, x - vector in the
  // plane, d - closest distance of plane to origin i.e. d=r0.n with r0 the
  // vector to the center of the plane (here: .=dot product)
  // 		       line: l(alpha)=x0+alpha*l0, where x0 - starting point of line,
  // alpha - a real number parametrizing the line, l0 - direction or slope of
  // the line (here: *=scalar multiplication)
  // 		       input line into plane (substitute x by l(alpha)) and slove for
  // alpha:
  // 		       		x0.n+alpha*l0.n=r0.n => (r0-x0).n/(l0.n)=alpha (.=dot
  // product, *=scalar multiplication)
  // 		       put alpha back into line, to get the real impact point
  // 		       		p=x0+(r0-x0).n/(l0.n)*l0 (.=dot product, *=scalar
  // multiplication)
  //
  ...
  // cerr << "Plane number" << sensorID  << ". The old coordinates are: x=" << x
  // << ", y=" << y << ", z=" << z << ". "; // debug output

  // this is a normal vector to the plane
  TVector3 NormalVector(_siPlaneNormal[sensorID][0],
                        _siPlaneNormal[sensorID][1],
                        _siPlaneNormal[sensorID][2]);
  // this is a vector to the center of the plane
  TVector3 r0Vector(_siPlaneCenter[sensorID][0], _siPlaneCenter[sensorID][1],
                    _siPlaneCenter[sensorID][2]);
  ...
  // this solves the equation for the actual impact point, using the normal
  // vector to parametrise the plane and parametrises the track by a line
  trackImpact = x0Vector +
                NormalVector.Dot(r0Vector - x0Vector) /
                    NormalVector.Dot(slopeVec) * slopeVec;

  x = trackImpact(0);
  y = trackImpact(1);
  z = trackImpact(2);

  // cerr << "The new coordinates are: x=" << x << ", y=" << y << ", z=" << z
  // <<endl; // debug output
}
...
void EUTelTestFitter::getFastTrackImpactPoint(double &x, double &y, double &z,
  ...
  b[0] = r0Vector(0) * NormalVector(0) + r0Vector(1) * NormalVector(1) +
         r0Vector(2) * NormalVector(2);
  b[1] = offsetX;
  b[2] = offsetY;
...
void EUTelTestFitter::getTrackImpactPoint(double &x, double &y, double &z,
                                          Track *tr, LCEvent *ev) {
  ...
  if (!(foundHitDUT && foundHitNeighbour)) {
    cout << "Was not possible to find hits at dut and next-to-dut layers. "
            "Terminating."
         << endl;
    abort();
  }
  ...
  (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap["TrackSlopeX"]))
      ->fill(slopeX);
  (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap["TrackOffsetX"]))
      ->fill(offsetX);
  (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap["TrackSlopeY"]))
      ->fill(slopeY);
  (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap["TrackOffsetY"]))
      ->fill(offsetY);
  ...
  double z_sensor =
      _siPlanesLayerLayout->getSensitivePositionZ(_indexDUT) +
      0.5 * (_siPlanesLayerLayout->getLayerThickness(_indexDUT) +
             _siPlanesLayerLayout->getSensitiveThickness(_indexDUT));
...
int EUTelTestFitter::guessSensorID(Track * /*track*/) {
  ...
  streamlog_out(ERROR5) << " guessSensorID(Track *) called but no longer "
                           "implemented! Please use guessSensorID( double & x, "
                           "double & y, double & z) instead. "
                        << endl;
...
int EUTelTestFitter::guessSensorID(double *hit) {
  ...
  double minDistance = numeric_limits<double>::max();
    ...
    for (int iPlane = 0; iPlane < _siPlanesLayerLayout->getNLayers();
         ++iPlane) {
      double distance =
          std::abs(hit[2] - _siPlanesLayerLayout->getLayerPositionZ(iPlane));
      if (distance < minDistance) {
        minDistance = distance;
    ...
    if (minDistance > 30) {
      // advice the user that the guessing wasn't successful
      streamlog_out(WARNING3) << "A hit was found " << minDistance
                              << " mm far from the nearest plane\n"
                                 "Please check the consistency of the data "
                                 "with the GEAR file: hitPosition[2]="
                              << hit[2] << endl;
    }
