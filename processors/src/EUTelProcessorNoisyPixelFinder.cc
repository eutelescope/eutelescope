...
namespace eutelescope {
  ...
  EUTelProcessorNoisyPixelFinder::EUTelProcessorNoisyPixelFinder()
      : Processor("EUTelProcessorNoisyPixelFinder"), _zsDataCollectionName(""),
    ...
    _description = "EUTelProcessorNoisyPixelFinder computes the firing "
  ...
  void EUTelProcessorNoisyPixelFinder::noisyPixelFinder(EUTelEventImpl *evt) {
    ...
    try {
      ...
      for (size_t iDetector = 0; iDetector < zsInputCollectionVec->size();
        ...
        // loop over all pixels in the sparseData object, these are the hit
        // pixels!
  ...
  void EUTelProcessorNoisyPixelFinder::end() {
    if (_finished) {
      ...
      streamlog_out(ERROR3)
          ...
          << std::endl;
      streamlog_out(ERROR3)
          << "Increase the number or events to be processed for the run or "
  ...
  void EUTelProcessorNoisyPixelFinder::check(LCEvent * /*event*/) {
    ...
    // the data
    // since check() runs after the event and we increment the event counter
    // before that
    // the comparison is valid. If we set _noOfEvents=1 we only want to have
    // processed event
    // 0 before calling this. Since we increment 0++ before calling this
    // function, this
    ...
    if (_iEvt == _noOfEvents) {
      ...
      for (auto &thisSensor : _sensorMap) {
        ...
        streamlog_out(MESSAGE3)
            ...
            << " (max. fire freq set to: " << _maxAllowedFiringFreq << ")"
            << std::endl;
        streamlog_out(MESSAGE3) << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
                                   "~~~~~~~~~~~~~~~~~~~~~~~"
                                << std::endl;
  ...
  void EUTelProcessorNoisyPixelFinder::noisyPixelDBWriter() {
    ...
    try {
      ...
      streamlog_out(DEBUG5) << "otPixelDB file: run header and event created ok"
                            << std::endl;
      LCTime now;
    ...
    try {
      ...
      std::cout << "noisyPixelCollection: " << _noisyPixelCollectionName
                << " found found with "
    ...
    //_noisyPixelMap holds the sensor if (first) and a vector of noisy pixels
    //(second)
  ...
  void EUTelProcessorNoisyPixelFinder::bookAndFillHistos() {
    ...
    for (auto det : _sensorIDVec) {
      ...
      double xMax =
          static_cast<double>(currentSensor->offX + currentSensor->sizeX) + 0.5;

      int yBin = currentSensor->sizeY + 1;
      double yMin = static_cast<double>(currentSensor->offY) - 0.5;
      double yMax =
          static_cast<double>(currentSensor->offY + currentSensor->sizeY) + 0.5;
      ...
      if (!firing2DHisto) {
        streamlog_out(ERROR5)
            << "CreateHistogram2D for " << (basePath + tempHistoName).c_str()
            << " failed " << std::endl
            << "Execution stopped, check that your path (" << basePath.c_str()
            << ")exists  " << std::endl;
        return;
      }

      firing2DHisto->setTitle("Firing frequency map of hot pixels (in percent, "
                              "rounded to the next integer); Pixel Index X; "
                              "Pixel Index Y; Percent (%)");
      ...
      AIDA::IHistogram1D *firing1DHisto =
          marlin::AIDAProcessor::histogramFactory(this)->createHistogram1D(
              (basePath + tempHistoName).c_str(), nBin, min, max);
      firing1DHisto->setTitle("Firing frequency distribution of hot pixels (in "
                              "percent, rounded to the next integer);Firing "
                              "Frequency (%); Count (#)");

      // actually fill both histos for each detector
      for (auto &pixel : _noisyPixelMap[det]) {
        firing2DHisto->fill(pixel.getXCoord(), pixel.getYCoord(),
                            pixel.getSignal());
