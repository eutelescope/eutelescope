...
EUTelPedestalNoiseProcessor::EUTelPedestalNoiseProcessor()
    : Processor("EUTelPedestalNoiseProcessor") {

  // modify processor description
  _description = "EUTelPedestalNoiseProcessor computes the pedestal and noise "
                 "values of a pixel detector";
  ...
  registerInputCollections(LCIO::TRACKERRAWDATA, "RawDataCollectionNameVec",
                           ...
                           _rawDataCollectionNameVec,
                           rawDataCollectionNameVecExample);
  ...
  registerProcessorParameter("CalculationAlgorithm",
                             ...
                             _pedestalAlgo, string(EUTELESCOPE::MEANRMS));

  registerProcessorParameter(
      "CommonModeAlgorithm",
      "Select the algorithm for the common mode calculation. Possible values "
      "are:\n"
  ...
  registerProcessorParameter(
      "BadPixelMaskingAlgorithm",
      "Select the algorithm for bad pixel masking. Possible values are:\n"
      " NoiseDistribution: removing pixels with noise above "
      "PixelMaskUpperNoiseCut in sigma unit\n"
      " AbsoluteNoiseValue: removing pixels with noise above "
      "PixelMaskUpperAbsNoiseCut in ADC value\n"
      " DeadPixel: removing pixels with noise below PixelMaskLowerAbsNoiseCut "
      "in ADC value\n"
      " AbsolutePedeValue: removing pixels having pedestal too low or high "
      "using PixelMaskUpperAbsPedeCut and PixelMaskLowerAbsPedeCut",
  ...
  registerOptionalParameter("PedestalCollectionName",
                            "Pedestal collection name", _pedestalCollectionName,
                            string("pedestalDB"));
  registerOptionalParameter("NoiseCollectionName", "Noise collection name",
                            _noiseCollectionName, string("noiseDB"));
  registerOptionalParameter("StatusCollectionName", "Status collection name",
...
void EUTelPedestalNoiseProcessor::processRunHeader(LCRunHeader *rdr) {
  ...
  streamlog_out(DEBUG4) << "Event range for pedestal calculation is from "
                        << _firstEvent << " to " << _lastEvent
                        << "\nMaxRecordNumber from the global section is   "
                        << maxRecordNumber << endl;
  ...
  if (_lastEvent == -1) {
    ...
    if (maxRecordNumber != 0) {
      streamlog_out(WARNING2)
          << "The MaxRecordNumber in the Global section of the steering file "
             "has been set to "
          << maxRecordNumber << ".\n"
          << "This means that in order to properly perform the pedestal "
             "calculation the maximum allowed number of events is "
    ...
    // different from 0
    if (maxRecordNumber != 0) {
      if ((_lastEvent - _firstEvent) *
              (_noOfCMIterations + 1 + additionalLoop) >
          maxRecordNumber) {
        streamlog_out(ERROR4)
            << "The pedestal calculation should be done on "
            << _lastEvent - _firstEvent << " times " << _noOfCMIterations + 1
            << " iterations = "
            << (_lastEvent - _firstEvent) *
                   (_noOfCMIterations + 1 + additionalLoop)
            << " records.\n"
            << "The global variable MarRecordNumber is limited to "
            << maxRecordNumber << endl;
  ...
  if (_iLoop == 0) {
    ...
    newHeader->lcRunHeader()->setRunNumber(
        runHeader->lcRunHeader()->getRunNumber());
    newHeader->lcRunHeader()->setDetectorName(
        runHeader->lcRunHeader()->getDetectorName());
...
void EUTelPedestalNoiseProcessor::initializeGeometry(LCEvent *evt) {
  ...
  for (size_t iCol = 0; iCol < _rawDataCollectionNameVec.size(); ++iCol) {
    ...
    try {
      LCCollectionVec *collectionVec = dynamic_cast<LCCollectionVec *>(
          evt->getCollection(_rawDataCollectionNameVec.at(iCol)));
      ...
      for (size_t iDetector = 0; iDetector < collectionVec->size();
           ++iDetector) {
        TrackerRawDataImpl *rawData = dynamic_cast<TrackerRawDataImpl *>(
            collectionVec->getElementAt(iDetector));
      ...
      streamlog_out(WARNING2)
          << "No input collection " << _rawDataCollectionNameVec.at(iCol)
          << " is not available in the current event" << endl
          << "If this collection is not present in the input file, please "
             "remove it from the steering file"
          << endl
...
void EUTelPedestalNoiseProcessor::end() {
  ...
  if (_iLoop == _noOfCMIterations + 1 + additionalLoop) {
    ...
    streamlog_out(ERROR4)
        << "Not all the iterations have been done because of a too "
           "MaxRecordNumber.\n"
...
void EUTelPedestalNoiseProcessor::fillHistos() {
  ...
  for (size_t iDetector = 0; iDetector < _noOfDetector; iDetector++) {
    ...
    for (int yPixel = _minY[iDetector]; yPixel <= _maxY[iDetector]; yPixel++) {
      for (int xPixel = _minX[iDetector]; xPixel <= _maxX[iDetector];
           xPixel++) {
        if (_histogramSwitch) {
          tempHistoName = _statusMapHistoName + "_d" +
                          to_string(_orderedSensorIDVec.at(iDetector)) + "_l" +
                          to_string(_iLoop);
          if (AIDA::IHistogram2D *histo = dynamic_cast<AIDA::IHistogram2D *>(
                  _aidaHistoMap[tempHistoName]))
            histo->fill(static_cast<double>(xPixel),
                        static_cast<double>(yPixel),
        ...
        if (_status[iDetector][iPixel] == EUTELESCOPE::GOODPIXEL) {
          if (_histogramSwitch) {
            tempHistoName = _pedeDistHistoName + "_d" +
                            to_string(_orderedSensorIDVec.at(iDetector)) +
                            "_l" + to_string(_iLoop);
            if (AIDA::IHistogram1D *histo = dynamic_cast<AIDA::IHistogram1D *>(
                    _aidaHistoMap[tempHistoName]))
          ...
          if (streamlog::out.write<streamlog::DEBUG0>()) {
            if ((xPixel == 10) && (yPixel == 10)) {
              streamlog::out()
                  << "Detector " << _orderedSensorIDVec.at(iDetector)
                  << " pedestal " << (_pedestal[iDetector][iPixel]) << endl;
            }
          }

          if (_histogramSwitch) {
            tempHistoName = _noiseDistHistoName + "_d" +
                            to_string(_orderedSensorIDVec.at(iDetector)) +
                            "_l" + to_string(_iLoop);
            if (AIDA::IHistogram1D *histo = dynamic_cast<AIDA::IHistogram1D *>(
                    _aidaHistoMap[tempHistoName]))
          ...
          if (_histogramSwitch) {
            tempHistoName = _pedeMapHistoName + "_d" +
                            to_string(_orderedSensorIDVec.at(iDetector)) +
                            "_l" + to_string(_iLoop);
            if (AIDA::IHistogram2D *histo = dynamic_cast<AIDA::IHistogram2D *>(
                    _aidaHistoMap[tempHistoName]))
          ...
          if (_histogramSwitch) {
            tempHistoName = _noiseMapHistoName + "_d" +
                            to_string(_orderedSensorIDVec.at(iDetector)) +
                            "_l" + to_string(_iLoop);
            if (AIDA::IHistogram2D *histo = dynamic_cast<AIDA::IHistogram2D *>(
                    _aidaHistoMap[tempHistoName]))
...
void EUTelPedestalNoiseProcessor::maskBadPixel() {
  ...
  if ((!_additionalMaskingLoop) || (_iLoop < _noOfCMIterations + 1)) {
    ...
    vector<double> thresholdAbsNoiseVec(_noOfDetector,
                                        _pixelMaskUpperAbsNoiseCut);

    // thresholdDeadPixelVec is the vector of threshold for the
    // DEADPIXEL algorithm
    vector<double> thresholdDeadPixelVec(_noOfDetector,
                                         _pixelMaskLowerAbsNoiseCut);

    // thresholdLowerAbsPedeVec is the vector of threshold for the
    // ABSOLUTEPEDEVALUE algorithm
    vector<double> thresholdLowerAbsPedeVec(_noOfDetector,
                                            _pixelMaskLowerAbsPedeCut);
    vector<double> thresholdUpperAbsPedeVec(_noOfDetector,
                                            _pixelMaskUpperAbsPedeCut);
    ...
    if (_badPixelNoiseDistributionSwitch) {
      ...
      for (size_t iDetector = 0; iDetector < _noOfDetector; iDetector++) {
        ...
        thresholdNoiseDistVec.push_back(meanw +
                                        (rms * _pixelMaskUpperNoiseCut));
        streamlog_out(DEBUG4) << "Mean noise value is " << meanw
                              << " ADC\n"
                                 "RMS of noise is "
                              << rms << " ADC\n"
                                        "Masking threshold is set to "
        ...
        for (size_t iPixel = 0; iPixel < _status[iDetector].size(); ++iPixel) {
          if ((_noise[iDetector][iPixel] > thresholdNoiseDistVec[iDetector]) &&
            ...
            streamlog_out(DEBUG0)
                << "Masking pixel number " << iPixel << " on detector "
                << _orderedSensorIDVec.at(iDetector) << " ("
                << _noise[iDetector][iPixel] << " > "
                << thresholdNoiseDistVec[iDetector] << ")" << endl;
    ...
    if (_badPixelAbsPedeSwitch) {
      ...
      for (size_t iDetector = 0; iDetector < _noOfDetector; ++iDetector) {
        for (size_t iPixel = 0; iPixel < _status[iDetector].size(); ++iPixel) {
          if (((_pedestal[iDetector][iPixel] >
                thresholdUpperAbsPedeVec[iDetector]) ||
               (_pedestal[iDetector][iPixel] <
                thresholdLowerAbsPedeVec[iDetector])) &&
              _status[iDetector][iPixel] == EUTELESCOPE::GOODPIXEL) {
            _status[iDetector][iPixel] = EUTELESCOPE::BADPIXEL;
            streamlog_out(DEBUG0) << "Masking pixel number " << iPixel
                                  << " on detector "
                                  << _orderedSensorIDVec.at(iDetector) << endl;
            badPixelCounterVec[iDetector]++;
          }
        }
      }
    ...
    streamlog_out(MESSAGE4) << "Masking summary after loop " << _iLoop << ": "
                            << endl;
    int totalBad = 0;
    int total = 0;
    for (size_t iDetector = 0; iDetector < _noOfDetector; ++iDetector) {
      streamlog_out(MESSAGE4) << "Detector ID " << setw(4)
                              << _orderedSensorIDVec[iDetector] << " has "
                              << setw(8) << badPixelCounterVec[iDetector]
                              << " bad pixels ("
                              << 100 * (1.0 * badPixelCounterVec[iDetector]) /
                                     _status[iDetector].size()
                              << "%). " << endl;
      totalBad += badPixelCounterVec[iDetector];
      total += _status[iDetector].size();
    }
    streamlog_out(MESSAGE4) << "Total masked pixels = " << totalBad << " ("
                            << 100 * (1.0 * totalBad) / total << "%). " << endl;
  }
  ...
  if ((_additionalMaskingLoop) && (_iLoop == _noOfCMIterations + 1)) {
    ...
    for (size_t iDetector = 0; iDetector < _noOfDetector; iDetector++) {
      ...
      for (unsigned int iPixel = 0; iPixel < _status[iDetector].size();
           iPixel++) {
#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
        if (_histogramSwitch) {
          string tempHistoName;
          tempHistoName = _fireFreqHistoName + "_d" +
                          to_string(_orderedSensorIDVec.at(iDetector)) + "_l" +
                          to_string(_iLoop);
          if (AIDA::IHistogram1D *histo = dynamic_cast<AIDA::IHistogram1D *>(
                  _aidaHistoMap[tempHistoName]))
            histo->fill((static_cast<double>(_hitCounter[iDetector][iPixel])) /
                        _iEvt * 100.);
        }
#endif
        if (static_cast<double>(_hitCounter[iDetector][iPixel]) / _iEvt * 100. >
            _maxFiringFreq) {
          _status[iDetector][iPixel] = EUTELESCOPE::BADPIXEL;
          badPixelCounterVec[iDetector]++;
        }
...
void EUTelPedestalNoiseProcessor::preLoop(LCEvent *event) {
  ...
  if ((_lastEvent != -1) && (_iEvt >= _lastEvent)) {
    streamlog_out(DEBUG4)
        << "Looping limited by _lastEvent: calling simpleRewind()." << endl;
    ...
    for (size_t iCol = 0; iCol < _rawDataCollectionNameVec.size(); ++iCol) {
      ...
      try {
        ...
        for (size_t iDetector = 0; iDetector < collectionVec->size();
          ...
          TrackerRawData *trackerRawData = dynamic_cast<TrackerRawData *>(
              collectionVec->getElementAt(iDetector));
  ...
  for (size_t iCol = 0; iCol < _rawDataCollectionNameVec.size(); ++iCol) {
    ...
    try {
      ...
      streamlog_out(WARNING2) << "No input collection "
                              << _rawDataCollectionNameVec.at(iCol)
...
void EUTelPedestalNoiseProcessor::firstLoop(LCEvent *event) {
  ...
  if ((_lastEvent != -1) && (_iEvt >= _lastEvent)) {
    streamlog_out(DEBUG4)
        << "Looping limited by _lastEvent: calling finalizeProcessor()."
        << endl;
    finalizeProcessor(false);
  }
      ...
      try {
        ...
        for (size_t iDetector = 0; iDetector < collectionVec->size();
          ...
          if (_pedestalAlgo == EUTELESCOPE::MEANRMS) {
            ...
            int iPixel = 0;
            size_t detectorOffset =
                (iCol == 0) ? 0 : _noOfDetectorVec.at(iCol - 1);
            stringstream ss;
            ss << _tempProfile2DName << "_d"
               << _orderedSensorIDVec.at(iDetector + detectorOffset);
            for (int yPixel = _minY[iDetector + detectorOffset];
                 yPixel <= _maxY[iDetector + detectorOffset]; yPixel++) {
              for (int xPixel = _minX[iDetector + detectorOffset];
                   xPixel <= _maxX[iDetector + detectorOffset]; xPixel++) {
                double temp = static_cast<double>(adcValues[iPixel]);
                if (AIDA::IProfile2D *profile =
                        dynamic_cast<AIDA::IProfile2D *>(
                            _aidaHistoMap[ss.str()])) {
                  profile->fill(static_cast<double>(xPixel),
                                static_cast<double>(yPixel), temp);
                } else {
                  streamlog_out(ERROR4)
                      << "Irreversible error: " << ss.str()
                      << " is not available. Sorry for quitting." << endl;
                  exit(-1);
                }
                ++iPixel;
              }
          ...
          if (_additionalMaskingLoop)
            _hitCounter.push_back(ShortVec(adcValues.size(), 0));
          ...
          if (_pedestalAlgo == EUTELESCOPE::MEANRMS) {
            ...
            for (int yPixel = _minY[iDetector + detectorOffset];
              ...
              for (int xPixel = _minX[iDetector + detectorOffset];
                ...
                if (_preLoopSwitch &&
                    ((_iEvt ==
                      _maxValuePos[iDetector + detectorOffset][iPixel]) ||
                     (_iEvt ==
                      _minValuePos[iDetector + detectorOffset][iPixel]))) {
                  use = false;
                }
                ...
                if (use) {
                  ...
                  _tempEntries[iDetector + detectorOffset][iPixel] =
                      _tempEntries[iDetector + detectorOffset][iPixel] + 1;
                  _tempPede[iDetector + detectorOffset][iPixel] =
                      ((_tempEntries[iDetector + detectorOffset][iPixel] - 1) *
                           _tempPede[iDetector + detectorOffset][iPixel] +
                       currentVal) /
                      _tempEntries[iDetector + detectorOffset][iPixel];
                  _tempNoise[iDetector + detectorOffset][iPixel] = sqrt(
                      ((_tempEntries[iDetector + detectorOffset][iPixel] - 1) *
                           pow(_tempNoise[iDetector + detectorOffset][iPixel],
                               2) +
                       pow(currentVal -
                               _tempPede[iDetector + detectorOffset][iPixel],
                           2)) /
              ...
              for (int xPixel = _minX[iDetector + detectorOffset];
                ...
                if (use) {
                  if (AIDA::IProfile2D *profile =
                    ...
                    profile->fill(static_cast<double>(xPixel),
                                  static_cast<double>(yPixel),
                                  static_cast<double>(adcValues[iPixel]));
                  else {
...
void EUTelPedestalNoiseProcessor::otherLoop(LCEvent *event) {
  ...
  for (size_t iCol = 0; iCol < _rawDataCollectionNameVec.size(); ++iCol) {
    ...
    // let me get the rawDataCollection. This is should contain a
    // TrackerRawDataObject
      ...
      for (size_t iDetector = 0; iDetector < collectionVec->size();
        ...
        if (_commonModeAlgo == EUTELESCOPE::FULLFRAME) {
          ...
          for (int yPixel = _minY[iDetector + detectorOffset];
            ...
            for (int xPixel = _minX[iDetector + detectorOffset];
                 xPixel <= _maxX[iDetector + detectorOffset]; xPixel++) {
              bool isHit = ((adcValues[iPixel] -
                             _pedestal[iDetector + detectorOffset][iPixel]) >
                            _hitRejectionCut *
                                _noise[iDetector + detectorOffset][iPixel]);
              bool isGood = (_status[iDetector + detectorOffset][iPixel] ==
          ...
          if ((skippedPixel < _maxNoOfRejectedPixels) && (goodPixel != 0)) {

            commonMode = pixelSum / goodPixel;
            commonModeCorVec.insert(commonModeCorVec.begin(), iPixel + 1,
                                    commonMode);
            isEventValid = true;

#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
            string histoname =
                _commonModeHistoName + "_d" +
                to_string(_orderedSensorIDVec.at(iDetector + detectorOffset)) +
                "_l" + to_string(_iLoop);
            AIDA::IHistogram1D *histo =
                (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[histoname]));
            if (histo) {
          ...
          int iPixel = 0;
          int colCounter = 0;
          int rowLength = _maxX[iDetector + detectorOffset] -
                          _minX[iDetector + detectorOffset] + 1;
          ...
          for (int yPixel = _minY[iDetector + detectorOffset];
            ...
            if ((skippedPixelPerRow < _maxNoOfRejectedPixelPerRow) &&
              ...
              commonModeCorVec.insert(commonModeCorVec.begin() +
                                          colCounter * rowLength,
                                      rowLength, commonMode);

#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
              string histoname = _commonModeHistoName + "_d" +
                                 to_string(_orderedSensorIDVec.at(
                                     iDetector + detectorOffset)) +
                                 "_l" + to_string(_iLoop);
              AIDA::IHistogram1D *histo = (dynamic_cast<AIDA::IHistogram1D *>(
                  _aidaHistoMap[histoname]));
              if (histo) {
                histo->fill(commonMode);
              }
#endif
          ...
          streamlog_out(ERROR4)
              << "Unknown common mode algorithm. Using flat null correction"
              << endl;
          commonModeCorVec.insert(commonModeCorVec.begin(),
                                  (_maxY[iDetector + detectorOffset] -
                                   _minY[iDetector + detectorOffset] + 1) *
                                      (_maxX[iDetector + detectorOffset] -
                                       _minX[iDetector + detectorOffset] + 1),
                                  0.);
          isEventValid = true;
        }
        ...
        if (isEventValid) {
          ...
          for (int yPixel = _minY[iDetector + detectorOffset];
            ...
            for (int xPixel = _minX[iDetector + detectorOffset];
                 xPixel <= _maxX[iDetector + detectorOffset]; xPixel++) {
              if (_status[iDetector + detectorOffset][iPixel] ==
                  EUTELESCOPE::GOODPIXEL) {
                double pedeCorrected =
                    adcValues[iPixel] - commonModeCorVec[iPixel];
                if (std::abs(pedeCorrected -
                             _pedestal[iDetector + detectorOffset][iPixel]) <
                    _hitRejectionCut *
                        _noise[iDetector + detectorOffset][iPixel]) {
                  if (_pedestalAlgo == EUTELESCOPE::MEANRMS) {
                    ...
                    if (use) {
                      ...
                      _tempNoise[iDetector + detectorOffset][iPixel] = sqrt(
                          ((_tempEntries[iDetector + detectorOffset][iPixel] -
                            1) *
                               pow(_tempNoise[iDetector + detectorOffset]
                                             [iPixel],
                                   2) +
                           pow(pedeCorrected -
                                   _tempPede[iDetector + detectorOffset]
                                            [iPixel],
                               2)) /
                          _tempEntries[iDetector + detectorOffset][iPixel]);
                    }
                    ...
                    if (use) {
                      ...
                      (dynamic_cast<AIDA::IProfile2D *>(
                           _aidaHistoMap[ss.str()]))
                          ->fill(static_cast<double>(xPixel),
                                 static_cast<double>(yPixel), pedeCorrected);
                    }
#endif
                  }
                }
          ...
          if (_commonModeAlgo == EUTELESCOPE::FULLFRAME) {
            streamlog_out(WARNING2)
                << "Skipping event " << _iEvt
                << " because of max number of rejected pixels exceeded. ("
                << skippedPixel << ") on detector "
                << _orderedSensorIDVec.at(iDetector) << endl;
          } else if (_commonModeAlgo == EUTELESCOPE::ROWWISE) {
            streamlog_out(WARNING2)
                << "Skipping event " << _iEvt
                << " because of max number of skipped rows is reached. ("
                << skippedRow << ") on detector "
                << _orderedSensorIDVec.at(iDetector) << endl;
          }
  ...
  for (int iLoop = 0; iLoop < _noOfCMIterations + 1; iLoop++) {
    ...
    for (size_t iDetector = 0; iDetector < _noOfDetector; iDetector++) {
      ...
      string detectorDirName =
          "detector_" + to_string(_orderedSensorIDVec.at(iDetector));
      ...
      AIDA::IHistogram1D *pedeDistHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              (basePath + tempHistoName).c_str(), pedeDistHistoNBin,
              pedeDistHistoMin, pedeDistHistoMax);
      if (pedeDistHisto) {
      ...
      AIDA::IHistogram1D *noiseDistHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              (basePath + tempHistoName).c_str(), noiseDistHistoNBin,
              noiseDistHistoMin, noiseDistHistoMax);
      if (noiseDistHisto) {
      ...
      if (iLoop >= 1) {
        ...
        tempHistoName = _commonModeHistoName + "_d" +
                        to_string(_orderedSensorIDVec.at(iDetector)) + "_l" +
                        to_string(iLoop);
        if (isHistoManagerAvailable) {
        ...
        AIDA::IHistogram1D *commonModeHisto =
            AIDAProcessor::histogramFactory(this)->createHistogram1D(
                (basePath + tempHistoName).c_str(), commonModeHistoNBin,
                commonModeHistoMin, commonModeHistoMax);
        if (commonModeHisto) {
          _aidaHistoMap.insert(make_pair(tempHistoName, commonModeHisto));
          commonModeHisto->setTitle("Common mode distribution");
        } else {
          streamlog_out(ERROR1)
              << "Problem booking the " << (basePath + tempHistoName) << ".\n"
              << "Very likely a problem with path name. Switching off "
                 "histogramming and continue w/o"
              << endl;
          _histogramSwitch = false;
        }
      }
      ...
      AIDA::IHistogram2D *pedeMapHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram2D(
              (basePath + tempHistoName).c_str(), xNoOfPixel, xMin, xMax,
              yNoOfPixel, yMin, yMax);
      if (pedeMapHisto) {
      ...
      if ((_pedestalAlgo == EUTELESCOPE::AIDAPROFILE) && (iLoop == 0)) {
  ...
  if (_additionalMaskingLoop) {
    ...
    for (size_t iDetector = 0; iDetector < _noOfDetector; iDetector++) {
      ...
      AIDA::IHistogram1D *fireFreqHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              (basePath + tempHistoName).c_str(), fireFreqHistoNBin,
              fireFreqHistoMin, fireFreqHistoMax);
      if (fireFreqHisto) {
      ...
      tempHistoName = _aPixelHistoName + "_d" +
                      to_string(_orderedSensorIDVec.at(iDetector)) + "_l" +
                      to_string(iLoop);
      ...
      AIDA::IHistogram1D *aPixelHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              (basePath + tempHistoName).c_str(), aPixelNBin, aPixelMin,
              aPixelMax);
      if (aPixelHisto) {
...
void EUTelPedestalNoiseProcessor::finalizeProcessor(bool fromMaskingLoop) {
  ...
  if (_iLoop > 0) {
    ...
    _nextEventToSkip = _skippedEventList.begin();
    streamlog_out(MESSAGE4)
        << "Skipped " << _skippedEventList.size()
        << " event because of common mode ("
  ...
  if (!fromMaskingLoop) {
    ...
    if (_pedestalAlgo == EUTELESCOPE::MEANRMS) {
      ...
      for (size_t iDetector = 0; iDetector < _noOfDetector; iDetector++) {
        ...
        FloatVec tempPede;
        FloatVec tempNoise;
        for (int yPixel = _minY[iDetector]; yPixel <= _maxY[iDetector];
             yPixel++) {
          for (int xPixel = _minX[iDetector]; xPixel <= _maxX[iDetector];
               xPixel++) {
            if (AIDA::IProfile2D *profile =
              ...
              tempPede.push_back(
                  static_cast<float>(profile->binHeight(xPixel, yPixel)));
              // WARNING: the noise part of this algorithm is still not
              // working probably because of a bug in RAIDA implementation
              tempNoise.push_back(
                  static_cast<float>(profile->binRms(xPixel, yPixel)));
              // cout << xPixel << " " << yPixel << " " << tempPede.back() << "
              // " << tempNoise.back() << endl;
            } else {
              streamlog_out(ERROR4)
                  << "Problem with the AIDA temporary profile.\n"
  ...
  if (_iLoop == _noOfCMIterations + 1 + additionalLoop) {
    ...
    for (size_t iDetector = 0; iDetector < _noOfDetector; iDetector++) {
      ...
      CellIDEncoder<TrackerDataImpl> idPedestalEncoder(
          EUTELESCOPE::MATRIXDEFAULTENCODING, pedestalCollection);
      CellIDEncoder<TrackerDataImpl> idNoiseEncoder(
          EUTELESCOPE::MATRIXDEFAULTENCODING, noiseCollection);
      CellIDEncoder<TrackerRawDataImpl> idStatusEncoder(
          EUTELESCOPE::MATRIXDEFAULTENCODING, statusCollection);
      ...
      if (_asciiOutputSwitch) {
        if (iDetector == 0)
          streamlog_out(MESSAGE4) << "Writing the ASCII pedestal files" << endl;
        stringstream ss;
        ss << _outputPedeFileName << "-b" << iDetector << ".dat";
        ofstream asciiPedeFile(ss.str().c_str());
        asciiPedeFile << "# Pedestal and noise for board number " << iDetector
                      << endl
                      << "# calculated from run " << _outputPedeFileName
                      << endl;
        ...
        for (int yPixel = _minY[iDetector]; yPixel <= _maxY[iDetector];
          ...
          for (int xPixel = _minX[iDetector]; xPixel <= _maxX[iDetector];
               xPixel++) {
            asciiPedeFile << setiosflags(ios::left) << setw(subMatrixWidth)
                          << iDetector << setw(xPixelWidth) << xPixel
                          << setw(yPixelWidth) << yPixel
                          << resetiosflags(ios::left) << setiosflags(ios::fixed)
                          << setprecision(precision) << setw(pedeWidth)
                          << _pedestal[iDetector][iPixel] << setw(noiseWidth)
                          << _noise[iDetector][iPixel]
                          << resetiosflags(ios::fixed) << setw(statusWidth)
                          << _status[iDetector][iPixel] << endl;
            ++iPixel;
          }
        }
        asciiPedeFile.close();
...
// remember to loop over all detectors
      ...
      for (size_t iDetector = 0; iDetector < _noOfDetector; iDetector++) {
        ...
        if (AIDA::IProfile2D *profile =
          ...
          streamlog_out(ERROR4)
              << "Unable to reset the AIDA temporary profile.\n"
              << "Sorry for quitting..." << endl;
          exit(-1);
        }
      }
    ...
    setReturnValue("IsPedestalFinished", false);
    throw RewindDataFilesException(this);
  } else if ((_additionalMaskingLoop) && (_iLoop == _noOfCMIterations + 1)) {
    // additional loop!
    // now we need to loop again
    // so reset the event counter
    _iEvt = 0;
...
void EUTelPedestalNoiseProcessor::additionalMaskingLoop(LCEvent *event) {
  ...
  if (*_nextEventToSkip == _iEvt) {
    streamlog_out(MESSAGE4)
        << "Event " << _iEvt
        << " is skipped because labelled bad by the common mode procedure."
        << endl;
    ++_nextEventToSkip;
    return;
  }
        ...
        for (unsigned int iPixel = 0; iPixel < adcValues.size(); iPixel++) {
          if (_status[iDetector + detectorOffset][iPixel] ==
            ...
            if (_histogramSwitch && iPixel == 1 + (adcValues.size() / 10)) {
              string tempHistoName = _aPixelHistoName + "_d" +
                                     to_string(_orderedSensorIDVec.at(
                                         iDetector + detectorOffset)) +
                                     "_l" + to_string(_iLoop);
              if (AIDA::IHistogram1D *histo =
...
void EUTelPedestalNoiseProcessor::setBadPixelAlgoSwitches() {
  ...
  if (find(_badPixelAlgoVec.begin(), _badPixelAlgoVec.end(),
           EUTELESCOPE::NOISEDISTRIBUTION) != _badPixelAlgoVec.end()) {
    _badPixelNoiseDistributionSwitch = true;
    streamlog_out(DEBUG4) << "Using " << EUTELESCOPE::NOISEDISTRIBUTION
                          << " algorithm" << endl;
  } else {
  ...
  if (find(_badPixelAlgoVec.begin(), _badPixelAlgoVec.end(),
           EUTELESCOPE::ABSOLUTENOISEVALUE) != _badPixelAlgoVec.end()) {
    _badPixelAbsNoiseSwitch = true;
    streamlog_out(DEBUG4) << "Using " << EUTELESCOPE::ABSOLUTENOISEVALUE
                          << " algorithm" << endl;
  } else {
  ...
  if (find(_badPixelAlgoVec.begin(), _badPixelAlgoVec.end(),
           EUTELESCOPE::DEADPIXEL) != _badPixelAlgoVec.end()) {
    _badPixelDeadPixelSwitch = true;
    streamlog_out(DEBUG4) << "Using " << EUTELESCOPE::DEADPIXEL << " algorithm"
                          << endl;
  } else {
  ...
  if (find(_badPixelAlgoVec.begin(), _badPixelAlgoVec.end(),
           EUTELESCOPE::ABSOLUTEPEDEVALUE) != _badPixelAlgoVec.end()) {
    _badPixelAbsPedeSwitch = true;
    streamlog_out(DEBUG4) << "Using " << EUTELESCOPE::ABSOLUTEPEDEVALUE
                          << " algorithm" << endl;
  } else {
