...
void EUTelHistogramMaker::initializeGeometry(LCEvent *event) {
  ...
  try {
    for (size_t i = 0; i < _noiseCollectionName.size(); ++i) {
      LCCollectionVec *collection = dynamic_cast<LCCollectionVec *>(
          event->getCollection(_noiseCollectionName[i]));
      ...
      for (size_t iDetector = 0; iDetector < collection->size(); ++iDetector) {
        TrackerDataImpl *data = dynamic_cast<TrackerDataImpl *>(
            collection->getElementAt(iDetector));
...
void EUTelHistogramMaker::processEvent(LCEvent *evt) {
  ...
  if (_noiseHistoSwitch) {
    ...
    for (size_t i = 0; i < _noiseCollectionName.size(); ++i) {
      ...
      try {
        statusCollectionVec.push_back(dynamic_cast<LCCollectionVec *>(
            evt->getCollection(_statusCollectionName[i])));
      } catch (lcio::DataNotAvailableException &e) {
        streamlog_out(ERROR1)
            << e.what() << endl
            << "Switching off the noise histogram filling and continuing"
      ...
      if (_noiseHistoSwitch) {
        ...
        while (!found && i < noiseCollectionVec.size()) {
          ...
          if (noiseCollectionVec[i]->getNumberOfElements() >
            ...
            noiseMatrix = dynamic_cast<TrackerDataImpl *>(
                noiseCollectionVec[i]->getElementAt(_ancillaryMap[detectorID]));
            statusMatrix = dynamic_cast<TrackerRawDataImpl *>(
                statusCollectionVec[i]->getElementAt(
                    _ancillaryMap[detectorID]));
        ...
        try {
          ...
          streamlog_out(ERROR1) << e.what() << endl
                                << "Continuing without filling the histograms"
                                << endl;
          _noiseHistoSwitch = false;
        }
      }
...
void EUTelHistogramMaker::bookHistos() {
  ...
  for (int iDetector = 0; iDetector < _noOfDetector; iDetector++) {
    ...
    tempHistoName =
        _clusterSignalHistoName + "_d" + to_string(_sensorIDVec.at(iDetector));
    ...
    AIDA::IHistogram1D *nhitsHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + tempHistoName).c_str(), nhitsNBin, nhitsMin, nhitsMax);
    ...
    if (_noiseHistoSwitch) {
      // cluster SNR
      tempHistoName =
          _clusterSNRHistoName + "_d" + to_string(_sensorIDVec.at(iDetector));
    ...
    while (iter != _clusterSpectraNVector.end()) {
      ...
      if (_noiseHistoSwitch) {
        // this is for the SNR
        tempHistoName = _clusterSNRHistoName + to_string(*iter) + "_d" +
                        to_string(_sensorIDVec.at(iDetector));
    ...
    if (_noiseHistoSwitch) {
      // seed SNR
      tempHistoName =
          _seedSNRHistoName + "_d" + to_string(_sensorIDVec.at(iDetector));
      ...
      tempHistoName =
          _clusterNoiseHistoName + "_d" + to_string(_sensorIDVec.at(iDetector));
    ...
    int yBin = _maxY[_sensorIDVec.at(iDetector)] -
