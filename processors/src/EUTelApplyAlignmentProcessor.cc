...
#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
std::string EUTelApplyAlignmentProcessor::_densityPlotBeforeAlignName =
    "DensityPlotBeforeAlign";
std::string EUTelApplyAlignmentProcessor::_densityPlotAfterAlignName =
    "DensityPloAfterAlign";
std::string EUTelApplyAlignmentProcessor::_hitHistoBeforeAlignName =
    "HitHistoBeforeAlign";
std::string EUTelApplyAlignmentProcessor::_hitHistoAfterAlignName =
    "HitHistoAfterAlign";
#endif

EUTelApplyAlignmentProcessor::EUTelApplyAlignmentProcessor()
    : Processor("EUTelApplyAlignmentProcessor"), _conversionIdMap(), _nRun(0),
      _nEvt(0), _iDUT(0), _indexDUT(0), _xPitch(0.0), _yPitch(0.0), _rot00(0.0),
      _rot01(0.0), _rot10(0.0), _rot11(0.0),
      internal_inputHitCollectionName(""), internal_inputCollectionVec(NULL),
      internal_referenceHitCollectionName(""), internal_referenceHitVec(NULL),
      _inputHitCollectionName(""), _inputCollectionVec(NULL),
      _alignmentCollectionName(""), _alignmentCollectionVec(NULL),
      _outputHitCollectionName(""), _outputCollectionVec(NULL),
      _applyToReferenceHitCollection(true), _referenceHitCollectionName(""),
      _referenceHitVec(NULL), _outputReferenceHitCollectionName(""),
      _outputReferenceHitVec(NULL), _correctionMethod(0),
      _applyAlignmentDirection(0), _alignmentCollectionNames(),
      _alignmentCollectionSuffixes(), _hitCollectionNames(),
      _hitCollectionSuffixes(), _refhitCollectionNames(),
      _refhitCollectionSuffixes(), _doGear(false), _doAlignCollection(false),
      _doAlignmentInOneGo(false), _debugSwitch(false), _alpha(0.0), _beta(0.0),
      _gamma(0.0), _printEvents(0), _iRun(0), _iEvt(0), _lookUpTable(),
      _fevent(false), _aidaHistoMap(), _siPlanesParameters(NULL),
      _siPlanesLayerLayout(NULL), _siPlaneZPosition(NULL),
      _orderedSensorIDVec(), _histogramSwitch(false) {
  // modify processor description
  _description = "Apply to the input hit the alignment corrections";
  ...
  registerOptionalParameter(
      "ReferenceCollection", "This is the name of the reference hit collection",
      internal_referenceHitCollectionName, static_cast<string>("referenceHit"));

  registerOptionalParameter(
      "OutputReferenceCollection",
  ...
  registerOptionalParameter(
      "ApplyToReferenceCollection", "Do you want the reference hit collection "
                                    "to be corrected by the shifts and tilts "
                                    "from the alignment collection?",
      _applyToReferenceHitCollection, static_cast<bool>(true));

  // now the optional parameters
  registerProcessorParameter("CorrectionMethod", "Available methods are:\n"
                                                 " 0 -> shift only \n"
                                                 " 1 -> rotation first \n"
                                                 " 2 -> shift first ",
  ...
  registerProcessorParameter(
      "hitCollectionNames", "List of hit collections. First one is INPUT "
                            "collection, every subsequent corresponds to "
  ...
  registerProcessorParameter(
      "refhitCollectionNames", "List of refhit collections. First one is INPUT "
                               "collection, every subsequent corresponds to "
                               "applying alignment collection",
      _refhitCollectionSuffixes, _refhitCollectionSuffixExamples);

  registerOptionalParameter("DoAlignCollection", "Implement geometry shifts "
                                                 "and rotations as described "
                                                 "in alignmentCollectionName ",
                            _doAlignCollection, static_cast<bool>(0));

  registerOptionalParameter("DoGear", "Implement geometry shifts and rotations "
                                      "as described in GEAR steering file ",
                            _doGear, static_cast<bool>(0));

  registerOptionalParameter("DoAlignmentInOneGo",
                            "Apply alignment steps in one go. Is supposed to "
                            "be used for reversealignment in reverse order, "
                            "like: undoAlignment, undoPreAlignment, undoGear ",
  ...
  registerOptionalParameter("Alpha", "Rotation Angle around X axis", _alpha,
                            static_cast<double>(0.00));
  registerOptionalParameter("Beta", "Rotation Angle around Y axis", _beta,
                            static_cast<double>(0.00));
  registerOptionalParameter("Gamma", "Rotation Angle around Z axis", _gamma,
                            static_cast<double>(0.00));
  registerOptionalParameter(
      "PrintEvents", "Events number to have DEBUG1 printed outs (default=10)",
...
//  _histogramSwitch = false;
  ...
  for (int iPlane = 0; iPlane < _siPlanesParameters->getSiPlanesNumber();
       ++iPlane) {
...
void EUTelApplyAlignmentProcessor::CheckIOCollections(LCEvent *event) {
  ...
  if (_applyToReferenceHitCollection) {
    try {
      ...
      if (_iEvt % 1000 == 0)
        streamlog_out(WARNING2) << "_referenceHitCollectionName "
                                << _referenceHitCollectionName.c_str()
                                << " could not be retrieved, creating a dummy "
                                   "one (all elements are null) "
                                << endl;
    ...
    try {
      _outputReferenceHitVec = dynamic_cast<LCCollectionVec *>(
          event->getCollection(_outputReferenceHitCollectionName));

    } catch (...) {
      _outputReferenceHitVec = CreateDummyReferenceHitCollection();
      streamlog_out(DEBUG5) << "NOT found _outputReferenceHitVec ["
                            << _outputReferenceHitCollectionName.c_str()
                            << "] at " << _outputReferenceHitVec << endl;
      event->addCollection(_outputReferenceHitVec,
  ...
  try {
    _inputCollectionVec = dynamic_cast<LCCollectionVec *>(
        evt->getCollection(_inputHitCollectionName));
    streamlog_out(DEBUG2) << " input [" << _inputHitCollectionName
                          << "] collection found on event "
                          << event->getEventNumber() << endl;
    streamlog_out(DEBUG2) << " input [" << _inputHitCollectionName
                          << "] collection found at       "
                          << _inputCollectionVec << endl;
  } catch (DataNotAvailableException &e) {
    streamlog_out(DEBUG3) << "No input [" << _inputHitCollectionName
                          << "] collection found on event "
  ...
  if (_alignmentCollectionName != "gear") {
    try {
      _alignmentCollectionVec = dynamic_cast<LCCollectionVec *>(
          evt->getCollection(_alignmentCollectionName));
      streamlog_out(DEBUG2) << "found Alignment [" << _alignmentCollectionName
                            << "] collection found on event "
                            << event->getEventNumber() << " at "
                            << _alignmentCollectionVec << endl;
    } catch (DataNotAvailableException &e) {
      streamlog_out(DEBUG3) << "No Alignment [" << _alignmentCollectionName
                            << "] collection found on event "
                            << event->getEventNumber() << " in run "
                            << event->getRunNumber() << endl;
    }
  ...
  try {
    _outputCollectionVec = dynamic_cast<LCCollectionVec *>(
        evt->getCollection(_outputHitCollectionName));
    if (_outputCollectionVec == 0) {
      ...
      streamlog_out(DEBUG2) << " created new " << _outputHitCollectionName
                            << endl;
    } else {
    ...
    _outputCollectionVec = new LCCollectionVec(LCIO::TRACKERHIT);
    evt->addCollection(_outputCollectionVec, _outputHitCollectionName);
    streamlog_out(DEBUG2) << " try catch. nevertheless created new "
                          << _outputHitCollectionName << endl;
  }
}
...
void EUTelApplyAlignmentProcessor::processRunHeader(LCRunHeader *runHeader) {
  ...
  if (_alignmentCollectionSuffixes.size() < 1) {
    message<ERROR5>(log() << "You must define at least one Alignmen Collection "
                             "to start ApplyAlignment Processor"
                          << endl);
    throw StopProcessingException(this);
  } else if (_alignmentCollectionSuffixes.size() == 1) {
    message<MESSAGE4>(
        log() << "Only one Alignment Collection has been specified" << endl);
    message<MESSAGE4>(log()
                      << "Ignore other Suffix collections, proceed with the "
                         "defaults given by _referenceHitCollectionName and "
    ...
    message<MESSAGE4>(log() << " _alignmentCollectionNames[0] = "
                            << _alignmentCollectionNames[0] << endl);
    message<MESSAGE4>(log() << " _hitCollectionNames[0] = "
                            << _hitCollectionNames[0] << endl);
    message<MESSAGE4>(log() << " _refhitCollectionNames[0] = "
                            << _refhitCollectionNames[0] << endl);
  } else if ((_alignmentCollectionSuffixes.size() !=
              _hitCollectionSuffixes.size()) ||

             (_alignmentCollectionSuffixes.size() !=
              _refhitCollectionSuffixes.size()) ||
    ...
    for (unsigned i = 0; i < _alignmentCollectionSuffixes.size(); i++) {
      message<ERROR>(log() << " _alignmentCollectionSuffixes[" << i
                           << "] : " << _alignmentCollectionSuffixes[i]);
    }
    for (unsigned i = 0; i < _hitCollectionSuffixes.size(); i++) {
      message<ERROR>(log() << " _hitCollectionSuffixes[" << i
                           << "] : " << _hitCollectionSuffixes[i]);
    }
    for (unsigned i = 0; i < _refhitCollectionSuffixes.size(); i++) {
      message<ERROR>(log() << " _refhitCollectionSuffixes[" << i
                           << "] : " << _refhitCollectionSuffixes[i]);
    }

    message<ERROR>(log() << endl);
    TString ExceptionMessage =
        " Input collection names are inconsistent in number of elements: \n \n";
    ExceptionMessage += " alignment [ ";
    ExceptionMessage += _alignmentCollectionSuffixes.size();
    ExceptionMessage += "]\n";
    ExceptionMessage += " hit [ ";
    ExceptionMessage += _hitCollectionSuffixes.size();
    ExceptionMessage += "]\n";
    ExceptionMessage += " refhit [ ";
    ExceptionMessage += _refhitCollectionSuffixes.size();
    ExceptionMessage += "]\n";
...
void EUTelApplyAlignmentProcessor::processEvent(LCEvent *event) {
  ...
  if (_alignmentCollectionNames.size() <= 0) {
    streamlog_out(ERROR5) << "Alignment collections are UNDEFINED, the "
                             "processor can not continue. EXIT "
                          << endl;
    throw StopProcessingException(this);
  } else {
    ...
    for (int i = static_cast<int>(_alignmentCollectionNames.size()) - 1; i >= 0;
         i--) {
      // read the first available alignment collection
      // CAUTION 1: it might be important to keep the order of alignment
      // collections (if many) given in the opposite direction
      ...
      if (i == static_cast<int>(_alignmentCollectionNames.size()) - 1) {
        ...
        _outputHitCollectionName = _hitCollectionNames.at(
            i); //_input_inputHitCollectionName + _alignmentCollectionName;
        _outputReferenceHitCollectionName = _refhitCollectionNames.at(
            i); // referenceH_referenceHitCollectionName +
                // _alignmentCollectionName;

      } else if (i >= 0) {
        _inputHitCollectionName = _hitCollectionNames.at(i + 1);
        _outputHitCollectionName =
            _hitCollectionNames.at(i); // temp + _alignmentCollectionName;

        _referenceHitCollectionName = _refhitCollectionNames.at(i + 1);
        _outputReferenceHitCollectionName =
            _refhitCollectionNames.at(i); // temp + _alignmentCollectionName;
      }

      CheckIOCollections(event);

      //            streamlog_out ( MESSAGE4 ) << "_doAlignmentInOneGo == " <<
      //            _doAlignmentInOneGo << endl;

      if (_doAlignmentInOneGo != true) // keep for backward compatibility ,
                                       // hopefully to declare OBSOLETE soon
      {
        ...
        } else if (GetApplyAlignmentDirection() == 1) {
          if (_doAlignCollection) {
            Reverse(event);
          }
          if (_doGear) {
      ...
      } else if (_doAlignmentInOneGo ==
                 true) // logic from 01.09.2012 . hopefully to declare PRO soon
      {
        //                      streamlog_out ( MESSAGE4 ) <<
        //                      "_doAlignmentInOneGo == true " << endl;
        ...
        streamlog_out(ERROR5) << "You MUST specifiy whether you want the "
                                 "alignment to be done in one go or not"
                              << endl;
  ...
  if (_applyToReferenceHitCollection) {
    for (size_t jj = 0; jj < _alignmentCollectionNames.size(); jj++) {
      ...
      LCCollectionVec *ref =
          static_cast<LCCollectionVec *>(event->getCollection(_colName));
      for (size_t ii = 0; ii < static_cast<size_t>(ref->getNumberOfElements());
           ii++) {
        streamlog_out(DEBUG5) << " check output_refhit at : " << ref << " ";
        EUTelReferenceHit *output_refhit =
            static_cast<EUTelReferenceHit *>(ref->getElementAt(ii));
...
void EUTelApplyAlignmentProcessor::ApplyGear6D(LCEvent *event) {
...

  {
    ...
    map<int, int>::iterator mapIter =
        _lookUpTable[_alignmentCollectionName].begin();
  ...
  if (_inputCollectionVec == 0) {
    streamlog_out(DEBUG5) << "EUTelApplyAlignmentProcessor::ApplyGear6D. Skip "
                             "this event. Input Collection not found. "
                          << endl;
    return;
  }
  ...
  for (size_t iHit = 0; iHit < _inputCollectionVec->size(); iHit++) {
    ...
    TrackerHitImpl *inputHit =
        dynamic_cast<TrackerHitImpl *>(_inputCollectionVec->getElementAt(iHit));
    ...
    if (_conversionIdMap.size() !=
        static_cast<unsigned>(_siPlanesParameters->getSiPlanesNumber())) {
    ...
    for (int iPlane = 0; iPlane < _siPlanesLayerLayout->getNLayers();
      ...
      if (sensorID == _siPlanesLayerLayout->getID(iPlane)) {
        z_sensor = _siPlanesLayerLayout->getSensitivePositionZ(iPlane) +
                   0.5 * _siPlanesLayerLayout->getSensitiveThickness(iPlane);
        break;
      }
    }
    ...
    if (1 == 1) {
      ...
      if (_debugSwitch) {
        ...
        gRotation[0] = _siPlanesLayerLayout->getLayerRotationXY(
            layerIndex); // Euler alpha ;
        gRotation[1] = _siPlanesLayerLayout->getLayerRotationZX(
            layerIndex); // Euler alpha ;
        gRotation[2] = _siPlanesLayerLayout->getLayerRotationZY(
            layerIndex); // Euler alpha ;
        ...
        telPos[0] =
            _siPlanesLayerLayout->getSensitivePositionX(layerIndex); // mm
        telPos[1] =
            _siPlanesLayerLayout->getSensitivePositionY(layerIndex); // mm
        telPos[2] =
            _siPlanesLayerLayout->getSensitivePositionZ(layerIndex); // mm
      }
      ...
      //          streamlog_out ( DEBUG3 ) << "Sensor ID " << sensorID << " not
      //          found. Skipping alignment for hit "
      //                                    << iHit << endl;
    ...
    if (_iEvt < _printEvents) {
      streamlog_out(DEBUG2) << "ApplyGear: INPUT: Sensor ID " << sensorID << " "
                            << inputPosition[0] << " " << inputPosition[1]
                            << " " << inputPosition[2] << " " << z_sensor
                            << endl;
      streamlog_out(DEBUG2) << "ApplyGear: OUTPUT:Sensor ID " << sensorID << " "
...
void EUTelApplyAlignmentProcessor::RevertGear6D(LCEvent *event) {
  ...
  // final check
  if (_inputCollectionVec == 0) {
    streamlog_out(DEBUG5) << "EUTelApplyAlignmentProcessor::RevertGear6D. Skip "
                             "this event. Input Collection not found. "
                          << endl;
    return;
  }
       ...
       ++iPlane )
          {
              if (sensorID == _siPlanesLayerLayout->getID( iPlane ) )
              {
                  z_sensor = _siPlanesLayerLayout -> getSensitivePositionZ(
       iPlane ) + 0.5 * _siPlanesLayerLayout->getSensitiveThickness( iPlane );
                  break;
              }
          }
    */
    ...
    if (!_applyToReferenceHitCollection || _referenceHitVec == 0) {
      ...
      //        streamlog_out( MESSAGE5 ) << "reference Hit collection name : "
      //        << _referenceHitCollectionName << endl;
      ...
      for (size_t ii = 0;
        ...
        if (sensorID != refhit->getSensorID()) {
          //   streamlog_out( MESSAGE5 ) << "Looping through a varity of sensor
          //   IDs" << endl;
          continue;
        } else {
          ...
          if (_iEvt < _printEvents) {
            streamlog_out(DEBUG2) << "Sensor ID and Alignment plane ID match!"
                                  << endl;
    ...
    const double *inputPosition =
        const_cast<const double *>(inputHit->getPosition());
    double outputPosition[3] = {0., 0., 0.};
    ...
    if (1 == 1) {
      ...
      if (_debugSwitch) {
        ...
        telPos[1] =
            _siPlanesLayerLayout->getSensitivePositionY(layerIndex); // mm
        telPos[2] =
            _siPlanesLayerLayout->getSensitivePositionZ(layerIndex) +
            0.5 * _siPlanesLayerLayout->getSensitiveThickness(layerIndex); // mm
      }
      ...
      // undo the shifts = go back to the center of the sensor frame (rotations
      // unchanged)
      ...
      if (_iEvt < _printEvents) {
        streamlog_out(DEBUG2)
            << "RevertGear: intermediate: " << outputPosition[0] << " "
            << outputPosition[1] << " " << outputPosition[2] << endl;
      }
      TVector3 iCenterOfSensorFrame(outputPosition[0], outputPosition[1],
                                    outputPosition[2]);

      iCenterOfSensorFrame.RotateZ(-gRotation[0]);
      iCenterOfSensorFrame.RotateY(-gRotation[1]);
      iCenterOfSensorFrame.RotateX(
          -gRotation[2]); // first rotaiton in ZY plane -> around X axis (gamma)
      ...
      */
      ...
      // the users if running in DEBUG and copy the not aligned hit
      // in the new collection.
      streamlog_out(DEBUG5) << "Sensor ID " << sensorID
                            << " not found. Skipping alignment for hit " << iHit
                            << endl;
    ...
    if (_iEvt < _printEvents) {
      streamlog_out(DEBUG2) << "RevertGear: INPUT: Sensor ID " << sensorID
                            << " " << inputPosition[0] << " "
                            << inputPosition[1] << " " << inputPosition[2]
                            << endl;
      streamlog_out(DEBUG2) << "RevertGear: OUTPUT:Sensor ID " << sensorID
                            << " " << outputPosition[0] << " "
...
void EUTelApplyAlignmentProcessor::Direct(LCEvent *event) {
  ...
  if (_fevent) {
    streamlog_out(DEBUG2) << "DIRECT:FIRST:REFHIT: The alignment collection ["
                          << _alignmentCollectionName.c_str() << "] contains: "
                          << _alignmentCollectionVec->getNumberOfElements()
                          << " planes " << endl;
      ...
      for (size_t iPos = 0; iPos < _alignmentCollectionVec->size(); ++iPos) {
        EUTelAlignmentConstant *alignment =
            static_cast<EUTelAlignmentConstant *>(
                _alignmentCollectionVec->getElementAt(iPos));
        _lookUpTable[_alignmentCollectionName][alignment->getSensorID()] = iPos;
        streamlog_out(DEBUG3)
            << " _alignmentCollectionVec element position : " << iPos << " ";
    ...
    streamlog_out(DEBUG5) << "DIRECT:FIRST:REFHIT: The alignment collection ["
                          << _alignmentCollectionName.c_str() << "] contains: "
                          << _alignmentCollectionVec->getNumberOfElements()
                          << " planes " << endl;
    ...
    while (mapIter != _lookUpTable[_alignmentCollectionName].end()) {
      streamlog_out(DEBUG5)
          << "DIRECT:FIRST:REFHIT: Sensor ID = " << mapIter->first
          << " is in position " << mapIter->second << endl;
      ++mapIter;
    }
#endif
  ...
  // final check
  if (_inputCollectionVec == 0) {
    streamlog_out(DEBUG5) << "DIRECT:-----:-----: "
                             "EUTelApplyAlignmentProcessor::Direct. Skip this "
                             "event. Input Collection not found. "
                          << endl;
    return;
  }

  streamlog_out(DEBUG5) << "DIRECT:-----:-----: "
                           "EUTelApplyAlignmentProcessor::Direct. going to "
                           "proceeed with "
                        << _inputCollectionVec->size() << " hits " << endl;

  // go-go
  for (size_t iHit = 0; iHit < _inputCollectionVec->size(); iHit++) {

    streamlog_out(DEBUG5) << "DIRECT:-----:-----: hit " << iHit << " of  "
                          << _inputCollectionVec->size() << " hits " << endl;
    ...
    map<int, int>::iterator positionIter =
        _lookUpTable[_alignmentCollectionName].find(sensorID);

    streamlog_out(DEBUG5) << "DIRECT:-----:-----: iHit [" << iHit
                          << "] for sensor  " << sensorID << endl;
    if (positionIter == _lookUpTable[_alignmentCollectionName].end()) {
      streamlog_out(DEBUG5)
          << "DIRECT:-----:-----: wrong sensorID : " << sensorID << " ?? "
          << endl;
      //          continue; //do nothing as if alignment == 0.
    } else {
      EUTelAlignmentConstant *alignment = static_cast<EUTelAlignmentConstant *>(
          _alignmentCollectionVec->getElementAt(positionIter->second));
      streamlog_out(DEBUG5) << "DIRECT:-----:-----: iHit [" << iHit << "] at "
                            << positionIter->second
                            << " found at (EUTelAlignmentConstant*) "
                            << alignment;
      streamlog_out(DEBUG5)
          << " with alignment collection name : " << _alignmentCollectionName
          << endl;
    ...
    if (!_applyToReferenceHitCollection || _referenceHitVec == 0) {
      // todo: is this case (no reference vector) treated correctly?
      streamlog_out(DEBUG5) << "DIRECT:-----:-----: _referenceHitVec is empty"
                            << endl;
    } else {
      streamlog_out(DEBUG5)
          << "DIRECT:-----:-----: reference Hit collection name : "
          << _referenceHitCollectionName << endl;
      ...
      for (size_t ii = 0;
        ...
        EUTelReferenceHit *refhit = static_cast<EUTelReferenceHit *>(
            _referenceHitVec->getElementAt(ii));
        if (sensorID != refhit->getSensorID()) {
          //          streamlog_out( DEBUG5 ) << "DIRECT:-----:-----: Looping
          //          through a varity of sensor IDs" << endl;
          continue;
        } else {
          streamlog_out(DEBUG5)
              << "DIRECT:-----:-----: Sensor ID and Alignment plane ID match!"
              << endl;
          x_refhit = refhit->getXOffset();
          y_refhit = refhit->getYOffset();
          z_refhit = refhit->getZOffset();

          // possible source of inconsistency; Apply to reference hit collection
          // flag should be enabled in steering file
          // otherwise the following three lines (undo alignment shifts) makes
          // no sense !!!
          x_refhit += offsetX;
          y_refhit += offsetY;
          z_refhit += offsetZ;
        ...
        streamlog_out(DEBUG5)
            << "DIRECT:-----:-----: sensorID " << refhit->getSensorID()
            << " X: " << refhit->getXOffset() << " Y: " << refhit->getYOffset()
            << " Z: " << refhit->getZOffset() << " X_REFHIT: " << x_refhit
            << " Y_REFHIT: " << y_refhit << " Z_REFHIT: " << z_refhit << "  "
            << endl;
      }
    }
    streamlog_out(DEBUG5) << "DIRECT:-----:-----: refhit found for sensorID "
                          << sensorID << endl;
    ...
    // hit coordinates in the center-of-the sensor frame (axis coincide with the
    // global frame)
    ...
    // calculate hit position on a sensor relative to its center (assuming that
    // refhit is still without alignment corrections)
    inputPosition[0] = inputPosition[0] - x_refhit;
    inputPosition[1] = inputPosition[1] - y_refhit;
    inputPosition[2] = inputPosition[2] - z_refhit;

    // initial setting of the sensor = center-of-the sensor coordinates in the
    // global frame
    double outputPosition[3] = {0., 0., 0.};
...
//      if ( positionIter != _lookUpTable[ _alignmentCollectionName ].end() ) {
    ...
    if (_histogramSwitch) {
      ...
      if (AIDA::IHistogram2D *histo = dynamic_cast<AIDA::IHistogram2D *>(
        ...
        streamlog_out(ERROR1)
            << "DIRECT:-----:-----: Not able to retrieve histogram pointer for "
            << tempHistoName << ".\nDisabling histogramming from now on "
            << endl;
        _histogramSwitch = false;
      }
      //          histo3D = dynamic_cast<AIDA::IHistogram3D*> (_aidaHistoMap[
      //          _densityPlotBeforeAlignName ] );
      //          if ( histo3D ) histo3D->fill( inputPosition[0],
      //          inputPosition[1], inputPosition[2] );
      //          else {
      //            streamlog_out ( ERROR1 )  << "DIRECT:-----:-----: Not able
      //            to retrieve histogram pointer for " <<
      //            _densityPlotBeforeAlignName
      //                                      << ".\nDisabling histogramming
      //                                      from now on " << endl;
      //            _histogramSwitch = false;
      //          }
    }
    ...
    if (_correctionMethod == 0) {
      ...
      if (_iEvt < _printEvents) {
        ...
        streamlog_out(DEBUG5)
            << "DIRECT:-----:-----: _correctionMethod == rotation first "
            << endl;
        streamlog_out(DEBUG5)
            << "DIRECT:-----:-----:  alignment->getAlpha() = " << alpha << endl;
        streamlog_out(DEBUG5)
            << "DIRECT:-----:-----:  alignment->getBeta()  = " << beta << endl;
        streamlog_out(DEBUG5)
            << "DIRECT:-----:-----:  alignment->getGamma() = " << gamma << endl;
        streamlog_out(DEBUG5)
            << "DIRECT:-----:-----:  alignment->getXOffest() = " << offsetX
            << endl;
        streamlog_out(DEBUG5)
            << "DIRECT:-----:-----:  alignment->getYOffest() = " << offsetY
            << endl;
        streamlog_out(DEBUG5)
            << "DIRECT:-----:-----:  alignment->getZOffest() = " << offsetZ
            << endl;
      }
      // this is the rotation first
      TVector3 iCenterOfSensorFrame(inputPosition[0], inputPosition[1],
                                    inputPosition[2]);
    ...
    if (_histogramSwitch) {
      ...
      //          histo3D = dynamic_cast<AIDA::IHistogram3D*> (_aidaHistoMap[
      //          _densityPlotAfterAlignName ] );
      //          if ( histo3D ) histo3D->fill( outputPosition[0],
      //          outputPosition[1], outputPosition[2] );
      //          else {
      //            streamlog_out ( ERROR1 )  << "DIRECT:-----:-----: Not able
      //            to retrieve histogram pointer for " <<
    ...
    /*
          ...
          else
          {
            ...
            // the users if running in DEBUG and copy the not aligned hit
            // in the new collection.
    //        streamlog_out ( DEBUG5 ) << "Sensor ID " << sensorID << " not
    found. Skipping alignment for hit "
    //                                << iHit << endl;
    ...
    */
    ...
    if (_iEvt < _printEvents) {
      streamlog_out(DEBUG1)
          << "DIRECT:-----:-----:" << _alignmentCollectionName.c_str()
          << " : ORIGI: Sensor ID " << sensorID << " " << inputPosition_orig[0]
          << " " << inputPosition_orig[1] << " " << inputPosition_orig[2]
          << endl;
      streamlog_out(DEBUG1)
          << "DIRECT:-----:-----:" << _alignmentCollectionName.c_str()
          << " : INPUT: Sensor ID " << sensorID << " " << inputPosition[0]
          << " " << inputPosition[1] << " " << inputPosition[2] << endl;
      streamlog_out(DEBUG1)
          << "DIRECT:-----:-----:" << _alignmentCollectionName.c_str()
          << " : OUTPUT:Sensor ID " << sensorID << " " << outputPosition[0]
          << " " << outputPosition[1] << " " << outputPosition[2] << endl;
    }
...
void EUTelApplyAlignmentProcessor::Reverse(LCEvent *event) {
  ...
  if (_fevent) {
    streamlog_out(MESSAGE5) << "REVERSE: The alignment collection ["
                            << _alignmentCollectionName.c_str()
                            << "] contains: " << _alignmentCollectionVec
                            << endl;
    streamlog_out(MESSAGE5) << "REVERSE: The alignment collection ["
                            << _alignmentCollectionName.c_str()
                            << "] contains: " << _alignmentCollectionVec->size()
                            << " planes " << endl;
    ...
    if (_alignmentCollectionVec->size() > 0) {
      ...
      for (size_t iPos = 0; iPos < _alignmentCollectionVec->size(); ++iPos) {
        ...
        streamlog_out(MESSAGE5)
            << " _alignmentCollectionVec element position : " << iPos << " "
            << " align for " << alignment->getSensorID() << endl;
...
//            }
  ...
  streamlog_out(DEBUG5) << "REVERSE: The alignment collection ["
                        << _alignmentCollectionName.c_str()
                        << "] contains: " << _alignmentCollectionVec->size()
                        << " planes " << endl;
  streamlog_out(DEBUG5) << "REVERSE: _lookUpTable contains "
                        << _lookUpTable.size() << " elements " << endl;
  ...
  // final check
  if (_inputCollectionVec == 0) {
    streamlog_out(DEBUG5) << "REVERSE: EUTelApplyAlignmentProcessor::Reverse. "
                             "Skip this event. Input Collection not found. "
                          << endl;
    return;
  }
  ...
  for (size_t iHit = 0; iHit < _inputCollectionVec->size(); iHit++) {
    ...
    if (positionIter == _lookUpTable[_alignmentCollectionName].end()) {
      ...
      streamlog_out(DEBUG5) << "REVERSE: FOUND alignment collection "
                            << _alignmentCollectionName.c_str()
                            << " record for sensorId " << sensorID << endl;
      EUTelAlignmentConstant *alignment = static_cast<EUTelAlignmentConstant *>(
          _alignmentCollectionVec->getElementAt(positionIter->second));
    ...
    if (!_applyToReferenceHitCollection || _referenceHitVec == 0) {
      ...
      if (_fevent)
        streamlog_out(MESSAGE5) << "REVERSE: reference Hit collection name : "
                                << _referenceHitCollectionName << " at "
                                << _referenceHitVec << endl;
      ...
      for (size_t ii = 0;
        ...
        if (sensorID != refhit->getSensorID()) {
          ...
          // do not apply this part: refhits should be just as they where befre
          // the alignment has been applied
          // = it means the anti-apply alignment should have been applied
          // already in the AlignReferenceHit
          //---//
          //          x_refhit -= offsetX;
          //          y_refhit -= offsetY;
          //          z_refhit -= offsetZ;
    ...
    // now that we know at which sensor the hit belongs to, we can
    // get the corresponding alignment constants
    // map< int , int >::iterator  positionIter = _lookUpTable[
    // _alignmentCollectionName ].find( sensorID );
    ...
    //            const double * inputPosition = const_cast< const double * > (
    //            inputHit->getPosition() ) ;
    //            double   outputPosition[3]   = { 0., 0., 0. };
...
//            if ( positionIter != _lookUpTable[ _alignmentCollectionName
//            ].end() )
//            {
    ...
    if (_histogramSwitch) {
      ...
      //                    histo3D = dynamic_cast<AIDA::IHistogram3D*>
      //                    (_aidaHistoMap[ _densityPlotBeforeAlignName ] );
      //                    if ( histo3D )
      //                    {
      //                        histo3D->fill( inputPosition[0],
      //                        inputPosition[1], inputPosition[2] );
      //                    }
      //                    else
      //                    {
      //                        streamlog_out ( ERROR1 )  << "Not able to
      //                        retrieve histogram pointer for " <<
      //                        _densityPlotBeforeAlignName
      //                                                  << ".\nDisabling
      //                                                  histogramming from now
      //                                                  on " << endl;
      //                       _histogramSwitch = false;
      //                    }
    }
    ...
    if (_correctionMethod == 0) {
      ...
      if (_iEvt < _printEvents) {
        ...
        streamlog_out(DEBUG1) << " alignment->getXOffest() = " << offsetX
                              << endl;
        streamlog_out(DEBUG1) << " alignment->getYOffest() = " << offsetY
                              << endl;
        streamlog_out(DEBUG1) << " alignment->getZOffest() = " << offsetZ
                              << endl;
      }
    ...
    if (_histogramSwitch) {
      ...
      //                    histo3D = dynamic_cast<AIDA::IHistogram3D*>
      //                    (_aidaHistoMap[ _densityPlotAfterAlignName ] );
      //                    if ( histo3D )
      //                    {
      //                        histo3D->fill( outputPosition[0],
      //                        outputPosition[1], outputPosition[2] );
      //                    }
      //                    else
      //                    {
    ...
    /*
                ...
                else
                {
                    ...
                    // this hit belongs to a plane whose sensorID is not in the
                    // alignment constants. So the idea is to eventually advice
                    // the users if running in DEBUG and copy the not aligned
       hit
                    // in the new collection.

                    streamlog_out ( DEBUG5 ) << "Sensor ID " << sensorID << "
       not found. Skipping alignment for hit "
                                              << iHit << endl;
    ...
    */
    if (_iEvt < _printEvents) {
      streamlog_out(DEBUG0)
          << _alignmentCollectionName.c_str() << " REVERT: ORIGI: Sensor ID "
          << sensorID << " " << inputPosition_orig[0] << " "
          << inputPosition_orig[1] << " " << inputPosition_orig[2] << endl;
      streamlog_out(DEBUG0) << _alignmentCollectionName.c_str()
                            << " REVERT: INPUT: Sensor ID " << sensorID << " "
                            << inputPosition[0] << " " << inputPosition[1]
                            << " " << inputPosition[2] << endl;
      streamlog_out(DEBUG0) << _alignmentCollectionName.c_str()
                            << " REVERT: OUTPUT:Sensor ID " << sensorID << " "
                            << outputPosition[0] << " " << outputPosition[1]
...
// cout << "19" << endl;
  ...
  try {
    ...
    for (int iDet = 0; iDet < _siPlanesParameters->getSiPlanesNumber();
         iDet++) {
      int sensorID = _siPlanesLayerLayout->getID(iDet);

      streamlog_out(MESSAGE4) << "Booking histograms for sensorID: " << sensorID
                              << endl;
      ...
      double xMin = safetyFactor *
                    (_siPlanesLayerLayout->getSensitivePositionX(iDet) -
                     (0.5 * _siPlanesLayerLayout->getSensitiveSizeX(iDet)));
      double xMax = safetyFactor *
                    (_siPlanesLayerLayout->getSensitivePositionX(iDet) +
                     (0.5 * _siPlanesLayerLayout->getSensitiveSizeX(iDet)));

      double yMin = safetyFactor *
                    (_siPlanesLayerLayout->getSensitivePositionY(iDet) -
                     (0.5 * _siPlanesLayerLayout->getSensitiveSizeY(iDet)));
      double yMax = safetyFactor *
                    (_siPlanesLayerLayout->getSensitivePositionY(iDet) +
                     (0.5 * _siPlanesLayerLayout->getSensitiveSizeY(iDet)));

      int xNBin = static_cast<int>(safetyFactor) *
                  _siPlanesLayerLayout->getSensitiveNpixelX(iDet);
      int yNBin = static_cast<int>(safetyFactor) *
                  _siPlanesLayerLayout->getSensitiveNpixelY(iDet);
      ...
      AIDA::IHistogram2D *hitHistoBeforeAlign =
          AIDAProcessor::histogramFactory(this)->createHistogram2D(
              (basePath + tempHistoName).c_str(), xNBin, xMin, xMax, yNBin,
              yMin, yMax);

      if (hitHistoBeforeAlign) {
        hitHistoBeforeAlign->setTitle(
            "Hit map in the telescope frame of reference before align");
        ...
        streamlog_out(ERROR1) << "Problem booking the "
                              ...
                              << "Very likely a problem with path name. "
      ...
      if (hitHistoAfterAlign) {
        hitHistoAfterAlign->setTitle(
            "Hit map in the telescope frame of reference after align");
    ...
    for (int iPlane = 0; iPlane < _siPlanesParameters->getSiPlanesNumber();
      ...
      // x axis
      xMin = min(_siPlanesLayerLayout->getSensitivePositionX(iPlane) -
                     (0.5 * _siPlanesLayerLayout->getSensitiveSizeX(iPlane)),
                 xMin);
      xMax = max(_siPlanesLayerLayout->getSensitivePositionX(iPlane) +
                     (0.5 * _siPlanesLayerLayout->getSensitiveSizeX(iPlane)),
                 xMax);
      xNBin = max(_siPlanesLayerLayout->getSensitiveNpixelX(iPlane), xNBin);

      // y axis
      yMin = min(_siPlanesLayerLayout->getSensitivePositionY(iPlane) -
                     (0.5 * _siPlanesLayerLayout->getSensitiveSizeY(iPlane)),
                 yMin);
      yMax = max(_siPlanesLayerLayout->getSensitivePositionY(iPlane) +
                     (0.5 * _siPlanesLayerLayout->getSensitiveSizeY(iPlane)),
                 yMax);
    ...
    /*    AIDA::IHistogram3D * densityPlot =
       AIDAProcessor::histogramFactory(this)->createHistogram3D(
       _densityPlotBeforeAlignName ,
                                                                                                     "Hit position in the telescope frame of reference before align",
                                                                                                     xAxis, yAxis, zAxis, "");

        if ( densityPlot ) {
          _aidaHistoMap.insert( make_pair ( _densityPlotBeforeAlignName,
       densityPlot ) ) ;
        } else {
          streamlog_out ( ERROR1 )  << "Problem booking the " <<
       (_densityPlotBeforeAlignName) << ".\n"
       ...
       Switching off histogramming and continue w/o" << endl;
        ...
        densityPlot = AIDAProcessor::histogramFactory(this)->createHistogram3D(
       _densityPlotAfterAlignName ,
                                                                                "Hit position in the telescope frame of reference after align",
                                                                                xAxis, yAxis, zAxis, "");

        if ( densityPlot ) {
          _aidaHistoMap.insert( make_pair ( _densityPlotAfterAlignName,
       densityPlot ) ) ;
        } else {
          streamlog_out ( ERROR1 )  << "Problem booking the " <<
       (_densityPlotAfterAlignName) << ".\n"
    ...
    */
    ...
    streamlog_out(ERROR1) << "No AIDAProcessor initialized. Type q to exit or "
                             "c to continue without histogramming"
                          << endl;
    string answer;
    while (true) {
...
void EUTelApplyAlignmentProcessor::TransformToLocalFrame(
    TrackerHitImpl *outputHit) {
  ...
  xPointing[0] =
      _siPlanesLayerLayout->getSensitiveRotation1(layerIndex); // was -1 ;
  xPointing[1] =
      _siPlanesLayerLayout->getSensitiveRotation2(layerIndex); // was  0 ;
  yPointing[0] =
      _siPlanesLayerLayout->getSensitiveRotation3(layerIndex); // was  0 ;
  yPointing[1] =
      _siPlanesLayerLayout->getSensitiveRotation4(layerIndex); // was -1 ;
  ...
  TVector2 clusterCenter(flip1(0, 0) / xPitch - 0.5,
                         flip1(1, 0) / yPitch - 0.5);
  if (_iEvt < _printEvents) {
    streamlog_out(DEBUG1) << "RevertGear: antiflip: " << antiflip(0, 0) << ":"
                          << antiflip(1, 0) << endl;
    streamlog_out(DEBUG1) << "RevertGear: antiflip: " << antiflip(0, 1) << ":"
                          << antiflip(1, 1) << endl;
    streamlog_out(DEBUG1) << "RevertGear: matrix: " << flip1(0, 0) << ":"
                          << flip1(1, 0) << endl;
    streamlog_out(DEBUG1) << "RevertGear: cluster coordinates [pitch:" << xPitch
                          << ":" << yPitch << "]:" << clusterCenter.X() << " "
                          << clusterCenter.Y() << endl;
  }

  EUTelVirtualCluster *cluster;
  ClusterType type =
      static_cast<ClusterType>(static_cast<int>(outputHit->getType()));
  if (_iEvt < _printEvents) {
    streamlog_out(DEBUG2) << "RevertGear: hit/cluster type:"
  ...
  for (size_t i = 0; i < clusterVec.size(); i++) {
    ...
    if (type == kEUTelSparseClusterImpl) {
      cluster = new EUTelSparseClusterImpl<EUTelGenericSparsePixel>(
          static_cast<TrackerDataImpl *>(clusterVec[i]));
    } else {
    ...
    if (_iEvt < _printEvents) {
      streamlog_out(DEBUG0) << "[CoG:" << xCoG << ":" << yCoG
                            << " -> Det:" << xDet << ":" << yDet
                            << "] == " << telPos[0] << ":" << telPos[1];
    }
  }
...
void EUTelApplyAlignmentProcessor::revertAlignment(double &x, double &y,
                                                   double &z) {
  ...
  for (size_t iPos = 0; iPos < _alignmentCollectionVec->size(); ++iPos) {
    ...
    c = static_cast<EUTelAlignmentConstant *>(
        _alignmentCollectionVec->getElementAt(iPos));
    if (c->getSensorID() == _manualDUTid)
      break; // this means we found the alignment constant corresponding
  ...
  // the way we apply constants is correctionMethod1 (first the rotations,
  // second the shifts)
  // to revert the alignment transformation properly, the constants have to be
  // applied in
  // a reverted way, i.e. first the shifts, second the rotations

  // not that the sign is different for all the constants wrt to what is done in
  ...
  // further more a more clear way would be to multiply an input vector by an
  // inverse rotation matrix
  // Igor Rubinsky 09-10-2011

  x = x_temp * (1 + alpha * alpha) + ((-1) * gamma - alpha * beta) * y_temp +
      ((-1) * beta + alpha * gamma) * z_temp;
  y = x_temp * (gamma - alpha * beta) + (1 + beta * beta) * y_temp +
      ((-1) * alpha - beta * gamma) * z_temp;
  z = x_temp * (beta + alpha * gamma) + (alpha - gamma * beta) * y_temp +
      (1 + gamma * gamma) * z_temp;
...
void EUTelApplyAlignmentProcessor::_EulerRotation(double *_telPos,
                                                  double *_gRotation) {

  TVector3 _RotatedSensorHit(_telPos[0], _telPos[1], _telPos[2]);

  if (TMath::Abs(_gRotation[2]) > 1e-6)
    _RotatedSensorHit.RotateX(_gRotation[2]); // in ZY
  if (TMath::Abs(_gRotation[1]) > 1e-6)
    _RotatedSensorHit.RotateY(_gRotation[1]); // in ZX
  if (TMath::Abs(_gRotation[0]) > 1e-6)
    _RotatedSensorHit.RotateZ(_gRotation[0]); // in XY
...
void EUTelApplyAlignmentProcessor::_EulerRotationInverse(double *_telPos,
                                                         double *_gRotation) {

  TVector3 _RotatedSensorHit(_telPos[0], _telPos[1], _telPos[2]);

  if (TMath::Abs(_gRotation[0]) > 1e-6)
    _RotatedSensorHit.RotateZ(-1. * _gRotation[0]); // in XY
  if (TMath::Abs(_gRotation[1]) > 1e-6)
    _RotatedSensorHit.RotateY(-1. * _gRotation[1]); // in ZX
  if (TMath::Abs(_gRotation[2]) > 1e-6)
    _RotatedSensorHit.RotateX(-1. * _gRotation[2]); // in ZY
...
void EUTelApplyAlignmentProcessor::AlignReferenceHit(
    EUTelEventImpl *evt, EUTelAlignmentConstant *alignment) {
  int iPlane = alignment->getSensorID();
  streamlog_out(DEBUG5) << "AlignReferenceHit for sensor ID:"
                        << alignment->getSensorID() << endl;
  ...
  streamlog_out(DEBUG2) << "reference Hit collection name : "
                        << _referenceHitCollectionName.c_str() << " elements "
                        << _referenceHitVec->getNumberOfElements() << endl;
  ...
  for (size_t ii = 0;
    ...
    try {
      output_refhit = static_cast<EUTelReferenceHit *>(
          _outputReferenceHitVec->getElementAt(ii));
      streamlog_out(DEBUG1) << "existing refhit[" << ii << "] found at  "
                            << output_refhit << endl;

    } catch (...) {
      streamlog_out(DEBUG2) << " creating new EUTelReferenceHit element " << ii
                            << endl;
      output_refhit = new EUTelReferenceHit();
      streamlog_out(DEBUG1) << "new refhit for [" << ii << "] created at  "
                            << output_refhit << endl;
    ...
    if (iPlane != refhit->getSensorID()) {
      streamlog_out(DEBUG3)
          << "Looping through a varity of sensor IDs for iPlane :" << iPlane
          << " and refhit->getSensorID()=" << refhit->getSensorID() << endl;
      continue;
    } else {
    ...
    streamlog_out(DEBUG2) << "reference Hit collection name : "
                          << _referenceHitCollectionName.c_str() << " elements "
                          << _referenceHitVec->getNumberOfElements() << " ";
    ...
    if (GetApplyAlignmentDirection() == 0) {
      ...
      TVector3 _RotatedVector(refhit->getAlpha(), refhit->getBeta(),
                              refhit->getGamma());
...
LCCollectionVec *
EUTelApplyAlignmentProcessor::CreateDummyReferenceHitCollection() {
