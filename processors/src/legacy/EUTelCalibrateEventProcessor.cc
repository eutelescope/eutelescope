...
#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
string
  EUTelCalibrateEventProcessor::_rawDataDistHistoName = "RawDataDistHisto";
string
  EUTelCalibrateEventProcessor::_dataDistHistoName = "DataDistHisto";
string
  EUTelCalibrateEventProcessor::_commonModeDistHistoName =
  "CommonModeDistHisto";
string
  EUTelCalibrateEventProcessor::_skippedRowDistHistoName =
  "SkippedRowDistHisto";
string
  EUTelCalibrateEventProcessor::_skippedPixelDistHistoName =
  "SkippedPixelDistHisto";
string
  EUTelCalibrateEventProcessor::_skippedPixelPerRowDistHistoName =
  "SkippedPixelPerRowDistHisto";
#endif

const unsigned short
  EUTelCalibrateEventProcessor::_maxNoOfConsecutiveMissing = 10;

EUTelCalibrateEventProcessor::EUTelCalibrateEventProcessor ():Processor
  ("EUTelCalibrateEventProcessor")
{

  // modify processor description
  _description = "EUTelCalibrateEventProcessor subtracts the pedestal value "
    "from the input data";
  ...
  registerProcessorParameter ("PerformCommonMode", "Flag to switch on the "
			      ...
			      "algorithm. 0 -> off, 1 -> "
			      "full frame,  2 -> row wise", _doCommonMode, 1);
...
void
EUTelCalibrateEventProcessor::init ()
{
  ...
  if (_fillDebugHisto == 1)
	...
	<< "Filling debug histograms is slowing down the procedure" << endl;
    }
...
void
EUTelCalibrateEventProcessor::processRunHeader (LCRunHeader * rdr)
{
...
void
EUTelCalibrateEventProcessor::initializeGeometry (LCEvent * event)
{
  ...
  try
  {
    LCCollectionVec *pedestalCol =
      dynamic_cast <
      LCCollectionVec * >(event->getCollection (_pedestalCollectionName));
    CellIDDecoder < TrackerDataImpl > pedestalDecoder (pedestalCol);
    for (size_t iDetector = 0; iDetector < pedestalCol->size (); ++iDetector)
      {
	TrackerDataImpl *pedestal =
	  dynamic_cast <
	  TrackerDataImpl * >(pedestalCol->getElementAt (iDetector));

	int sensorID = pedestalDecoder (pedestal)["sensorID"];

	_ancillaryIndexMap.insert (make_pair (sensorID, iDetector));

	unsigned int noOfPixel = (pedestalDecoder (pedestal)["xMax"] -
				  pedestalDecoder (pedestal)["xMin"] + 1) *
	  (pedestalDecoder (pedestal)["yMax"] -
	   pedestalDecoder (pedestal)["yMin"] + 1);

	_noOfPixelMap.insert (make_pair (sensorID, noOfPixel));

	unsigned int noOfPixelPerRow = (pedestalDecoder (pedestal)["xMax"] -
					pedestalDecoder (pedestal)["xMin"] +
					1);
	_noOfPixelPerRowMap.insert (make_pair (sensorID, noOfPixelPerRow));

	unsigned int noOfRow = (pedestalDecoder (pedestal)["yMax"] -
				pedestalDecoder (pedestal)["yMin"] + 1);
  ...
  } catch (lcio::DataNotAvailableException &)
  {
    streamlog_out (WARNING2) << "Unable to initialize the geometry with the "
      "current event. Trying with the next one" << endl;
    _isGeometryReady = false;
...
void
EUTelCalibrateEventProcessor::processEvent (LCEvent * event)
{
  ...
  try
  {
    ...
    LCCollectionVec *inputCollectionVec =
      dynamic_cast <
      LCCollectionVec * >(evt->getCollection (_rawDataCollectionName));
    LCCollectionVec *pedestalCollectionVec =
      dynamic_cast <
      LCCollectionVec * >(evt->getCollection (_pedestalCollectionName));
	...
	for (unsigned int iDetector = 0;
	     iDetector < inputCollectionVec->size (); iDetector++)
	  {
		...
		ss << "Input data and pedestal are incompatible\n"
		  << "Detector " << sensorID << " has "
		  << rawData->getADCValues ().
		  size () << " pixels in the input data \n" << "while " <<
		  pedestal->getChargeValues ().
		  size () << " in the pedestal data " << endl;
...
// histograms are grouped in directory identifying the detector
	    ...
	    if ((_fillDebugHisto == 1) || (_doCommonMode == 1) ||
		(_doCommonMode == 2))
	      {
		...
		tempHistoName =
		  _rawDataDistHistoName + "_d" + to_string (sensorID);

		int rawDataDistHistoNBin = 4096;
		double rawDataDistHistoMin = -2048.5;
		double rawDataDistHistoMax =
		  rawDataDistHistoMin + rawDataDistHistoNBin;
		string rawDataDistTitle = "Raw data distribution";
		if (isHistoManagerAvailable)
		  {
		    histoInfo =
		      histoMgr->getHistogramInfo (_rawDataDistHistoName);
		    if (histoInfo)
		      {
			...
			if (histoInfo->_title != "")
			  rawDataDistTitle = histoInfo->_title;
		      }
		  }
		AIDA::IHistogram1D * rawDataDistHisto =
		  AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath + tempHistoName).c_str (), rawDataDistHistoNBin, rawDataDistHistoMin, rawDataDistHistoMax);
		if (rawDataDistHisto)
		  {
		    _aidaHistoMap.
		      insert (make_pair (tempHistoName, rawDataDistHisto));
		    rawDataDistHisto->setTitle (rawDataDistTitle.c_str ());
		  }
		else
		  {
		    streamlog_out (ERROR1)
		      << "Problem booking the " << (basePath +
						    tempHistoName) << ".\n" <<
		      "Very likely a problem with path name. Switching off "
		      "histogramming and continue w/o" << endl;
		    _fillDebugHisto = 0;
		  }

		// book the pedestal corrected data histogram
		tempHistoName =
		  _dataDistHistoName + "_d" + to_string (sensorID);
		...
		if (isHistoManagerAvailable)
		  {
		    histoInfo =
		      histoMgr->getHistogramInfo (_dataDistHistoName);
		    if (histoInfo)
		      {
			streamlog_out (DEBUG2) << (*histoInfo) << endl;
			...
			if (histoInfo->_title != "")
			  dataDistTitle = histoInfo->_title;
		      }
		  }
		AIDA::IHistogram1D * dataDistHisto =
		  AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath + tempHistoName).c_str (), dataDistHistoNBin, dataDistHistoMin, dataDistHistoMax);
		if (dataDistHisto)
		  {
		    _aidaHistoMap.
		      insert (make_pair (tempHistoName, dataDistHisto));
		...
		tempHistoName =
		  _commonModeDistHistoName + "_d" + to_string (sensorID);
		...
		if (isHistoManagerAvailable)
		  {
		    histoInfo =
		      histoMgr->getHistogramInfo (_commonModeDistHistoName);
		    if (histoInfo)
		      {
			streamlog_out (DEBUG2) << (*histoInfo) << endl;
			...
			if (histoInfo->_title != "")
			  commonModeTitle = histoInfo->_title;
		      }
		  }
		AIDA::IHistogram1D * commonModeDistHisto =
		  AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath + tempHistoName).c_str (), commonModeDistHistoNBin, commonModeDistHistoMin, commonModeDistHistoMax);
		if (commonModeDistHisto)
		  {
		    _aidaHistoMap.
		      insert (make_pair (tempHistoName, commonModeDistHisto));
		...
		tempHistoName =
		  _skippedPixelDistHistoName + "_d" + to_string (sensorID);

		int skippedPixelDistHistoNBin =
		  static_cast < int >(_noOfPixelMap[sensorID] / 100.);
		double skippedPixelDistHistoMin = 0.;
		double skippedPixelDistHistoMax =
		  static_cast < double >(_noOfPixelMap[sensorID]);
		string skippedPixelDistTitle =
		  "Pixel exceeding the hit rejection cut";

		AIDA::IHistogram1D * skippedPixelDistHisto =
		  AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath + tempHistoName).c_str (), skippedPixelDistHistoNBin, skippedPixelDistHistoMin, skippedPixelDistHistoMax);
		if (skippedPixelDistHisto)
		  {
		    _aidaHistoMap.
		      insert (make_pair
			      (tempHistoName, skippedPixelDistHisto));
		...
		tempHistoName =
		  _skippedPixelPerRowDistHistoName + "_d" +
		  to_string (sensorID);

		int skippedPixelPerRowDistHistoNBin =
		  _noOfPixelPerRowMap[sensorID] + 1;
		double skippedPixelPerRowDistHistoMin = -0.5;
		double skippedPixelPerRowDistHistoMax =
		  _noOfPixelPerRowMap[sensorID] + 0.5;
		string skippedPixelPerRowDistTitle =
		  "Pixel in a row exceeding the hit rejection cut";

		AIDA::IHistogram1D * skippedPixelPerRowDistHisto =
		  AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath + tempHistoName).c_str (), skippedPixelPerRowDistHistoNBin, skippedPixelPerRowDistHistoMin, skippedPixelPerRowDistHistoMax);
		if (skippedPixelPerRowDistHisto)
		  {
		    _aidaHistoMap.
		      insert (make_pair
			      (tempHistoName, skippedPixelPerRowDistHisto));
		    skippedPixelPerRowDistHisto->
		      setTitle (skippedPixelPerRowDistTitle);
		  }
		else
		  {
		    streamlog_out (ERROR1)
		...
		tempHistoName =
		  _skippedRowDistHistoName + "_d" + to_string (sensorID);
		...
		string skippedRowDistTitle =
		  "Row rejected in common mode calculation";

		AIDA::IHistogram1D * skippedRowDistHisto =
		  AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath + tempHistoName).c_str (), skippedRowDistHistoNBin, skippedRowDistHistoMin, skippedRowDistHistoMax);
		if (skippedRowDistHisto)
		  {
		    _aidaHistoMap.
		      insert (make_pair (tempHistoName, skippedRowDistHisto));
    ...
    LCCollectionVec *correctedDataCollection =
    ...
    for (unsigned int iDetector = 0; iDetector < inputCollectionVec->size ();
	 iDetector++)
      {
	...
	TrackerDataImpl *pedestal =
	  dynamic_cast <
	  TrackerDataImpl *
	  >(pedestalCollectionVec->getElementAt (ancillaryPos));
	TrackerDataImpl *noise =
	  dynamic_cast <
	  TrackerDataImpl *
	  >(noiseCollectionVec->getElementAt (ancillaryPos));
	TrackerRawDataImpl *status =
	  dynamic_cast <
	  TrackerRawDataImpl *
	  >(statusCollectionVec->getElementAt (ancillaryPos));

	TrackerDataImpl *corrected = new TrackerDataImpl;
	CellIDEncoder < TrackerDataImpl >
	  idDataEncoder (EUTELESCOPE::MATRIXDEFAULTENCODING,
			 correctedDataCollection);
	idDataEncoder["sensorID"] = sensorID;
	idDataEncoder["xMin"] =
	  static_cast < int >(cellDecoder (rawData)["xMin"]);
	idDataEncoder["xMax"] =
	  static_cast < int >(cellDecoder (rawData)["xMax"]);
	idDataEncoder["yMin"] =
	  static_cast < int >(cellDecoder (rawData)["yMin"]);
	idDataEncoder["yMax"] =
	  static_cast < int >(cellDecoder (rawData)["yMax"]);
	...
	FloatVec::const_iterator pedIter =
	  pedestal->getChargeValues ().begin ();
	FloatVec::const_iterator noiseIter =
	  noise->getChargeValues ().begin ();
	ShortVec::const_iterator statusIter =
	  status->getADCValues ().begin ();
	...
	if (_doCommonMode == 1)
	  {
	    ...
	    if (((_maxNoOfRejectedPixels == -1) ||
		 (skippedPixel < _maxNoOfRejectedPixels)) && (goodPixel != 0))
	      {
		...
		if (AIDA::IHistogram1D * histo =
		    dynamic_cast <
		    AIDA::IHistogram1D * >(_aidaHistoMap[tempHistoName]))
	...
	else if (_doCommonMode == 2)
	  {
		...
		for (int xPixel = _minX[iDetector];
		     xPixel <= _maxX[iDetector]; xPixel++)
		  {
		    bool isHit = ((adcValues[iPixel] - _pedestal[iPixel]) >
				  _hitRejectionCut * _noise[iPixel]);
	...
	if (isEventValid)
	  {
		...
		for (int yPixel = _minY[iDetector];
		     yPixel <= _maxY[iDetector]; yPixel++)
		  {
		    for (int xPixel = _minX[iDetector];
			 xPixel <= _maxX[iDetector]; xPixel++)
		      {
			...
			double correctedValue =
			  adcValues[iPixel] - _pedestal[iPixel] -
			  commonModeCorVec[iPixel];
	...
	else
	  {
		...
		streamlog_out (WARNING4)
		  << "Skipping event " << evt->getEventNumber ()
		  << " because of maximum number of pixel exceeded ("
		  << skippedPixel << ")" << endl;
	      }
	    else if (_doCommonMode == 2)
	      {
		streamlog_out (WARNING4)
		  << "Skipping event " << evt->getEventNumber ()
		  << " because of maximum number of skipped row exceeded ("
		  << skippedRow << ")" << endl;
	      }
	    else
	      {
		streamlog_out (WARNING4) << "Skipping event " << evt->
		  getEventNumber () << " for an unknown reason " << endl;
	      }
    ...
    evt->addCollection (correctedDataCollection,
			_calibratedDataCollectionName);
	...
	if (_noOfConsecutiveMissing == _maxNoOfConsecutiveMissing)
	  {
	    streamlog_out (MESSAGE2)
	      <<
	      "Assuming the run was taken in ZS. Not issuing any other warning"
	      << endl;
	  }
...
void
EUTelCalibrateEventProcessor::check (LCEvent * /* evt */ )
{
...
void
EUTelCalibrateEventProcessor::end ()
{
