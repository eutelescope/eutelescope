...
string EUTelCalibrateEventProcessor::_commonModeDistHistoName =
    "CommonModeDistHisto";
string EUTelCalibrateEventProcessor::_skippedRowDistHistoName =
    "SkippedRowDistHisto";
string EUTelCalibrateEventProcessor::_skippedPixelDistHistoName =
    "SkippedPixelDistHisto";
string EUTelCalibrateEventProcessor::_skippedPixelPerRowDistHistoName =
    "SkippedPixelPerRowDistHisto";
#endif

const unsigned short EUTelCalibrateEventProcessor::_maxNoOfConsecutiveMissing =
    10;

EUTelCalibrateEventProcessor::EUTelCalibrateEventProcessor()
    : Processor("EUTelCalibrateEventProcessor") {
...
void EUTelCalibrateEventProcessor::initializeGeometry(LCEvent *event) {
  ...
  try {
    LCCollectionVec *pedestalCol = dynamic_cast<LCCollectionVec *>(
        event->getCollection(_pedestalCollectionName));
    ...
    for (size_t iDetector = 0; iDetector < pedestalCol->size(); ++iDetector) {
      TrackerDataImpl *pedestal =
          dynamic_cast<TrackerDataImpl *>(pedestalCol->getElementAt(iDetector));
    ...
    if (isFirstEvent()) {
      ...
      for (unsigned int iDetector = 0; iDetector < inputCollectionVec->size();
        ...
        TrackerDataImpl *pedestal = dynamic_cast<TrackerDataImpl *>(
            pedestalCollectionVec->getElementAt(_ancillaryIndexMap[sensorID]));
...
// histograms are grouped in directory identifying the detector
        ...
        if (_fillDebugHisto == 1) {
          ...
          AIDA::IHistogram1D *rawDataDistHisto =
              AIDAProcessor::histogramFactory(this)->createHistogram1D(
                  (basePath + tempHistoName).c_str(), rawDataDistHistoNBin,
                  rawDataDistHistoMin, rawDataDistHistoMax);
          if (rawDataDistHisto) {
          ...
          AIDA::IHistogram1D *dataDistHisto =
              AIDAProcessor::histogramFactory(this)->createHistogram1D(
                  (basePath + tempHistoName).c_str(), dataDistHistoNBin,
                  dataDistHistoMin, dataDistHistoMax);
          if (dataDistHisto) {
        ...
        if (_doCommonMode == 1 || _doCommonMode == 2) {
          ...
          AIDA::IHistogram1D *commonModeDistHisto =
              AIDAProcessor::histogramFactory(this)->createHistogram1D(
                  (basePath + tempHistoName).c_str(), commonModeDistHistoNBin,
                  commonModeDistHistoMin, commonModeDistHistoMax);
        ...
        if (_doCommonMode == 1) {
          ...
          AIDA::IHistogram1D *skippedPixelDistHisto =
              AIDAProcessor::histogramFactory(this)->createHistogram1D(
                  (basePath + tempHistoName).c_str(), skippedPixelDistHistoNBin,
                  skippedPixelDistHistoMin, skippedPixelDistHistoMax);
          if (skippedPixelDistHisto) {
            _aidaHistoMap.insert(
                make_pair(tempHistoName, skippedPixelDistHisto));
        ...
        if (_doCommonMode == 2) {
          ...
          tempHistoName =
              _skippedPixelPerRowDistHistoName + "_d" + to_string(sensorID);
          ...
          AIDA::IHistogram1D *skippedPixelPerRowDistHisto =
              AIDAProcessor::histogramFactory(this)->createHistogram1D(
                  (basePath + tempHistoName).c_str(),
                  skippedPixelPerRowDistHistoNBin,
                  skippedPixelPerRowDistHistoMin,
                  skippedPixelPerRowDistHistoMax);
          if (skippedPixelPerRowDistHisto) {
            _aidaHistoMap.insert(
                make_pair(tempHistoName, skippedPixelPerRowDistHisto));
          ...
          AIDA::IHistogram1D *skippedRowDistHisto =
              AIDAProcessor::histogramFactory(this)->createHistogram1D(
                  (basePath + tempHistoName).c_str(), skippedRowDistHistoNBin,
                  skippedRowDistHistoMin, skippedRowDistHistoMax);
    ...
    for (unsigned int iDetector = 0; iDetector < inputCollectionVec->size();
      ...
      CellIDEncoder<TrackerDataImpl> idDataEncoder(
          EUTELESCOPE::MATRIXDEFAULTENCODING, correctedDataCollection);
      ...
      if (_doCommonMode == 1) {
        ...
        while (rawIter != rawData->getADCValues().end()) {
          bool isHit =
              (((*rawIter) - (*pedIter)) > _hitRejectionCut * (*noiseIter));
        ...
        if (((_maxNoOfRejectedPixels == -1) ||
             (skippedPixel < _maxNoOfRejectedPixels)) &&
            (goodPixel != 0)) {
      ...
      if (isEventValid) {
        if (_doCommonMode == 2) {
          ...
          for (int yPixel = _minY[iDetector]; yPixel <= _maxY[iDetector];
            ...
            for (int xPixel = _minX[iDetector]; xPixel <= _maxX[iDetector];
              ...
              double correctedValue = adcValues[iPixel] - _pedestal[iPixel] -
        ...
        if (_doCommonMode == 1) {
          ...
          streamlog_out(WARNING4) << "Skipping event " << evt->getEventNumber()
                                  << " for an unknown reason " << endl;
        }
