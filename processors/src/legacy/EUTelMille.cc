...
EUTelMille::EUTelMille() : Processor("EUTelMille") {
  ...
  registerOptionalParameter(
      ...
      "\n0 - hits read from hitfile and simple straight line trackfinding will "
  ...
  registerInputCollections(LCIO::TRACKERHIT, "HitCollectionName",
                           "Hit collections name", _hitCollectionName,
                           HitCollectionNameVecExample);

  registerInputCollection(LCIO::TRACK, "TrackCollectionName",
                          "Track collection name. This is only relevant if "
                          "InputMode is set to larger to 1",
  ...
  registerOptionalParameter("DistanceMaxVec",
                            ...
                            "the fit per 10 cm space between the planes. One "
                            "value for each neighbor planes. DistanceMax will "
                            "be used for each pair if this vector is empty.",
  ...
  registerOptionalParameter("TelescopeResolution",
  ...
  registerOptionalParameter("AlignmentConstantCollectionName",
                            "This is the name of the alignment collection to "
                            "be saved into the slcio file",
                            _alignmentConstantCollectionName,
                            static_cast<string>("alignment"));

  registerOptionalParameter("ResidualsXMin",
                            "Minimal values of the hit residuals in the X "
                            "direction for a track. Note: these numbers are "
                            "ordered according to the z position of the "
                            "sensors and NOT according to the sensor id.",
                            _residualsXMin, MinimalResidualsX);

  registerOptionalParameter("ResidualsYMin",
                            "Minimal values of the hit residuals in the Y "
                            "direction for a track. Note: these numbers are "
                            "ordered according to the z position of the "
                            "sensors and NOT according to the sensor id.",
  ...
  registerOptionalParameter("ResidualsXMax",
                            "Maximal values of the hit residuals in the X "
                            "direction for a track. Note: these numbers are "
                            "ordered according to the z position of the "
                            "sensors and NOT according to the sensor id.",
  ...
  registerOptionalParameter("ResidualsYMax",
                            "Maximal values of the hit residuals in the Y "
                            "direction for a track. Note: these numbers are "
                            "ordered according to the z position of the "
                            "sensors and NOT according to the sensor id.",
  ...
  registerOptionalParameter(
      ...
      "given, then all parameters will be free). bit 0 = x shift, bit 1 = y "
  ...
  registerOptionalParameter("PedeSteeringAdditionalCmds",
                            "FOR EXPERTS: List of commands that should be "
                            "included in the pede steering file. Use '\\' to "
                            "seperate options and introduce a line break.",
                            _pedeSteerAddCmds, StringVec());
  ...
  registerOptionalParameter("TestModeSensorXShifts",
                            "X shifts of the sensors in test mode (to be "
                            "determined by the alignment).",
                            _testModeSensorXShifts, SensorXShifts);

  registerOptionalParameter("TestModeSensorYShifts",
                            "Y shifts of the sensors in test mode (to be "
                            "determined by the alignment).",
                            _testModeSensorYShifts, SensorYShifts);

  registerOptionalParameter("TestModeSensorGamma",
                            "Rotation around the z axis of the sensors in test "
                            "mode (to be determined by the alignment).",
                            _testModeSensorGamma, SensorGamma);

  registerOptionalParameter("TestModeSensorAlpha",
                            "Rotation around the x axis of the sensors in test "
                            "mode (to be determined by the alignment).",
                            _testModeSensorAlpha, SensorAlpha);

  registerOptionalParameter("TestModeSensorBeta",
                            "Rotation around the y axis of the sensors in test "
                            "mode (to be determined by the alignment).",
                            _testModeSensorBeta, SensorBeta);

  IntVec initRect;
  registerOptionalParameter("UseSensorRectangular",
                            "Do not use all pixels for alignment, only these "
                            "in the rectangular (A|B) e.g. (0,0) and (C|D) "
                            "e.g. (100|100) of sensor S. Type in the way S1 A1 "
                            "B1 C1 D1 S2 A2 B2 C2 D2 ...",
...
void EUTelMille::init() {
  ...
  if (_inputMode == 0 || _inputMode == 2) {
    ...
    if (_useSensorRectangular.empty()) {
      streamlog_out(MESSAGE4)
          << "No rectangular limits on pixels of sensorplanes applied" << endl;
    } else {
      if (_useSensorRectangular.size() % 5 != 0) {
        streamlog_out(WARNING2) << "Wrong number of arguments in "
                                   "RectangularLimits! Ignoring this cut!"
                                << endl;
      } else {
  ...
  if (_alignMode == Utility::alignMode::XYShiftsAllRot) {
    ...
    if (_resolutionY.size() != _resolutionZ.size()) {
      throw InvalidParameterException(
          "WARNING, length of resolution Y and Z is not the same \n");
    }

    if (_resolutionX.size() != static_cast<unsigned int>(_nPlanes) ||
      ...
      streamlog_out(WARNING2)
          << "Consistency check of the resolution parameters failed. The array "
             "size is different than the number of found planes! "
             "The resolution parameters are set to default values now (see "
             "variable TelescopeResolution). "
             "This introduces a bias if the real values for X,Y and Z are "
             "rather different."
          << endl;
      _resolutionX.clear();
      _resolutionY.clear();
    ...
    if (_FixParameter.size() != static_cast<unsigned int>(_nPlanes) &&
        !_FixParameter.empty()) {
      streamlog_out(WARNING2)
          << "Consistency check of the fixed parameters array failed. The "
             "array size is different than the number of found planes! The "
             "array is now set to default values, which means that all "
    ...
    if (_FixParameter.empty()) {
      streamlog_out(WARNING2)
          << "The fixed parameters array was found to be empty. It will be "
             "filled with default values. All parameters are free in the fit "
             "now."
          << endl;
      _FixParameter.clear();
      for (size_t i = 0; i < _nPlanes; i++) {
  ...
  if (!_distanceMaxVec.empty()) {
    if (_distanceMaxVec.size() != static_cast<unsigned int>(_nPlanes)) {
      streamlog_out(WARNING2)
          << "Consistency check of the DistanceMaxVec array failed. Its size "
             "is different compared to the number of planes! Will now use "
             "_distanceMax for each pair of planes."
          << endl;
...
void EUTelMille::findtracks2(
    int missinghits, std::vector<IntVec> &indexarray, IntVec vec,
  ...
  if (missinghits > getAllowedMissingHits()) {
    ...
    streamlog_out(DEBUG9) << "indexarray size:" << indexarray.size()
                          << std::endl;
    return;
  }
    ...
    if (i < _allHitsArray.size() - 1) {
      ...
      if (taketrack) {
        ...
        streamlog_out(DEBUG9)
            << "indexarray size at last plane:" << indexarray.size()
            << std::endl;
      }
...
void EUTelMille::findtracks(
    std::vector<IntVec> &indexarray, IntVec vec,
    std::vector<std::vector<EUTelMille::HitsInPlane>> &_hitsArray, int i,
    int y) {
  if (i > 0)
    vec.push_back(y);
  ...
  for (size_t j = 0; j < _hitsArray[i].size(); j++) {
    ...
    if (i < static_cast<int>((_hitsArray.size())) - 1) {
      vec.push_back(
          static_cast<int>(j)); // index of the cluster in the last plane

      // track candidate requirements
      bool taketrack = true;
      const int e = vec.size() - 2;
      if (e >= 0) {
        double distance = sqrt(pow(_hitsArray[e][vec[e]].measuredX -
                                       _hitsArray[e + 1][vec[e + 1]].measuredX,
                                   2) +
                               pow(_hitsArray[e][vec[e]].measuredY -
                                       _hitsArray[e + 1][vec[e + 1]].measuredY,
                                   2));
        double distance_z = _hitsArray[e + 1][vec[e + 1]].measuredZ -
                            _hitsArray[e][vec[e]].measuredZ;
        ...
        if (_onlySingleHitEvents == 1 &&
            (_hitsArray[e].size() != 1 || _hitsArray[e + 1].size() != 1))
          taketrack = false;
      }
      vec.pop_back();
 ...
 *
 */
void EUTelMille::FitTrack(unsigned int nPlanesFitter, double xPosFitter[],
                          double yPosFitter[], double zPosFitter[],
                          double xResFitter[], double yResFitter[],
                          double chi2Fit[2], double residXFit[],
  ...
  for (counter = 0; counter < static_cast<int>(nPlanesFitter); counter++) {
    residXFit[counter] =
        (Ybar[0] - Xbar[0] * A2[0] + zPosFitter[counter] * A2[0]) -
        xPosFitter[counter];
    residYFit[counter] =
        (Ybar[1] - Xbar[1] * A2[1] + zPosFitter[counter] * A2[1]) -
        yPosFitter[counter];
  }
  ...
  for (int i = 0; i < hotPixelCollectionVec->getNumberOfElements(); i++) {
    ...
    if (type == kEUTelGenericSparsePixel) {
      ...
      for (auto &m26Pixel : pixelVec) {
        streamlog_out(DEBUG3) << "Size: " << m26Data->size()
                              << " HotPixelInfo:  " << m26Pixel.getXCoord()
                              << " " << m26Pixel.getYCoord() << " "
                              << m26Pixel.getSignal() << endl;
        try {
          char ix[100];
...
void EUTelMille::findMatchedHits(int &_ntrack, Track *TrackHere) {
  ...
  for (int nHits = 0; nHits < int(TrackHitsHere.size()); nHits++) {
    ...
    if (((hitCellDecoder(HitHere)["properties"] & kFittedHit) >> 1) == 0) {
      hit.push_back(HitHere);
    }

    if (((hitCellDecoder(HitHere)["properties"] & kFittedHit) >> 1) == 1) {
      fit.push_back(HitHere);
    }
  }

  nPlaneHere = 0;

  for (std::vector<TrackerHit *>::iterator ihit = hit.begin();
       ihit != hit.end(); ihit++) {
    ...
    for (std::vector<TrackerHit *>::iterator ifit = fit.begin();
      ...
      _trackResidX[_ntrack][nPlaneHere] =
          (fitPosition[0] - hitPosition[0]) * 1000.;
      _trackResidY[_ntrack][nPlaneHere] =
          (fitPosition[1] - hitPosition[1]) * 1000.;
...
void EUTelMille::processEvent(LCEvent *event) {
  ...
  if (_iEvt % 1000 == 0) {
    streamlog_out(MESSAGE5) << "Currently having " << _nMilleDataPoints
                            << " data points in " << _nMilleTracks << " tracks "
                            << endl;
  }
  ...
  std::vector<std::vector<EUTelMille::HitsInPlane>> _hitsArray(
      _nPlanes - _nExcludePlanes, std::vector<EUTelMille::HitsInPlane>());
  IntVec indexconverter(_nPlanes, -1);

  std::vector<std::vector<EUTelMille::HitsInPlane>> _allHitsArray(
      _nPlanes, std::vector<EUTelMille::HitsInPlane>());
    ...
    for (size_t i = 0; i < _hitCollectionName.size(); i++) {
      ...
      if (_inputMode == 0) {
        ...
        for (int iHit = 0; iHit < collection->getNumberOfElements(); iHit++) {
          ...
          TrackerHitImpl *hit =
              static_cast<TrackerHitImpl *>(collection->getElementAt(iHit));
        ...
        for (unsigned int help = 0; help < _nPlanes; help++) {
          ...
          hitsInPlane.measuredX =
              xhitpos + gRandom->Gaus(0.0, resolX) +
              _testModeSensorXShifts[help] +
              _testModeSensorZPositions[help] * tan(xslope) -
              _testModeSensorGamma[help] * yhitpos -
              _testModeSensorBeta[help] * _testModeSensorZPositions[0];
          hitsInPlane.measuredY =
              yhitpos + gRandom->Gaus(0.0, resolY) +
              _testModeSensorYShifts[help] +
              _testModeSensorZPositions[help] * tan(yslope) +
              _testModeSensorGamma[help] * xhitpos -
              _testModeSensorAlpha[help] * _testModeSensorZPositions[help];
  ...
  if (_inputMode == 0 || _inputMode == 2) {
    ...
    try {
      ...
      streamlog_out(WARNING2) << "No input track collection "
                              << _trackCollectionName << " found for event "
                              << event->getEventNumber() << " in run "
                              << event->getRunNumber() << endl;
      throw SkipEventException(this);
    }
    ...
    for (int nTracksEvent = 0;
         nTracksEvent < nTracksHere && nTracksEvent < _maxTrackCandidates;
         nTracksEvent++) {

      Track *TrackHere =
          dynamic_cast<Track *>(collection->getElementAt(nTracksEvent));
    ...
    for (int nTracksEvent = 0;
      ...
      // hit list assigned to track
      std::vector<EVENT::TrackerHit *> TrackHitsHere =
          TrackHere->getTrackerHits();

      size_t number_of_planes =
          (TrackHitsHere.size() - _excludePlanes.size()) / 2;
      ...
      if ((_nPlanes - _excludePlanes.size()) == number_of_planes) {
        ...
        // check for a hit in every telescope plane
        {
          ...
          try {
            ...
            streamlog_out(WARNING2)
                << "No input collection " << _hitCollectionName[i]
                << " found for event " << event->getEventNumber() << " in run "
                << event->getRunNumber() << endl;
            throw SkipEventException(this);
          }
          for (int iHit = 0; iHit < collection->getNumberOfElements(); iHit++) {
            ...
            hitsplane.push_back(EUTelMille::HitsInPlane(
                1000. * hit->getPosition()[0], 1000. * hit->getPosition()[1],
            ...
            for (int nHits = 0; nHits < int(TrackHitsHere.size());
                 nHits++) // end loop over all hits and fill arrays
            {
              TrackerHit *HitHere = TrackHitsHere.at(nHits);
              ...
              //                      if( std::abs( measuredz - PositionsHere[2]
              //                      ) > 5.0 /* mm */)
              {
                ...
                if ((hitCellDecoder(HitHere)["properties"] & kFittedHit) > 0) {
                  hitsplane.push_back(EUTelMille::HitsInPlane(
                      PositionsHere[0] * 1000., PositionsHere[1] * 1000.,
    ...
    for (int track = 0; track < _nTracks; track++) {
      ...
      //    }else
      {
        if (_alignMode == Utility::alignMode::XYShiftsAllRot) {
          ...
          for (unsigned int help = 0; help < _nPlanes; help++) {
            ...
            if (xresid < _residualsXMin[help] ||
                xresid > _residualsXMax[help]) {
              continue;
            }
            if (yresid < _residualsYMin[help] ||
                yresid > _residualsYMax[help]) {
              continue;
            }
            ...
            if (!excluded) {
              ...
              hitsarray[index_hitsarray] =
                  (hit(x, y, z, sigmax, sigmay, sigmaz, help));
          ...
          TMinuit *gMinuit =
              new TMinuit(4); // initialize TMinuit with a maximum of 4 params
          ...
          double ps = atan(linfit_x_a1 /
                           sqrt(1.0 + linfit_y_a1 * linfit_y_a1)); // guess
          // of psi
          ...
          gMinuit->mnparm(2, "delta", vstart[2], step[2], -1.0 * TMath::Pi(),
                          1.0 * TMath::Pi(), ierflg);
          gMinuit->mnparm(3, "psi", vstart[3], step[3], -1.0 * TMath::Pi(),
                          1.0 * TMath::Pi(), ierflg);
          ...
          if (ok) {
            ...
            for (unsigned int help = 0; help < _nPlanes; help++) {
              ...
              if (!(abs(x) < 1e-06 && abs(y) < 1e-06)) {
                ...
                // cout << "z residual w/ ref vec: " << _waferResidZ[help] << ",
                // w/o ref vec: " << la*sqrt(1.0 - c0*c0 - c1*c1) - z << endl;
              } else {
                _waferResidX[help] = 0.;
                _waferResidY[help] = 0.;
                _waferResidZ[help] = 0.;
          ...
          streamlog_out(DEBUG9) << " AlignMode = " << static_cast<int>(_alignMode)
                                  ...
                                  << std::endl;
          // Calculate residuals
          FitTrack(_nPlanes, _xPosHere, _yPosHere, _zPosHere, _telescopeResolX,
                   _telescopeResolY, Chiquare, _waferResidX, _waferResidY,
                   angle);
        }
      }
      ...
      if (_useResidualCuts != 0) {
        ...
        for (unsigned int help = 0; help < _nPlanes; help++) {
          ...
          if (excluded == 0) {
            if (_waferResidX[help] < _residualsXMin[help] ||
                _waferResidX[help] > _residualsXMax[help]) {
              residualsXOkay = 0;
            }
            if (_waferResidY[help] < _residualsYMin[help] ||
                _waferResidY[help] > _residualsYMax[help]) {
              residualsYOkay = 0;
            }
          }
      ...
      if (_useResidualCuts == 0 ||
          (residualsXOkay == 1 && residualsYOkay == 1)) {
        ...
        if (_alignMode == Utility::alignMode::XYShifts) {
          ...
          const int nGL =
              (_nPlanes - _nExcludePlanes) * 2; // number of global parameters
          ...
          float *derGL =
              new float[nGL]; // array of derivatives for global parameters
          ...
          if (validminuittrack || _inputMode == 1) {
            ...
            for (unsigned int help = 0; help < _nPlanes; help++) {
              ...
              if (excluded == 0) {
                ...
                //   (a_X*x+b_X, a_Y*y+b_Y)  :   /  1   -g    b \   / x
                //   \   :: shouldn't it be x-xcenter-of-the-sensor ??
                //                           : - |  g    1   -a |   | y
                //                           |   ::  and y-ycenter-of-the-sensor
                //                           ??
                //                           :   \ -b    a    1 /   \ z-z_sensor
                //                           /   ::  == z-zcenter-of-the-sensor
                //                           ?? (already)
                ...
                derGL[((helphelp * 6) + 4)] =
                    -1.0 * (_zPosHere[help] - z_sensor); // beta  - ZX :: Z->X
                derGL[((helphelp * 6) + 5)] =
                    1.0 * (_yPosHere[help] - y_sensor); // gamma - XY :: X->Y
                ...
                // rotation in ZX
                derGL[((helphelp * 6) + 3)] =
                    1.0 * (_zPosHere[help] - z_sensor);
                derGL[((helphelp * 6) + 4)] = 0.0;
                derGL[((helphelp * 6) + 5)] =
                    -1.0 * (_xPosHere[help] - x_sensor);
...
// -------------------------------------
        ...
        for (unsigned int iDetector = 0; iDetector < _nPlanes; iDetector++) {
          ...
          if (abs(_waferResidX[iDetector]) < 1e-06 &&
...
void EUTelMille::end() {
  ...
  if (_generatePedeSteerfile) {
    ...
    streamlog_out(MESSAGE4)
        << endl
        << "Generating the steering file for the pede program..." << endl;
    ...
    if (steerFile.is_open()) {
      ...
      steerFile << endl;
      for (StringVec::iterator it = _pedeSteerAddCmds.begin();
           it != _pedeSteerAddCmds.end(); ++it) {
      ...
      streamlog_out(MESSAGE5) << "File " << _pedeSteerfileName << " written."
                              << endl;
  ...
  streamlog_out(MESSAGE7) << "Number of data points used: " << _nMilleDataPoints
                          << endl;
  streamlog_out(MESSAGE7) << "Number of tracks used: " << _nMilleTracks << endl;

  // monitor the number of tracks in CDash when running tests
  CDashMeasurement meas_ntracks("ntracks", _nMilleTracks); // cout <<
                                                           // meas_ntracks;  //
                                                           // output only if
                                                           // DO_TESTING is set
  ...
  if (_runPede == 1) {
    ...
    if (_generatePedeSteerfile == 1) {
      ...
      if (!pede.is_open()) {
        streamlog_out(ERROR5)
            << "Pede cannot be executed: command not found in the path" << endl;
        encounteredError = true;
      } else {

        // output multiplexing: parse pede output in both stdout and stderr and
        // echo messages accordingly
        char buf[1024];
        std::streamsize n;
        ...
        // check for some of those here by parsing the output
        {
          ...
          if (pch) {
            streamlog_out(ERROR5)
                << "Pede stopped due to the large number of rejects. " << endl;
            encounteredError = true;
          }
        }
        ...
        if (pede.rdbuf()->status() == 0 && !encounteredError) {
          ...
          streamlog_out(ERROR5)
              ...
              << pede.rdbuf()->status()
              << ", error messages (repeated here): " << endl;
          ...
          // exit(EXIT_FAILURE); // FIXME: can lead to (ROOT?) seg faults -
          // points to corrupt memory? run valgrind...
        ...
        streamlog_out(MESSAGE6) << "Reading back the " << millepedeResFileName
                                << endl
                                << "Saving the alignment constant into "
                                << _alignmentConstantLCIOFile << endl;
        ...
        event->addCollection(constantsCollection,
                             _alignmentConstantCollectionName);
      ...
      streamlog_out(ERROR2)
          << "Unable to run pede. No steering file has been generated." << endl;
    }
...
void EUTelMille::bookHistos() {
  ...
  try {
    ...
    streamlog_out(ERROR5) << "No AIDAProcessor initialized. Type q to exit or "
...
#else
    streamlog_out(WARNING6)
        << "No AIDAProcessor initialized. Continue without histogramming"
        << endl;
