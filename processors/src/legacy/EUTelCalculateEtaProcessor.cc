...
string
  EUTelCalculateEtaProcessor::_cogHistogramXName = "CoG_X";
string
  EUTelCalculateEtaProcessor::_cogHistogramYName = "CoG_Y";
string
  EUTelCalculateEtaProcessor::_cogIntegralXName = "Integral_CoG_X";
string
  EUTelCalculateEtaProcessor::_cogIntegralYName = "Integral_CoG_Y";
string
  EUTelCalculateEtaProcessor::_etaHistoXName = "EtaProfile_X";
string
  EUTelCalculateEtaProcessor::_etaHistoYName = "EtaProfile_Y";
string
  EUTelCalculateEtaProcessor::_cogHisto2DName = "CoG_Histo2D";
#endif

const double
  EUTelCalculateEtaProcessor::_min = -0.5;
const double
  EUTelCalculateEtaProcessor::_max = 0.5;

EUTelCalculateEtaProcessor::EUTelCalculateEtaProcessor ():Processor
  ("EUTelCalculateEtaProcessor")
{

  // modify processor description
  _description =
    "EUTelCalculateEtaProcessor calculates the eta function for a "
  ...
  registerInputCollection (LCIO::TRACKERPULSE, "ClusterCollectionName",
			   "Input cluster collection", _clusterCollectionName,
			   string ("cluster"));
  ...
  registerProcessorParameter ("NxMPixelClusterSize",
			      ...
			      _xyCluSize, xyCluSizeExample,
			      xyCluSizeExample.size ());
  ...
  registerOptionalParameter ("RejectSinglePixelCluster",
...
void
EUTelCalculateEtaProcessor::init ()
{
  ...
  if (_rejectsingplepixelcluster != 0 && _rejectsingplepixelcluster != 1 &&
      _rejectsingplepixelcluster != 2)
    {
      streamlog_out (ERROR4) <<
	"the parameter RejectSinglePixelCluster must set "
	"to 0,1 or 2, but it is " << _rejectsingplepixelcluster << endl;
      exit (-1);
    }
}

void
EUTelCalculateEtaProcessor::processRunHeader (LCRunHeader * rdr)
{
  ...
  if (!_isEtaCalculationFinished)
	...
	exit (-1);
      ...
      newHeader->lcRunHeader ()->setRunNumber (runHeader->lcRunHeader ()->
					       getRunNumber ());
      newHeader->lcRunHeader ()->setDetectorName (runHeader->lcRunHeader ()->
						  getDetectorName ());
...
void
EUTelCalculateEtaProcessor::processEvent (LCEvent * event)
{
  ...
  if (evt->getEventType () == kEORE)
    {
      ...
      // streamlog_out ( DEBUG4 ) << "Found kEORE, calling finalizeProcessor()" <<
      // endl;
      // finalizeProcessor();
      // return;

      streamlog_out (DEBUG4) <<
	"Found kEORE, calling finalizeProcessor() now!" << endl;
      finalizeProcessor ();
      return;
  ...
  if (!_isEtaCalculationFinished)
	...
	LCCollectionVec *clusterCollectionVec =
	  dynamic_cast <
	  LCCollectionVec * >(evt->getCollection (_clusterCollectionName));
	CellIDDecoder < TrackerPulseImpl > cellDecoder (clusterCollectionVec);

	for (int iCluster = 0;
	     iCluster < clusterCollectionVec->getNumberOfElements ();
	     iCluster++)
	  {
	    ...
	    TrackerPulseImpl *pulse =
	      dynamic_cast <
	      TrackerPulseImpl *
	      >(clusterCollectionVec->getElementAt (iCluster));
		...
		cluster =
		  new EUTelFFClusterImpl (static_cast <
					  TrackerDataImpl *
					  >(pulse->getTrackerData ()));
		// streamlog_out ( MESSAGE2 ) <<  "seen a kEUTelFFClusterImpl" <<
		// endl; //!HACK TAKI
		...
		// streamlog_out ( MESSAGE2 ) <<  "seen a kEUTelBrickedClusterImpl" <<
		// endl; //!HACK TAKI
		...
		// streamlog_out ( MESSAGE2 ) <<  "seen a kEUTelSparseClusterImpl" <<
		// endl; //!HACK TAKI
		...
		if (type == kEUTelBrickedClusterImpl)
		  {
		    ...
		    // Static of cluster to EUTelBrickedClusterImpl* was done for sure
		    // in the case of
		    //( type == kEUTelBrickedClusterImpl ).
		    // So this cast must work as well!
		    // This is just a (different) pointer to the same memory as
		    // "cluster". So no additional delete needed.
		  }
		...
		if (_clusterTypeSelection == "FULL")
		  {
		    ...
		    if (p_tmpBrickedCluster)
		      {
			// streamlog_out ( MESSAGE2 ) <<  "DEBUG: doing eta FULL on a
			// bricked cluster!" << endl;
			p_tmpBrickedCluster->
		...
		else if (_clusterTypeSelection == "NxMPixel")
		  {
			...
			streamlog_out (WARNING4)
			  <<
			  "NxM not applicable for a bricked cluster!! Doing FULL!"
			  << endl;
			p_tmpBrickedCluster->
			  getCenterOfGravityShiftWithOutGlobalSeedCoordinateCorrection
			  (xShift, yShift);
		...
		else if (_clusterTypeSelection == "NPixel")
		  {
		    ...
		    if (p_tmpBrickedCluster)
		      {
			// streamlog_out ( MESSAGE2 ) <<  "DEBUG: doing eta NPixel on a
			// bricked cluster!" << endl;
			p_tmpBrickedCluster->
			  getCenterOfGravityShiftWithOutGlobalSeedCoordinateCorrection
			  (xShift, yShift, _nPixel);
		      }
		    else
		      {
			cluster->getCenterOfGravityShift (xShift, yShift,
...
//#define TAKI_DEBUG_ETA 1
		...
		if (p_tmpBrickedCluster)
		  {
		    ...
		    streamlog_out (MESSAGE2) <<
		      "Just done ETA on a BrickedCluster!" << endl;
		...
		if (_cogHistogramX.find (detectorID) == _cogHistogramX.end ())
		  {
		    _cogHistogramX[detectorID] =
		      new EUTelPseudo1DHistogram (_noOfBin[0], _min, _max);
		    _integralHistoX[detectorID] =
		      new EUTelPseudo1DHistogram (_noOfBin[0], _min, _max);
		  }
		if (_cogHistogramY.find (detectorID) == _cogHistogramY.end ())
		  {
		    _cogHistogramY[detectorID] =
		      new EUTelPseudo1DHistogram (_noOfBin[1], _min, _max);
		    _integralHistoY[detectorID] =
		      new EUTelPseudo1DHistogram (_noOfBin[0], _min, _max);
		  }
		...
		if (_rejectsingplepixelcluster == 2)
		  {
		    spc_cut = type != kEUTelDFFClusterImpl &&
		      (abs (static_cast < double >(xShift)) <
		       numeric_limits < double >::min () ||
		       abs (static_cast < double >(yShift)) <
		       numeric_limits < double >::min ());
		  }
		...
		if (!spc_cut)
		  {
		    _cogHistogramX[detectorID]->fill (static_cast <
						      double >(xShift), 1.0);
		    _cogHistogramY[detectorID]->fill (static_cast <
						      double >(yShift), 1.0);
		  }
#if defined(USE_AIDA) || defined(MARLIN_USE_AIDA)
		{

		  if (_alreadyBookedSensorID.find (detectorID) ==
			...
			createHistogram1D ((path + name).c_str (),
		      ...
		      _aidaHistoMap.
			insert (make_pair (name, cogIntegralHistoX));

		      // Integral along y
		      name = _cogIntegralYName + "_" + to_string (detectorID);
		      title = "Integral CoG (y) shift histogram on " +
			to_string (detectorID);
			...
			createHistogram1D ((path + name).c_str (),
		      ...
		      _aidaHistoMap.
			insert (make_pair (name, cogIntegralHistoY));

		      // Eta histogram along x
		      name = _etaHistoXName + "_" + to_string (detectorID);
		      title =
			"Eta profile x for detector " +
			to_string (detectorID);
		      AIDA::IProfile1D * etaHistoX =
			AIDAProcessor::histogramFactory (this)->
			createProfile1D ((path + name).c_str (), _noOfBin[0],
					 _min, _max, _min, _max);
		      ...
		      title =
			"Eta profile y for detector " +
			to_string (detectorID);
		      AIDA::IProfile1D * etaHistoY =
			AIDAProcessor::histogramFactory (this)->
			createProfile1D ((path + name).c_str (), _noOfBin[1],
					 _min, _max, _min, _max);
		      ...
		      title =
			"2D Histo with the CoG within the seed pixel for detector "
			+ to_string (detectorID);
		      AIDA::IHistogram2D * cogHisto2D =
			AIDAProcessor::histogramFactory (this)->
			createHistogram2D ((path + name).c_str (),
					   _noOfBin[0], _min, _max,
					   _noOfBin[1], _min, _max);
		  ...
		  if (!spc_cut)
			...
			_cogHistogramXName + "_" + to_string (detectorID);
		      (dynamic_cast <
		       AIDA::IHistogram1D *
		       >(_aidaHistoMap[name]))->fill (xShift);

		      name =
			_cogHistogramYName + "_" + to_string (detectorID);
		      (dynamic_cast <
		       AIDA::IHistogram1D *
		       >(_aidaHistoMap[name]))->fill (yShift);

		      name = _cogHisto2DName + "_" + to_string (detectorID);
		      (dynamic_cast <
		       AIDA::IHistogram2D *
		       >(_aidaHistoMap[name]))->fill (xShift, yShift);
		    }
		}
#endif
	      }
	      ...
	      {
		// streamlog_out ( MESSAGE2 ) <<  "CLUSTER QUALITY NOT GOOD ENOUGH!!!"
		// << endl;
	      }
  ...
  if (_iEvt >= _nEvent && _nEvent != -1 && !_isEtaCalculationFinished)
    {
      streamlog_out (DEBUG4) << _nEvent
	<< " events done, calling finalizeProcessor()" << endl;
      finalizeProcessor ();
      return;
    }
...
void
EUTelCalculateEtaProcessor::check (LCEvent * /* evt */ )
{
...
void
EUTelCalculateEtaProcessor::finalizeProcessor ()
{
  ...
  streamlog_out (MESSAGE4) << "Writing the output eta file "
    << _outputEtaFileName << endl;
  ...
  {
    streamlog_out (ERROR4) << e.
      what () << endl << "Sorry for quitting." << endl;
    exit (-1);
  }
  ...
  LCCollectionVec *etaXCollection =
    new LCCollectionVec (LCIO::LCGENERICOBJECT);
  LCCollectionVec *etaYCollection =
    new LCCollectionVec (LCIO::LCGENERICOBJECT);

  map < int, EUTelPseudo1DHistogram * >::iterator iter =
    _cogHistogramX.begin ();
  while (iter != _cogHistogramX.end ())
    {
      ...
      for (int iBin = 1;
	   iBin <= _cogHistogramX[iDetector]->getNumberOfBins (); iBin++)
	{
      ...
      for (int iBin = 1;
	   iBin <= _integralHistoX[iDetector]->getNumberOfBins (); iBin++)
	{
	  etaBinCenter.push_back (_integralHistoX[iDetector]->
				  getBinCenter (iBin));
	  etaBinValue.push_back (_integralHistoX[iDetector]->getBinContent (iBin) / integral - 0.5);	// - 0.5);
	}

      EUTelEtaFunctionImpl *etaX =
	new EUTelEtaFunctionImpl (etaBinCenter.size (), etaBinCenter,
				  etaBinValue);
#if ETA_VERSION >= 2
      ...
      for (int iBin = 1;
	   iBin <= _cogHistogramY[iDetector]->getNumberOfBins (); iBin++)
	{
      ...
      for (int iBin = 1;
	   iBin <= _integralHistoY[iDetector]->getNumberOfBins (); iBin++)
	{
	  etaBinCenter.push_back (_integralHistoY[iDetector]->
				  getBinCenter (iBin));
	  etaBinValue.push_back (_integralHistoY[iDetector]->getBinContent (iBin) / integral - 0.5);	// 0.5);
	}

      EUTelEtaFunctionImpl *etaY =
	new EUTelEtaFunctionImpl (etaBinCenter.size (), etaBinCenter,
				  etaBinValue);
#if ETA_VERSION >= 2
      ...
      AIDA::IHistogram1D * integralHisto =
	dynamic_cast < AIDA::IHistogram1D * >(_aidaHistoMap[name]);

      name = _cogHistogramXName + "_" + to_string (iDetector);
      AIDA::IHistogram1D * cogHisto =
	dynamic_cast < AIDA::IHistogram1D * >(_aidaHistoMap[name]);

      integral = 0;
      for (int iBin = 0; iBin < cogHisto->axis ().bins (); iBin++)
	{
	  double x = cogHisto->axis ().binLowerEdge (iBin) +
	    0.5 * cogHisto->axis ().binWidth (iBin);
	  integral += cogHisto->binHeight (iBin);
	  integralHisto->fill (x, integral);
	}

      name = _etaHistoXName + "_" + to_string (iDetector);
      AIDA::IProfile1D * etaXHisto =
	dynamic_cast < AIDA::IProfile1D * >(_aidaHistoMap[name]);

      for (int iBin = 0; iBin < integralHisto->axis ().bins (); iBin++)
	{
	  double x = integralHisto->axis ().binLowerEdge (iBin) +
	    0.5 * integralHisto->axis ().binWidth (iBin);
	...
	dynamic_cast < AIDA::IHistogram1D * >(_aidaHistoMap[name]);
      ...
      for (int iBin = 0; iBin < cogHisto->axis ().bins (); iBin++)
	{
	  double y = cogHisto->axis ().binLowerEdge (iBin) +
	    0.5 * cogHisto->axis ().binWidth (iBin);
	  integral += cogHisto->binHeight (iBin);
      ...
      AIDA::IProfile1D * etaYHisto =
	dynamic_cast < AIDA::IProfile1D * >(_aidaHistoMap[name]);

      for (int iBin = 0; iBin < integralHisto->axis ().bins (); iBin++)
	{
	  double y = integralHisto->axis ().binLowerEdge (iBin) +
	    0.5 * integralHisto->axis ().binWidth (iBin);
	  double v = integralHisto->binHeight (iBin);
...
void
EUTelCalculateEtaProcessor::end ()
{
