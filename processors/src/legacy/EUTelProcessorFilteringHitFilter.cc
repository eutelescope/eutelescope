 ...
 *
 *  Processor selects hits that fulfill all specified requirements from input
 * collection.
 *
...
EUTelProcessorFilteringHitFilter::EUTelProcessorFilteringHitFilter()
    : Processor("EUTelProcessorFilteringHitFilter"),
      _hitInputCollectionName("HitCollection"), _nProcessedRuns(0),
      _nProcessedEvents(0) {

  // Processor description
  _description = "EUTelProcessorFilteringHitFilter selects hits that fulfill "
                 "all specified requirements from input collection.";

  //--------------------------------------------------------------------------------------------------------------------------------------------------------//

  // Hits input collection
  registerInputCollection(LCIO::TRACKERHIT, "HitInputCollectionName",
                          "Input hits collection name", _hitInputCollectionName,
                          "HitCollection");

  // Filtered hits output collection
  registerOutputCollection(LCIO::TRACKERHIT, "HitOutputCollectionName",
                           "Output hits collection name",
                           _hitOutputCollectionName, "FilteredHitCollection");

  // HotPixel colelction: how the hits can be sorted out (first example)
  registerOutputCollection(LCIO::TRACKERHIT, "HotPixelCollectionName",
                           "Output hits collection name",
                           _hotpixelCollectionName, "HotPixelCollection");
  ...
  registerOptionalParameter("WantPlaneID",
                            "Select hits with given plane IDs (all - if empty)",
                            _wantPlaneID, defaultPlaneIDs);
}

void EUTelProcessorFilteringHitFilter::init() {

  streamlog_out(DEBUG) << "EUTelProcessorFilteringHitFilter::init( )"
                       << std::endl;
...
void EUTelProcessorFilteringHitFilter::processEvent(LCEvent *event) {
  ...
  try {
    ...
    hitOutputCollection = static_cast<LCCollectionVec *>(
        event->getCollection(_hitOutputCollectionName));
  } catch (...) {
    bHitOutputCollectionExists = false;
    hitOutputCollection = new LCCollectionVec(LCIO::TRACKERHIT);
  }
  // cout << " processEvent continue: OutputCollection  " << hitOutputCollection
  // << endl;
  ...
  if (hitInputCollection != nullptr && hitOutputCollection != nullptr) {
    for (int iHit = 0; iHit < hitInputCollection->getNumberOfElements();
         iHit++) {
      TrackerHitImpl *hit =
          static_cast<TrackerHitImpl *>(hitInputCollection->getElementAt(iHit));

      if (Utility::HitContainsHotPixels(hit, _hotPixelMap)) {
        streamlog_out(MESSAGE5) << "Hit " << iHit
                                << " contains hot pixels; skip this one. "
                                << std::endl;
        continue;
      }
    ...
    if (!bHitOutputCollectionExists)
      event->addCollection(hitOutputCollection, _hitOutputCollectionName);
  }
...
void EUTelProcessorFilteringHitFilter::end() {
  ...
  streamlog_out(MESSAGE) << "EUTelProcessorFilteringHitFilter::end()  "
                         << name() << " processed " << _nProcessedEvents
                         << " events in " << _nProcessedRuns << " runs "
                         << std::endl;
}
