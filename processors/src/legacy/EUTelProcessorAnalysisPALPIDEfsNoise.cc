...
EUTelProcessorAnalysisPALPIDEfsNoise::EUTelProcessorAnalysisPALPIDEfsNoise()
    : Processor("EUTelProcessorAnalysisPALPIDEfsNoise"),
      _zsDataCollectionName(""), _fillHistos(false), _nEvent(0), _nFiredPixel(),
      _nLayer(0), _xPixel(), _yPixel(), _energy(6.0), _chipID(), _irradiation(),
      _dutIDs(), _rate(""), _outputSettingsFolderName("./") {
  ...
  registerOptionalParameter("Rate", "Data taking rate", _rate,
                            static_cast<string>(""));
  registerOptionalParameter("dutIDs", "DUT IDs", _dutIDs, _stringVecExample);
...
void EUTelProcessorAnalysisPALPIDEfsNoise::init() {
  ...
  for (unsigned int i = 0; i < _dutIDs.size(); i++) {
    ...
    string _outputSettingsFileName =
        _outputSettingsFolderName + "settings_DUT" + _dutIDs[i] + ".txt";
    if (!std::ifstream(_outputSettingsFileName.c_str()))
      newFile = true;
    settingsFile[i].open(_outputSettingsFileName.c_str(), ios::out | ios::app);
    if (newFile)
      settingsFile[i] << "Run number;Energy;Chip ID;Irradiation "
                         "level(0-nonIrradiated,1-2.5e12,2-1e13,3-700krad,4-"
                         "combined:1e13+700krad);Rate;BB;Ithr;Idb;Vcasn;Vaux;"
                         "Vcasp;Vreset;Threshold and their RMS for all four "
                         "sectors;Noise and their RMS for all four "
                         "sectors;Readout delay;Trigger delay;Strobe "
                         "length;StrobeB length;Data (1) or noise (0);Number "
                         "of events;Efficiency,Number of tracks,Number of "
                         "tracks with associated hit for all sectors"
                      << endl;
  }
}
...
//  cerr << evt->getEventNumber() << endl;
  ...
  if (_isFirstEvent) {
    for (unsigned int i = 0; i < _dutIDs.size(); i++) {
      ...
      settingsFile[i]
          << evt->getRunNumber() << ";" << _energy << ";" << _chipID[dutID]
          << ";" << _irradiation[dutID] << ";" << _rate << ";"
          << evt->getParameters().getFloatVal("BackBiasVoltage") << ";"
          << evt->getParameters().getIntVal(Form("Ithr_%d", dutID)) << ";"
          << evt->getParameters().getIntVal(Form("Idb_%d", dutID)) << ";"
          << evt->getParameters().getIntVal(Form("Vcasn_%d", dutID)) << ";"
          << evt->getParameters().getIntVal(Form("Vaux_%d", dutID)) << ";"
          << evt->getParameters().getIntVal(Form("Vcasp_%d", dutID)) << ";"
          << evt->getParameters().getIntVal(Form("Vreset_%d", dutID)) << ";";
      for (int iSector = 0; iSector < 4; iSector++)
        settingsFile[i] << evt->getParameters().getFloatVal(
                               Form("Thr_%d_%d", dutID, iSector))
                        << ";"
                        << evt->getParameters().getFloatVal(
                               Form("ThrRMS_%d_%d", dutID, iSector))
                        << ";";
      for (int iSector = 0; iSector < 4; iSector++)
        settingsFile[i] << evt->getParameters().getFloatVal(
                               Form("Noise_%d_%d", dutID, iSector))
                        << ";"
                        << evt->getParameters().getFloatVal(
                               Form("NoiseRMS_%d_%d", dutID, iSector))
                        << ";";
      settingsFile[i]
          << evt->getParameters().getIntVal(Form("m_readout_delay_%d", dutID))
          << ";"
          << evt->getParameters().getIntVal(Form("m_trigger_delay_%d", dutID))
          << ";"
          << evt->getParameters().getIntVal(Form("m_strobe_length_%d", dutID))
          << ";"
          << evt->getParameters().getIntVal(Form("m_strobeb_length_%d", dutID))
          << ";0;";
    }
    if (_fillHistos)
      bookHistos();
  ...
  for (unsigned int iDetector = 0; iDetector < zsInputDataCollectionVec->size();
    ...
    for (auto &sparsePixel : pixelVec) {
      noiseMap[iDetector]->Fill(sparsePixel.getXCoord(),
                                sparsePixel.getYCoord());
      for (int iSector = 0; iSector < 4; iSector++)
        //      {
        //        cerr << iSector*_xPixel[iDetector]/4 << "\t"
        //        <<(iSector+1)*_xPixel[iDetector]/4 << endl;
        if (sparsePixel.getXCoord() >= iSector * _xPixel[iDetector] / 4 &&
            sparsePixel.getXCoord() < (iSector + 1) * _xPixel[iDetector] / 4)
          _nFiredPixel[iDetector][iSector]++;
      //      }
      //      cerr << evt->getEventNumber() << "\t" << iDetector << "\t" <<
...
void EUTelProcessorAnalysisPALPIDEfsNoise::bookHistos() {
  ...
  for (int iLayer = 0; iLayer < _nLayer; iLayer++) {
    noiseMap[iLayer] =
        new TH2I(Form("noiseMap_%d", iLayer),
                 Form("Noise map of layer %d", iLayer), _xPixel[iLayer], 0,
                 _xPixel[iLayer], _yPixel[iLayer], 0, _yPixel[iLayer]);
    noiseOccupancy[iLayer] =
        new TH1F(Form("noiseOccupancy_%d", iLayer),
                 Form("Noise occupancy in layer %d", iLayer), 4, 0, 4);
  }
}
...
void EUTelProcessorAnalysisPALPIDEfsNoise::end() {
  ...
  for (int iLayer = 0; iLayer < _nLayer; iLayer++) {
    for (int iSector = 0; iSector < 4; iSector++) {
      //      cerr << "Total number of fired pixels in layer " << iLayer << ",
      //      sector " << iSector << " is " << _nFiredPixel[iLayer][iSector] <<
      //      endl;
