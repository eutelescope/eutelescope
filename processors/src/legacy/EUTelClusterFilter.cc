...
EUTelClusterFilter::EUTelClusterFilter ():Processor ("EUTelClusterFilter")
{
  ...
  registerProcessorParameter ("ClusterNMinCharge",
  ...
  registerProcessorParameter ("ClusterNMinSNR",
  ...
  // again for 3 planes
  FloatVec
    minNxNChargeVecExample;
  minNxNChargeVecExample.push_back (3);
  minNxNChargeVecExample.push_back (0);
  minNxNChargeVecExample.push_back (0);
  minNxNChargeVecExample.push_back (0);

  registerProcessorParameter ("ClusterNxNMinCharge",
  ...
  registerProcessorParameter ("ClusterNxNMinSNR",
			      "This is the minimum SNR that a cluster of N times N "
			      "pixels has to have.\n"
			      ...
			      "the first number to zero.", _minNxNSNRVec,
			      minNxNSNRVecExample);

  FloatVec
    minSeedChargeVecExample;
  ...
  IntVec
    clusterQualityVecExample;
  clusterQualityVecExample.push_back (0);
  clusterQualityVecExample.push_back (0);
  clusterQualityVecExample.push_back (0);

  registerProcessorParameter ("ClusterQuality",
  ...
  registerProcessorParameter ("InsideRegion",
  ...
  registerProcessorParameter ("OutsideRegion",
			      "Define here ROI's. The first number (integer) is the detector ID.\n"
			      "The other four float are xBotLeft  yBotLeft xTopRight yTopRight.\n"
			      "To disable it, just put a negative number as detector ID.",
			      _tempOutsideROI, roiExample,
			      roiExample.size ());

  IntVec
    minClusterNoVecExample;
  minClusterNoVecExample.push_back (0);
  minClusterNoVecExample.push_back (0);
  minClusterNoVecExample.push_back (0);

  registerProcessorParameter ("MinClusterPerPlane",
  ...
  IntVec
    maxClusterNoVecExample;
  maxClusterNoVecExample.push_back (-1);
  maxClusterNoVecExample.push_back (-1);
  maxClusterNoVecExample.push_back (-1);

  registerProcessorParameter ("MaxClusterPerPlane",
  ...
  registerProcessorParameter ("DFFNumberOfHits",
			      "This is a cut on the number of hit pixels inside the "
			      "digital fixed frame\n"
...
EUTelClusterFilter::init ()
{
  ...
  streamlog_out (DEBUG2) << "MinNxNChargeSwitch " << _minNxNChargeSwitch <<
    endl;
  ...
  streamlog_out (DEBUG2) << "MinSeedSNRSwitch " << _minSeedChargeSwitch <<
    endl;
  ...
  streamlog_out (DEBUG2) << "MinTotalSNRSwitch " << _minTotalSNRSwitch <<
    endl;
  ...
  streamlog_out (DEBUG2) << "MinClusterNoSwitch " << _minClusterNoSwitch <<
    endl;
  ...
  if (count_if (_maxClusterNoVec.begin (), _maxClusterNoVec.end (),
	...
	{
	  start =
	    find_if (start, _maxClusterNoVec.end (),
		     bind2nd (less < int >(), 0));
  ...
  streamlog_out (DEBUG2) << "MaxClusterNoSwitch " << _maxClusterNoSwitch <<
    endl;
...
void
EUTelClusterFilter::initializeGeometry (LCEvent * event)
{
  ...
  try
  {
    ...
    LCCollectionVec *noiseCollection =
      dynamic_cast <
	...
	TrackerDataImpl *noise =
	  dynamic_cast <
	  TrackerDataImpl * >(noiseCollection->getElementAt (iDetector));
...
void
EUTelClusterFilter::checkCriteria ()
{
  ...
  if (_minTotalChargeSwitch)
      ...
      else
	{
	  streamlog_out (DEBUG1)
	    << "Total cluster charge criterion verified and switched on" <<
	    endl;
	  vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
	  _rejectionMap.
	    insert (make_pair ("MinTotalChargeCut", rejectedCounter));
	}
    }
  ...
  if (_minTotalSNRSwitch)
      ...
      if (_minTotalSNRVec.size () != static_cast < unsigned >(_noOfDetectors))
	{
	  streamlog_out (ERROR1)
	    ...
	    " while the thresholds are " << _minTotalSNRVec.
	    size () << "\n" <<
	    "Disabling the selection criterion and continue without" << endl;
	  _minTotalSNRSwitch = false;
	}
      else
	{
	  ...
	  vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
	  _rejectionMap.
	    insert (make_pair ("MinTotalSNRCut", rejectedCounter));
	}
    }
  ...
  if (_minNChargeSwitch)
      ...
      if (_minNChargeVec.size () % module != 0)
	{
	  streamlog_out (ERROR1)
	    ...
	    "right size \n " << "The number of planes is " << _noOfDetectors
	    << " while the thresholds are " << _minNChargeVec.
	    size () << "\n" <<
	    "Disabling the selection criterion and continue without" << endl;
	  _minNChargeSwitch = false;
	}
  ...
  if (_minNSNRSwitch)
	...
	{
	  streamlog_out (DEBUG1) <<
	    "N pixel SNR criterion verified and switched on" << endl;
	  vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
  ...
  if (_minNxNChargeSwitch)
      ...
      else
	{
	  ...
	  vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
	  _rejectionMap.
	    insert (make_pair ("MinNxNChargeCut", rejectedCounter));
	}
    }
  ...
  if (_minNxNSNRSwitch)
      ...
      if (_minNxNSNRVec.size () % module != 0)
	{
	  streamlog_out (ERROR1)
	    ...
	    "right size \n " << "The number of planes is " << _noOfDetectors
	    << " while the thresholds are " << _minNxNSNRVec.
	    size () << "\n" <<
	    "Disabling the selection criterion and continue without" << endl;
	  _minNxNSNRSwitch = false;
	}
  ...
  if (_minSeedChargeSwitch)
    {
      if (_minSeedChargeVec.size () != static_cast <
	  unsigned >(_noOfDetectors))
	{
	  streamlog_out (ERROR1)
	    <<
	    ...
	    "size \n" << "The number of planes is " << _noOfDetectors <<
	    " while the thresholds are " << _minSeedChargeVec.
	    size () << "\n" <<
	    "Disabling the selection criterion and continue without" << endl;
	  _minSeedChargeSwitch = false;
	}
	...
	{
	  streamlog_out (DEBUG1) <<
	    "Seed charge criterion verified and switched on" << endl;
	  vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
	  _rejectionMap.
	    insert (make_pair ("MinSeedChargeCut", rejectedCounter));
	}
    }
  ...
  if (_minSeedSNRSwitch)
	...
	{
	  streamlog_out (DEBUG1) <<
	    "Seed SNR criterion verified and switched on" << endl;
	  vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
  ...
  if (_clusterQualitySwitch)
    {
      if (_clusterQualityVec.size () != static_cast <
	  unsigned >(_noOfDetectors))
	{
	  streamlog_out (ERROR1)
      ...
      else
	{
	  ...
	  vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
	  _rejectionMap.
	    insert (make_pair ("ClusterQualityCut", rejectedCounter));
	}
    }

  if (_minClusterNoSwitch)
    {
      if (_minClusterNoVec.size () != static_cast <
	  unsigned >(_noOfDetectors))
	{
	  streamlog_out (ERROR1)
	    <<
	    ...
	    << "The number of planes is " << _noOfDetectors <<
	    " while the thresholds are " << _minClusterNoVec.
	    size () << "\n" <<
	    "Disabling the selection criterion and continue without" << endl;
	  _minClusterNoSwitch = false;
	}
      else
	{
	  ...
	  vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
	  _rejectionMap.
	    insert (make_pair ("MinClusterNoCut", rejectedCounter));
	}
    }

  if (_maxClusterNoSwitch)
    {
      if (_maxClusterNoVec.size () != static_cast <
	  unsigned >(_noOfDetectors))
	{
	  streamlog_out (ERROR1)
	    <<
	    ...
	    << "The number of planes is " << _noOfDetectors <<
	    " while the thresholds are " << _maxClusterNoVec.
	    size () << "\n" <<
	    "Disabling the selection criterion and continue without" << endl;
	  _maxClusterNoSwitch = false;
	}
      else
	{
	  ...
	  vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
	  _rejectionMap.
	    insert (make_pair ("MaxClusterNoCut", rejectedCounter));
	}
    }

  if (_maxClusterNoiseSwitch)
    {
      if (_maxClusterNoiseVec.size () != static_cast <
	  unsigned >(_noOfDetectors))
	{
	  streamlog_out (ERROR1)
	    <<
	    ...
	    << "The number of planes is " << _noOfDetectors <<
	    " while the thresholds are " << _maxClusterNoiseVec.
	    size () << "\n" <<
	    "Disabling the selection criterion and continue without" << endl;
	  _maxClusterNoSwitch = false;
	}
	...
	{
	  streamlog_out (DEBUG1)
	    << "Maximum cluster noise criterion verified and switched on" <<
	    endl;
	  vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
	  _rejectionMap.
	    insert (make_pair ("MaxClusterNoiseCut", rejectedCounter));
	}
    }
  ...
  if (_insideROISwitch)
    {
      streamlog_out (DEBUG1) <<
	"Inside ROI criterion verified and switched on" << endl;
      vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
  ...
  if (_outsideROISwitch)
    {
      streamlog_out (DEBUG1) <<
	"Outside ROI criterion verified and switched on" << endl;
      vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
  ...
  if (_sameNumberOfHitSwitch)
	...
	<< "Same number of hits criterion verified and switched on" << endl;
      vector < unsigned int >rejectedCounter (_noOfDetectors, 0);
      _rejectionMap.
	insert (make_pair ("SameNumberOfHitCut", rejectedCounter));
    }
}

void
EUTelClusterFilter::processRunHeader (LCRunHeader * rdr)
{
...
void
EUTelClusterFilter::processEvent (LCEvent * event)
{
  ...
  try
  {
    LCCollectionVec *pulseCollectionVec =
      dynamic_cast <
      LCCollectionVec * >(evt->getCollection (_inputPulseCollectionName));
    LCCollectionVec *filteredCollectionVec =
      new LCCollectionVec (LCIO::TRACKERPULSE);
    CellIDEncoder < TrackerPulseImpl >
      outputEncoder (EUTELESCOPE::PULSEDEFAULTENCODING,
		     filteredCollectionVec);
	...
	if (type == kEUTelDFFClusterImpl)
	...
	else if (type == kEUTelBrickedClusterImpl)
	  {
		...
		try
		{
		  ...
		  CellIDDecoder < TrackerDataImpl >
		    noiseDecoder (noiseCollectionVec);
		  ...
		  EUTelMatrixDecoder noiseMatrixDecoder (noiseDecoder,
							 noiseMatrix);
		  ...
		  for (int yPixel = ySeed - (yClusterSize / 2);
			   ...
			   xPixel <= xSeed + (xClusterSize / 2); xPixel++)
			{
			  ...
			  if ((xPixel >= noiseMatrixDecoder.getMinX ()) &&
			    ...
			    {
			      int index =
				noiseMatrixDecoder.getIndexFromXY (xPixel,
								   yPixel);
	...
	else if (type == kEUTelSparseClusterImpl)
	  {
		...
		if (_noiseRelatedCuts)
		  {
			...
			{
			  int index =
			    noiseMatrixDecoder.getIndexFromXY (sparsePixel.
							       getXCoord (),
							       sparsePixel.
							       getYCoord ());
			  noiseValues.push_back (noiseMatrix->
		    ...
		    streamlog_out (DEBUG1) << "Noise related cuts may be used"
		      << endl;
		  }
	      }
	    else
		...
		streamlog_out (ERROR4) <<
		  "Unknown pixel type. Sorry for quitting" << endl;
	...
	TrackerPulseImpl *pulse =
	  dynamic_cast <
	  TrackerPulseImpl * >(pulseCollectionVec->getElementAt (*cluIter));
	...
	if (!areClusterEnoughTemp)
	  {
	    streamlog_out (DEBUG1) <<
	      "Not enough clusters passed the selection " << endl;
	  }
	else if (areClusterTooManyTemp)
	  {
	    streamlog_out (DEBUG1) <<
	      "Too many clusters passed the selection " << endl;
	  }
	else
	  {
	    streamlog_out (DEBUG1) << "No clusters passed the selection" <<
	      endl;
	  }
	if (_skipEmptyEvent)
	  {
	...
	while (iter != acceptedClusterVec.end ())
	  {
	    TrackerPulseImpl *pulse =
	      dynamic_cast <
	      TrackerPulseImpl * >(pulseCollectionVec->getElementAt (*iter));
	    ...
	    _acceptedClusterCounter[_ancillaryIndexMap
				    [inputDecoder (pulse)["sensorID"]]]++;
	    ++iter;
	  }
	evt->addCollection (filteredCollectionVec,
			    _outputPulseCollectionName);
      }
  }
...
bool
EUTelClusterFilter::areClusterEnough (std::vector < int >clusterNoVec) const
{
  ...
  for (size_t iDetector = 0; iDetector < _noOfDetectors; iDetector++)
      ...
      if (clusterNoVec[iDetector] < _minClusterNoVec[iDetector])
	{
	  streamlog_out (DEBUG2)
	    << "Rejected event because on detector " << iDetector <<
...
bool
EUTelClusterFilter::areClusterTooMany (std::vector < int >clusterNoVec) const
{
  ...
  for (size_t iDetector = 0; iDetector < _noOfDetectors; iDetector++)
      ...
      if (clusterNoVec[iDetector] > _maxClusterNoVec[iDetector])
	{
	  streamlog_out (DEBUG2)
	    << "Rejected event because on detector " << iDetector <<
	    " there are " << clusterNoVec[iDetector] << "\nwhile " << "only "
	    << _maxClusterNoVec[iDetector] << " were allowed" << endl;
...
bool
EUTelClusterFilter::hasSameNumberOfHit (std::vector < int >clusterNoVec) const
{
  ...
  while (iter != clusterNoVec.end ())
	...
	{
	  streamlog_out (DEBUG2) <<
	    "Rejected event because the number of hit is "
	    "different from one sensor to the other" << endl;
	  hasSameNumber = false;
	  break;
	}
      ++iter;
...
bool
EUTelClusterFilter::isAboveNumberOfHitPixel (EUTelVirtualCluster * cluster) const
{
  if (!_dffnhitsswitch)
    {
      return true;
  ...
  if (static_cast < int >(cluster->getTotalCharge ()) >=
      _DFFNHitsCuts[detectorPos])
    return true;
  else
    {
      streamlog_out (DEBUG2)
...
bool
EUTelClusterFilter::isAboveMinTotalCharge (EUTelVirtualCluster * cluster) const
{
...
bool
EUTelClusterFilter::isAboveMinTotalSNR (EUTelVirtualCluster * cluster) const
{
  ...
  streamlog_out (DEBUG1) << "Filtering against the minimum total SNR " <<
    endl;
...
bool
EUTelClusterFilter::isAboveNMinCharge (EUTelVirtualCluster * cluster) const
{
  ...
  while (iter != _minNChargeVec.end ())
	...
	{
	  streamlog_out (DEBUG2) <<
	    "Rejected cluster because its charge over " << (*iter) << " is "
	    << charge << " and the threshold is " << threshold << endl;
...
bool
EUTelClusterFilter::isAboveNMinSNR (EUTelVirtualCluster * cluster) const
{
...
bool
EUTelClusterFilter::isAboveNxNMinCharge (EUTelVirtualCluster * cluster) const
{

  if (!_minNxNChargeSwitch)
    return true;

  streamlog_out (DEBUG1) << "Filtering against the N x N pixel charge" <<
    endl;
  ...
  while (iter != _minNxNChargeVec.end ())
	...
	{
	  streamlog_out (DEBUG2) <<
	    "Rejected cluster because its charge within a " << (*iter) <<
	    " x " << (*iter) << " subcluster is " << charge <<
	    " and the threshold is " << threshold << endl;
...
bool
EUTelClusterFilter::isAboveNxNMinSNR (EUTelVirtualCluster * cluster) const
{
  ...
  while (iter != _minNxNSNRVec.end ())
	...
	{
	  streamlog_out (DEBUG2) <<
	    "Rejected cluster because its SNR within a " << (*iter) << " x "
	    << (*iter) << " subcluster is " << snr << " and the threshold is "
	    << threshold << endl;
...
bool
EUTelClusterFilter::isAboveMinSeedCharge (EUTelVirtualCluster * cluster) const
{
...
bool
EUTelClusterFilter::isAboveMinSeedSNR (EUTelVirtualCluster * cluster) const
{
...
bool
EUTelClusterFilter::hasQuality (EUTelVirtualCluster * cluster) const
{
...
bool
EUTelClusterFilter::isBelowMaxClusterNoise (EUTelVirtualCluster * cluster) const
{
...
bool
EUTelClusterFilter::isInsideROI (EUTelVirtualCluster * cluster) const
{
...
bool
EUTelClusterFilter::isOutsideROI (EUTelVirtualCluster * cluster) const
{
...
void
EUTelClusterFilter::check (LCEvent * /* evt */ )
{
...
void
EUTelClusterFilter::end ()
{
  streamlog_out (MESSAGE4) << printSummary () << endl;
}

string
EUTelClusterFilter::printSummary () const
{
  ...
  ss << doubleLine.str () << endl
    << " Rejection summary " << endl << doubleLine.str () << endl;

  map < string, vector < unsigned int >>::iterator iter =
    _rejectionMap.begin ();
  while (iter != _rejectionMap.end ())
    {
      ss << " " << setiosflags (ios::left) << setw (bigSpacer) << (*iter).
	first << resetiosflags (ios::left);
  ...
  ss << singleLine.str () << "\n"
    << " " << setiosflags (ios::
			   left) << setw (bigSpacer) << "Accepted clusters "
    << resetiosflags (ios::left);
  vector < unsigned int >::const_iterator iter3 =
    _acceptedClusterCounter.begin ();
  ...
  ss << "\n"
    ...
    << resetiosflags (ios::left);
  vector < unsigned int >::const_iterator iter4 =
    _totalClusterCounter.begin ();
