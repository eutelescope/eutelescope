...
EUTelDafAlign::EUTelDafAlign()
    : EUTelDafBase("EUTelDafAlign"), _runPede(false), _pedeSteerfileName(""),
...
void EUTelDafAlign::dafParams() {
  _description = "This processor preforms track reconstruction. The tracks are "
                 "used for alignment of a partially aligned system.";
  ...
  registerOptionalParameter(
      "Translate", "List of sensor IDs to where translations should be free",
      _translate, std::vector<int>());
  registerOptionalParameter("TranslateX", "List of sensor IDs to where "
                                          "translations should be free in the "
                                          "x direction",
                            _translateX, std::vector<int>());
  registerOptionalParameter("TranslateY", "List of sensor IDs to where "
                                          "translations should be free in the "
                                          "y direction",
                            _translateY, std::vector<int>());
  registerOptionalParameter(
      "ZRotate",
      "List of sensor IDs to where rotation over z-axis should be free", _zRot,
      std::vector<int>());
  registerOptionalParameter(
      "Scale",
      "List of sensor IDs to where scales of X and Y axis should be free",
      _scale, std::vector<int>());
  registerOptionalParameter(
      "ScaleX", "List of sensor IDs to where scales of X axis should be free",
      _scaleX, std::vector<int>());
  registerOptionalParameter(
      "ScaleY", "List of sensor IDs to where scales of Y axis should be free",
      _scaleY, std::vector<int>());
  // Track cuts
  registerOptionalParameter("ResidualXMin", "Min X for residual cuts. All "
                                            "sensors shoule be included, same "
                                            "order as parameter DutPlanes",
                            _resXMin, std::vector<float>());
  registerOptionalParameter("ResidualXMax", "Max X for residual cuts. All "
                                            "sensors shoule be included, same "
                                            "order as parameter DutPlanes",
                            _resXMax, std::vector<float>());
  registerOptionalParameter("ResidualYMin", "Min Y for residual cuts. All "
                                            "sensors shoule be included, same "
                                            "order as parameter DutPlanes",
                            _resYMin, std::vector<float>());
  registerOptionalParameter("ResidualYMax", "Max Y for residual cuts. All "
                                            "sensors shoule be included, same "
                                            "order as parameter DutPlanes",
                            _resYMax, std::vector<float>());
  // Region of cell
  registerOptionalParameter("MinCol", "Minimum allowed local X of hit. All "
                                      "clusters containing hits outside will "
                                      "be discarded from alignment.",
                            _colMin, std::vector<int>());
  registerOptionalParameter("MaxCol", "Maximum allowed local X of hit. All "
                                      "clusters containing hits outside will "
                                      "be discarded from alignment.",
                            _colMax, std::vector<int>());
  registerOptionalParameter("MinRow", "Minimum allowed local Y of hit. All "
                                      "clusters containing hits outside will "
                                      "be discarded from alignment.",
                            _rowMin, std::vector<int>());
  registerOptionalParameter("MaxRow", "Maximum allowed local Y of hit. All "
                                      "clusters containing hits outside will "
                                      "be discarded from alignment.",
                            _rowMax, std::vector<int>());
}
...
void EUTelDafAlign::dafInit() {
  ...
  if (_runPede) {
    ...
    streamlog_out(MESSAGE5) << "The filename for the mille binary file is: "
                            << _binaryFilename.c_str() << endl;
  }
...
int EUTelDafAlign::checkDutResids(daffitter::TrackCandidate<float, 4> &track) {
  ...
  for (size_t ii = 0; ii < _system.planes.size(); ii++) {
    ...
    for (size_t w = 0; w < plane.meas.size(); w++) {
      ...
      // Resids
      if ((estim.getX() - meas.getX()) < resX.first) {
        measWeight = 0.0f;
      }
      if ((estim.getX() - meas.getX()) > resX.second) {
        measWeight = 0.0f;
      }
      if ((estim.getY() - meas.getY()) < resY.first) {
        measWeight = 0.0f;
      }
      if ((estim.getY() - meas.getY()) > resY.second) {
        measWeight = 0.0f;
      }
      if (measWeight > 0.5) {
        nHits++;
        if (not meas.goodRegion()) {
          measWeight = 0.0f;
        }
      }
...
void EUTelDafAlign::dafEvent(LCEvent * /*event*/) {
  ...
  for (size_t ii = 0; ii < _system.getNtracks(); ii++) {
    ...
    // This guy includes DUT planes and adds weights to measurements based on
    // resid cuts
...
void EUTelDafAlign::addToMille(daffitter::TrackCandidate<float, 4> &track) {
  ...
  for (size_t ii = 0; ii < _system.planes.size(); ii++) {
    ...
    derGL[(ii * 5) + 2] =
        meas.getY(); // Derivatives of residuals w.r.t. z rotations
    derGL[(ii * 5) + 3] =
        meas.getX();          // Derivatives of residuals w.r.t. scale of x axis
    derLC[0] = 1;             // Derivatives of fit pos w.r.t. x
    derLC[2] = pl.getMeasZ(); // Derivatives of fit pos w.r.t. dx/dz
    _mille->mille(nLC, derLC, nGL, derGL, label, estim.getX() - meas.getX(),
                  pl.getSigmaX());
    ...
    derGL[(ii * 5) + 2] =
        -1 * meas.getX(); // Derivatives of residuals w.r.t. z rotations
    derGL[(ii * 5) + 4] =
        meas.getY(); // Derivatives of residuals w.r.t. scales of y axis

    derLC[1] = 1;             // Derivatives of fit pos w.r.t. y
    derLC[3] = pl.getMeasZ(); // Derivatives of fit pos w.r.t. dy/dz

    _mille->mille(nLC, derLC, nGL, derGL, label, estim.getY() - meas.getY(),
                  pl.getSigmaY());
...
void EUTelDafAlign::generatePedeSteeringFile() {
  ...
  if (not steerFile.is_open()) {
    streamlog_out(ERROR2) << "Unable to open pede steering file, quitting"
                          << endl;
...
void EUTelDafAlign::steerLine(ofstream &steerFile, int label, int iden,
...
void EUTelDafAlign::runPede() {
  ...
  if (which.rdbuf()->status() == 255) {
    streamlog_out(ERROR5) << "Cannot find pede program in path. Nothing to do."
                          << endl;
  ...
  while (getline(pede, output)) {
    streamlog_out(MESSAGE5) << output << endl;
  }
  ...
  while (not millepede.eof()) {
    ...
    if ((nValues != 3) and (nValues != 5) and (nValues != 6)) {
      continue;
    }
    ...
    case 0:
      ...
      if (not isFixed) {
        constant->setXOffsetError(values.at(4) / 1000.0);
      }
      break;
    case 1:
      constant->setYOffset(values.at(1) / 1000.0);
      if (not isFixed) {
        constant->setYOffsetError(values.at(4) / 1000.0);
      }
      break;
    case 2:
      constant->setGamma(values.at(1));
      if (not isFixed) {
        constant->setGammaError(values.at(4));
      }
      break;
    case 3:
      constant->setAlpha(values.at(1));
      if (not isFixed) {
        constant->setAlphaError(values.at(4));
      }
      break;
    case 4:
      constant->setBeta(values.at(1));
      if (not isFixed) {
        constant->setBetaError(values.at(4));
      }
