...
EUTelAutoPedestalNoiseProcessor::EUTelAutoPedestalNoiseProcessor ():Processor ("EUTelAutoPedestalNoiseProcessor"), _pedestalCollectionName (""),
_noiseCollectionName (""), _statusCollectionName (""), _initPedestal (),
_initNoise (), _iRun (0), _iEvt (0), _pedestalCollectionVec (nullptr),
_noiseCollectionVec (nullptr), _statusCollectionVec (nullptr), _minX (0),
_maxX (0), _minY (0), _maxY (0), _sensorIDVec ()
{

  // modify processor description
  _description =
    "EUTelAutoPedestalNoiseProcessor produces initial pedestal / "
    "noise / status with user provided values";
  ...
  IntVec
  minXExample (nDetectorExample, 0);
  registerOptionalParameter ("MinXVector",
			     "The minimum pixel along x (default 0, one value per detector)",
			     _minX, minXExample);

  IntVec
  maxXExample (nDetectorExample, 255);
  registerOptionalParameter ("MaxXVector",
			     "The maximum pixel along x (default 255, one value per detector)",
			     _maxX, maxXExample);

  IntVec
  minYExample (nDetectorExample, 0);
  registerOptionalParameter ("MinYVector",
			     "The minimum pixel along y (default 0, one value per detector)",
			     _minY, minYExample);

  IntVec
  maxYExample (nDetectorExample, 255);
  registerOptionalParameter ("MaxYVector",
  ...
  FloatVec
  initPedeExample (nDetectorExample, 0.);
  ...
  FloatVec
  initNoiseExample (nDetectorExample, 1.);
...
void
EUTelAutoPedestalNoiseProcessor::init ()
{
...
void
EUTelAutoPedestalNoiseProcessor::processRunHeader (LCRunHeader * rdr)
{
  ++_iRun;
...
void
EUTelAutoPedestalNoiseProcessor::processEvent (LCEvent * event)
{
  ...
  if (isFirstEvent ())
    {
      ...
      for (unsigned int iDetector = 0; iDetector < _initPedestal.size ();
	   iDetector++)
	{

	  int nPixel = (_maxX[iDetector] - _minX[iDetector] + 1) *
	    (_maxY[iDetector] - _minY[iDetector] + 1);

	  TrackerRawDataImpl *status = new TrackerRawDataImpl;
	  CellIDEncoder < TrackerRawDataImpl >
	    statusEncoder (EUTELESCOPE::MATRIXDEFAULTENCODING,
	  ...
	  CellIDEncoder < TrackerDataImpl >
	    pedestalEncoder (EUTELESCOPE::MATRIXDEFAULTENCODING,
	  ...
	  CellIDEncoder < TrackerDataImpl >
	    noiseEncoder (EUTELESCOPE::MATRIXDEFAULTENCODING,
			  _noiseCollectionVec);
...
void
EUTelAutoPedestalNoiseProcessor::end ()
{
