...
EUTelFitTuple::EUTelFitTuple() : Processor("EUTelFitTuple") {
  ...
  registerInputCollection(LCIO::TRACKERHIT, "InputDUTCollectionName",
                          "Name of the input DUT hit collection",
                          _inputDUTColName, std::string("duthit"));
...
void EUTelFitTuple::init() {
  ...
  message<MESSAGE5>(
      log() << "Reading telescope geometry description from GEAR ");
  ...
  if (_useManualDUT) {
    ...
    if (!_manualOK) {
      message<ERROR5>(
          log() << "Manual DUT flag set, layer not found ID = " << _manualDUTid
  ...
  message<MESSAGE5>(log() << "Telescope configuration with " << _nTelPlanes
                          << " planes");
...
void EUTelFitTuple::processRunHeader(LCRunHeader *runHeader) {
  ...
  message<MESSAGE5>(log() << "Processing run header " << _nRun << ", run nr "
                          << _runNr);
...
void EUTelFitTuple::processEvent(LCEvent *event) {
  ...
  int nDUT = 0;
  if (_DUTok)
    nDUT = hitcol->getNumberOfElements();
  ...
  for (int itrack = 0; itrack < nTrack; itrack++) {
    ...
    message<DEBUG5>(log() << "Track " << itrack << " with " << nHit
                          << " hits, Chi2 = " << fittrack->getChi2() << "/"
                          << fittrack->getNdf());
    ...
    for (int ihit = 0; ihit < nHit; ihit++) {
      ...
      if (hitPlane < 0) {
        message<ERROR5>(log() << "Hit outside telescope plane at z [mm] = "
                              << pos[2]);
        continue;
      }
      ...
      if ((hitCellDecoder(meshit)["properties"] & kFittedHit) == 0) {
        ...
        message<DEBUG5>(log() << "Measured hit in plane " << hitPlane
                              << " at  X = " << pos[0] << ", Y = " << pos[1]
                              << ", Q = " << _measuredQ[hitPlane]);
        ...
        message<DEBUG5>(log() << "Fitted  hit  in plane " << hitPlane
                              << " at  X = " << pos[0] << ", Y = " << pos[1]);
      }
    }
    ...
    if (_DUTok) {
      ...
      for (int ihit = 0; ihit < nDUT; ihit++) {
        TrackerHit *meshit =
            dynamic_cast<TrackerHit *>(hitcol->getElementAt(ihit));
      ...
      if (imin >= 0) {
        TrackerHit *meshit =
            dynamic_cast<TrackerHit *>(hitcol->getElementAt(imin));
        ...
        message<DEBUG5>(log() << "Matched DUT hit at X = " << dutX
                              << "   Y = " << dutY << "   Dxy = " << dutR
                              << "   Q = " << dutQ);
      } else
...
void EUTelFitTuple::end() {
  ...
  message<MESSAGE5>(log() << "N-tuple with " << _FitTuple->rows()
                          << " rows created");
...
void EUTelFitTuple::bookHistos() {
  ...
  _FitTuple = AIDAProcessor::tupleFactory(this)->create(
      _FitTupleName, _FitTupleName, _columnNames, _columnType, "");
