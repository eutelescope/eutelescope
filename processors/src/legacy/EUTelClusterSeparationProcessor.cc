...
EUTelClusterSeparationProcessor::EUTelClusterSeparationProcessor ():Processor
  ("EUTelClusterSeparationProcessor")
{
  ...
  registerProcessorParameter ("SeparationAlgorithm",
			      ...
			      _separationAlgo,
			      string (EUTELESCOPE::FLAGONLY));

  registerProcessorParameter ("MinimumDistance", "Minimum distance allowed "
			      "between separated clusters (0 "
			      "== only touching clusters)",
			      _minimumDistance, static_cast < float >(0));
}

void
EUTelClusterSeparationProcessor::init ()
{
...
void
EUTelClusterSeparationProcessor::processRunHeader (LCRunHeader * rdr)
{
...
void
EUTelClusterSeparationProcessor::processEvent (LCEvent * event)
{
  ...
  catch (lcio::DataNotAvailableException & e)
  {
    streamlog_out (DEBUG2)
      << "No cluster collection found on this event. Returnin now" << endl;
    return;
  }
  ...
  CellIDEncoder < TrackerPulseImpl >
    outputEncoder (EUTELESCOPE::PULSEDEFAULTENCODING, outputCollectionVec);
  ...
  for (int iCluster = 0;
	...
	dynamic_cast <
	TrackerPulseImpl * >(clusterCollectionVec->getElementAt (iCluster));
      ClusterType type =
      ...
      else if (type == kEUTelSparseClusterImpl)
	{
	  ...
	  TrackerDataImpl *oneCluster =
	    dynamic_cast <
	    TrackerDataImpl * >(sparseClusterCollectionVec->getElementAt (0));
      ...
      else
	{
      ...
      bool isExisisting =
	(iOtherCluster < clusterCollectionVec->getNumberOfElements ());
      ...
      while (isOnSameDetector && isExisisting)
	{
	  ...
	  TrackerPulseImpl *otherPulse =
	    dynamic_cast <
	    TrackerPulseImpl *
	    >(clusterCollectionVec->getElementAt (iOtherCluster));
	  ...
	  if (cluster->getDetectorID () == otherCluster->getDetectorID ())
	      ...
	      if (distance < _minimumDistance)
		{
		  ...
		  // algorithm
		  mergingPairVector.
		    push_back (make_pair (iCluster, iOtherCluster));
		}
  ...
  if (mergingPairVector.empty ())
    {
      ...
      // exactly the same
      for (int iPulse = 0;
	   iPulse < clusterCollectionVec->getNumberOfElements (); iPulse++)
	{
	  TrackerPulseImpl *pulse =
	    dynamic_cast <
	    TrackerPulseImpl * >(clusterCollectionVec->getElementAt (iPulse));
...
bool
EUTelClusterSeparationProcessor::applySeparationAlgorithm (std::vector <
							   std::set <
							   int >>setVector,
							   LCCollectionVec *
							   inputCollectionVec,
  ...
  const
{
  ...
  //  " << _separationAlgo );
  streamlog_out (DEBUG0) << "Found " << setVector.size ()
    << " group(s) of merging clusters " << endl;
  ...
  if (_separationAlgo == EUTELESCOPE::FLAGONLY)
    {
      ...
      // follow.
      for (int iPulse = 0;
	   iPulse < inputCollectionVec->getNumberOfElements (); iPulse++)
	{
	  TrackerPulseImpl *pulse =
	    dynamic_cast <
	    TrackerPulseImpl * >(inputCollectionVec->getElementAt (iPulse));
	  TrackerPulseImpl *newPulse = new TrackerPulseImpl;
	  newPulse->setCellID0 (pulse->getCellID0 ());
	  newPulse->setCellID1 (pulse->getCellID1 ());
	  newPulse->setTime (pulse->getTime ());
      ...
      while (vectorIterator != setVector.end ())
	{
	  ...
	  streamlog_out (DEBUG4) << "     Group " << (iCounter++)
	    << " with the following clusters " << endl;
	  ...
	  while (setIterator != (*vectorIterator).end ())
		...
		dynamic_cast <
		TrackerPulseImpl *
		>(outputCollectionVec->getElementAt (*setIterator));
	      ClusterType type =
		static_cast < ClusterType > (static_cast <
					     int
					     >(cellDecoder (pulse)["type"]));
		...
		{
		  message < ERROR5 >
		    ("Unknown cluster type. Sorry for quitting");
		  throw UnknownDataTypeException ("Cluster type unknown");
		}
	      ...
	      try
	      {
		// TODO: FIxMe//cluster->setClusterQuality (
		// cluster->getClusterQuality() | kMergedCluster );
	      }
	      catch (lcio::ReadOnlyException & e)
	      {
		streamlog_out (WARNING2)
		  <<
		  "Attempt to change the cluster quality on the original data\n"
		  "This is possible only when the " << name () <<
		  " is not applied to data already on tape\n"
		  "In this case only the pulse containing this "
		  "cluster will have the proper quality" << endl;
	      }
	      pulse->setQuality (static_cast <
				 int
				 >(ClusterQuality
				   (pulse->getQuality () | kMergedCluster)));
	      ++setIterator;
	      delete cluster;
	    }
	  ++vectorIterator;
...
void
EUTelClusterSeparationProcessor::groupingMergingPairs (std::vector <
						       std::pair < int,
						       int >>pairVector,
						       std::vector <
						       std::set <
						       int >>*setVector) const
{
  ...
  while (iter != pairVector.end ())
	...
	int >>::iterator >::reverse_iterator iterIter = tempIterVec.rbegin ();
...
void
EUTelClusterSeparationProcessor::check (LCEvent * /* evt */ )
{
...
void
EUTelClusterSeparationProcessor::end ()
{
