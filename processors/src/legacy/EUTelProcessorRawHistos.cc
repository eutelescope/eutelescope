...
EUTelProcessorRawHistos::EUTelProcessorRawHistos ():Processor ("EUTelProcessorRawHistos"), _zsDataCollectionName (""), _iRun (0),
_iEvt (0), _sensorIDVec ()
{
  // processor description
  _description = "EUTelProcessorRawHistos computes the firing frequency of "
    "pixels and applies a cut on this value to mask (NOT remove) "
    "hot pixels.";

  registerInputCollection (LCIO::TRACKERDATA, "ZSDataCollectionName",
			   "Input of Zero Suppressed data",
			   _zsDataCollectionName, std::string ("zsdata"));
  registerOptionalParameter ("NoisyPixelCollectionName",
			     "Name of the noisy pixel collection",
			     _noisyPixCollectionName,
			     std::string ("noisypixel"));
}

void
EUTelProcessorRawHistos::init ()
{
...
int
EUTelProcessorRawHistos::cantorEncode (int X, int Y)
{
...
void
EUTelProcessorRawHistos::initialiseNoisyPixels (LCCollectionVec *
						const noisyPixCollectionVec)
{
  ...
  for (int i = 0; i < noisyPixCollectionVec->getNumberOfElements (); i++)
    {
      ...
      TrackerDataImpl *noisyTrackerData =
	dynamic_cast <
	TrackerDataImpl * >(noisyPixCollectionVec->getElementAt (i));
	...
	{
	  auto noisyPixelData = std::make_unique <
	    EUTelTrackerDataInterfacerImpl <
	    EUTelGenericSparsePixel >> (noisyTrackerData);
	  auto & pixelVec = noisyPixelData->getPixels ();
	  // Store all the noisy pixels in the noise vector, use the provided
	  // encoding to map two int's to an unique int
	for (auto & pixel:pixelVec)
	    {
	      noiseSensorVector->
		push_back (cantorEncode
			   (pixel.getXCoord (), pixel.getYCoord ()));
	    }
	}
      else
	{
...
void
EUTelProcessorRawHistos::processRunHeader (LCRunHeader * rdr)
{
...
void
EUTelProcessorRawHistos::processEvent (LCEvent * event)
{
  if (event == nullptr)
    {
      streamlog_out (WARNING2) << "Event does not exist! Skipping!" << std::
	endl;
      return;
    }
  ...
  if (_iEvt == 0)
    {
      try
      {
	auto noisyPixelCollection =
	  static_cast <
	  LCCollectionVec * >(event->getCollection (_noisyPixCollectionName));
	...
	if (!_noisyPixCollectionName.empty ())
	  {
	    streamlog_out (WARNING1) << "_noisyPixCollectionName "
	      << _noisyPixCollectionName << " not found" << endl;
	  }
	_treatNoise = false;
      }
    }
  ...
  for (auto sensorID:_sensorIDVec)
	...
	TrackerDataImpl *zsData =
	  dynamic_cast <
	  TrackerDataImpl * >(zsInputCollectionVec->getElementAt (iDetector));
	...
	auto & pixelVec = sparseData->getPixels ();
      ...
      for (auto & genericPixel:pixelVec)
	  {
	    ...
	    if (_treatNoise)
	      isNoisy =
		std::binary_search (_noisyPixelVecMap.at (sensorID).begin (),
				    _noisyPixelVecMap.at (sensorID).end (),
				    encoded);
		...
		_chargeHistoNoNoise.at (sensorID)->fill (genericPixel.
							 getSignal ());
		_timeHistoNoNoise.at (sensorID)->fill (genericPixel.
						       getTime ());
	      }
	  }
      }
...
void
EUTelProcessorRawHistos::end ()
{
}

void
EUTelProcessorRawHistos::bookHistos ()
{
...
for (auto sensorID:_sensorIDVec)
    {
      ...
      auto const countHistoName =
	"raw_hits_per_trigger_d" + to_string (sensorID);
      ...
      auto rawHitCountHisto =
	AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath +
								    countHistoName).
								   c_str (),
								   20, -0.5,
								   19.5);
      auto rawHitChargeHisto =
	AIDAProcessor::histogramFactory (this)->
	createHistogram1D ((basePath + chargeHistoName).c_str (), 20, -0.5,
			   19.5);
      auto rawHitTimeHisto =
	AIDAProcessor::histogramFactory (this)->
	createHistogram1D ((basePath + timeHistoName).c_str (), 20, -0.5,
			   19.5);

      auto rawHitCountHistoNoNoise =
	AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath +
								    countHistoName
								    +
								    "_noNoise").
								   ...
								   20, -0.5,
								   19.5);
      auto rawHitChargeHistoNoNoise =
	AIDAProcessor::histogramFactory (this)->
	createHistogram1D ((basePath + chargeHistoName + "_noNoise").c_str (),
			   20, -0.5, 19.5);
      auto rawHitTimeHistoNoNoise =
	AIDAProcessor::histogramFactory (this)->
	createHistogram1D ((basePath + timeHistoName + "_noNoise").c_str (),
			   20, -0.5, 19.5);
