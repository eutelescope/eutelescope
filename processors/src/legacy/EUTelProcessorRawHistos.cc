...
EUTelProcessorRawHistos::EUTelProcessorRawHistos()
    : Processor("EUTelProcessorRawHistos"), _zsDataCollectionName(""), _iRun(0),
  ...
  registerOptionalParameter("NoisyPixelCollectionName",
                            ...
                            _noisyPixCollectionName, std::string("noisypixel"));
}
...
void EUTelProcessorRawHistos::initialiseNoisyPixels(
    LCCollectionVec *const noisyPixCollectionVec) {
  ...
  for (int i = 0; i < noisyPixCollectionVec->getNumberOfElements(); i++) {
    ...
    TrackerDataImpl *noisyTrackerData =
        dynamic_cast<TrackerDataImpl *>(noisyPixCollectionVec->getElementAt(i));
...
void EUTelProcessorRawHistos::processEvent(LCEvent *event) {
  ...
  if (_iEvt == 0) {
    try {
      auto noisyPixelCollection = static_cast<LCCollectionVec *>(
          event->getCollection(_noisyPixCollectionName));
      initialiseNoisyPixels(noisyPixelCollection);
    } catch (...) {
      if (!_noisyPixCollectionName.empty()) {
        streamlog_out(WARNING1) << "_noisyPixCollectionName "
                                << _noisyPixCollectionName << " not found"
                                << endl;
      }
      _treatNoise = false;
    }
        ...
        if (_treatNoise)
          isNoisy =
              std::binary_search(_noisyPixelVecMap.at(sensorID).begin(),
                                 _noisyPixelVecMap.at(sensorID).end(), encoded);
  ...
  for (auto sensorID : _sensorIDVec) {
    ...
    auto rawHitCountHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + countHistoName).c_str(), 20, -0.5, 19.5);
    auto rawHitChargeHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + chargeHistoName).c_str(), 20, -0.5, 19.5);
    auto rawHitTimeHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + timeHistoName).c_str(), 20, -0.5, 19.5);

    auto rawHitCountHistoNoNoise =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + countHistoName + "_noNoise").c_str(), 20, -0.5, 19.5);
    auto rawHitChargeHistoNoNoise =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + chargeHistoName + "_noNoise").c_str(), 20, -0.5, 19.5);
    auto rawHitTimeHistoNoNoise =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + timeHistoName + "_noNoise").c_str(), 20, -0.5, 19.5);
