...
EUTelProcessorTrueHitAnalysis::EUTelProcessorTrueHitAnalysis()
    : Processor("EUTelProcessorTrueHitAnalysis"), _trueHitCollectionName(""),
...
int EUTelProcessorTrueHitAnalysis::findPairIndex(
...
void EUTelProcessorTrueHitAnalysis::fillHistos(LCEvent *event) {
  ...
  try {
    ...
    for (int i = 0; i < _reconstructedHitCollectionVec->getNumberOfElements();
      ...
      _2DHistos[2 * (clusterSizeX - 1)]
          .at(detectorID)
          ->fill(diff_x, diff_y, 1.0);
...
void EUTelProcessorTrueHitAnalysis::bookHistos() {
  ...
  for (size_t i = 0; i < _sensorIDVec.size(); i++) {
    ...
    for (size_t i = 0; i < _1DHistos.size(); i++) {
      ...
      std::string histoName = coordinate + "PositionDifference_" + coordinate +
                              "ClusterSize" + clusterSize + "_d" +
                              to_string(sensorID);
      ...
      std::string histoTitle =
          ...
          coordinate + " cluster size " + clusterSize + ";#Delta" + coordinate +
          " /#mum;Entries";
      ...
      _1DHistos[i].insert(std::make_pair(
          sensorID,
    ...
    for (size_t i = 0; i < _2DHistos.size(); i++) {
      ...
      std::string histoTitle = "difference in position between true simulated "
                               "hits and reconstructed hits for " +
                               coordinate + " cluster size " + clusterSize +
      ...
      _2DHistos[i].insert(std::make_pair(
    ...
    if (sensorID >= 10) {
      ...
      for (size_t i = 0; i < 10; i++) {
        ...
        std::string histoName = "xPositionDifference" + modifier + "_TOTDiff" +
                                TOTDiff + "_d" + to_string(sensorID);

        int histoNBin = 161;
        double histoMin = -140;
        double histoMax = 140;
        std::string histoTitle = "difference in x position between true "
                                 "simulated hits and reconstructed hits for x "
                                 "cluster size 2 with TOT difference " +
                                 TOTDiff + ";#Deltax /#mum;Entries";

        _xClustSize2Histos.push_back(
            AIDAProcessor::histogramFactory(this)->createHistogram1D(
                (tempPath + histoName).c_str(), histoNBin, histoMin, histoMax));
    ...
    if (sensorID == 0) {
      ...
      for (size_t i = 0; i < 4; i++) {
        ...
        double histoMin = -20;
        double histoMax = 20;
        std::string histoTitle = "difference in y position between true "
                                 "simulated hits and reconstructed hits for y "
                                 "cluster size 3 with x cluster size " +
                                 xClustSize + ";#Deltay /#mum;Entries";

        _yClustSize3Histos.push_back(
            AIDAProcessor::histogramFactory(this)->createHistogram1D(
                (tempPath + histoName).c_str(), histoNBin, histoMin, histoMax));
