...
EUTelFitHistograms::EUTelFitHistograms() : Processor("EUTelFitHistograms") {
  ...
  registerInputCollection(LCIO::TRACK, "InputCollectionName",
                          "Name of the input Track collection", _inputColName,
...
void EUTelFitHistograms::init() {
  ...
  streamlog_out(MESSAGE5) << "Reading telescope geometry description from GEAR "
                          << endl;
  ...
  for (int iz = 0; iz < _nTelPlanes; iz++) {
    ...
    if (_planeID[iz] == _BeamReferenceID)
      _beamID = iz;
    if (_planeID[iz] == _TelescopeReferenceIDs.at(0))
      _referenceID0 = iz;
    if (_planeID[iz] == _TelescopeReferenceIDs.at(1))
      _referenceID1 = iz;
  }
  ...
  streamlog_out(MESSAGE5) << "Telescope configuration with " << _nTelPlanes
                          << " planes" << endl;
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    ...
    if (ipl == _iDUT)
      ss << "D.U.T.  plane";
    else if (_isActive[ipl])
      ss << "Active  plane";
    else
      ss << "Passive plane";

    ss << "  ID = " << _planeID[ipl] << "  at Z [mm] = " << _planePosition[ipl];
...
void EUTelFitHistograms::processRunHeader(LCRunHeader *runHeader) {
  std::unique_ptr<EUTelRunHeaderImpl> eutelHeader =
      std::make_unique<EUTelRunHeaderImpl>(runHeader);
  ...
  streamlog_out(MESSAGE5) << "Processing run header " << _nRun << ", run nr "
                          << runNr << endl;
  ...
  streamlog_out(MESSAGE5) << detectorName << " : " << detectorDescription
                          << endl;
...
void EUTelFitHistograms::processEvent(LCEvent *event) {
  ...
  try {
    ...
    streamlog_out(WARNING) << "Not able to get collection " << _inputColName
  ...
  if (debug) {
    streamlog_out(TESTFITTERMESSAGE) << "Total of " << nTrack
                                     << " tracks in input collection " << endl;
  }
  ...
  for (int itrack = 0; itrack < nTrack; itrack++) {
    ...
    if (debug) {
      streamlog_out(TESTFITTERMESSAGE) << "Track " << itrack << " with " << nHit
                                       << " hits " << endl;
    }
    ...
    for (int ihit = 0; ihit < nHit; ihit++) {
      ...
      if (hitPlane < 0) {
        streamlog_out(ERROR5)
            << "Hit outside telescope plane at z [mm] = " << pos[2] << endl;
        continue;
      }
      ...
      if ((hitCellDecoder(meshit)["properties"] & kFittedHit) == 0) {
        ...
        if (debug) {
          streamlog_out(TESTFITTERMESSAGE)
              << "Measured hit in plane " << hitPlane << " at  X = " << pos[0]
              << ", Y = " << pos[1] << ", Q = " << _measuredQ[hitPlane] << endl;
        }
        ...
        if (debug) {
          streamlog_out(TESTFITTERMESSAGE) << "Fitted  hit  in plane "
                                           << hitPlane << " at  X = " << pos[0]
                                           << ", Y = " << pos[1] << endl;
        }
      }
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isMeasured[ipl]) {
        ...
        (dynamic_cast<AIDA::IHistogram2D *>(_aidaHistoMap[tempHistoName]))
            ->fill(_measuredX[ipl], _measuredY[ipl]);
        ...
        (dynamic_cast<AIDA::IProfile1D *>(_aidaHistoMap[tempHistoName]))
            ->fill(_measuredX[ipl], _measuredQ[ipl]);

        stringstream nam6;
        nam6 << _meanSignalYHistoName << "_" << _planeID[ipl];
        tempHistoName = nam6.str();
        (dynamic_cast<AIDA::IProfile1D *>(_aidaHistoMap[tempHistoName]))
            ->fill(_measuredY[ipl], _measuredQ[ipl]);

        stringstream nam7;
        nam7 << _meanSignalXYHistoName << "_" << _planeID[ipl];
        tempHistoName = nam7.str();
        (dynamic_cast<AIDA::IProfile2D *>(_aidaHistoMap[tempHistoName]))
            ->fill(_measuredX[ipl], _measuredY[ipl], _measuredQ[ipl]);

        stringstream nam8;
        nam8 << _ShiftXvsYHistoName << "_" << _planeID[ipl];
        tempHistoName = nam8.str();
        (dynamic_cast<AIDA::IProfile1D *>(_aidaHistoMap[tempHistoName]))
            ->fill(_measuredX[ipl], _measuredY[ipl] - _fittedY[ipl]);

        stringstream nam9;
        nam9 << _ShiftYvsXHistoName << "_" << _planeID[ipl];
        tempHistoName = nam9.str();
        (dynamic_cast<AIDA::IProfile1D *>(_aidaHistoMap[tempHistoName]))
            ->fill(_measuredY[ipl], _measuredX[ipl] - _fittedX[ipl]);
      }
    }
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isFitted[ipl]) {
        ...
        AIDA::IHistogram1D *histo =
            (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[tempHistoName]));
        if (histo)
          histo->fill(_fittedX[ipl]);
        else
          cout << tempHistoName << endl;
        ...
        AIDA::IHistogram2D *histo2D =
            (dynamic_cast<AIDA::IHistogram2D *>(_aidaHistoMap[tempHistoName]));
        if (histo2D)
          histo2D->fill(_fittedX[ipl], _fittedY[ipl]);
        else
          cout << tempHistoName << endl;
      }
    }
    ...
    for (int ipl = 1; ipl < _nTelPlanes - 1; ipl++) {
      if (_isFitted[ipl] && _isFitted[ipl + 1] && _isFitted[ipl - 1]) {
        ...
        if (ipl > 0)
          scatX -= (_fittedX[ipl] - _fittedX[ipl - 1]) /
                   (_planePosition[ipl] - _planePosition[ipl - 1]);
        ...
        if (ipl > 0)
          scatY -= (_fittedY[ipl] - _fittedY[ipl - 1]) /
                   (_planePosition[ipl] - _planePosition[ipl - 1]);
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isMeasured[ipl] && _isFitted[ipl]) {
        ...
        tempHistoName = nam.str();
        (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[tempHistoName]))
            ->fill(_fittedX[ipl] - _measuredX[ipl]);

        stringstream nam2;
        nam2 << _ResidualYHistoName << "_" << _planeID[ipl];
        tempHistoName = nam2.str();
        (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[tempHistoName]))
            ->fill(_fittedY[ipl] - _measuredY[ipl]);
    ...
    if (_isMeasured[_beamID] && _alignCheckHistograms) {
      for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
        if (ipl != _beamID && _isMeasured[ipl]) {
          ...
          tempHistoName = nam.str();
          (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[tempHistoName]))
              ->fill(_measuredX[ipl] - _measuredX[_beamID]);

          stringstream nam1;
          nam1 << _beamShiftYHistoName << "_" << _planeID[ipl];
          tempHistoName = nam1.str();
          (dynamic_cast<AIDA::IHistogram1D *>(_aidaHistoMap[tempHistoName]))
              ->fill(_measuredY[ipl] - _measuredY[_beamID]);
          ...
          (dynamic_cast<AIDA::IHistogram2D *>(_aidaHistoMap[tempHistoName]))
              ->fill(_measuredX[ipl] - _measuredX[_beamID],
                     _measuredY[ipl] - _measuredY[_beamID]);

          stringstream nam3;
          nam3 << _beamRotXHistoName << "_" << _planeID[ipl];
          tempHistoName = nam3.str();
          (dynamic_cast<AIDA::IProfile1D *>(_aidaHistoMap[tempHistoName]))
              ->fill(_measuredY[_beamID],
                     _measuredX[ipl] - _measuredX[_beamID]);

          stringstream nam4;
          nam4 << _beamRotYHistoName << "_" << _planeID[ipl];
          tempHistoName = nam4.str();
          (dynamic_cast<AIDA::IProfile1D *>(_aidaHistoMap[tempHistoName]))
              ->fill(_measuredX[_beamID],
                     _measuredY[ipl] - _measuredY[_beamID]);
          ...
          tempHistoName = nam5.str();
          (dynamic_cast<AIDA::IProfile2D *>(_aidaHistoMap[tempHistoName]))
              ->fill(_measuredX[_beamID], _measuredY[_beamID],
                     _measuredX[ipl] - _measuredX[_beamID]);
    ...
    if (_isMeasured[_referenceID0] && _isMeasured[_referenceID1] &&
        _alignCheckHistograms) {
      for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
        if (ipl != _referenceID0 && ipl != _referenceID1 && _isMeasured[ipl]) {
          string tempHistoName;

          double lineX =
              (_measuredX[_referenceID0] *
                   (_planePosition[_referenceID1] - _planePosition[ipl]) +
               _measuredX[_referenceID1] *
                   (_planePosition[ipl] - _planePosition[_referenceID0])) /
              (_planePosition[_referenceID1] - _planePosition[_referenceID0]);

          double lineY =
              (_measuredY[_referenceID0] *
                   (_planePosition[_referenceID1] - _planePosition[ipl]) +
               _measuredY[_referenceID1] *
                   (_planePosition[ipl] - _planePosition[_referenceID0])) /
              (_planePosition[_referenceID1] - _planePosition[_referenceID0]);
...
void EUTelFitHistograms::bookHistos() {
  ...
  streamlog_out(MESSAGE5) << "Histogram information searched in "
  ...
  //
  {
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl]) {
        ...
        AIDA::IProfile1D *tempHisto =
            AIDAProcessor::histogramFactory(this)->createProfile1D(
                tempHistoName.c_str(), shiftXNBin, shiftXMin, shiftXMax);
  ...
  //
  {
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl]) {
        ...
        AIDA::IProfile1D *tempHisto =
            AIDAProcessor::histogramFactory(this)->createProfile1D(
                tempHistoName.c_str(), shiftYNBin, shiftYMin, shiftYMax);
    ...
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), measXNBin, measXMin, measXMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), measYNBin, measYMin, measYMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
    ...
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram2D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram2D(
              tempHistoName.c_str(), measXNBin, measXMin, measXMax, measYNBin,
              measYMin, measYMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), fitXNBin, fitXMin, fitXMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), fitYNBin, fitYMin, fitYMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram2D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram2D(
              tempHistoName.c_str(), fitXNBin, fitXMin, fitXMax, fitYNBin,
              fitYMin, fitYMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  ...
  for (int ipl = 1; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), angleXNBin, angleXMin, angleXMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 1; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), angleYNBin, angleYMin, angleYMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 1; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram2D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram2D(
              tempHistoName.c_str(), angleXNBin, angleXMin, angleXMax,
              angleYNBin, angleYMin, angleYMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 1; ipl < _nTelPlanes - 1; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), scatXNBin, scatXMin, scatXMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 1; ipl < _nTelPlanes - 1; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), scatYNBin, scatYMin, scatYMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 1; ipl < _nTelPlanes - 1; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram2D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram2D(
              tempHistoName.c_str(), scatXNBin, scatXMin, scatXMax, scatYNBin,
              scatYMin, scatYMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), residXNBin, residXMin, residXMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), residYNBin, residYMin, residYMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram2D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram2D(
              tempHistoName.c_str(), residXNBin, residXMin, residXMax,
              residYNBin, residYMin, residYMax);
      tempHisto->setTitle(tempHistoTitle.c_str());
      _aidaHistoMap.insert(make_pair(tempHistoName, tempHisto));
    }
  }
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IHistogram1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              tempHistoName.c_str(), clusterNBin, clusterMin, clusterMax);
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IProfile1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createProfile1D(
              tempHistoName.c_str(), meanXNBin, meanXMin, meanXMax);
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IProfile1D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createProfile1D(
              tempHistoName.c_str(), meanYNBin, meanYMin, meanYMax);
  ...
  for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
    if (_isActive[ipl]) {
      ...
      AIDA::IProfile2D *tempHisto =
          AIDAProcessor::histogramFactory(this)->createProfile2D(
              tempHistoName.c_str(), meanXNBin, meanXMin, meanXMax, meanYNBin,
              meanYMin, meanYMax);
  ...
  if (_alignCheckHistograms) {
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl] && ipl != _beamID) {
        ...
        tempHistoTitle = shiftXTitle + " for plane " +
                         to_string(_planeID[ipl]) + " w.r.t. " +
                         to_string(_planeID[_beamID]);
        AIDA::IHistogram1D *tempHisto =
            AIDAProcessor::histogramFactory(this)->createHistogram1D(
                tempHistoName.c_str(), shiftXNBin, shiftXMin, shiftXMax);
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl] && ipl != _beamID) {
        ...
        tit << shiftYTitle << " for plane " << _planeID[ipl] << " w.r.t. plane "
            << _planeID[_beamID];
        tempHistoTitle = tit.str();
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl] && ipl != _beamID) {
        ...
        tit << shiftXYTitle << " for plane " << _planeID[ipl]
            << " w.r.t. plane " << _planeID[_beamID];
        tempHistoTitle = tit.str();

        AIDA::IHistogram2D *tempHisto =
            AIDAProcessor::histogramFactory(this)->createHistogram2D(
                tempHistoName.c_str(), shiftXNBin, shiftXMin, shiftXMax,
                shiftYNBin, shiftYMin, shiftYMax);
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl] && ipl != _beamID) {
        ...
        tit << rotXTitle << " for plane " << _planeID[ipl] << " w.r.t. plane "
            << _planeID[_beamID];
        tempHistoTitle = tit.str();

        AIDA::IProfile1D *tempHisto =
            AIDAProcessor::histogramFactory(this)->createProfile1D(
                tempHistoName.c_str(), rotXNBin, rotXMin, rotXMax, rotVMin,
                rotVMax);
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl] && ipl != _beamID) {
        ...
        tit << rotYTitle << " for plane " << _planeID[ipl] << " w.r.t. plane "
            << _planeID[_beamID];
        tempHistoTitle = tit.str();

        AIDA::IProfile1D *tempHisto =
            AIDAProcessor::histogramFactory(this)->createProfile1D(
                tempHistoName.c_str(), rotYNBin, rotYMin, rotYMax, rotVMin,
                rotVMax);
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl] && ipl != _beamID) {
        ...
        AIDA::IHistogram2D *tempHisto =
            AIDAProcessor::histogramFactory(this)->createHistogram2D(
                tempHistoName.c_str(), rotXNBin, rotXMin, rotXMax, rotVNBin,
                rotVMin, rotVMax);
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl] && ipl != _beamID) {
        ...
        AIDA::IHistogram2D *tempHisto =
            AIDAProcessor::histogramFactory(this)->createHistogram2D(
                tempHistoName.c_str(), rotYNBin, rotYMin, rotYMax, rotVNBin,
                rotVMin, rotVMax);
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl] && ipl != _beamID) {
        ...
        AIDA::IProfile2D *tempHisto =
            AIDAProcessor::histogramFactory(this)->createProfile2D(
                tempHistoName.c_str(), rotXNBin, rotXMin, rotXMax, rotYNBin,
                rotYMin, rotYMax, rotVMin, rotVMax);
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl] && ipl != _referenceID0 && ipl != _referenceID1) {
        ...
        tit << shiftXTitle << " for plane " << _planeID[ipl]
            << " w.r.t. planes " << _planeID[_referenceID0] << " and "
            << _planeID[_referenceID1];
        tempHistoTitle = tit.str();
    ...
    for (int ipl = 0; ipl < _nTelPlanes; ipl++) {
      if (_isActive[ipl] && ipl != _referenceID0 && ipl != _referenceID1) {
        ...
        tit << shiftYTitle << " for plane " << _planeID[ipl]
            << " w.r.t. planes " << _planeID[_referenceID0] << " and "
            << _planeID[_referenceID1];
        tempHistoTitle = tit.str();
  ...
  streamlog_out(MESSAGE5) << _aidaHistoMap.size() << " histograms booked"
                          << endl;
