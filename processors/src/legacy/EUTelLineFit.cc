...
EUTelLineFit::EUTelLineFit() : Processor("EUTelLineFit") {
  ...
  _description = "EUTelLineFit will fit a straight line";

  // input collection

  registerInputCollection(LCIO::TRACKERHIT, "HitCollectionName",
                          "Hit collection name", _hitCollectionName,
                          string("hit"));
  ...
  registerOptionalParameter("AlignmentConstantsFourthLayer",
                            "Alignment Constants for fourth Telescope Layer:\n "
                            "off_x, off_y, theta_x, theta_y, theta_z",
                            _alignmentConstantsFourthLayer,
                            constantsFourthLayer);
  registerOptionalParameter("AlignmentConstantsFifthLayer",
                            "Alignment Constants for fifth Telescope Layer:\n "
                            "off_x, off_y, theta_x, theta_y, theta_z",
                            _alignmentConstantsFifthLayer, constantsFifthLayer);
  registerOptionalParameter("AlignmentConstantsSixthLayer",
...
// check if Marlin was built with GEAR support or not
  ...
  message<ERROR5>("You need to install GEAR and recompile Marlin with "
...
void EUTelLineFit::processEvent(LCEvent *event) {
  ...
  try {
    ...
    LCCollectionVec *hitCollection = static_cast<LCCollectionVec *>(
    ...
    for (int iHit = 0; iHit < hitCollection->getNumberOfElements(); iHit++) {
      ...
      TrackerHitImpl *hit =
          static_cast<TrackerHitImpl *>(hitCollection->getElementAt(iHit));
      ...
      if (layerIndex == 0) {
        ...
        _xPos[iHit] =
            (cos(theta_y) * cos(theta_z)) * hit->getPosition()[0] * 1000 +
            ((-1) * sin(theta_x) * sin(theta_y) * cos(theta_z) +
             cos(theta_x) * sin(theta_z)) *
                hit->getPosition()[1] * 1000 +
            off_x;
        _yPos[iHit] = ((-1) * cos(theta_y) * sin(theta_z)) *
                          hit->getPosition()[0] * 1000 +
    ...
    for (counter = 0; counter < _nPlanes; counter++) {
      Sxybar[0] =
          Sxybar[0] +
          Zbar_X[counter] * _xPos[counter] / pow(_intrResolX[counter], 2);
      Sxybar[1] =
          Sxybar[1] +
          Zbar_Y[counter] * _yPos[counter] / pow(_intrResolY[counter], 2);
    }
    ...
    for (counter = 0; counter < _nPlanes; counter++) {
      Sxxbar[0] =
          Sxxbar[0] +
          Zbar_X[counter] * Zbar_X[counter] / pow(_intrResolX[counter], 2);
      Sxxbar[1] =
          Sxxbar[1] +
          Zbar_Y[counter] * Zbar_Y[counter] / pow(_intrResolX[counter], 2);
    }
    ...
    for (counter = 0; counter < _nPlanes; counter++) {
      Chiquare[0] += pow(-_zPos[counter] * A2[0] + _xPos[counter] - Ybar[0] +
                             Xbar[0] * A2[0],
                         2) /
                     pow(_intrResolX[counter], 2);
      Chiquare[1] += pow(-_zPos[counter] * A2[1] + _yPos[counter] - Ybar[1] +
                             Xbar[1] * A2[1],
                         2) /
                     pow(_intrResolY[counter], 2);
    }

    for (counter = 0; counter < _nPlanes; counter++) {
      _waferResidX[counter] =
          (Ybar[0] - Xbar[0] * A2[0] + _zPos[counter] * A2[0]) - _xPos[counter];
      _waferResidY[counter] =
          (Ybar[1] - Xbar[1] * A2[1] + _zPos[counter] * A2[1]) - _yPos[counter];
    }
    ...
    if (_histogramSwitch) {
      ...
      if (AIDA::IHistogram1D *chi2x_histo = dynamic_cast<AIDA::IHistogram1D *>(
              _aidaHistoMap[_chi2XLocalname]))
        chi2x_histo->fill(Chiquare[0]);
      else {
        message<ERROR5>(log() << "Not able to retrieve histogram pointer for "
                              << _chi2XLocalname
    ...
    if (_histogramSwitch) {
      ...
      if (AIDA::IHistogram1D *chi2y_histo = dynamic_cast<AIDA::IHistogram1D *>(
    ...
    if (_histogramSwitch) {
      ...
      if (AIDA::IHistogram1D *anglex_histo = dynamic_cast<AIDA::IHistogram1D *>(
              _aidaHistoMap[_angleXLocalname]))
    ...
    if (_histogramSwitch) {
      ...
      if (AIDA::IHistogram1D *angley_histo = dynamic_cast<AIDA::IHistogram1D *>(
              _aidaHistoMap[_angleYLocalname]))
  ...
  try {
    ...
    AIDA::IHistogram1D *chi2XLocal =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            _chi2XLocalname, NBin, Chi2Min, Chi2Max);
    if (chi2XLocal) {
    ...
    AIDA::IHistogram1D *chi2YLocal =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            _chi2YLocalname, NBin, Chi2Min, Chi2Max);
    if (chi2YLocal) {
    ...
    AIDA::IHistogram1D *angleXLocal =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            _angleXLocalname, NBin, angleMin, angleMax);
    if (angleXLocal) {
    ...
    AIDA::IHistogram1D *angleYLocal =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            _angleYLocalname, NBin, angleMin, angleMax);
    if (angleYLocal) {
    ...
    for (int iDetector = 0; iDetector < _nPlanes; iDetector++) {
      ...
      if (tempXHisto) {
        ...
        message<ERROR5>(log()
                        << "Problem booking the " << (tempHistoName) << ".\n"
    ...
    message<ERROR5>(log() << "No AIDAProcessor initialized. Type q to exit or "
