...
EUTelProcessorSpuriousClusterFinder::EUTelProcessorSpuriousClusterFinder ():Processor ("EUTelProcessorSpuriousClusterFinder"),
...
void
EUTelProcessorSpuriousClusterFinder::init ()
{
...
void
EUTelProcessorSpuriousClusterFinder::processRunHeader (LCRunHeader * rhdr)
{

  std::unique_ptr < EUTelRunHeaderImpl >
    runHeader (new EUTelRunHeaderImpl (rhdr));
...
void
EUTelProcessorSpuriousClusterFinder::readCollections (LCEvent * event)
{

  try
  {

    _trueHitCollectionVec =
      dynamic_cast <
      LCCollectionVec * >(event->getCollection (_trueHitCollectionName));
  ...
  } catch (lcio::DataNotAvailableException & e)
  {
    ...
    streamlog_out (DEBUG4) << "_trueHitCollectionVec: "
      ...
      << " not found in event " << event->getEventNumber () << std::endl;
  }
  ...
  try
  {
    ...
    streamlog_out (DEBUG4) << "_reconstructedHitCollectionVec: " <<
      _reconstructedHitCollectionName.c_str () << "found\n";
  } catch (lcio::DataNotAvailableException & e)
  {
...
void
EUTelProcessorSpuriousClusterFinder::processEvent (LCEvent * event)
{
...
void
EUTelProcessorSpuriousClusterFinder::check (LCEvent *)
{
}

void
EUTelProcessorSpuriousClusterFinder::end ()
{
...
int
EUTelProcessorSpuriousClusterFinder::findPairIndex (double const *a,
						    std::vector <
						    double const *>vec)
{
...
void
EUTelProcessorSpuriousClusterFinder::fillHistos (LCEvent * event)
{
  ...
  try
  {
    ...
    LCCollectionVec *_reconstructedHitCollectionVec =
      ...
      LCCollectionVec *
      >(event->getCollection (_reconstructedHitCollectionName));
    CellIDDecoder < TrackerHitImpl >
      reconstructedHitDecoder (_reconstructedHitCollectionVec);
    CellIDDecoder < TrackerDataImpl >
      rawDataDecoder ("sensorID:7,sparsePixelType:5");
	...
	trueHitMap.
	  insert (std::
		  make_pair (_sensorIDVec[i],
			     std::vector < double const *>()));
	trueHitEDepMap.
	  insert (std::make_pair (_sensorIDVec[i], std::vector < double >()));
      }
	...
	TrackerHitImpl *trueHit =
	  dynamic_cast <
	  TrackerHitImpl * >(_trueHitCollectionVec->getElementAt (i));
	int detectorID =
	  static_cast < int >(trueHitDecoder (trueHit)["sensorID"]);
    ...
    for (int i = 0;
	 i < _reconstructedHitCollectionVec->getNumberOfElements (); i++)
      {

	TrackerHitImpl *reconstructedHit =
	  dynamic_cast <
	  TrackerHitImpl *
	  >(_reconstructedHitCollectionVec->getElementAt (i));
	int detectorID =
	  static_cast <
	  int >(reconstructedHitDecoder (reconstructedHit)["sensorID"]);
	...
	TrackerDataImpl *zsData =
	  dynamic_cast <
	  TrackerDataImpl * >(reconstructedHit->getRawHits ().front ());
	...
	for (size_t j = pixelEntries; j < chargeValues.size ();
	     j += pixelEntries)
	  {
	    ...
	    // stored there
	    if (std::
		find (XHitPixels.begin (), XHitPixels.end (),
		      chargeValues[j]) == XHitPixels.end ())
	...
	int pairIndex = findPairIndex (hitPos,
	...
	if ((clusterSizeX == 2) && (clusterSizeY == 3))
	  {			// potentially a spurious cluster
		...
		shape.push_back (static_cast <
				 int >(chargeValues[j] - chargeValues[0]));
		shape.push_back (static_cast <
				 int >(chargeValues[j + 1] -
				       chargeValues[1]));
	      }
		...
		_2DHistos[1].at (detectorID)->fill (hitPos[0], hitPos[1],
						    1.0);
		_2DHistos[2].at (detectorID)->fill (pair[0], pair[1], 1.0);
		_2DHistos[9].at (detectorID)->fill (diff_x, diff_y + 9.2,
						    1.0);
		_2DHistos[12].at (detectorID)->fill (diff_x, diff_y + 9.2,
						     1.0);
		for (size_t j = 0; j < shape.size (); j += 2)
		  {

		    _2DHistos[3].at (detectorID)->fill (shape[j],
							shape[j + 1], 1.0);
		  }
	      }
		...
		_2DHistos[6].at (detectorID)->fill (hitPos[0], hitPos[1],
						    1.0);
		_2DHistos[7].at (detectorID)->fill (pair[0], pair[1], 1.0);
		for (size_t j = 0; j < shape.size (); j += 2)
		  {

		    _2DHistos[8].at (detectorID)->fill (shape[j],
							shape[j + 1], 1.0);
		  }
	      }
		...
		_2DHistos[4].at (detectorID)->fill (shape[j], shape[j + 1],
						    1.0);
	      }
	  }
		...
		for (size_t j = 0; j < shape.size (); j += 2)
		  {
		    ...
		    _2DHistos[11].at (detectorID)->fill (shape[j],
							 shape[j + 1], 1.0);
		  }
	      }
	    else
		...
		for (size_t j = 0; j < shape.size (); j += 2)
		  {
		    ...
		    _2DHistos[14].at (detectorID)->fill (shape[j],
							 shape[j + 1], 1.0);
		  }
	      }
	  }

	trueHitMap.at (detectorID).erase (trueHitMap.at (detectorID).
					  begin () + pairIndex);
      }
  }
  catch (lcio::DataNotAvailableException & e)
...
void
EUTelProcessorSpuriousClusterFinder::bookHistos ()
{
  ...
  for (size_t i = 0; i < _sensorIDVec.size (); i++)
	...
	insert (std::
		make_pair (sensorID,
			   ...
			   createHistogram1D ((basePath + histoName).c_str (),
					      histo1DNBin, xPosDiffMin,
					      xPosDiffMax)));
	...
	insert (std::
      ...
      histoTitle =
	"x position of reconstructed hits for spurious clusters (x "
	...
	insert (std::
      ...
      histoTitle =
	"y position of reconstructed hits for spurious clusters (x "
	"cluster size 2, y cluster size 3);#Deltay /#mum;Entries";
      _1DHistos[3].
	insert (std::
		make_pair (sensorID,
      ...
      histoTitle =
	"x position of true simulated hits for spurious clusters (x "
	"cluster size 2, y cluster size 3);#Deltay /#mum;Entries";
      _1DHistos[4].
	insert (std::
		make_pair (sensorID,
      ...
      histoTitle =
	"y position for true simulated hits for spurious clusters (x "
	"cluster size 2, y cluster size 3);#Deltay /#mum;Entries";
      _1DHistos[5].
	insert (std::
		make_pair (sensorID,
      ...
      histoTitle =
	"total deposition energy for true simulated hits for spurious "
	...
	insert (std::
      ...
      // set up reconstructed hit x position histogram
      histoName =
	"xReconstructedHitPosition_NonSpurious_d" + to_string (sensorID);
      histoTitle =
	"x position of reconstructed hits for non spurious "
	...
	insert (std::
      ...
      // set up reconstructed hit y position histogram
      histoName =
	"yReconstrutedHitPosition_NonSpurious_d" + to_string (sensorID);
      histoTitle =
	"y position of reconstructed hits for non spurious "
	"clusters;#Deltay /#mum;Entries";
      _1DHistos[10].
	insert (std::
		make_pair (sensorID,
	...
	"y cluster size 3);#Deltax /#mum;#Deltay /#mum;Entries";
      _2DHistos[0].insert (std::make_pair (sensorID,
					   AIDAProcessor::
					   histogramFactory (this)->
					   createHistogram2D ((basePath +
							       histoName).
							      c_str (),
							      histo2DXNBin,
							      xPosDiff2DMin,
	...
	"#mum;#Deltax /#mum;#Deltay /#mum;Entries";
      _2DHistos[9].insert (std::make_pair (sensorID,
					   AIDAProcessor::
					   histogramFactory (this)->
					   createHistogram2D ((basePath +
							       histoName).
	...
	"/#mum;Entries";
      _2DHistos[1].insert (std::make_pair (sensorID,
					   AIDAProcessor::
					   histogramFactory (this)->
					   createHistogram2D ((basePath +
							       histoName).
	...
	"cluster size 2, y cluster size 3);#Deltax /#mum;#Deltay "
	"/#mum;Entries";
      _2DHistos[2].insert (std::make_pair (sensorID,
					   AIDAProcessor::
					   histogramFactory (this)->
					   createHistogram2D ((basePath +
							       histoName).
      ...
      histoTitle =
	"shape of clusters for spurious clusters (x cluster size 2, y "
	...
	insert (std::
		make_pair (sensorID,
			   AIDAProcessor::histogramFactory (this)->
			   createHistogram2D ((basePath + histoName).c_str (),
					      shapeXNBin, shapeXMin,
					      shapeXMax, shapeYNBin,
					      shapeYMin, shapeYMax)));
      ...
      histoTitle =
	"shape of all clusters of x cluster size 2 and y cluster size "
	...
	"reconstructed hits for non spurious clusters;#Deltax "
	"/#mum;#Deltay /#mum;Entries";
      _2DHistos[5].insert (std::make_pair (sensorID,
					   AIDAProcessor::
					   histogramFactory (this)->
					   createHistogram2D ((basePath +
							       histoName).
	...
	"clusters;#Deltax /#mum;#Deltay /#mum;Entries";
      _2DHistos[6].insert (std::make_pair (sensorID,
					   AIDAProcessor::
					   histogramFactory (this)->
					   createHistogram2D ((basePath +
							       histoName).
	...
	"clusters;#Deltax /#mum;#Deltay /#mum;Entries";
      _2DHistos[7].insert (std::make_pair (sensorID,
					   AIDAProcessor::
					   histogramFactory (this)->
					   createHistogram2D ((basePath +
							       histoName).
	...
	"2x2;#Deltax /#mum;#Deltay /#mum;Entries";
      _2DHistos[13].insert (std::make_pair (sensorID,
					    AIDAProcessor::
					    histogramFactory (this)->
					    createHistogram2D ((basePath +
								histoName).
      ...
      histoTitle =
	"shape of clusters for x cluster size 2 y cluster size 2 non "
	...
	"/#mum;Entries";
      _2DHistos[10].insert (std::make_pair (sensorID,
					    AIDAProcessor::
					    histogramFactory (this)->
					    createHistogram2D ((basePath +
								histoName).
	...
	insert (std::
      ...
      // set up position difference histogram
      histoName =
	"2DPositionDifference_2x2+spurious_d" + to_string (sensorID);
      histoTitle =
	"difference in position between true simulated hits and "
	"reconstructed hits for 2x2 and spurious clusters;#Deltax "
	"/#mum;#Deltay /#mum;Entries";
      _2DHistos[12].
	insert (std::
		make_pair (sensorID,
			   AIDAProcessor::histogramFactory (this)->
			   createHistogram2D ((basePath + histoName).c_str (),
					      histo2DXNBin, xPosDiff2DMin,
					      xPosDiff2DMax, histo2DYNBin,
					      yPosDiff2DMin, yPosDiff2DMax)));
