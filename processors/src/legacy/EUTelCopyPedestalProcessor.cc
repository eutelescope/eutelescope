...
EUTelCopyPedestalProcessor::EUTelCopyPedestalProcessor ():Processor ("EUTelCopyPedestalProcessor"), _pedestalCollectionVec (nullptr),
_noiseCollectionVec (nullptr),
_statusCollectionVec (nullptr)
{

  // modify processor description
  _description = "EUTelCopyPedestalProcessor copies the condition data into "
    "local writable collections";

  // first of all we need to register the input collection
  registerInputCollection (LCIO::TRACKERDATA, "PedestalConditionName",
			   "Pedestal input condition", _pedestalConditionName,
			   string ("pedestalDB"));

  registerInputCollection (LCIO::TRACKERDATA, "NoiseConditionName",
			   "Noise input condition", _noiseConditionName,
			   string ("noiseDB"));

  registerInputCollection (LCIO::TRACKERRAWDATA, "StatusConditionName",
			   "Status input condition", _statusConditionName,
			   string ("statusDB"));
...
void
EUTelCopyPedestalProcessor::init ()
{
...
void
EUTelCopyPedestalProcessor::processRunHeader (LCRunHeader * rdr)
{
...
void
EUTelCopyPedestalProcessor::processEvent (LCEvent * event)
{
  ...
  if (isFirstEvent ())
    {
      ...
      LCCollectionVec *pedestalCollectionVecDB =
	dynamic_cast <
	LCCollectionVec * >(evt->getCollection (_pedestalConditionName));
      LCCollectionVec *noiseCollectionVecDB =
	dynamic_cast <
	LCCollectionVec * >(evt->getCollection (_noiseConditionName));
      LCCollectionVec *statusCollectionVecDB =
	dynamic_cast <
	LCCollectionVec * >(evt->getCollection (_statusConditionName));
	...
	pedeDecoder (EUTELESCOPE::MATRIXDEFAULTENCODING,
		     _pedestalCollectionVec);
      CellIDEncoder < TrackerDataImpl >
	noiseDecoder (EUTELESCOPE::MATRIXDEFAULTENCODING,
		      _noiseCollectionVec);
      CellIDEncoder < TrackerRawDataImpl >
	statusDecoder (EUTELESCOPE::MATRIXDEFAULTENCODING,
		       _statusCollectionVec);

      for (int iDetector = 0;
	   iDetector < pedestalCollectionVecDB->getNumberOfElements ();
	   iDetector++)
	{

	  TrackerRawDataImpl *statusDB =
	    dynamic_cast <
	    TrackerRawDataImpl *
	    >(statusCollectionVecDB->getElementAt (iDetector));
	  ...
	  TrackerDataImpl *pedestalDB =
	    dynamic_cast <
	    TrackerDataImpl *
	    >(pedestalCollectionVecDB->getElementAt (iDetector));
	  ...
	  TrackerDataImpl *noiseDB =
	    dynamic_cast <
	    TrackerDataImpl *
	    >(noiseCollectionVecDB->getElementAt (iDetector));
...
void
EUTelCopyPedestalProcessor::end ()
{
