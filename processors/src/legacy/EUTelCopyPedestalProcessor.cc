...
void EUTelCopyPedestalProcessor::processEvent(LCEvent *event) {
  ...
  if (isFirstEvent()) {
    ...
    LCCollectionVec *pedestalCollectionVecDB = dynamic_cast<LCCollectionVec *>(
        evt->getCollection(_pedestalConditionName));
    LCCollectionVec *noiseCollectionVecDB = dynamic_cast<LCCollectionVec *>(
        evt->getCollection(_noiseConditionName));
    LCCollectionVec *statusCollectionVecDB = dynamic_cast<LCCollectionVec *>(
        evt->getCollection(_statusConditionName));
    ...
    CellIDEncoder<TrackerDataImpl> pedeDecoder(
        EUTELESCOPE::MATRIXDEFAULTENCODING, _pedestalCollectionVec);
    CellIDEncoder<TrackerDataImpl> noiseDecoder(
        EUTELESCOPE::MATRIXDEFAULTENCODING, _noiseCollectionVec);
    CellIDEncoder<TrackerRawDataImpl> statusDecoder(
        EUTELESCOPE::MATRIXDEFAULTENCODING, _statusCollectionVec);

    for (int iDetector = 0;
         iDetector < pedestalCollectionVecDB->getNumberOfElements();
         iDetector++) {

      TrackerRawDataImpl *statusDB = dynamic_cast<TrackerRawDataImpl *>(
      ...
      TrackerDataImpl *pedestalDB = dynamic_cast<TrackerDataImpl *>(
      ...
      TrackerDataImpl *noiseDB = dynamic_cast<TrackerDataImpl *>(
