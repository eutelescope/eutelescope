...
std::string EUTelHistogramMaker::_eventMultiplicityHistoName =
  "eventMultiplicity";
std::string EUTelHistogramMaker::_clusterNumberOfHitPixelName =
  "numberofhitpixel";
#endif
...
EUTelHistogramMaker::EUTelHistogramMaker ():Processor ("EUTelHistogramMaker")
{
  ...
  _description = "EUTelHistogramMaker fills reference and control histograms";
  ...
  // LCIO::TRACKERDATA
  registerInputCollections (LCIO::TRACKERDATA, "NoiseCollectionName",
			    "The name of the noise collections",
			    _noiseCollectionName,
			    NoiseCollectionNameVecExample);

  registerInputCollections (LCIO::TRACKERRAWDATA, "StatusCollectionName",
			    "The name of the status collections.",
			    _statusCollectionName,
			    StatusCollectionNameVecExample);
...
void
EUTelHistogramMaker::processRunHeader (LCRunHeader * rdr)
{
...
void
EUTelHistogramMaker::initializeGeometry (LCEvent * event)
{
  ...
  try
  {
	...
	LCCollectionVec *collection =
	  dynamic_cast <
	  LCCollectionVec * >(event->getCollection (_noiseCollectionName[i]));
	_noOfDetector += collection->size ();
	CellIDDecoder < TrackerDataImpl > decoder (collection);
	for (size_t iDetector = 0; iDetector < collection->size ();
	     ++iDetector)
	  {
	    TrackerDataImpl *data =
	      dynamic_cast <
	      TrackerDataImpl * >(collection->getElementAt (iDetector));
...
void
EUTelHistogramMaker::processEvent (LCEvent * evt)
{
  ...
  std::vector < LCCollectionVec * >noiseCollectionVec;	// noiseCollectionVec =
  // 0x0, *
  // statusCollectionVec =
  // 0x0;
  ...
  if (_noiseHistoSwitch)
      ...
      for (size_t i = 0; i < _noiseCollectionName.size (); ++i)
	{
	  try
	  {
	    noiseCollectionVec.push_back (dynamic_cast <
					  LCCollectionVec *
					  >(evt->
					    getCollection
					    (_noiseCollectionName[i])));
	  }
	  ...
	  try
	  {
	    statusCollectionVec.push_back (dynamic_cast <
					   LCCollectionVec *
					   >(evt->
					     getCollection
					     (_statusCollectionName[i])));
	  }
	  catch (lcio::DataNotAvailableException & e)
	  {
	    streamlog_out (ERROR1)
  ...
  try
  {
	...
	string tempHistoName =
	  _clusterSignalHistoName + "_d" + to_string (detectorID);
	(dynamic_cast <
	 AIDA::IHistogram1D *
	 >(_aidaHistoMap[tempHistoName]))->fill (cluster->getTotalCharge ());

	if (type == kEUTelDFFClusterImpl)
	  {
	    tempHistoName =
	      _clusterNumberOfHitPixelName + "_d" + to_string (detectorID);
	    (dynamic_cast <
	     AIDA::IHistogram1D *
	     >(_aidaHistoMap[tempHistoName]))->fill (cluster->
						     getTotalCharge ());
	  }
	...
	(dynamic_cast <
	 AIDA::IHistogram1D *
	 >(_aidaHistoMap[tempHistoName]))->fill (cluster->getSeedCharge ());

	vector < int >::iterator iter = _clusterSpectraNVector.begin ();
	while (iter != _clusterSpectraNVector.end ())
	  {
	    tempHistoName =
	      _clusterSignalHistoName + to_string (*iter) + "_d" +
	      to_string (detectorID);
	    (dynamic_cast <
	     AIDA::IHistogram1D *
	     >(_aidaHistoMap[tempHistoName]))->fill (cluster->
						     getClusterCharge ((*iter)));
	    ++iter;
	  }
	...
	while (iter != _clusterSpectraNxNVector.end ())
	  {
	    tempHistoName =
	      _clusterSignalHistoName + to_string (*iter) + "x" +
	      to_string (*iter) + "_d" + to_string (detectorID);
	    (dynamic_cast <
	     AIDA::IHistogram1D *
	     >(_aidaHistoMap[tempHistoName]))->fill (cluster->
						     getClusterCharge ((*iter), (*iter)));
	    ++iter;
	  }
	...
	(dynamic_cast <
	 AIDA::IHistogram2D *
	 >(_aidaHistoMap[tempHistoName]))->fill (static_cast <
						 double >(xSeed),
						 static_cast <
						 double >(ySeed), 1.);
	...
	if (_noiseHistoSwitch)
	  {
		...
		CellIDDecoder < TrackerDataImpl >
		  noiseDecoder (noiseCollectionVec[i]);

		// get the noise TrackerDataImpl corresponding to the detector
		// under analysis and the status matrix as well

		if (noiseCollectionVec[i]->getNumberOfElements () >
		    _ancillaryMap[detectorID])
		  {
		    noiseMatrix =
		      dynamic_cast <
		      TrackerDataImpl *
		      >(noiseCollectionVec[i]->
			getElementAt (_ancillaryMap[detectorID]));
		    statusMatrix =
		      dynamic_cast <
		      TrackerRawDataImpl *
		      >(statusCollectionVec[i]->
			getElementAt (_ancillaryMap[detectorID]));

		    CellIDDecoder < TrackerDataImpl >
		      noiseDecoder2 (noiseCollectionVec[i]);
	    ...
	    if (!found)
	      {
		cout << "no noise and status maps were found for sensor "
		  << detectorID << endl;
		exit (-1);
	      }
		...
		<< "Continuing without filling the histograms" << endl;
	      _noiseHistoSwitch = false;
	    }
	  }
	...
	if (_noiseHistoSwitch)
	  {
	    ...
	    tempHistoName =
	      _clusterNoiseHistoName + "_d" + to_string (detectorID);
	    histo =
	      dynamic_cast <
	    ...
	    tempHistoName =
	      _clusterSNRHistoName + "_d" + to_string (detectorID);
	    histo =
	      dynamic_cast <
	      AIDA::IHistogram1D * >(_aidaHistoMap[tempHistoName]);
	    if (histo)
	    ...
	    while (iter != _clusterSpectraNxNVector.end ())
	      {
		tempHistoName =
		  _clusterSNRHistoName + to_string (*iter) + "x" +
		  to_string (*iter) + "_d" + to_string (detectorID);
		histo =
		  dynamic_cast <
		  AIDA::IHistogram1D * >(_aidaHistoMap[tempHistoName]);
	    ...
	    for (unsigned int i = 0; i < snrs.size (); i++)
	      {
		tempHistoName = _clusterSNRHistoName +
		  to_string (_clusterSpectraNVector[i]) + "_d" +
		  to_string (detectorID);
		histo =
		  dynamic_cast <
		  AIDA::IHistogram1D * >(_aidaHistoMap[tempHistoName]);
    ...
    for (int iDetector = 0; iDetector < _noOfDetector; iDetector++)
      {
	tempHistoName = _eventMultiplicityHistoName + "_d" +
	  to_string (_sensorIDVec[iDetector]);
	AIDA::IHistogram1D * histo =
  ...
  catch (DataNotAvailableException & e)
  {
    streamlog_out (WARNING2) << "No input collection found on event "
      << evt->getEventNumber () << " in run " << evt->getRunNumber () << endl;
    return;
  }
#endif
}
...
void
EUTelHistogramMaker::check (LCEvent * /*  evt */ )
{
...
void
EUTelHistogramMaker::bookHistos ()
{
  ...
  } catch (ios::failure & e)
  {
    streamlog_out (ERROR1) << "I/O problem with " << _histoInfoFileName <<
      endl << "Continuing without histogram manager" << endl;
  ...
  for (int iDetector = 0; iDetector < _noOfDetector; iDetector++)
    {
      ...
      tempHistoName =
	_clusterSignalHistoName + "_d" +
	to_string (_sensorIDVec.at (iDetector));
	...
	AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath +
      ...
      tempHistoName = _clusterNumberOfHitPixelName + "_d" +
	to_string (_sensorIDVec.at (iDetector));
	...
	{
	  histoInfo =
	    histoMgr->getHistogramInfo (_clusterNumberOfHitPixelName);
	  if (histoInfo)
	    {
	      streamlog_out (DEBUG1) << (*histoInfo) << endl;
	      clusterNBin = histoInfo->_xBin;
      ...
      while (iter != _clusterSpectraNVector.end ())
	{
	  ...
	  _aidaHistoMap.
	    insert (make_pair (tempHistoName, clusterSignalNHisto));
	  string tempTitle =
	  ...
	  if (_noiseHistoSwitch)
		...
		" most significant pixels";
	      _aidaHistoMap.
		insert (make_pair (tempHistoName, clusterSNRNHisto));
      ...
      while (iter != _clusterSpectraNxNVector.end ())
	{
	  ...
	  AIDA::IHistogram1D * clusterSignalNxNHisto =
	    AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath + tempHistoName).c_str (), clusterNBin, clusterMin, clusterMax);
	  _aidaHistoMap.
	    insert (make_pair (tempHistoName, clusterSignalNxNHisto));
	  string tempTitle =
	  ...
	  if (_noiseHistoSwitch)
		...
		AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath + tempHistoName).c_str (), clusterSNRNBin, clusterSNRMin, clusterSNRMax);
	      _aidaHistoMap.
		insert (make_pair (tempHistoName, clusterSNRNxNHisto));
	      tempTitle =
      ...
      tempHistoName =
	_seedSignalHistoName + "_d" + to_string (_sensorIDVec.at (iDetector));
      ...
      if (_noiseHistoSwitch)
	{
	  ...
	  AIDA::IHistogram1D * seedSNRHisto =
	    AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath + tempHistoName).c_str (), seedSNRNBin, seedSNRMin, seedSNRMax);
	  _aidaHistoMap.insert (make_pair (tempHistoName, seedSNRHisto));
	  seedSNRHisto->setTitle (seedSNRTitle.c_str ());

	  tempHistoName =
	    _clusterNoiseHistoName + "_d" +
	    to_string (_sensorIDVec.at (iDetector));
	  ...
	  AIDA::IHistogram1D * clusterNoiseHisto =
	    AIDAProcessor::histogramFactory (this)->createHistogram1D ((basePath + tempHistoName).c_str (), clusterNoiseNBin, clusterNoiseMin, clusterNoiseMax);
	  _aidaHistoMap.insert (make_pair (tempHistoName, clusterNoiseHisto));
	  clusterNoiseHisto->setTitle (clusterNoiseTitle.c_str ());
	}

      tempHistoName =
	_hitMapHistoName + "_d" + to_string (_sensorIDVec.at (iDetector));
      int xBin = _maxX[_sensorIDVec.at (iDetector)] -
	_minX[_sensorIDVec.at (iDetector)] + 1;
      double xMin =
	static_cast < double >(_minX[_sensorIDVec.at (iDetector)]) - 0.5;
      double xMax =
	static_cast < double >(_maxX[_sensorIDVec.at (iDetector)]) + 0.5;
      int yBin =
	_maxY[_sensorIDVec.at (iDetector)] -
	_minY[_sensorIDVec.at (iDetector)] + 1;
      double yMin =
	static_cast < double >(_minY[_sensorIDVec.at (iDetector)]) - 0.5;
      double yMax =
	static_cast < double >(_maxY[_sensorIDVec.at (iDetector)]) + 0.5;
      AIDA::IHistogram2D * hitMapHisto =
	AIDAProcessor::histogramFactory (this)->
	createHistogram2D ((basePath + tempHistoName).c_str (), xBin, xMin,
			   xMax, yBin, yMin, yMax);
