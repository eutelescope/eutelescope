...
void EUTelProcessorGeometricClustering::geometricClustering(
  ...
  for (unsigned int idetector = 0;
    ...
    for (auto &pixelRef : *sparseData) {
      ...
      EUTelGeometricPixel hitPixel(
          dynamic_cast<EUTelGenericSparsePixel const &>(pixel));

      // And get the path to the given pixel
      std::string pixelPath =
          geoDescr->getPixName(hitPixel.getXCoord(), hitPixel.getYCoord());
    ...
    while (!hitPixelVec.empty()) {
      ...
      std::unique_ptr<EUTelGenericSparseClusterImpl<EUTelGeometricPixel>>
          sparseCluster = std::make_unique<
              EUTelGenericSparseClusterImpl<EUTelGeometricPixel>>(
              zsCluster.get());
      ...
      while (!newlyAdded.empty()) {
        bool newlyDone = true;
        float x1, x2, y1, y2, dX, dY, cx1, cy1, cx2, cy2, cutX, cutY, t1, t2,
            dT;
        ...
        for (std::vector<EUTelGeometricPixel>::iterator hitVec =
          ...
          dT = t1 - t2;
          cutX = (cx1 + cx2) *
                 1.01; // this additional 1% is accounting for precision
  ...
  try {
    ...
    for (int iPulse = _initialPulseCollectionSize;
      ...
      if (type == kEUTelGenericSparseClusterImpl) {
        cluster = new EUTelGeometricClusterImpl(
      ...
      (dynamic_cast<AIDA::IHistogram2D *>(_hitMapGeomHistos[detectorID]))
...
void EUTelProcessorGeometricClustering::bookHistos() {
  ...
  for (size_t iDetector = 0; iDetector < _sensorIDVec.size(); iDetector++) {
    ...
    std::string clusterTitle = "Total Signal per Cluster (in Detector specific "
    ...
    std::string clusterTotTitle = "Total size of Cluster (in Hit Pixels);No. "
    ...
    _clusterSizeTotalHistos.insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + tempHistoName).c_str(), clusterTotBin,
                      clusterTotMin, clusterTotMax)));
    ...
    _clusterSignalHistos.insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
    ...
    _clusterSizeXHistos.insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
    ...
    _clusterSizeYHistos.insert(std::make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
    ...
    hitMapHisto->setTitle(
        "Pixel Index Hit Map;X Index [#];Y Index [#];Count [#]");
    ...
    AIDA::IHistogram2D *hitMapGeomHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + tempHistoName).c_str(), xGeomBin, xGeomMin, xGeomMax,
            yGeomBin, yGeomMin, yGeomMax);
    _hitMapGeomHistos.insert(std::make_pair(sensorID, hitMapGeomHisto));
    hitMapGeomHisto->setTitle(
