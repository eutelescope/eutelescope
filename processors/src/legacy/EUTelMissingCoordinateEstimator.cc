...
_missingHitPos (0),
_knownHitPos (0),
_nDutHits (0), _nDutHitsCreated (0), _maxExpectedCreatedHitPerDUTHit (10)
{
  ...
  registerProcessorParameter ("ReferencePlanes",
  ...
  registerProcessorParameter ("DUTPlanes",
  ...
  registerProcessorParameter ("MaxResidual",
			      ...
			      _maxResidual, float (10.0));

  registerProcessorParameter ("MultiHitMode",
...
void
EUTelMissingCoordinateEstimator::init ()
{
  ...
  if (_missingCoordinate == string ("X")
      || _missingCoordinate == string ("Y"))
    {
      streamlog_out (MESSAGE4) << "MissingCoordinate value set as: " <<
	_missingCoordinate << std::endl;
    }
  else
    {
      streamlog_out (ERROR5) << "MissingCoordinate value:" <<
	_missingCoordinate << " is not valid!" << std::endl;
      exit (-1);
    }
...
void
EUTelMissingCoordinateEstimator::processRunHeader (LCRunHeader * rdr)
{
...
void
EUTelMissingCoordinateEstimator::processEvent (LCEvent * event)
{
  ...
  {
    streamlog_out (DEBUG5) << "No input collection " <<
      _inputHitCollectionName << " found on event " << event->
  ...
  for (int i = 0; i < inputHitCollection->getNumberOfElements (); i++)
	...
	inputCellIDDecoder (EUTELESCOPE::HITENCODING);

      TrackerHitImpl *inputHit =
	dynamic_cast <
	TrackerHitImpl * >(inputHitCollection->getElementAt (i));
  ...
  if (_multihitmode == true)
      ...
      for (unsigned int k = 0; k < dutPlaneHits.size (); k++)
	{
	  TrackerHitImpl *dutHit =
	    dynamic_cast <
	    TrackerHitImpl *
	    >(inputHitCollection->getElementAt (dutPlaneHits[k]));
	  ...
	  for (unsigned int i = 0; i < referencePlaneHits1.size (); i++)
	    {
	      TrackerHitImpl *refHit1 =
		dynamic_cast <
		TrackerHitImpl *
		>(inputHitCollection->getElementAt (referencePlaneHits1[i]));
		...
		{
		  TrackerHitImpl *refHit2 =
		    dynamic_cast <
		    TrackerHitImpl *
		    >(inputHitCollection->
		      getElementAt (referencePlaneHits2[j]));
		  const double *refHit2Pos = refHit2->getPosition ();

		  // t = (z-z1)/(z2-z1)
		  double t =
		    (dutHitPos[2] - refHit1Pos[2]) / (refHit2Pos[2] -
						      refHit1Pos[2]);

		  // find the known coordinate value that correcponds to that z on the line
		  double knownHitPosOnLine =
		    refHit1Pos[_knownHitPos] + (refHit2Pos[_knownHitPos] -
						refHit1Pos[_knownHitPos]) * t;

		  pointhitmaphisto->
		    fill ((refHit1Pos[0] +
			   (refHit2Pos[0] - refHit1Pos[0]) * t),
			  (refHit1Pos[1] +
			   (refHit2Pos[1] - refHit1Pos[1]) * t));

		  // if knownHitPosOnLine is close to the actual DUT hit position
		  if (fabs (knownHitPosOnLine - dutHitPos[_knownHitPos]) <
		      _maxResidual)
		    {
			...
			(refHit2Pos[_missingHitPos] -
			 refHit1Pos[_missingHitPos]) * t;
		      ...
		      streamlog_out (DEBUG0) << "New hit: x: " <<
			newDutHitPos[0] << ", y: " << newDutHitPos[1] <<
			", z: " << newDutHitPos[2] << endl;
		    ...
		    {
		      streamlog_out (DEBUG0) << "Failing residual cut!" <<
			endl;
		      failhitmaphisto->fill (dutHitPos[0], dutHitPos[1]);
		      faildistancehisto->fill (knownHitPosOnLine -
					       dutHitPos[_knownHitPos]);
		      _nResidualFailCount++;
		    }
	      ...
	      for (unsigned int j = 0; j < referencePlaneHits2.size (); j++)
		{
		  ...
		  if (fabs (knownHitPosOnLine - dutHitPos[_knownHitPos]) <
		      closestresidual)
		    {
		      foundhit = true;
		      closestresidual =
			knownHitPosOnLine - dutHitPos[_knownHitPos];
		      // first copy old DUT hit position to the new one
		      newDutHitPos[0] = dutHitPos[0];
		      newDutHitPos[1] = dutHitPos[1];
		      newDutHitPos[2] = dutHitPos[2];
	  ...
	  if (foundhit == true)
	    {
	      ...
	      newHit->setPosition (&hitpos[0]);
	      outputHitCollection->push_back (newHit);
	      streamlog_out (DEBUG0) << "New hit: x: " << newDutHitPos[0] <<
		", y: " << newDutHitPos[1] << ", z: " << newDutHitPos[2] <<
		endl;
	      hitmaphisto->fill (newDutHitPos[0], newDutHitPos[1]);
  ...
  if (referencePlaneHits1.size () == 0 || referencePlaneHits2.size () == 0)
	...
	"Couldn't create hit, no input in reference plane 1 or 2!" << endl;
      _nNoReferenceCount++;
    }
...
void
EUTelMissingCoordinateEstimator::end ()
{

  streamlog_out (MESSAGE4) << "Number of input DUT hits:   " << _nDutHits <<
    endl;
  streamlog_out (MESSAGE4) << "Number of DUT hits created: " <<
    _nDutHitsCreated << endl;
  streamlog_out (MESSAGE4) << " " << endl;
  streamlog_out (MESSAGE4) << "Number of DUT hits failing the residual cut : "
    << _nResidualFailCount << endl;
  streamlog_out (MESSAGE4) << "Number of DUT hits with no reference hits : "
    << _nNoReferenceCount << endl;
  for (unsigned int i = 0; i < 10; i++)
    {
      streamlog_out (MESSAGE4) << "You created " << i << " hits per DUT hit "
	<< _numberOfCreatedHitsPerDUTHit[i] << " times" << endl;
    }
...
void
EUTelMissingCoordinateEstimator::bookHistos ()
{

  hitmaphisto =
    AIDAProcessor::histogramFactory (this)->createHistogram2D ("hitmap", 1000,
							       -20, 20, 1000,
							       -20, 20);
  hitmaphisto->setTitle ("Created Hitmap;X [mm];Y [mm]");

  failhitmaphisto =
    AIDAProcessor::histogramFactory (this)->createHistogram2D ("failhitmap",
							       1000, -20, 20,
							       1000, -20, 20);
  failhitmaphisto->setTitle ("Hitmap where no hit was created;X [mm];Y [mm]");

  faildistancehisto =
    AIDAProcessor::histogramFactory (this)->createHistogram1D ("faildistance",
							       1000, -50, 50);
  faildistancehisto->
    setTitle
    ("Residual distance where no hit was created;Distance [mm];Entries");

  pointhitmaphisto =
    AIDAProcessor::histogramFactory (this)->createHistogram2D ("pointhitmap",
							       1000, -20, 20,
							       1000, -20, 20);
