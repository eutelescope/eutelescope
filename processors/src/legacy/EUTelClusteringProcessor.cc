...
EUTelClusteringProcessor::EUTelClusteringProcessor()
    : Processor("EUTelClusteringProcessor"), _nzsDataCollectionName(""),
  ...
  registerInputCollection(
      LCIO::TRACKERDATA, "NZSDataCollectionName",
  ...
  registerInputCollection(LCIO::TRACKERDATA, "NoiseCollectionName",
                          "Noise (input) collection name", _noiseCollectionName,
                          string("noise"));
  ...
  registerOptionalParameter(
      "HotPixelCollectionName", "This is the name of the hotpixel collection",
  ...
  registerOptionalParameter(
      "ClusterNxN", "The list of cluster NxN to be filled."
...
void EUTelClusteringProcessor::init() {
  ...
  if (_nzsClusteringAlgo == EUTELESCOPE::BRICKEDCLUSTER ||
    ...
    if (!(_ffXClusterSize == 3) && (_ffYClusterSize == 3)) {
      streamlog_out(ERROR2) << "[init()] For bricked pixel clustering the "
                               "cluster size has to be 3x3 at the moment(!). "
                               "Sorry!";
      throw InvalidParameterException(
...
void EUTelClusteringProcessor::initializeGeometry(LCEvent *event) {
  ...
  try {
    nzsInputDataCollectionVec = dynamic_cast<LCCollectionVec *>(
    ...
    for (size_t i = 0; i < nzsInputDataCollectionVec->size(); ++i) {
      TrackerDataImpl *data = dynamic_cast<TrackerDataImpl *>(
          nzsInputDataCollectionVec->getElementAt(i));
...
void EUTelClusteringProcessor::modifyEvent(LCEvent * /* event */) { return; }
...
void EUTelClusteringProcessor::initializeHotPixelMapVec() {
  ...
  for (unsigned int iDetector = 0; iDetector < hotPixelCollectionVec->size();
    ...
    TrackerDataImpl *hotData = dynamic_cast<TrackerDataImpl *>(
        hotPixelCollectionVec->getElementAt(iDetector));
    ...
    streamlog_out(DEBUG1) << "Processing sparse data on detector " << sensorID
                          << " with " << sparseData->size() << " pixels "
                          << endl;

    for (auto &sparsePixel : pixelVec) {
      int decoded_XY_index = matrixDecoder.getIndexFromXY(
      ...
      if (_hitIndexMapVec[iDetector].find(decoded_XY_index) ==
        ...
        _hitIndexMapVec[iDetector].insert(
            make_pair(decoded_XY_index, EUTELESCOPE::FIRINGPIXEL));
      } else {
        streamlog_out(ERROR5) << "hot pixel [index " << decoded_XY_index
                              << "] reoccured ?!" << endl;
      }
    }
  }
...
void EUTelClusteringProcessor::initializeStatusCollection() {
  ...
  for (unsigned int iDetector = 0; iDetector < zsInputDataCollectionVec->size();
       iDetector++) {
    if (_hitIndexMapVec.size() < iDetector + 1)
      _hitIndexMapVec.resize(iDetector + 1);
    ...
    // get the noise and the status matrix with the right detectorID
    status = dynamic_cast<TrackerRawDataImpl *>(
    ...
    for (auto &sparsePixel : pixelVec) {
      ...
      if (_hitIndexMapVec[iDetector].find(decoded_XY_index) ==
        ...
        _hitIndexMapVec[iDetector].insert(
            make_pair(decoded_XY_index, last_element));

        status->adcValues()[last_element] =
            EUTELESCOPE::HITPIXEL; // adcValues is a vector, there fore must
                                   // address the elements incrementally
      } else {
        status->adcValues()[_hitIndexMapVec[iDetector][decoded_XY_index]] =
...
void EUTelClusteringProcessor::readCollections(LCEvent *event) {
  ...
  try {
    noiseCollectionVec = dynamic_cast<LCCollectionVec *>(
        event->getCollection(_noiseCollectionName));
    streamlog_out(DEBUG4) << "noiseCollectionName: "
                          << _noiseCollectionName.c_str() << " found " << endl;
  } catch (lcio::DataNotAvailableException &e) {
    streamlog_out(DEBUG4) << "No noise pixel DB collection found in the event"
                          << endl;
  }
  ...
  if (isFirstEvent() && statusCollectionVec != nullptr) {
    ...
    try {
      ...
      streamlog_out(WARNING5) << "No hot pixel DB collection found in the event"
                              << endl;
    }
  }
  ...
  if (!hasNZSData && !hasZSData) {
    streamlog_out(MESSAGE2) << "The current event doesn't contain neither ZS "
                               "nor NZS data collections: skip # "
...
void EUTelClusteringProcessor::processEvent(LCEvent *event) {
  ...
  if (_fillHistos)
    (dynamic_cast<AIDA::IHistogram1D *>(_timeStampHisto))
  ...
  try {
    pulseCollection = dynamic_cast<LCCollectionVec *>(
...
void EUTelClusteringProcessor::digitalFixedFrameClustering(
    LCEvent *evt, LCCollectionVec *pulseCollection) {
  streamlog_out(DEBUG4) << "Looking for clusters in the zs data with digital "
                           "FixedFrame algorithm "
                        << endl;
  ...
  CellIDEncoder<TrackerDataImpl> idZSClusterEncoder(
      EUTELESCOPE::CLUSTERDEFAULTENCODING, sparseClusterCollectionVec);
  ...
  for (unsigned int i = 0; i < zsInputDataCollectionVec->size(); i++) {
    ...
    if (type == kEUTelGenericSparsePixel) {
      ...
      for (auto &sparsePixel : pixelVec) {
        int index = matrixDecoder.getIndexFromXY(sparsePixel.getXCoord(),
    ...
    for (pos = sensormatrix.begin(); pos != sensormatrix.end(); ++pos) {
      ...
      for (sec = sensormatrix[(*pos).first].begin();
        ...
        if (sensormatrix[pos->first][sec->first]) {
          ...
          for (unsigned int index_x = i - stepx; index_x <= (i + stepx);
               index_x++) {
            for (unsigned int index_y = j - stepy; index_y <= (j + stepy);
                 index_y++) {

              if (index_x > 0 && index_y > 0) {
                std::map<unsigned int,
                         std::map<unsigned int, bool>>::const_iterator z =
                    sensormatrix.find(index_x);
                if (z != sensormatrix.end() &&
                    z->second.find(index_y) != z->second.end()) {
    ...
    if (!seedcandidates.empty()) {
      ...
      for (i = seedcandidates.begin(); i != seedcandidates.end(); ++i) {
        ...
        std::map<unsigned int, std::map<unsigned int, bool>>::const_iterator z =
            sensormatrix.find(i->x);
        if (z != sensormatrix.end() &&
          ...
          if (sensormatrix[i->x][i->y]) {
            ...
            if (i->x >= static_cast<unsigned int>(stepx) &&
              ...
              for (int index_x = static_cast<int>(i->x - stepx);
                   index_x <= static_cast<int>(i->x + stepx); index_x++) {
                for (int index_y = static_cast<int>(i->y - stepy);
                     index_y <= static_cast<int>(i->y + stepy); index_y++) {
                  if (index_x >= 0 && index_y >= 0) {
                    std::map<unsigned int,
                             std::map<unsigned int, bool>>::const_iterator z =
                        sensormatrix.find(index_x);
            ...
            if (!pix.empty()) {
              ...
              for (unsigned int j = 0; j < pix.size(); j++) {
                ...
                if (z != sensormatrix.end() &&
                    z->second.find(pix[j].y) != z->second.end()) {
              ...
              if (seedX == -1 || seedY == -1) {
                streamlog_out(DEBUG5)
                    ...
                    << endl;
                streamlog_out(DEBUG5) << pix.size() << " " << i->x << " "
                                      << i->y << endl;
                exit(-1);
              }
              ...
              // the sensor matrix
              if ((seedX - stepx) < _minX || (seedX + stepx) > _maxX ||
                  (seedY - stepy) < _minY || (seedY + stepy) > _maxY) {
              ...
              CellIDEncoder<TrackerDataImpl> idClusterEncoder(
              ...
              streamlog_out(DEBUG0) << "  Cluster no " << clusterID << " seedX "
                                    << seedX << " seedY " << seedY << endl;
              /*
              ...
              */
              ...
              if (clusterID >= MAXCLUSTERSIZE) {
                ...
                --clusterID;
                streamlog_out(WARNING2)
                    << "Event " << evt->getEventNumber() << " in run "
                    << evt->getRunNumber() << " on detector " << _sensorID
                    << " contains more than " << MAXCLUSTERSIZE << " cluster ("
...
void EUTelClusteringProcessor::zsFixedFrameClustering(
    LCEvent *evt, LCCollectionVec *pulseCollection) {
      ...
      while (rMapIter != seedCandidateMap.rend()) {
        ...
        if (_hitIndexMapVec.size() > static_cast<unsigned int>(sensorID)) {
          if (_hitIndexMapVec[sensorID].find((*rMapIter).second) !=
            ...
            streamlog_out(DEBUG5) << "Detector " << sensorID << " Pixel "
                                  << seedX << " " << seedY
        ...
        if (status->adcValues()[(*rMapIter).second] == EUTELESCOPE::GOODPIXEL) {
          ...
          for (int yPixel = seedY - (_ffYClusterSize / 2);
            ...
            for (int xPixel = seedX - (_ffXClusterSize / 2);
              ...
              if ((xPixel >= minX) && (xPixel <= maxX) && (yPixel >= minY) &&
                  (yPixel <= maxY)) {
                int index = matrixDecoder.getIndexFromXY(xPixel, yPixel);

                bool isHit =
                    (status->getADCValues()[index] == EUTELESCOPE::HITPIXEL);
                bool isGood =
                    (status->getADCValues()[index] == EUTELESCOPE::GOODPIXEL);
          ...
          if (clusterCandidateSignal >
            ...
            if (clusterID >= MAXCLUSTERSIZE) {
              ...
              streamlog_out(WARNING2)
                  << "Event " << evt->getEventNumber() << " in run "
                  << evt->getRunNumber() << " on detector " << sensorID
                  << " contains more than " << MAXCLUSTERSIZE << " cluster ("
                  << clusterID + limitExceed << ")" << endl;
            }
          }
...
void EUTelClusteringProcessor::zsBrickedClustering(
    LCEvent *evt, LCCollectionVec *pulseCollection) {

  streamlog_out(DEBUG4) << "Looking for clusters in the zs data with "
                           "zsBrickedClustering algorithm "
                        << endl;
  ...
  for (unsigned int i = 0; i < zsInputDataCollectionVec->size(); i++) {
    ...
    if (!seedCandidateMap.empty()) {
      ...
      streamlog_out(DEBUG0) << "  Seed candidates " << seedCandidateMap.size()
                            << endl;
        ...
        if (status->adcValues()[(*rMapIter).second] == EUTELESCOPE::GOODPIXEL) {
          ...
          for (int yPixel = seedY - (_ffYClusterSize / 2);
            ...
            for (int xPixel = seedX - (_ffXClusterSize / 2);
              ...
              if ((xPixel >= minX) && (xPixel <= maxX) && (yPixel >= minY) &&
                ...
                bool isHit = (status->getADCValues()[index] ==
                              EUTELESCOPE::HITPIXEL); // this is set for pixels
                                                      // already used for
                                                      // another cluster
                bool isGood =
                    (status->getADCValues()[index] == EUTELESCOPE::GOODPIXEL);
                ...
                {
                  clusterCandidateCharges.push_back(dataVec[index]);
                  clusterCandidateIndeces.push_back(
                      index); // used to flag used pixels afterwards!
                ...
                } else if (isHit) // used by another cluster
                {
                  ...
                  clusterCandidateCharges.push_back(0.0);
                  clusterCandidateIndeces.push_back(
                      -1);          // used to flag used pixels afterwards!
          ...
          if (!((clusterCandidateCharges.size() == 9) &&
            ...
            streamlog_out(ERROR2)
                ...
                << noiseValueVec.size() << "." << endl
                << "There should be 9 signal values,           but there are"
                << clusterCandidateCharges.size() << "." << endl
                << "There are " << clusterCandidateIndeces.size()
                << " candidate pixel indeces." << endl;
            throw IncompatibleDataSetException(
                "NOT ENOUGH/TOO MUCH DATA GATHERED TO FORM A 3x3 CLUSTER");
          }
          ...
          TrackerPulseImpl *pulse =
              new TrackerPulseImpl; // this will be deleted if the candidate
          ...
          TrackerDataImpl *clusterData =
              new TrackerDataImpl; // this will be deleted if the candidate does
          ...
          EUTelBrickedClusterImpl *brickedClusterCandidate =
              new EUTelBrickedClusterImpl(
                  clusterData); // this will be deleted in any case
          ...
          if (brickedClusterCandidate->getClusterSNR(3) >
              _ffClusterCut) //! HACK TAKI !! important
          {
            //! the cluster candidate is a good cluster
            //! mark all pixels belonging to the cluster as hit
...
void EUTelClusteringProcessor::sparseClustering(
    LCEvent *evt, LCCollectionVec *pulseCollection) {
  // only for noise
  CellIDDecoder<TrackerDataImpl> noiseDecoder(noiseCollectionVec);
    ...
    if (type == kEUTelGenericSparsePixel) {
      ...
      while (!hitPixelVec.empty()) {
        ...
        while (!cluCandidate.empty()) {
          ...
          int index = matrixDecoder.getIndexFromXY(pixel.getXCoord(),
                                                   pixel.getYCoord());
...
void EUTelClusteringProcessor::fixedFrameClustering(
    LCEvent *evt, LCCollectionVec *pulseCollection) {
  ...
  if (isFirstEvent()) {
    ...
    for (unsigned int i = 0; i < nzsInputDataCollectionVec->size(); i++) {
      TrackerDataImpl *nzsData = dynamic_cast<TrackerDataImpl *>(
          nzsInputDataCollectionVec->getElementAt(i));
      ...
      if (foundexcludedsensor)
        continue;
      TrackerDataImpl *noise = dynamic_cast<TrackerDataImpl *>(
          noiseCollectionVec->getElementAt(_ancillaryIndexMap[detectorID]));
      TrackerRawDataImpl *status = dynamic_cast<TrackerRawDataImpl *>(
          statusCollectionVec->getElementAt(_ancillaryIndexMap[detectorID]));
  ...
  for (int i = 0; i < nzsInputDataCollectionVec->getNumberOfElements(); i++) {
    ...
    for (unsigned int iPixel = 0; iPixel < nzsData->getChargeValues().size();
         iPixel++) {
      if (status->getADCValues()[iPixel] == EUTELESCOPE::GOODPIXEL) {
        if (nzsData->getChargeValues()[iPixel] >
          ...
          _seedCandidateMap.push_back(
              make_pair(nzsData->getChargeValues()[iPixel], iPixel));
        }
      }
    }
    ...
    if (!_seedCandidateMap.empty()) {
      ...
      streamlog_out(DEBUG0) << "There are << " << _seedCandidateMap.size()
                            << " seed candidates." << endl;
      ...
      while (mapIter != _seedCandidateMap.begin()) {
        ...
        if (status->adcValues()[(*mapIter).second] == EUTELESCOPE::GOODPIXEL) {
            ...
            for (int xPixel = seedX - (_ffXClusterSize / 2);
              ...
              if ((xPixel >= minX) && (xPixel <= maxX) && (yPixel >= minY) &&
                ...
                if (isGood && !isHit) {
                  ...
                  clusterCandidateNoise2 +=
                      pow(noise->getChargeValues()[index], 2);
                  clusterCandidateCharges.push_back(
          ...
          if (clusterCandidateSignal >
            ...
            TrackerDataImpl *cluster = new TrackerDataImpl;
            CellIDEncoder<TrackerDataImpl> idClusterEncoder(
                EUTELESCOPE::CLUSTERDEFAULTENCODING, dummyCollection);
            idClusterEncoder["sensorID"] = sensorID;
            idClusterEncoder["xSeed"] = seedX;
            idClusterEncoder["ySeed"] = seedY;
            idClusterEncoder["xCluSize"] = _ffXClusterSize;
            ...
            streamlog_out(DEBUG0) << "  Cluster no " << clusterCounter
...
void EUTelClusteringProcessor::nzsBrickedClustering(
    LCEvent *evt, LCCollectionVec *pulseCollection) {
  streamlog_out(DEBUG4) << "Looking for clusters in the RAW data with "
                           "nzsBrickedClustering algorithm "
                        << endl;
  ...
  for (int i = 0; i < nzsInputDataCollectionVec->getNumberOfElements(); i++) {
    ...
    for (unsigned int iPixel = 0; iPixel < nzsData->getChargeValues().size();
      ...
      if (status->getADCValues()[iPixel] == EUTELESCOPE::GOODPIXEL) {
        ...
        if (nzsData->getChargeValues()[iPixel] >
            _ffSeedCut * noise->getChargeValues()[iPixel]) {
          seedCandidateMap.push_back(
              make_pair(nzsData->getChargeValues()[iPixel], iPixel));
          streamlog_out(MESSAGE2) << "Added pixel at (index=" << iPixel
                                  << ") with signal "
                                  << nzsData->getChargeValues()[iPixel]
                                  << " to the seedCandidateMap" << endl;
        ...
        if (status->adcValues()[(*rMapIter).second] == EUTELESCOPE::GOODPIXEL) {
          ...
          TrackerDataImpl *clusterData =
              new TrackerDataImpl; // this will be deleted if the candidate does
                                   ...
                                   // otherwise it will be added to a collection
          CellIDEncoder<TrackerDataImpl> idClusterEncoder(
              EUTELESCOPE::CLUSTERDEFAULTENCODING, dummyCollection); // GOBACK
          idClusterEncoder["sensorID"] = sensorID;
          idClusterEncoder["xSeed"] = seedX;
          idClusterEncoder["ySeed"] = seedY;
          idClusterEncoder["xCluSize"] = _ffXClusterSize;
...
void EUTelClusteringProcessor::end() {
  ...
  if (_fillHistos) {
    ...
    for (int iBin = 0; iBin < 1000; iBin++) {
      int binEntry = (dynamic_cast<AIDA::IHistogram1D *>(_timeStampHisto))
                         ->binEntries(iBin);
      if (binEntry > max) {
        max = binEntry;
        maxBin = iBin;
...
void EUTelClusteringProcessor::fillHistos(LCEvent *evt) {
  ...
  try {
    ...
    for (int iPulse = _initialPulseCollectionSize;
      ...
      if (type == kEUTelFFClusterImpl) {
        ...
        if (pixelType == kEUTelGenericSparsePixel) {
          ...
          streamlog_out(ERROR4)
              << "Unknown sparse cluster type. Sorry for quitting" << endl;
      ...
      (dynamic_cast<AIDA::IHistogram1D *>(_seedSignalHistos[detectorID]))
          ->fill(cluster->getSeedCharge());
      ...
      for (unsigned int i = 0; i < charges.size(); i++) {
        (dynamic_cast<AIDA::IHistogram1D *>(
             _clusterSignal_NHistos[_clusterSpectraNVector[i]][detectorID]))
            ->fill(charges[i]);
      }
      ...
      while (iter != _clusterSpectraNxNVector.end()) {
        (dynamic_cast<AIDA::IHistogram1D *>(
             _clusterSignal_NxNHistos[*iter][detectorID]))
      ...
      TrackerDataImpl *noiseMatrix = dynamic_cast<TrackerDataImpl *>(
          noiseCollectionVec->getElementAt(_ancillaryIndexMap[detectorID]));
      TrackerRawDataImpl *statusMatrix = dynamic_cast<TrackerRawDataImpl *>(
          statusCollectionVec->getElementAt(_ancillaryIndexMap[detectorID]));
      ...
      if (type == kEUTelFFClusterImpl || type == kEUTelBrickedClusterImpl) {
        for (int yPixel = ySeed - (_ffYClusterSize / 2);
          ...
          for (int xPixel = xSeed - (_ffXClusterSize / 2);
            ...
            if ((xPixel >= minX) && (xPixel <= maxX) && (yPixel >= minY) &&
              ...
              bool isMissing = (statusMatrix->getADCValues()[index] ==
        ...
        if (pixelType == kEUTelGenericSparsePixel) {
          ...
          for (auto &pixel : pixelVec) {
            int index = noiseMatrixDecoder.getIndexFromXY(pixel.getXCoord(),
                                                          pixel.getYCoord());
            noiseValues.push_back(noiseMatrix->getChargeValues()[index]);
          }
        }
      ...
      if (fillSNRSwitch) {
        ...
        histo =
            dynamic_cast<AIDA::IHistogram1D *>(_clusterNoiseHistos[detectorID]);
        if (histo) {
          histo->fill(cluster->getClusterNoise());
        }

        histo =
            dynamic_cast<AIDA::IHistogram1D *>(_clusterSNRHistos[detectorID]);
        if (histo) {
        ...
        AIDA::IHistogram2D *histo2d = dynamic_cast<AIDA::IHistogram2D *>(
            _cluster_vs_seedSNRHistos[detectorID]);
        if (histo2d) {
        ...
        vector<int>::iterator iter = _clusterSpectraNxNVector.begin();
        while (iter != _clusterSpectraNxNVector.end()) {
          histo = dynamic_cast<AIDA::IHistogram1D *>(
        ...
        for (unsigned int i = 0; i < snrs.size(); i++) {
          histo = dynamic_cast<AIDA::IHistogram1D *>(
              _clusterSNR_NHistos[_clusterSpectraNVector[i]][detectorID]);
          if (histo) {
            histo->fill(snrs[i]);
          }
        }
...
void EUTelClusteringProcessor::bookHistos() {
  ...
  for (size_t iDetector = 0; iDetector < _sensorIDVec.size(); iDetector++) {
    ...
    _clusterSignalHistos.insert(make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + tempHistoName).c_str(), clusterNBin,
    ...
    _clusterSizeXHistos.insert(make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + tempHistoName).c_str(), clusterXNBin,
                      clusterXMin, clusterXMax)));
    ...
    _clusterSizeYHistos.insert(make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + tempHistoName).c_str(), clusterYNBin,
                      clusterYMin, clusterYMax)));
    _clusterSizeYHistos[sensorID]->setTitle(clusterYTitle.c_str());

    // cluster SNR
    tempHistoName = _clusterSNRHistoName + "_d" + to_string(sensorID);
    _clusterSNRHistos.insert(make_pair(
        sensorID, AIDAProcessor::histogramFactory(this)->createHistogram1D(
                      (basePath + tempHistoName).c_str(), clusterSNRNBin,
                      clusterSNRMin, clusterSNRMax)));
    ...
    AIDA::IHistogram2D *cluster_vs_seedSNRHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + tempHistoName).c_str(), cluster_vs_seedSNRNBin_X,
            cluster_vs_seedSNRMin_X, cluster_vs_seedSNRMax_X,
            cluster_vs_seedSNRNBin_Y, cluster_vs_seedSNRMin_Y,
            cluster_vs_seedSNRMax_Y);
    _cluster_vs_seedSNRHistos.insert(
        make_pair(sensorID, cluster_vs_seedSNRHisto));
    ...
    while (iter != _clusterSpectraNVector.end()) {
      ...
      _clusterSignal_NHistos.insert(
          make_pair(*iter, std::map<int, AIDA::IBaseHistogram *>()));
      _clusterSNR_NHistos.insert(
          make_pair(*iter, std::map<int, AIDA::IBaseHistogram *>()));
      ...
      AIDA::IHistogram1D *clusterSignalNHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              (basePath + tempHistoName).c_str(), clusterNBin, clusterMin,
              clusterMax);
      string tempTitle = "Cluster spectrum with the " + to_string(*iter) +
                         " most significant pixels ";
      clusterSignalNHisto->setTitle(tempTitle.c_str());
      _clusterSignal_NHistos.at(*iter).insert(
          make_pair(sensorID, clusterSignalNHisto));

      // this is for the SNR
      tempHistoName =
          _clusterSNRHistoName + to_string(*iter) + "_d" + to_string(sensorID);
      AIDA::IHistogram1D *clusterSNRNHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              (basePath + tempHistoName).c_str(), clusterSNRNBin, clusterSNRMin,
              clusterSNRMax);
      tempTitle = "Cluster SNR with the " + to_string(*iter) +
                  " most significant pixels";
      clusterSNRNHisto->setTitle(tempTitle.c_str());
      _clusterSNR_NHistos.at(*iter).insert(
          make_pair(sensorID, clusterSNRNHisto));
    ...
    while (iter != _clusterSpectraNxNVector.end()) {
      ...
      _clusterSignal_NxNHistos.insert(
          make_pair(*iter, std::map<int, AIDA::IBaseHistogram *>()));
      _clusterSNR_NxNHistos.insert(
          make_pair(*iter, std::map<int, AIDA::IBaseHistogram *>()));
      ...
      AIDA::IHistogram1D *clusterSignalNxNHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              (basePath + tempHistoName).c_str(), clusterNBin, clusterMin,
              clusterMax);
      string tempTitle = "Cluster spectrum with " + to_string(*iter) + " by " +
                         to_string(*iter) + " pixels ";
      clusterSignalNxNHisto->setTitle(tempTitle.c_str());
      _clusterSignal_NxNHistos.at(*iter).insert(
          make_pair(sensorID, clusterSignalNxNHisto));
      ...
      AIDA::IHistogram1D *clusterSNRNxNHisto =
          AIDAProcessor::histogramFactory(this)->createHistogram1D(
              (basePath + tempHistoName).c_str(), clusterSNRNBin, clusterSNRMin,
              clusterSNRMax);
      tempTitle = "SNR with " + to_string(*iter) + " by " + to_string(*iter) +
                  " pixels ";
      clusterSNRNxNHisto->setTitle(tempTitle.c_str());
      _clusterSNR_NxNHistos.at(*iter).insert(
          make_pair(sensorID, clusterSNRNxNHisto));
    ...
    AIDA::IHistogram1D *seedSignalHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + tempHistoName).c_str(), seedNBin, seedMin, seedMax);
    ...
    AIDA::IHistogram1D *seedSNRHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + tempHistoName).c_str(), seedSNRNBin, seedSNRMin,
            seedSNRMax);
    ...
    AIDA::IHistogram1D *clusterNoiseHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + tempHistoName).c_str(), clusterNoiseNBin,
            clusterNoiseMin, clusterNoiseMax);
    ...
    AIDA::IHistogram2D *hitMapHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram2D(
            (basePath + tempHistoName).c_str(), xBin, xMin, xMax, yBin, yMin,
            yMax);
    ...
    AIDA::IHistogram1D *eventMultiHisto =
        AIDAProcessor::histogramFactory(this)->createHistogram1D(
            (basePath + tempHistoName).c_str(), eventMultiNBin, eventMultiMin,
            eventMultiMax);
    _eventMultiplicityHistos.insert(make_pair(sensorID, eventMultiHisto));
    eventMultiHisto->setTitle(eventMultiTitle.c_str());
  }
  _timeStampHisto = AIDAProcessor::histogramFactory(this)->createHistogram1D(
      "timeStampHisto", 1000, 0, 50000);
