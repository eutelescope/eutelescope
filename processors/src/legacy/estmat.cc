...
inline double getScatterSigma(double eBeam, double radLength) {
  ...
  double scatterTheta =
      0.0136f / eBeam * sqrt(radLength) * (1.0f + 0.038f * std::log(radLength));
  return (scatterTheta);
}
...
void EstMat::readTracksToArray(float **measX, float **measY, int nTracks,
                               int nPlanes) {
  ...
  for (int tr = 0; tr < nTracks; tr++) {
    if (tracks.at(tr).size() != 9 or nPlanes != 9) {
      ...
      throw std::runtime_error("SDR2CL currently needs exactly nine "
                               "measurements in all the tracks.");
    }
...
void EstMat::getExplicitEstimate(TrackEstimate<FITTERTYPE, 4> &estim) {
  ...
  // Sparse multiply
  FITTERTYPE x(estim.params(0)), y(estim.params(1)), dx(estim.params(2)),
      dy(estim.params(3));
  Eigen::Matrix<FITTERTYPE, 4, 1> offdiag, reverse(dx, dy, x, y);
  offdiag(0) = offdiag(2) = estim.cov(0, 2);
  offdiag(1) = offdiag(3) = estim.cov(1, 3);
  estim.params = estim.params.array() * estim.cov.diagonal().array() +
                 offdiag.array() * reverse.array();
...
void FakeChi2::calibrate(TrackerSystem<FITTERTYPE, 4> &system) {
  ...
  for (size_t ii = 2; ii < system.planes.size(); ii++) {
    if (system.planes.at(ii).isExcluded()) {
      continue;
    }
...
void FakeChi2::operator()(size_t offset, size_t stride) {
  ...
  for (int track = offset; track < mat.itMax; track += stride) {
    // prepare system for new track: clear system from prev go around, read
    // track from memory, run track finder
    system.clear();
    ...
    for (size_t pl = 2; pl < system.planes.size(); pl++) {
      ...
      chi2 += resv(0) * resv(0) / resFWErrorX[pl] +
              resv(1) * resv(1) / resFWErrorY[pl];
    }
    ...
    for (size_t pl = 0; pl < system.planes.size() - 2; pl++) {
      ...
      chi2 += resv(0) * resv(0) / resBWErrorX[pl] +
              resv(1) * resv(1) / resBWErrorY[pl];
    }
  }
...
void FakeAbsDev::operator()(size_t offset, size_t stride) {
  ...
  for (int track = offset; track < mat.itMax; track += stride) {
    ...
    for (size_t pl = 2; pl < system.planes.size(); pl++) {
      ...
      Measurement<FITTERTYPE> &meas = system.planes[pl].meas.at(0);
      resv = system.getResiduals(meas, fw);
      chi2 += fabs(resv[0] / sqrt(resFWErrorX[pl])) +
              fabs(resv[1] / sqrt(resFWErrorY[pl]));
    }
    ...
    for (size_t pl = 0; pl < system.planes.size() - 2; pl++) {
      ...
      Measurement<FITTERTYPE> &meas = system.planes[pl].meas.at(0);
      resv = system.getResiduals(meas, bw);
      chi2 += fabs(resv[0] / sqrt(resBWErrorX[pl])) +
              fabs(resv[1] / sqrt(resBWErrorY[pl]));
    }
  }
...
void SDR::operator()(size_t offset, size_t stride) {
  // Get the mean^2 + (1 - variance) of the standardized residuals of chi2
  // increments and or pull distributions
  TrackerSystem<FITTERTYPE, 4> &system = systems.at(offset);
  ...
  std::vector<std::vector<double>> sqrParams(system.planes.size() - 3,
  ...
  for (int track = offset; track < mat.itMax; track += stride) {
    ...
    for (size_t pl = 0; pl < system.planes.size() - 2; pl++) {
      // We're using an information filter, we need explicit state and
      // covariance
    ...
    for (size_t pl = 2; pl < system.planes.size(); pl++) {
      ...
      Eigen::Matrix<FITTERTYPE, 2, 1> resids =
          system.getResiduals(meas, result);
      Eigen::Matrix<FITTERTYPE, 2, 1> variance =
          system.getBiasedResidualErrors(system.planes.at(pl), result);
      // Squared pulls to calculate pull variance
      Eigen::Matrix<FITTERTYPE, 2, 1> pull2 =
          resids.array().square() / variance.array();
    ...
    for (size_t pl = 0; pl < system.planes.size() - 2; pl++) {
      ...
      Eigen::Matrix<FITTERTYPE, 2, 1> resids =
          system.getResiduals(meas, result);
      Eigen::Matrix<FITTERTYPE, 2, 1> variance =
          system.getUnBiasedResidualErrors(system.planes.at(pl), result);
      // Squared pulls to calculate pull variance
      Eigen::Matrix<FITTERTYPE, 2, 1> pull2 =
          resids.array().square() / variance.array();
    ...
    if (not cholDec) {
      ...
      for (size_t pl = 1; pl < system.planes.size() - 2; pl++) {
        TrackEstimate<FITTERTYPE, 4> &fw = system.m_fitter.forward.at(pl);
        TrackEstimate<FITTERTYPE, 4> &bw = system.m_fitter.backward.at(pl);
        Eigen::Matrix<double, 4, 1> tmpDiff =
            (fw.params - bw.params).cast<double>();
...
void Minimizer::prepareThreads() {
  ...
  for (size_t ii = 0; ii < mat.system.planes.size(); ii++) {
    ...
    for (size_t thread = 0; thread < nThreads; thread++) {
      FitPlane<FITTERTYPE> &plT =
          systems.at(thread).planes.at(ii); // Thread plane
...
FITTERTYPE Minimizer::operator()(void) {
  ...
  for (int ii = 0; ii < nThreads; ii++) {
    threads.create_thread(
        boost::bind(&Minimizer::operator(), this, ii, nThreads));
  }
  threads.join_all();
#else
  (*this)(0, 1);
...
//   // Toy simulation of a straight track with Gaussian uncertainties and
//   scattering
...
//       simTrack.push_back( Measurement<FITTERTYPE>(x + g1 * sigmas(0), y + g2
//       * sigmas(1), system.planes.at(pl).getZpos(), true, pl) );
//     }
//     addTrack(simTrack);
//   }
// }
...
void EstMat::setPlane(int index, double sigmaX, double sigmaY,
                      double radLength) {
...
void EstMat::printAllFreeParams() {
  ...
  printParams("params[\"RadiationLengths\"]", radLengths,
              radLengthsIndex.size() or radLengthsMulti.size(), "%4.6f ");
  printParams("params[\"ResolutionX\"]", resX,
              resXIndex.size() or resXMulti.size() or resXYp, "%4.6f ");
  printParams("params[\"ResolutionY\"]", resY,
              resYIndex.size() or resYMulti.size() or resXYp, "%4.6f ");
...
void EstMat::printParams(std::string name, std::vector<FITTERTYPE> &params,
                         bool plot, const char *valString) {
...
void EstMat::plot(char *fname) {
  ...
  TH2D *corr34X =
      new TH2D("correlations34x", "correlations34x", 100, -4, 4, 100, -4, 4);
  TH2D *corr34Y =
      new TH2D("correlations34y", "correlations34y", 100, -4, 4, 100, -4, 4);
  TH2D *corr12X =
  ...
  for (size_t track = 0; track < tracks.size(); track++) {
    ...
    for (size_t ii = 0; ii < system.getNtracks(); ii++) {
      ...
      for (size_t pl = 0; pl < system.planes.size(); pl++) {
        if (system.planes.at(pl).meas.size() < 1) {
          continue;
        }
        Measurement<FITTERTYPE> &meas = system.planes.at(pl).meas.at(0);
        ...
        Eigen::Matrix<FITTERTYPE, 2, 1> variance =
            system.getUnBiasedResidualErrors(system.planes.at(pl), est);
      ...
      if ((system.planes.at(3).meas.size() > 0) and
        ...
        Eigen::Matrix<FITTERTYPE, 2, 1> resids3 =
            system.getResiduals(meas3, est3);
        Eigen::Matrix<FITTERTYPE, 2, 1> variance3 =
            system.getUnBiasedResidualErrors(system.planes.at(3), est3);

        TrackEstimate<FITTERTYPE, 4> &est4 = candidate.estimates.at(4);
        Measurement<FITTERTYPE> &meas4 = system.planes.at(4).meas.at(0);
        Eigen::Matrix<FITTERTYPE, 2, 1> resids4 =
            system.getResiduals(meas4, est4);
        Eigen::Matrix<FITTERTYPE, 2, 1> variance4 =
            system.getUnBiasedResidualErrors(system.planes.at(4), est4);

        corr34X->Fill(resids3(0) / sqrt(variance3(0)),
                      resids4(0) / sqrt(variance4(0)));
        corr34Y->Fill(resids3(1) / sqrt(variance3(1)),
                      resids4(1) / sqrt(variance4(1)));
      ...
      if ((system.planes.at(1).meas.size() > 0) and
        ...
        Eigen::Matrix<FITTERTYPE, 2, 1> resids6 =
            system.getResiduals(meas6, est6);
        Eigen::Matrix<FITTERTYPE, 2, 1> variance6 =
            system.getUnBiasedResidualErrors(system.planes.at(6), est6);

        TrackEstimate<FITTERTYPE, 4> &est2 = candidate.estimates.at(2);
        Measurement<FITTERTYPE> &meas2 = system.planes.at(2).meas.at(0);
        Eigen::Matrix<FITTERTYPE, 2, 1> resids2 =
            system.getResiduals(meas2, est2);
        Eigen::Matrix<FITTERTYPE, 2, 1> variance2 =
            system.getUnBiasedResidualErrors(system.planes.at(2), est2);

        // corr12X->Fill( resids2(0), resids6(0));
        // corr12Y->Fill( resids2(1), resids6(1));
        corr12X->Fill(resids2(0) / sqrt(variance2(0)),
                      resids6(0) / sqrt(variance6(0)));
        corr12Y->Fill(resids2(1) / sqrt(variance2(1)),
                      resids6(1) / sqrt(variance6(1)));
...
void EstMat::vectToEst(gsl_vector *v, std::vector<int> &indexVector,
                       vector<FITTERTYPE> &dataVector, size_t &param) {
  for (size_t ii = 0; ii < indexVector.size(); ii++) {
    gsl_vector_set(v, param++, dataVector.at(indexVector.at(ii)));
  }
}
void EstMat::multiVectToEst(gsl_vector *v, std::vector<int> &indexVector,
                            vector<FITTERTYPE> &dataVector, size_t &param) {
...
gsl_vector *EstMat::systemToEst() {
  // Read the state of the tracker system into a gsl vector for the simplex
  // search
...
void EstMat::simplexSearch(Minimizer *minimizeMe, size_t iterations,
                           int restarts, size_t maxIterations) {
...
FITTERTYPE EstMat::stepVector(gsl_vector *vc, size_t index, FITTERTYPE value,
                              bool doMSE, Minimizer *minimize) {
...
void EstMat::quasiNewtonHomeMade(Minimizer *minimizeMe, int iterations) {
  // Try to find minimum using independent steps of variables using Newtons
  // method
  minimizeMe->init();
  ...
  for (int iteration = 0; iteration < iterations; iteration++) {
    ...
    for (size_t ii = 0; ii < nParams; ii++) {
      ...
      double firstDeriv =
          (-1.0 * p2h + 8.0 * p1h - 8.0 * m1h + 1.0 * m2h) / (12.0 * step);
      double secondDeriv =
          (-1.0 * p2h + 16 * p1h - 30.0 * p0h + 16.0 * m1h - 1.0 * m2h) /
          (12.0 * step * step);
      ...
      if (std::isnan(newVal) or std::isinf(newVal)) {
        ...
        while (fabs(newVal) > 5 * fabs(val) + 0.00001) {
          cout << "Large step, damping: " << val << " , " << val - newVal
               << endl;
          newVal *= 0.1;
        }
