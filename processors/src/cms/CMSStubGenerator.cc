...
CMSStubGenerator::CMSStubGenerator ():Processor ("CMSStubGenerator")
{
  ...
  registerProcessorParameter ("DUTPlane1", "This is the first DUT sensorID.",
			      _dutPlane1, 6);

  registerProcessorParameter ("DUTPlane2", "This is the second DUT sensorID.",
			      _dutPlane2, 7);

  registerProcessorParameter ("KeepDUTHits",
  ...
  registerProcessorParameter ("OutputSensorID",
			      "SensorID of the output stub.", _outputSensorID,
			      8);

  registerProcessorParameter ("RequireStub",
...
void
CMSStubGenerator::processRunHeader (LCRunHeader * rdr)
{
...
void
CMSStubGenerator::processEvent (LCEvent * event)
{
  ...
  catch (DataNotAvailableException & e)
  {
    streamlog_out (MESSAGE2) << "No input collection " <<
      _inputHitCollectionName << " found in event " << event->
      getEventNumber () << " in run " << event->getRunNumber () << endl;
    return;
  }
  ...
  try
  {
    outputHitCollection =
      static_cast <
      LCCollectionVec * >(event->getCollection (_outputHitCollectionName));
  }
  catch ( ...)
  {
  ...
  CellIDEncoder < TrackerHitImpl >
    outputCellIDEncoder (EUTELESCOPE::HITENCODING, outputHitCollection);
  ...
  for (int iInputHits = 0;
	...
	dynamic_cast <
	TrackerHitImpl * >(inputHitCollection->getElementAt (iInputHits));
      ...
      else
	{
	  streamlog_out (ERROR5) <<
	    "Illegal setting for Mode! Only 0, 1 or 2 is a valid setting!" <<
	    endl;
	  exit (-1);
	}
    }
  ...
  if (_runMode == 0)
    {
      for (unsigned int iHitPlane1 = 0; iHitPlane1 < dutPlane1Hits.size ();
	   iHitPlane1++)
	{
	  TrackerHitImpl *Hit1 =
	    dynamic_cast <
	    TrackerHitImpl *
	    >(inputHitCollection->getElementAt (dutPlane1Hits[iHitPlane1]));
	  float x1 = -1.0;
	  float y1 = -1.0;
	  float q1 = -1.0;
	  const double *pos1 = Hit1->getPosition ();
	  TrackerDataImpl *clusterVector1 =
	    static_cast < TrackerDataImpl * >(Hit1->getRawHits ()[0]);
	  ...
	  for (unsigned int iHitPlane2 = 0;
	       iHitPlane2 < dutPlane2Hits.size (); iHitPlane2++)
	    {
	      TrackerHitImpl *Hit2 =
		dynamic_cast <
		TrackerHitImpl *
		>(inputHitCollection->
		  getElementAt (dutPlane2Hits[iHitPlane2]));
	      float x2 = -1.0;
	      float y2 = -1.0;
	      float q2 = -1.0;
	      const double *pos2 = Hit2->getPosition ();
	      TrackerDataImpl *clusterVector2 =
		static_cast < TrackerDataImpl * >(Hit2->getRawHits ()[0]);
	      ...
	      if (cluster2 != nullptr)
		{
	      ...
	      streamlog_out (DEBUG0) << " x1 " << x1 << " y1 " << y1 << " q1 "
		<< q1 << " x2 " << x2 << " y2 " << y2 << " q2 " << q2 <<
		" evt " << evt->getRunNumber () << endl;
	      ...
	      if (dx < _maxResidual && dy < _maxResidual)
		{
		  ...
		  CellIDEncoder < TrackerHitImpl >
		    idHitEncoder (EUTELESCOPE::HITENCODING,
				  outputHitCollection);
  ...
  double ratio =
    stubsinthisevent * 1.0 /
    ((dutPlane1Hits.size () + dutPlane2Hits.size ()) / 2.0);
  stubratiodt->fill (event->getEventNumber (), ratio);
  stubdt->fill ((event->getEventNumber ()) * 1.0,
		((_totalstubs * 1.0) / (event->getEventNumber () * 1.0)));

}


TrackerHitImpl *
CMSStubGenerator::cloneHit (TrackerHitImpl * inputHit)
{
...
void
CMSStubGenerator::end ()
{

  streamlog_out (MESSAGE4) << "Created " << _totalstubs <<
    " stubs from on average " << (_totalpl1 +
				  _totalpl2) /
    2.0 << " hits -> " << _totalstubs / ((_totalpl1 + _totalpl2) / 2.0) *
    100.0 << "% efficiency!" << endl;

}

void
CMSStubGenerator::bookHistos ()
{
  try
  {
    ...
    stubdistx =
      ...
      createHistogram1D ("Stub Distance in x", 101, -50, 50);
    stubdistx->
      setTitle ("Stub Distance in x;x_{0} - x_{1} [channels];Entries");

    stubdisty =
      AIDAProcessor::histogramFactory (this)->
      createHistogram1D ("Stub Distance in y", 101, -50, 50);
    stubdisty->
      setTitle ("Stub Distance in y;y_{0} - y_{1} [channels];Entries");

    stubdistx_bit =
      AIDAProcessor::histogramFactory (this)->
      createHistogram1D ("Stub Distance in x, bit required", 101, -50, 50);
    stubdistx_bit->
      setTitle
      ("Stub Distance in x, bit required;x_{0} - x_{1} [channels];Entries");

    stubdisty_bit =
      AIDAProcessor::histogramFactory (this)->
      createHistogram1D ("Stub Distance in y, bit required", 101, -50, 50);
    stubdisty_bit->
      setTitle
      ("Stub Distance in y, bit required;y_{0} - y_{1} [channels];Entries");

    faildistx =
      AIDAProcessor::histogramFactory (this)->
      createHistogram1D ("Fail Distance in x", 1000, -500, 500);
    faildistx->
      setTitle ("Fail Distance in x;x_{0} - x_{1} [channels];Entries");

    faildisty =
      AIDAProcessor::histogramFactory (this)->
      createHistogram1D ("Fail Distance in y", 1000, -500, 500);
    faildisty->
      setTitle ("Fail Distance in y;y_{0} - y_{1} [channels];Entries");

    correx =
      AIDAProcessor::histogramFactory (this)->
      createHistogram2D ("Cluster correlation in x", 1016, 0, 1015, 1016, 0,
			 1015);
    correx->
      setTitle ("Cluster Correlation in x;x_{0} [Channel];x_{1} [Channel]");

    correy =
      AIDAProcessor::histogramFactory (this)->
      createHistogram2D ("Cluster correlation in y", 1016, 0, 1015, 1016, 0,
			 1015);
    correy->
      setTitle ("Cluster Correlation in y;y_{0} [Channel];y_{1} [Channel]");

    stubdt =
      AIDAProcessor::histogramFactory (this)->
      createHistogram2D ("Stubs per Event", 10000, 0, 500000, 100, 0, 1);
    stubdt->setTitle ("Stubs per Event;Events;Stubs per Event");

    stubratiodt =
      AIDAProcessor::histogramFactory (this)->
      createHistogram2D ("Stubs per Cluster Ratio over Events", 10000, 0,
			 500000, 100, 0, 1);
    stubratiodt->
      setTitle
      ("Stubs per Cluster Ratio over Events;Events;Stubs per Cluster");

    stubmap_top_x =
      AIDAProcessor::histogramFactory (this)->
      createHistogram1D ("Stub map, top, x", 1016, 0, 1015);
    stubmap_top_x->setTitle ("Stub Map, Top Sensor in x;Channel;Entries");

    stubmap_bot_x =
      AIDAProcessor::histogramFactory (this)->
      createHistogram1D ("Stub map, bot, x", 1016, 0, 1015);
    stubmap_bot_x->setTitle ("Stub Map, Bottom Sensor in x;Channel;Entries");

    stubmap_top_y =
      AIDAProcessor::histogramFactory (this)->
      createHistogram1D ("Stub map, top, y", 1016, 0, 1015);
    stubmap_top_y->setTitle ("Stub Map, Top Sensor in y;Channel;Entries");

    stubmap_bot_y =
      AIDAProcessor::histogramFactory (this)->
      createHistogram1D ("Stub map, bot, y", 1016, 0, 1015);
