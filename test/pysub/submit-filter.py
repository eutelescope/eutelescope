...
print red, "Filtering job submitter" , black
...
gridFolderFilterResults   = "$LFC_HOME/2008/tb-cern-summer/results-depfet/filter/filter-1"
gridFolderDB              = "$LFC_HOME/2008/tb-cern-summer/db-depfet"
gridFolderFilterHisto     = "$LFC_HOME/2008/tb-cern-summer/joboutput/depfet/filter/filter-1"
gridFolderFilterJobout    = "$LFC_HOME/2008/tb-cern-summer/joboutput/depfet/filter/filter-1"
...
for run in runList[:]:
    ...
    print blue, "Generating the steering file... (filter-%(run)s.xml)" % { "run": runString }, black

    # open the template steering file for reading
    templateSteeringFile = open( "./template/filter-tmp.xml", "r")
    ...
    actualSteeringFile = open( "filter-%(run)s.xml" % { "run": runString }, "w" )
    ...
    if optionAllGrid == 0 :
        ...
        if optionCPULocal == 1:
            print blue, "Getting the input pre-analyzed file from the GRID...", black
            command = ( "lcg-cp -v lfn:%(inputFolder)s/run%(run)s-clu-p%(pede)s.slcio " + \
                        " file:$PWD/results/run%(run)s-clu-p%(pede)s.slcio" ) % \
                      { "inputFolder": gridFolderLcioRaw , "run": runString, "pede": pedestalRunShortString }
            returnvalue = os.system( command )
            if returnvalue != 0:
        ...
        logfile = open( "filter-%(run)s.log " % { "run": runString }, "w" )
        marlin = popen2.Popen4( "Marlin filter-%(run)s.xml" % { "run": runString } )
        while marlin.poll() == -1:
        ...
        if optionCPULocal == 1:
            ...
            command = ( "lcg-cr -v -l lfn:%(gridFolder)s/run%(run)s-filter-p%(pede)s.slcio" + \
                        " file:$PWD/results/run%(run)s-filter-p%(pede)s.slcio" ) % \
        ...
        command = "mkdir /tmp/filter-%(run)s" % { "run": runString }
        returnvalue = os.system( command )
        if returnvalue != 1 and returnvalue != 0:
            ...
            command = "cp filter-%(run)s*  histo/run%(run)s-filter-histo.root /tmp/filter-%(run)s" % { "run" : runString }
            returnvalue = os.system( command )
            if returnvalue != 0:
                ...
                command = "tar czvf filter-%(run)s.tar.gz /tmp/filter-%(run)s"  % { "run" : runString }
                returnvalue = os.system( command )
                if returnvalue != 0:
                    ...
                    if optionCPULocal == 1:
                        ...
                        command = "lcg-cr -v -l lfn:%(gridFolder)s/filter-%(run)s.tar.gz file:$PWD/filter-%(run)s.tar.gz" % \
                        ...
                        if returnvalue != 0:
                            ...
                            copiedTARFile.append( "%(gridFolder)s/filter-%(run)s.tar.gz" \
                                                  %  { "gridFolder": gridFolderFilterJobout , "run": runString } )

        # clean up the execution envirotment
        print blue, "Cleaning up enviroment", black
        command = "rm -vrf /tmp/filter-%(run)s filter-%(run)s*" % { "run": runString }
        returnvalue = os.system( command )
        if returnvalue != 0:
        ...
        if optionKeepOutput == 0 and optionKeepInput == 0:
            command = "rm -v results/run%(run)s-clu-p%(pede)s.slcio histo/run%(run)s-filter-histo.root results/run%(run)s-filter-p%(pede)s.slcio" \
            ...
            command = "rm -v results/run%(run)s-clu-p%(pede)s.slcio" % { "run": runString,  "pede": pedestalRunShortString }
            returnvalue = os.system( command )
            if returnvalue != 0:
