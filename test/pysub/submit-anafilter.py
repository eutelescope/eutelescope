...
for run in runList[:]:
    ...
    print blue, "Generating the steering file... (anafilter-%(run)s.xml)" % { "run": runString }, black

    # open the template steering file for reading
    templateSteeringFile = open( "./template/anafilter-tmp.xml", "r")
    ...
    actualSteeringFile = open( "anafilter-%(run)s.xml" % { "run": runString }, "w" )
    ...
    if optionAllGrid == 0 :
        ...
        logfile = open( "anafilter-%(run)s.log " % { "run": runString }, "w" )
        marlin = popen2.Popen4( "Marlin anafilter-%(run)s.xml" % { "run": runString } )
        while marlin.poll() == -1:
