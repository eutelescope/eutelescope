  ...
  namespace geo {
                                  ...
                                  93.660734)          // rad length
    {
      ...
      TGeoVolume *centreregion =
          _tGeoManager->MakeBox("fei4dcentreregion", Si, 0.45, 8.4, 0.0125);
      TGeoVolume *edgeregion =
          _tGeoManager->MakeBox("fei4dedgeregion", Si, 9.875, 8.4, 0.0125);

      // Divide the regions to create pixels
      TGeoVolume *edgerow =
          edgeregion->Divide("fei4dedgerow", 2, 336, 0, 1, 0, "N");
      edgerow->Divide("fei4dedgepixel", 1, 79, 0, 1, 0, "N");
      TGeoVolume *centrerow =
          centreregion->Divide("fei4dcentrerow", 2, 336, 0, 1, 0, "N");
    ...
    std::string FEI4Double::getPixName(int x, int y) {
      ...
      if (x < 79) {
        snprintf(buffer, 200, "/sensarea_fei4d_1/fei4dedgeregion_1/"
                              "fei4dedgerow_%d/fei4dedgepixel_%d",
                 336 - y, x + 1);
      }

      else if (x == 79) {
        snprintf(buffer, 200, "/sensarea_fei4d_1/fei4dcentreregion_1/"
                              "fei4dcentrerow_%d/fei4dcentrepixel_1",
                 336 - y);
      }

      else if (x == 80) {
        snprintf(buffer, 200, "/sensarea_fei4d_1/fei4dcentreregion_1/"
                              "fei4dcentrerow_%d/fei4dcentrepixel_2",
                 336 - y);
      }

      else {

        snprintf(buffer, 200, "/sensarea_fei4d_1/fei4dedgeregion_2/"
                              "fei4dedgerow_%d/fei4dedgepixel_%d",
                 336 - y, x - 80);
      }
    ...
    std::pair<int, int> FEI4Double::getPixIndex(char const *) {
