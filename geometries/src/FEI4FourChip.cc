...
namespace eutelescope {
  namespace geo {
    ...
    FEI4FourChip::FEI4FourChip()
        : EUTelGenericPixGeoDescr(40.4, 35.18, 0.025, // size X, Y, Z
                                  ...
                                  93.660734)          // rad length
    {
      ...
      plane =
          _tGeoManager->MakeBox("sensarea_fei4four", Si, 20.2, 17.59, 0.0125);

      // Create volumes for the double chipe
      TGeoVolume *doublechip =
          _tGeoManager->MakeBox("fei4double", Si, 20.2, 8.4, 0.0125);

      // Create volumes for the different regions
      TGeoVolume *normalRegion =
          _tGeoManager->MakeBox("fei4normreg", Si, 9.875, 8.4, 0.0125);
      TGeoVolume *centreRegion =
          _tGeoManager->MakeBox("fei4centreg", Si, 0.45, 8.4, 0.0125);
    ...
    std::string FEI4FourChip::getPixName(int x, int y) {
      ...
      // If y index is larger than 335 then we are on double chip two (the upper
      // one)
      if (y > 335) {
        doublechip = 2;
        y -= 336;
      ...
      if (x < 79) {
        ...
        snprintf(
            buffer, 100,
            "/sensarea_fei4four_1/fei4double_%d/fei4normreg_1/col_%d/pixel_%d",
            doublechip, x + 1, y + 1);
      }
        ...
        snprintf(
            buffer, 100,
            "/sensarea_fei4four_1/fei4double_%d/fei4centreg_1/col_1/pixel_%d",
            doublechip, y + 1);
      } else if (x == 80) {
        // in centreRegion_1, col_2
        snprintf(
            buffer, 100,
            "/sensarea_fei4four_1/fei4double_%d/fei4centreg_1/col_2/pixel_%d",
            doublechip, y + 1);
      } else {
        // in normalRegion_2
        snprintf(
            buffer, 100,
            "/sensarea_fei4four_1/fei4double_%d/fei4normreg_2/col_%d/pixel_%d",
            doublechip, x - 80, y + 1);
      }
    ...
    std::pair<int, int> FEI4FourChip::getPixIndex(char const *) {
