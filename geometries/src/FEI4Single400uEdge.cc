  ...
  namespace geo {
                                  ...
                                  93.660734)          // rad length
    {
      ...
      TGeoVolume *centreregion =
          _tGeoManager->MakeBox("fei4centreregion", Si, 9.75, 8.4, 0.0125);
      TGeoVolume *edgeregion =
          _tGeoManager->MakeBox("fei4edgeregion", Si, 0.2, 8.4, 0.0125);

      // Divide the regions to create pixels
      edgeregion->Divide("fei4edgepixel", 2, 336, 0, 1, 0, "N");
      TGeoVolume *centrerow =
          centreregion->Divide("fei4centrerow", 2, 336, 0, 1, 0, "N");
    ...
    std::string FEI4Single400uEdge::getPixName(int x, int y) {
      ...
      if (x == 0) {
        snprintf(buffer, 100,
                 "/sensarea_fei4_1/fei4edgeregion_1/fei4edgepixel_%d", 336 - y);
      }

      else if (x == 79) {
        snprintf(buffer, 100,
                 "/sensarea_fei4_1/fei4edgeregion_2/fei4edgepixel_%d", 336 - y);
      }
      if (x > 0 && x < 79) {

        snprintf(buffer, 100, "/sensarea_fei4_1/fei4centreregion_1/"
                              "fei4centrerow_%d/fei4centrepixel_%d",
                 336 - y, x);
      }
    ...
    std::pair<int, int> FEI4Single400uEdge::getPixIndex(char const *) {
