...
## Submit ApplyAlignment jobs
#
# This class is responsible to submit jobs for applying alignment.
# It is inheriting from SubmitBase and it is called by the submit-applyAlignment.py script
#
#
# @version $Id: submitapplyAlignment.py,v 1.13 2009-07-28 09:24:41 bulgheroni Exp $
# @author Antonio Bulgheroni, INFN <mailto:antonio.bulgheroni@gmail.com>
#

class SubmitApplyAlignment( SubmitBase ):
    ...
    def configure( self ):
        ...
        if self._option.split_job != 1 and self._option.execution in [ "all-local", "cpu-local" ] :
            self._logger.warning( "User wants to split the applyAlignment in %(job)d jobs, but this is not available in %(mode)s" % {
...
#        self._logger.critical( self._fqInputFileList[0] )
    ...
    def generateSteeringFileSplitting( self, index ):
        ...
        try:
            steeringFileTemplate = self._configParser.get( "SteeringTemplate", "ApplyAlignmentSteeringFile" )
        except ConfigParser.NoOptionError :
        ...
        try :
            record = self._configParser.get("ApplyAlignmentOptions", "Records")
        except ConfigParser.NoOptionError :
            record = 10000000
        ...
        if self._option.execution == "all-grid" :
            ...
            try :
                outputFolder =  self._configParser.get("LOCAL", "LocalFolderApplyAlignmentResults")
            except ConfigParser.NoOptionError :
                outputFolder = "results"
        ...
        if self._option.execution == "all-grid" :
            ...
            try:
                histoFolder = self._configParser.get("LOCAL", "LocalFolderApplyAlignmentHisto")
            except ConfigParser.NoOptionError :
                histoFolder = "histo"
...
# what's the point here? 
    ...
    #
    # results/@output@-alignedHit.XXX.slcio
    def checkOutputFile( self ) :


        try :
            outputFilePath = self._configParser.get( "LOCAL", "LocalFolderApplyAlignmentResults" )
        except ConfigParser.NoOptionError :
            outputFilePath = "results"

        outputFileName = "%(run)s-alignedHit.[0-9][0-9][0-9].slcio" % { "run": self._option.output  }
    ...
    def checkHistogramFile( self ) :
        ...
        try :
            histoFilePath = self._configParser.get( "LOCAL", "LocalFolderApplyAlignmentHisto" )
        except ConfigParser.NoOptionError :
            histoFilePath = "histo"

        histoFileName = "%(run)s-alignedHit-histo.root" % { "run": self._option.output }
    ...
    def prepareTarball( self ) :
        ...
        if self._option.execution != "all-grid":
            ...
            listOfFiles.append( os.path.join( histoFilePath, "%(run)s-alignedHit-histo.root" % { "run": self._option.output } ) )
        ...
        try:
            localFolder = self._configParser.get( "LOCAL", "LocalFolderApplyAlignmentJoboutput")
        except ConfigParser.NoOptionError :
            self._logger.debug( "LocalFolderApplyAlignmentJoboutput not available, using $PWD/log ")
            localFolder = "log/"
...
## Check the joboutput file
    ...
    def checkJoboutputFile( self ) :
        ...
        try :
            outputFilePath = self._configParser.get( "LOCAL", "LocalFolderApplyAlignmentJoboutput" )
        except ConfigParser.NoOptionError :
            outputFilePath = "log"
    ...
    def cleanup( self ):
        ...
        if self._keepOutput == False :
            ...
            outputFile = "%(run)s-alignedHit.[0-9][0-9][0-9].slcio" % { "run" : self._option.output }
            ...
            histoFile = "%(run)s-alignedHit-histo.root" % { "run": self._option.output }
    ...
    # Put the output aligned hits file on the GRID
    def putOutputOnGRID( self ):

        self._logger.info(  "Putting the output file to the GRID" )

        try :
            gridPath = self._configParser.get( "GRID", "GRIDFolderApplyAlignmentResults")
        except ConfigParser.NoOptionError :
            message = "GRIDFolderApplyAlignmentResults missing in the configuration file. Quitting."
            self._logger.critical( message )
            raise StopExecutionError( message )

        try :
            localPath = self._configParser.get( "LOCAL", "LocalFolderApplyAlignmentResults" )
        except ConfigParser.NoOptionError :
            localPath = "$PWD/results"
        ...
        fileglob = "%(localPath)s/%(output)s-alignedHit.[0-9][0-9][0-9].slcio" % { "localPath": localPath, "output" : self._option.output }
    ...
    def putHistogramOnGRID( self ):
        ...
        try:
            gridPath = self._configParser.get( "GRID", "GRIDFolderApplyAlignmentHisto")
        except ConfigParser.NoOptionError :
            message = "GRIDFolderApplyAlignmentHisto missing in the configuration file. Quitting."
            self._logger.critical( message )
            raise StopExecutionError( message )

        try :
            localPath = self._configParser.get( "LOCAL", "LocalFolderApplyAlignmentHisto" )
        except ConfigParser.NoOptionError :
            localPath = "$PWD/histo"
        ...
        command = "%(base)s -l lfn:%(gridFolder)s/%(run)s-alignedHit-histo.root file:%(localFolder)s/%(run)s-alignedHit-histo.root" % \
            { "base": baseCommand, "gridFolder": gridPath, "localFolder": localPath, "run" : self._option.output }
        if os.system( command ) != 0 :
            self._logger.critical( "Problem copying the histogram file %(run)s-alignedHit-histo.root to the GRID" % {"run" : self._option.output} )
            for index, entry in enumerate( self._summaryNTuple ):
                run, input, marlin, output, histo, tarball = entry
            ...
            raise GRID_LCG_CRError( "lfn:%(gridFolder)s/%(run)s-alignedHit-histo.root" %   { "gridFolder": gridPath, "run" : self._option.output } )
        else:
        ...
        if self._option.verify_output:
            ...
            filename = "%(run)s-alignedHit-histo.root"  % { "run" : self._option.output }
            ...
            filenametest = "%(run)s-alignedHit-histo-test.root"  % { "run" : self._option.output }
    ...
    def putJoboutputOnGRID( self ):
        ...
        try:
            gridPath = self._configParser.get( "GRID", "GRIDFolderApplyAlignmentJoboutput")
        except ConfigParser.NoOptionError :
            message = "GRIDFolderApplyAlignmentJoboutput missing in the configuration file. Quitting."
            self._logger.critical( message )
            raise StopExecutionError( message )

        try :
            localPath = self._configParser.get( "LOCAL", "LocalFolderApplyAlignmentJoboutput" )
        except ConfigParser.NoOptionError :
            localPath = "$PWD/log"
    ...
    def doPreliminaryTest( self ) :
        ...
        try :
            ...
            self._outputPathGRID    = self._configParser.get("GRID", "GRIDFolderApplyAlignmentResults" )
            self._joboutputPathGRID = self._configParser.get("GRID", "GRIDFolderApplyAlignmentJoboutput")
            self._histogramPathGRID = self._configParser.get("GRID", "GRIDFolderApplyAlignmentHisto")
...
#        lfcls.wait()
        ...
        pattern = re.compile( "%(output)s-alignedHit.[0-9]{3}.slcio" % { "output": self._option.output } )
        for line in list :
        ...
        filenameList.append( "%(outputPathGRID)s/%(output)s-alignedHit-histo.root"     % { "outputPathGRID": self._histogramPathGRID, "output": self._option.output } )
    ...
    def generateRunjobFileSingleJob( self ):
        ...
        self._logger.info( message )
        try :
            runTemplate = self._configParser.get( "SteeringTemplate", "ApplyAlignmentGRIDScript" )
        except ConfigParser.NoOptionError:
        ...
        variableList = [ "GRIDCE", "GRIDSE", "GRIDStoreProtocol", "GRIDVO",
                         "GRIDFolderBase", "GRIDFolderHitmakerResults", "GRIDFolderDBAlign", "GRIDFolderApplyAlignmentResults",
                         "GRIDFolderApplyAlignmentJoboutput", "GRIDFolderApplyAlignmentHisto", "GRIDLibraryTarball",
    ...
    def doPreliminaryTestSplitting( self, i, fullCheck ):
        ...
        lfcls.wait()
        list = lfcls.fromchild.read().splitlines() 
        pattern = re.compile( "%(output)s-s%(v)06d-alignedHit.[0-9]{3}.slcio" % { "v": i, "outputPathGRID": self._dbAlignGRID, "output": self._option.output } )
        for line in list :
            if pattern.search( line ) != None:
                filenameList.append( "%(outputPathGRID)s/%(file)s" % { "outputPathGRID": self._outputPathGRID, "file": line })
        ...
        filenameList.append( "%(outputPathGRID)s/%(output)s-s%(v)06d-alignedHit-histo.root"     % { "outputPathGRID": self._histogramPathGRID,
