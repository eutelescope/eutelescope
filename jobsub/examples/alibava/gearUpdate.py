    ...
    for imd in IDModeRes:
        ...
        for i, layer in enumerate(layers):
            ...
            if(int(id) == int(imd[0])):
                ...
                ##If the result is a rotation, multiply by 57.2957795 (180/pi)
                if imd[1] in trans:
                    newVal = float(val) - float(imd[2])*57.2957795
                else:
...
if __name__ == '__main__':
    ...
    for imr in IDModeRes:
        if imr[1] in trans:
	    print "The sensor ID: ", imr[0] , " Type: ", imr[1], " Value: ", float(imr[2])*57.2957795
        else:
