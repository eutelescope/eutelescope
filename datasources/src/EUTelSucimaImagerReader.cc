...
EUTelSucimaImagerReader::EUTelSucimaImagerReader()
    : DataSourceProcessor("EUTelSucimaImagerReader") {

  _description = "Reads SUCIMA Imager ASCII data files and creates LCEvent "
                 "with TrackerRawData collection.\n"
                 "Make sure to not specify any LCIOInputFiles in the steering "
                 "in order to read SUCIMA Imager files.";
...
void EUTelSucimaImagerReader::init() { printParameters(); }
...
void EUTelSucimaImagerReader::readDataSource(int numEvents) {
  ...
  while (true) {
    ...
    if (eventNumber % 10 == 0)
      message<MESSAGE5>(log() << "Converting event " << eventNumber);
    ...
    if (isFirstEvent()) {
      ...
      runHeader->lcRunHeader()->setDescription(
          " Events read from SUCIMA Imager ASCII input file: " + _fileName);
      ...
      ProcessorMgr::instance()->processRunHeader(
          static_cast<lcio::LCRunHeader *>(lcHeader.release()));
    ...
    try {
      ...
      if ((xPixel == 0) && (yPixel == 0)) {
        ...
        // break here
        message<ERROR5>(log() << "Event " << eventNumber
                              << " finished un-expectedly. ");
        message<ERROR5>(
            log()
            << "Consider to check the input file, or limit the conversion to "
            << eventNumber - 1 << " events" << endl
            << "Sorry to quit!");
        exit(-1);
      }
    }
