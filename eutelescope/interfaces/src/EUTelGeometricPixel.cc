...
EUTelGeometricPixel::EUTelGeometricPixel()
    : EUTelGenericSparsePixel(), _posX(0), _posY(0), _boundX(0), _boundY(0) {
  _noOfElementsDerived = 8;
  _typeDerived = kEUTelGeometricPixel;
}

// Constructor taking all possible eight arguments
EUTelGeometricPixel::EUTelGeometricPixel(short xCoord, short yCoord,
                                         float signal, short time, float posX,
                                         float posY, float boundX, float boundY)
    : EUTelGenericSparsePixel(xCoord, yCoord, signal, time), _posX(posX),
      _posY(posY), _boundX(boundX), _boundY(boundY) {
  _noOfElementsDerived = 8;
  _typeDerived = kEUTelGeometricPixel;
}

// Constructor taking a EUTelGenericSparsePixel, all geometry related entries
// are set to zero
EUTelGeometricPixel::EUTelGeometricPixel(
    EUTelGenericSparsePixel const &genericPixel)
    : EUTelGenericSparsePixel(genericPixel), _posX(0), _posY(0), _boundX(0),
      _boundY(0) {
  _noOfElementsDerived = 8;
  _typeDerived = kEUTelGeometricPixel;
}

// Constructor taking a EUTelGenericSparsePixel and all the four geometry
// related parameters
EUTelGeometricPixel::EUTelGeometricPixel(
    EUTelGenericSparsePixel const &genericPixel, float posX, float posY,
    float boundX, float boundY)
    : EUTelGenericSparsePixel(genericPixel), _posX(posX), _posY(posY),
      _boundX(boundX), _boundY(boundY) {
  _noOfElementsDerived = 8;
  _typeDerived = kEUTelGeometricPixel;
}
...
void EUTelGeometricPixel::print(std::ostream &os) const {
  ...
  os << std::endl;
  int width = 20;
  os << std::setw(width) << std::setiosflags(std::ios::left)
     << "Type: " << _type << std::endl
