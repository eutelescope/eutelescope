...
EUTelGeometricClusterImpl::EUTelGeometricClusterImpl(
    IMPL::TrackerDataImpl *data)
    : EUTelGenericSparseClusterImpl<EUTelGeometricPixel>(data) {}

EUTelGeometricClusterImpl::~EUTelGeometricClusterImpl() {}

void EUTelGeometricClusterImpl::getClusterGeomInfo(float &xPos, float &yPos,
                                                   float &xSize,
                                                   float &ySize) const {
  float xMin = std::numeric_limits<float>::max(); // stores the largest possible
                                                  // value every pixel will be
                                                  // lower,
...
void EUTelGeometricClusterImpl::getGeometricCenterOfGravity(float &xCoG,
                                                            float &yCoG) const {
  xCoG = 0;
  yCoG = 0;
