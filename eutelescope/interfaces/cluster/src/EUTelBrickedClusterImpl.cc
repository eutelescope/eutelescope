...
EUTelBrickedClusterImpl::EUTelBrickedClusterImpl(TrackerDataImpl *data)
...
float EUTelBrickedClusterImpl::getDistance(
    EUTelVirtualCluster *otherCluster) const {
  ...
  return sqrt(pow(static_cast<double>(xThisSeed - xOtherSeed), 2) +
              pow(static_cast<double>(yThisSeed - yOtherSeed), 2));
}
...
float EUTelBrickedClusterImpl::getExternalRadius() const {
  ...
  // NOTE fix this if you switch the implemenatation to a variable size (size
  // aka surrounding layers):
  ...
  {
      //xSize needs to be the distance |seedPixel-outermostPixelInTheSameRow|
  now:
      xSize /= 2; //e.g. 3 becomes 1, 5 becomes 2, ...
      return sqrt( pow( static_cast<double>(xSize) , 2) + pow(
  static_cast<double>(xSize*0.5) , 2) );
  }
...
void EUTelBrickedClusterImpl::getCenterOfGravityShift(float &xCoG,
                                                      float &yCoG) const {
  //! will be okay if we set zeros for the unwanted pixels
  FloatVec vectorCopy(_trackerData->getChargeValues());
  ...
  // NOTE this whole class could use a flag showing, if even rows are skewed
  // left or right!
  // NOTE then the offset would become +/-0.5 depending on the flag, instead of
  // fixed -0.5 here!
  ...
  if (!(xSize == 3 && ySize == 3)) {
    ...
    streamlog_out(WARNING4)
        << " BRICKED PIXEL FIXED FRAME CLUSTER SIZE MUST BE 3x3!!!" << endl;
    streamlog_out(WARNING4) << " BUT IT IS " << xSize << "x" << ySize << "!!!"
                            << endl;
  }
  ...
  if (seedY % 2 == 0)
    bSeedRowIsEven = true; // seed pixel's row is even
  ...
  for (int yPixel = -1 * (ySize / 2); yPixel <= (ySize / 2); yPixel++) {
    ...
    currRowIsEven =
        (bSeedRowIsEven && (yPixel % 2 == 0)) ||
        (!bSeedRowIsEven && (yPixel % 2 != 0)); // even+even or odd+odd
    if (currRowIsEven)
      skewCorrectionX = -0.5;
    else
      skewCorrectionX = 0.0;
...
void EUTelBrickedClusterImpl::getCenterOfGravityShift(float &xCoG, float &yCoG,
                                                      int, int) const {

  //! UGLY HACK
  streamlog_out(WARNING4) << "[getCenterOfGravityShift(float& xCoG, float& "
                             "yCoG, int xSize, int ySize)] DOES NOT MAKE SENSE "
                             "FOR A BRICKED PIXEL"
                          << endl;
  streamlog_out(WARNING4) << "[getCenterOfGravityShift(float& xCoG, float& "
                             "yCoG, int xSize, int ySize)] USING THE WHOLE "
                             "FRAME TO COMPUTE THE SHIFT!"
                          << endl;
...
void EUTelBrickedClusterImpl::getCenterOfGravityShift(float &xCoG, float &yCoG,
  ...
  //! = (9-2) in the 3x3 case. Adjust this to be more generic if you implement a
  //! variable cluster size!

  //! Implementation for 3x3 only!

  if (/* (size_t) nPixel >= _trackerData->getChargeValues().size() */ (nPixel >=
                                                                       7)) {
    ...
    // BEGIN sort the n highest pixels into the highSignalPixel map (will be
    // sorted decending via index as the key, but that doesn't matter)
    // BEGIN  (the highest pixel will be put in first but that doesn't matter
    // either)
    while (iPixel != nPixel) {
    ...
    // END sort the n highest pixels into the highSignalPixel map (will be
    // sorted decending via index as the key, but that doesn't matter))
    ...
    for (int yPixel = -1 * (ySize / 2); yPixel <= (ySize / 2); yPixel++) {
      ...
      for (int xPixel = -1 * (xSize / 2); xPixel <= (xSize / 2); xPixel++) {
        ...
        if (mapIter !=
            highSignalPixel.end()) // not found in map -> mapIter == map.end()
        {
...
void EUTelBrickedClusterImpl::
    getCenterOfGravityShiftWithOutGlobalSeedCoordinateCorrection(
  ...
  // (seed_XCoordinateCorrection is: in case the seed pixel lies in an even row,
  // the cog shift will reflect an additional 0.5 offset)
  //
  // So:
  //   let the normal COG Shift (with(!) seed coordinate correction) be computed
...
void EUTelBrickedClusterImpl::
    getCenterOfGravityShiftWithOutGlobalSeedCoordinateCorrection(
        float &xCoG, float &yCoG, int nPixel) const {
...
void EUTelBrickedClusterImpl::getCenterOfGravity(float &xCoG,
                                                 float &yCoG) const {
...
float EUTelBrickedClusterImpl::getClusterCharge(int nPixel) const {
  ...
  //! HACK TAKI, for an explanation see getCenterOfGravityShift(float& xCoG,
  //! float& yCoG, int nPixel)
  if (/*(size_t) nPixel >= vectorCopy.size()*/ nPixel >= 7)
...
std::vector<float>
EUTelBrickedClusterImpl::getClusterCharge(std::vector<int> nPixels) const {
...
void EUTelBrickedClusterImpl::setNoiseValues(std::vector<float> noiseValues) {
  ...
  if (noiseValues.size() != _trackerData->getChargeValues().size()) {
    _noiseSetSwitch = false;
    streamlog_out(ERROR2) << "[EUTelBrickedClusterImpl::setNoiseValues()] The "
                             "noiseValues size is different from the "
                             "TrackerData size!!!"
                          << endl;
    throw IncompatibleDataSetException(
        "[EUTelBrickedClusterImpl::setNoiseValues()] The noiseValues size is "
        "different from the TrackerData size");
  }

  _noiseValues = noiseValues;
  setOutsiderValuesInVectorInterpretedAsBrickedMatrix(_noiseValues,
                                                      0.0f); //! HACK TAKI
...
vector<float> EUTelBrickedClusterImpl::getNoiseValues() const {
  ...
  if (!_noiseSetSwitch)
    throw DataNotAvailableException("No noise values set");
  return _noiseValues;
}
...
float EUTelBrickedClusterImpl::getClusterNoise() const {
  ...
  float squaredSum = 0; //! will be okay if we set zeros @ noise for the
                        //! unwanted pixels (which we did)
...
float EUTelBrickedClusterImpl::getClusterSNR() const {
  ...
  if (!_noiseSetSwitch)
    throw DataNotAvailableException("No noise values set");
  float clusterNoise = getClusterNoise(); //! will be okay if we set zeros @
                                          //! noise for the unwanted pixels!!!
                                          //! (which we did)
  if (clusterNoise == 0)
    return 0.;
  float clusterSignal = getTotalCharge(); //! will be okay if we set zeros @
                                          //! charge for the unwanted pixels!!!
                                          //! (which we do in getTotalCharge() )
...
float EUTelBrickedClusterImpl::getSeedSNR() const {
  ...
  if (!_noiseSetSwitch)
    throw DataNotAvailableException("No noise values set");
  vector<float>::const_iterator chargeBegin =
      _trackerData->getChargeValues().begin();
  vector<float>::const_iterator seedChargeIter =
      max_element(chargeBegin, _trackerData->getChargeValues().end());
  vector<float>::const_iterator seedNoiseIter =
      _noiseValues.begin() + (seedChargeIter - chargeBegin);
  if (*seedNoiseIter == 0.0) {
    streamlog_out(ERROR4) << "[getSeedSNR()] Just found a noise value == 0.0 !"
                          << endl;
    return 0;
  }
...
float EUTelBrickedClusterImpl::getClusterSNR(int nPixel) const {
  ...
  if (/*(size_t) nPixel >= _trackerData->getChargeValues().size()*/ nPixel >=
      7) //! HACK TAKI, for an explanation see getCenterOfGravityShift(float&
         //! xCoG, float& yCoG, int nPixel)
...
std::vector<float>
EUTelBrickedClusterImpl::getClusterSNR(std::vector<int> nPixels) const {
...
#define USE_MULTIMAP
  ...
  streamlog_out(MESSAGE2)
      << "[getClusterSNR( std::vector<int > nPixels )] THE CLUSTER:" << endl;
  ...
  int index = 0;
  while (iter != vectorCopy.end()) {
    clusterSignalMap.insert(
        make_pair((*iter), index)); // sorted by charge value, ascending
#ifdef DEBUG_OUTPUT_FOR_getClusterSNR_ON_INT_VECTOR
    streamlog_out(MESSAGE2) << "Adding a pixel to the map! Value = " << (*iter)
                            << " Index = " << index << endl;
#endif
    ++index;
    ++iter;
...
#ifdef DEBUG_OUTPUT_FOR_getClusterSNR_ON_INT_VECTOR
  streamlog_out(MESSAGE2)
      << "[getClusterSNR( std::vector<int > nPixels )] SORTED CHARGES:" << endl;
#ifdef USE_MULTIMAP
  multimap<float, int>::reverse_iterator mapIterTmp =
      clusterSignalMap.rbegin(); // walking down descending!
#else
  map<float, int>::reverse_iterator mapIterTmp =
      clusterSignalMap.rbegin(); // walking down descending!
#endif
  while (mapIterTmp != clusterSignalMap.rend()) {
    streamlog_out(MESSAGE2) << "Value:" << mapIterTmp->first
                            << ", Index:" << mapIterTmp->second << endl;
    ++mapIterTmp;
  }
#endif
  ...
  while (pixelIter != nPixels.end()) {
    if (/*(size_t) (*pixelIter) >= _trackerData->getChargeValues().size()*/ (
            *pixelIter) >= 7) //! 7 because this is the maximum amount of
...
#ifdef USE_MULTIMAP
      multimap<float, int>::reverse_iterator mapIter =
          clusterSignalMap.rbegin(); // walking down descending!
#else
      map<float, int>::reverse_iterator mapIter =
          clusterSignalMap.rbegin(); // walking down descending!
#endif
      float signal = 0;
      float noise2 = 0;
      int iPixel = 0;
      while ((iPixel < (*pixelIter)) &&
             (mapIter !=
              clusterSignalMap
                  .rend())) // NOTE: mapIter should never come close to rend()!!
      {                     // rend() = 'begin()-1'
        signal += mapIter->first; // note the difference between rbegin and
                                  // begin
        noise2 += pow(_noiseValues[mapIter->second], 2);

        //! check, if we accidentally picked the fake strong negative values!
        //! (should not happen though!)
        if ((mapIter->first) < -10.0) {
          streamlog_out(ERROR4)
              << endl
              << "[getClusterSNR( std::vector<int > nPixels )] Might have "
                 "reached the outsider pixels!!!"
              << endl;
          streamlog_out(ERROR4)
              << "  Just found a signal value < -10.0 !!! VERY BAD !!!" << endl;
          streamlog_out(ERROR4) << "  Was looking for SNR with " << (*pixelIter)
                                << " MSP this time." << endl;
          streamlog_out(ERROR4) << "  Was working on the " << (iPixel) + 1
                                << "th MSP. With index " << (mapIter->second)
                                << endl;
          streamlog_out(ERROR4) << "  Signal for this pixel is "
                                << mapIter->first
                                << ", Signal values are:" << endl;
          outputBricked3x3MatrixFromVector(vectorCopy);
          int xSeed, ySeed;
          getSeedCoord(xSeed, ySeed);
          streamlog_out(ERROR4) << "  Seed Coord: x=" << xSeed
                                << ", y=" << ySeed << "." << endl;
        ...
        if (_noiseValues[mapIter->second] == 0.0) {
          streamlog_out(ERROR4)
              << endl
              << "[getClusterSNR( std::vector<int > nPixels )] Possible error "
                 "in noise map or pixel status!"
              << endl;
          streamlog_out(ERROR4) << "  Just found a noise value == 0.0 !"
                                << endl;
          streamlog_out(ERROR4) << "  Was looking for SNR with " << (*pixelIter)
                                << " MSP this time." << endl;
          streamlog_out(ERROR4) << "  Was working on the " << (iPixel) + 1
                                << "th MSP. With index " << (mapIter->second)
                                << endl;
          streamlog_out(ERROR4) << "  Noise for this pixel is "
                                << _noiseValues[mapIter->second]
                                << ", Noise values are:" << endl;
        ...
        //! check, if we accidentally picked the fake strong negative values!
        //! (should not happen though!)
        if (mapIter == clusterSignalMap.rend()) // this means that the whole map
                                                // has been worked on. the map
                                                // should always be 9 in size,
                                                // and it should only be used up
                                                // to 7 pixels!
        {
          streamlog_out(ERROR4)
              << endl
              << "[getClusterSNR( std::vector<int > nPixels )] !!! reached the "
                 "outsider pixels we wanted to ignore !!! VERY BAD !!!"
              << endl;
        }
      }
      if (noise2 == 0)
        ...
        snr.push_back(signal / sqrt(noise2));
    }
    ++pixelIter;
  }
  return snr;
...
float EUTelBrickedClusterImpl::getClusterSNR(int xSize, int ySize) const {
  ...
  if (!(xSize == 1 &&
        ySize == 1)) // NOTE adjust this if you implement a variable size
  {
    // throw
    // IncompatibleDataSetException("[EUTelBrickedClusterImpl::getClusterSNR(int
    // xSize, int ySize)] This is not really applicable within a bricked pixel
    // structure.");
    streamlog_out(WARNING2)
        << "[getClusterSNR( int x, int y )] Not applicable for a "
           "brickedCluster. Will return whole cluster's SNR!"
        << endl;
    return getClusterSNR();
  }
  return getSeedSNR();
...
float EUTelBrickedClusterImpl::getClusterCharge(int xSize, int ySize) const {
  ...
  {
    // throw
    // IncompatibleDataSetException("[EUTelBrickedClusterImpl::getClusterCharge(int
    // xSize, int ySize)] This is not really applicable within a bricked pixel
    // structure.");
    streamlog_out(WARNING2)
        << "[getClusterCharge( int x, int y )] Not applicable for a "
           "brickedCluster. Will return whole cluster's charge!"
        << endl;
    getTotalCharge();
  }
...
void EUTelBrickedClusterImpl::print(std::ostream &os) const {
  ...
  float xShift, yShift, xShift2, yShift2, xShift3,
      yShift3; // xShift3x3, yShift3x3
  ClusterQuality quality = getClusterQuality();
  getClusterSize(xSize, ySize);
  if (xSize != 3 || ySize != 3) {
    streamlog_out(ERROR2) << "[EUTelBrickedClusterImpl::print(std::ostream& os "
                             ")] Wrong Cluster Size!!!"
                          << endl;
  ...
  os << setw(bigspacer) << setiosflags(ios::left) << "Bricked pixel cluster "
     << "( one surrounding layer of neighbour pixels )\n"
     // NOTE adjust this if you implement a variable size
     << setw(bigspacer) << "Cluster quality: " << quality << "\n"
     << setw(bigspacer) << "Seed coords: "
     << "x=" << xSeed << ", y=" << ySeed << "\n"
     << setw(bigspacer) << "Seed charge: " << getSeedCharge() << " in ("
     << xSeed << ", " << ySeed << ")\n"
     ...
     //"\n"
     << setw(bigspacer) << "CoG shift FULL: "
     << "(" << xShift << ", " << yShift << ")\n"
     << setw(bigspacer) << "CoG shift 2MSP: "
     << "(" << xShift2 << ", " << yShift2 << ")\n"
     << setw(bigspacer) << "CoG shift 3MSP: "
     << "(" << xShift3 << ", " << yShift3
     << ")\n"; //<<  setw(bigspacer) <<  "CoG(3x3) shift " << "(" << xShift3x3
               //<< ", " << yShift3x3 << ")\n" ;
...
void EUTelBrickedClusterImpl::
  ...
  //     ...and _even rows_ being skewed to the left (aka their coordinates have
  //     to become 0.5 smaller!)
  ...
  //     Anyway the code down there can be used for such a variable
  //     implementation.
  //     It is very well suited to be adapted a little bit more.
  //     Ee should assume a square instead of a rectangle as well because
  //     everything would become
  //     even more complicated otherwise! =/

  //! General approach would be:
  //- with each row further away from the seed pixel row consider one pixel less
  //in x direction!
  //- only in odd rows away from seed pixel (  |yRowCurrent-yRowSeedpixel|=2n+1
  //) you have
  //  to consider on which side to drop a pixel.
  //- in even rows away from seed pixel there is an uneven number of pixels to
  //use
  ...
  // ...and even rows being skewed to the left (aka their coordinates have to
  // become 0.5 smaller!)
  ...
  if (!(xSize == 3 && ySize == 3)) {
    streamlog_out(WARNING4)
        << "EUTelBrickedClusterImpl::"
           "setOutsiderValuesInVectorInterpretedAsBrickedMatrix(FloatVec& v):"
        << endl;
    streamlog_out(WARNING4)
        << " BRICKED PIXEL FIXED FRAME CLUSTER SIZE MUST BE 3x3!!!" << endl;
    streamlog_out(WARNING4) << " BUT IT IS " << xSize << "x" << ySize << "!!!"
  ...
  if (!(static_cast<size_t>(xSize * ySize) == v.size())) {
    streamlog_out(WARNING4)
        << "EUTelBrickedClusterImpl::"
           "setOutsiderValuesInVectorInterpretedAsBrickedMatrix(FloatVec& v):"
        << endl;
    streamlog_out(WARNING4) << " BRICKED PIXEL FIXED FRAME CLUSTER SIZE DOES "
                               "NOT MATCH VECTOR SIZE!!!"
                            << endl;
    streamlog_out(WARNING4) << " FIXED FRAME: " << xSize << "x" << ySize
                            << ", VECTOR: " << v.size() << "." << endl;
  }
  ...
  for (int y = (-1) * ySize / 2; y <= ySize / 2; ++y) {
    if (y != 0) // only if we are not looking at the middle row: write zeros to
                // the two pixels we don't want.
    { // this will be a little bit more complicated in a variable approach
      //(have to take care of rows with even and odd distances of the seed pixel
      //row, etc.)
      if (bSeedRowIsEven) // seed pixel's row is even:
      {                   // delete one pixel in this row, on the right side
        startZerosX = (xSize / 2);
        endZerosX = startZerosX; // this will be a little bit more complicated
                                 // in a variable approach
...
void EUTelBrickedClusterImpl::debugOutput() const {
  ...
  int xSize, ySize;
  getClusterSize(xSize, ySize);
  streamlog_out(MESSAGE2) << "    Size: x=" << xSize << ", y= " << ySize
                          << endl;

  int xSeed, ySeed;
  getSeedCoord(xSeed, ySeed);
  streamlog_out(MESSAGE2) << "    Seed: x=" << xSeed << ", y= " << ySeed
                          << endl;

  //! CHARGE MATRIX
  FloatVec vectorCopy(_trackerData->getChargeValues());
  // setOutsiderValuesInVectorInterpretedAsBrickedMatrix( static_cast<
  // std::vector< float>& > (vectorCopy), 777.0f );
  ...
  streamlog_out(MESSAGE2) << "    MSP2  Charge: " << getClusterCharge(2)
                          << endl;
  streamlog_out(MESSAGE2) << "    MSP3  Charge: " << getClusterCharge(3)
                          << endl;
  streamlog_out(MESSAGE2) << "    MSP6  Charge: " << getClusterCharge(6)
                          << endl;
  streamlog_out(MESSAGE2) << "    MSP7  Charge: " << getClusterCharge(7)
                          << endl;
  ...
  float xShift, yShift;
  getCenterOfGravityShift(xShift, yShift);
  streamlog_out(MESSAGE2) << "  = CoG Shift Global Full: x=" << xShift
                          << ", y= " << yShift << endl;

  getCenterOfGravityShift(xShift, yShift, 2);
  streamlog_out(MESSAGE2) << "    CoG Shift Global 2MSP: x=" << xShift
                          << ", y= " << yShift << endl;

  getCenterOfGravityShift(xShift, yShift, 3);
  streamlog_out(MESSAGE2) << "    CoG Shift Global 3MSP: x=" << xShift
                          << ", y= " << yShift << endl;

  getCenterOfGravityShiftWithOutGlobalSeedCoordinateCorrection(xShift, yShift);
  streamlog_out(MESSAGE2) << "    CoG Shift ETA    FULL: x=" << xShift
                          << ", y= " << yShift << endl;
  ...
  if (_noiseSetSwitch) {
    ...
    streamlog_out(MESSAGE2) << "    ClusterSNR 2MSPv: " << fourMspSNRs[0]
                            << endl;
    streamlog_out(MESSAGE2) << "    ClusterSNR 3MSPv: " << fourMspSNRs[1]
                            << endl;
    streamlog_out(MESSAGE2) << "    ClusterSNR 6MSPv: " << fourMspSNRs[2]
                            << endl;
    streamlog_out(MESSAGE2) << "    ClusterSNR 7MSPv: " << fourMspSNRs[3]
                            << endl;
  } else {
...
void EUTelBrickedClusterImpl::outputBricked3x3MatrixFromVector(
  ...
  *    .so the seedRow (and other even rows if the cluster is bigger than just
  * one surroundling layer, aka 3x3)
  *     is virtually shifted left by 0.5 because that is the case in reality as
  * well.
  *    .for this output we can only shift stuff right tough. so we shift the
  * other (odd) rows right
  *     and leave the rest (including the pixel coordinate "header" of our small
  * table in place)
  *
  * b) seedRow is odd:
  *    .even Rows are substracted 0.5 from their x pixel coordinate number as
  * well
  *    .so odd rows are the refernce here as well
  *    .so the (other!) even rows next to the seed row are
  *     virtually shifted left by 0.5 because that is the case in reality as
  * well.
  *    .for this output we can only shift stuff right tough. so we shift the
  * seedRow right
  *     and leave the rest (including the pixel coordinate "header" of our small
  ...
  */
  ...
  if (v.size() != 9) {
    streamlog_out(ERROR2) << "    "
                             "[outputBricked3x3MatrixFromVector(std::vector<"
                             "float>& v)] Wrong vectorSize!!!"
                          << endl;
    return;
  }
  ...
  if (ySeed % 2 == 0) {
    streamlog_out(ERROR4) << "| "
                          ...
                          << setw(13) << v[0] << "," << setw(13) << v[1] << ","
                          << "(" << setw(11) << v[2] << ")" << endl
                          << "|" << endl;

    streamlog_out(ERROR4) << "| "
                          << "y= " << setw(4) << ySeed - 0 << " |" << setw(13)
                          << v[3] << "," << setw(13) << v[4] << "," << setw(13)
                          << v[5] << endl
                          << "|" << endl;
    ...
    streamlog_out(ERROR4) << "| "
                          << "y= " << setw(4) << ySeed + 1 << " |"
                          << "      " //! EXTRA SPACE HERE (THE SKEW)
                          << setw(13) << v[6] << "," << setw(13) << v[7] << ","
                          << "(" << setw(11) << v[8] << ")";
  } else {
    streamlog_out(ERROR4) << "| "
                          << "y= " << setw(4) << ySeed - 1 << " |"
                          << "(" << setw(11) << v[0] << ")," << setw(13) << v[1]
                          << "," << setw(13) << v[2] << endl
                          << "|" << endl;
    ...
    streamlog_out(ERROR4) << "| "
                          << "y= " << setw(4) << ySeed + 0 << " |"
                          << "      " //! EXTRA SPACE HERE (THE SKEW)
                          << setw(13) << v[3] << "," << setw(13) << v[4] << ","
                          << setw(13) << v[5] << endl
                          << "|" << endl;

    streamlog_out(ERROR4) << "| "
                          << "y= " << setw(4) << ySeed + 1 << " |"
                          << "(" << setw(11) << v[6] << ")," << setw(13) << v[7]
                          << "," << setw(13) << v[8];
  }
