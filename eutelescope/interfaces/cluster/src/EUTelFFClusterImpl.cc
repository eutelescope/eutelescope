...
EUTelFFClusterImpl::EUTelFFClusterImpl(TrackerDataImpl *data)
...
float EUTelFFClusterImpl::getExternalRadius() const {
  ...
  return 0.5 * sqrt(pow(static_cast<double>(xSize), 2) +
                    pow(static_cast<double>(ySize), 2));
}
...
void EUTelFFClusterImpl::getCenterOfGravityShift(float &xCoG,
                                                 float &yCoG) const {
  ...
  for (int yPixel = -1 * (ySize / 2); yPixel <= (ySize / 2); yPixel++) {
    for (int xPixel = -1 * (xSize / 2); xPixel <= (xSize / 2); xPixel++) {
      if (_trackerData->getChargeValues().size() <= iPixel)
        break;
  ...
  //  printf("%5.2f %5.2f  %5.2f %5.2f  norm=%5.2f\n", xCoG, yCoG, tempX,tempY,
  //  normalization);
}

void EUTelFFClusterImpl::getCenterOfGravityShift(float &xCoG, float &yCoG,
...
float EUTelFFClusterImpl::getClusterCharge(int nPixel) const {
  ...
  if (static_cast<size_t>(nPixel) >= vectorCopy.size())
...
std::vector<float>
EUTelFFClusterImpl::getClusterCharge(std::vector<int> nPixels) const {
...
void EUTelFFClusterImpl::setNoiseValues(std::vector<float> noiseValues) {
  ...
  if (noiseValues.size() != _trackerData->getChargeValues().size()) {
    ...
    throw IncompatibleDataSetException(
        "The noiseValues size is different from the TrackerData size");
  }
...
std::vector<float>
EUTelFFClusterImpl::getClusterSNR(std::vector<int> nPixels) const {
...
void EUTelFFClusterImpl::print(std::ostream &os) const {
  ...
  os << setw(bigspacer) << setiosflags(ios::left) << "Fixed frame cluster "
     << "(" << xSize << ", " << ySize << ")\n"
     ...
     << "\n"
     << setw(bigspacer) << "Seed charge " << getSeedCharge() << " in (" << xSeed
     << ", " << ySeed << ")\n"
     << setw(bigspacer) << "CoG shift "
     << "(" << xShift << ", " << yShift << ")\n"
     << setw(bigspacer) << "CoG(9) shift "
     << "(" << xShift9 << ", " << yShift9 << ")\n"
     << setw(bigspacer) << "CoG(3x3) shift "
     << "(" << xShift3x3 << ", " << yShift3x3 << ")\n";
  if (_noiseSetSwitch) {
