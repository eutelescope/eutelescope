...
namespace eutelescope {
  namespace geo {
    ...
    GEARPixGeoDescr::GEARPixGeoDescr(int xPixel, int yPixel, double xSize,
                                  ...
                                  radLength)                    // rad length
    {
      ...
      plane = _tGeoManager->MakeBox("sensarea_gen", Si, xSize / 2., ySize / 2.,
                                    zSize / 2.);
    ...
    std::string GEARPixGeoDescr::getPixName(int x, int y) {
      ...
      snprintf(buffer, 100, "/sensarea_gen_1/genrow_%d/genpixel_%d", x + 1,
               y + 1);
      return std::string(buffer);
    }
    /*TODO*/ std::pair<int, int> GEARPixGeoDescr::getPixIndex(char const *) {
