...
void EUTelGenericPixGeoMgr::addCastedPlane(int planeID, int xPixel, int yPixel,
  ...
  if (it != _castedDescriptions.end()) {
    ...
    streamlog_out(MESSAGE3) << "Didnt find " << name << " yet, thus creating"
                            << std::endl;
    pixgeodescrptr =
        new GEARPixGeoDescr(xPixel, yPixel, xSize, ySize, zSize, radLength);
    streamlog_out(MESSAGE3) << "Inserting " << name << " into map" << std::endl;
    _geoDescriptions.insert(std::make_pair(planeID, pixgeodescrptr));
    _castedDescriptions.insert(std::make_pair(name, pixgeodescrptr));
  }

  streamlog_out(MESSAGE3) << "Adding plane: " << planeID
...
void EUTelGenericPixGeoMgr::addPlane(int planeID, std::string geoName,
  ...
  if (it != _pixelDescriptions.end()) {
    ...
    pixgeodescrptr = (*it).second;
    streamlog_out(MESSAGE3) << "Inserting " << planeID << " into map"
                            << std::endl;
    _geoDescriptions.insert(std::make_pair(planeID, pixgeodescrptr));
  }
  // Otherwise we load it
  else {
    streamlog_out(MESSAGE3) << "Didnt find " << geoName << " yet, thus creating"
                            << std::endl;
    ...
    try {
      ...
      streamlog_out(ERROR3)
          << "Could not retrieve pointer to EUTelGenericPixGeoDescr object, "
             "did you include a maker() function w/o C++ name mangling?"
          << std::endl;
      // While runtime errors are not processed in Marlin, they will cause a
      // controlled crash!
...
EUTelGenericPixGeoDescr *EUTelGenericPixGeoMgr::getPixGeoDescr(int planeID) {
  std::map<int, EUTelGenericPixGeoDescr *>::iterator returnGeoDescrIt =
      _geoDescriptions.find(planeID);
  if (returnGeoDescrIt == _geoDescriptions.end()) {
    std::stringstream ss;
    ss << "Could not retrieve plane: " << planeID
