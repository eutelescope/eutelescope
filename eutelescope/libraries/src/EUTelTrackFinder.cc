...
namespace eutelescope {
  ...
  EUTelTrackFinder::EUTelTrackFinder()
      : _isReady(kIsNotReady), _searchResult(kFailed),
        _name("DefaultTrackFinder"), _allHits(), _trackCandidates() {}

  EUTelTrackFinder::EUTelTrackFinder(std::string name)
      : _isReady(kIsNotReady), _searchResult(kFailed), _name(name), _allHits(),
        _trackCandidates() {}

  EUTelTrackFinder::~EUTelTrackFinder() {}

  std::vector<EVENT::TrackerHitVec>
  EUTelTrackFinder::GetTrackCandidates() const {

    if (!_isReady) {
      streamlog_out(ERROR) << "Track finder " << _name << std::endl;
      streamlog_out(ERROR) << "Tracks were not searched. "
                           << "Initialise track finder by supplying hits first"
                           << std::endl;
      streamlog_out(ERROR) << "Returning empty list of track candidates"
                           << std::endl;
    }
    ...
    if (_isReady && _searchResult == kFailed) {
      ...
      streamlog_out(MESSAGE)
          << "Failed. Returning empty list of track candidates" << std::endl;
    }

    return _trackCandidates;
  }

  void EUTelTrackFinder::SetAllHits(
      const std::vector<EVENT::TrackerHitVec> &allHits) {
