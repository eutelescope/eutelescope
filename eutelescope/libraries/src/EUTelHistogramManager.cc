...
bool EUTelHistogramManager::init() {
  ...
  if (doc->LoadFile(_histoInfoFileName)) {
    ...
    if (!pElem) {
      delete doc;
      throw ParseException(string("EUTelHistogramManager::init: no root tag "
    ...
    if (!nHistosBlock) {
      delete doc;
      throw ParseException(string("EUTelHistogramManager::init: no <histos> "
                                  "... </histos> block found in ") +
                           _histoInfoFileName);
    }

    TiXmlElement *pHistoNode =
        hRoot.FirstChild("histos").FirstChild("histo").Element();
    while (pHistoNode) {
...
EUTelHistogramInfo *
EUTelHistogramManager::getHistogramInfo(std::string histoName) const {
