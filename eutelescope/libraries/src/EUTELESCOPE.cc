...
const char *EUTELESCOPE::MATRIXDEFAULTENCODING =
    "sensorID:7,xMin:12,xMax:12,yMin:12,yMax:12";
const char *EUTELESCOPE::CLUSTERDEFAULTENCODING =
    "sensorID:7,xSeed:12,ySeed:12,xCluSize:5,yCluSize:5,quality:7";

const char *EUTELESCOPE::PULSEDEFAULTENCODING =
    "sensorID:7,xSeed:12,ySeed:12,xCluSize:5,yCluSize:5,type:5,quality:5";
const char *EUTELESCOPE::ZSDATADEFAULTENCODING = "sensorID:7,sparsePixelType:5";
const char *EUTELESCOPE::ZSCLUSTERDEFAULTENCODING =
...
namespace eutelescope {
  ...
  std::ostream &operator<<(std::ostream &os, const SparsePixelType &type) {
    if (type == kEUTelBaseSparsePixel)
      os << "kEUTelBaseSparsePixel";
    else if (type == kEUTelSimpleSparsePixel)
      os << "kEUTelSimpleSparsePixel";
    else if (type == kEUTelGenericSparsePixel)
      os << "kEUTelGenericSparsePixel";
    else if (type == kEUTelGeometricPixel)
      os << "kEUTelGeometricPixel";
    // add here your type
    else if (type == kUnknownPixelType)
      os << "kUnknownPixelType";
  ...
  std::string escape(const std::string &inputString) {
    ...
    for (size_t iPos = 0; iPos < inputString.length(); ++iPos) {
      if (inputString[iPos] == '\\')
        ret << "\\\\";
      else if (inputString[iPos] < 32)
        ret << "\\x" << std::setw(2) << int(inputString[iPos]);
      else
        ret << inputString[iPos];
    }
    return ret.str();
  }
