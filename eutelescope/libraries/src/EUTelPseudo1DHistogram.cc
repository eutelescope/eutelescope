...
EUTelPseudo1DHistogram::EUTelPseudo1DHistogram(int NOfBins, double min,
                                               double max) {
...
void EUTelPseudo1DHistogram::fill(double x, double w) {
  ...
  if (x < _MinValue) {
    ...
    int index = static_cast<int>(floor(((static_cast<double>(_NumberOfBins)) /
                                        (_MaxValue - _MinValue)) *
                                       (x - _MinValue))) +
                1;
    ++_NOfEntries[index];
...
int EUTelPseudo1DHistogram::findBin(double x) {
  ...
  if (x < _MinValue) {
    ...
    return static_cast<int>(floor(((static_cast<double>(_NumberOfBins)) /
                                   (_MaxValue - _MinValue)) *
                                  (x - _MinValue))) +
           1;
  }
...
double EUTelPseudo1DHistogram::getBinContent(int bin) {
  ...
  if (isInRange(bin)) {
    ...
    std::cout << "Requested bin not in range of the 'EUTelPseudo1DHistogram'."
              << std::endl;
    return -1.0;
  }
}
...
double EUTelPseudo1DHistogram::integral(int startbin, int endbin) {
  ...
  if (isInRange(startbin) && isInRange(endbin)) {
    ...
    std::cout << "At least one requested bin is not in range of the "
...
void EUTelPseudo1DHistogram::printContent() {
  ...
  for (int i = 0; i < _FullNumberOfBins; ++i) {
    ...
    if (i == 0) {
      ...
      std::cout << "[" << _UpperIntervalLimit[i - 1] << ","
                << _UpperIntervalLimit[i] << "]" << std::endl;
    }
  }
}
...
int EUTelPseudo1DHistogram::getNumberOfBins() { return _NumberOfBins; }
