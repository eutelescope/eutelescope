...
EUTelRunHeaderImpl::EUTelRunHeaderImpl(const EUTelRunHeaderImpl &z)
  ...
  std::vector<std::string> *activesubdetectors =
      const_cast<std::vector<std::string> *>(
          zlcrunheader->getActiveSubdetectors());
  for (std::vector<std::string>::iterator it = activesubdetectors->begin();
       it != activesubdetectors->end(); ++it) {
...
EUTelRunHeaderImpl &EUTelRunHeaderImpl::operator=(const EUTelRunHeaderImpl &z) {
  if (this == &z)
    return *this; // This handles self assignment
  IMPL::LCRunHeaderImpl *zlcrunheader =
      const_cast<EUTelRunHeaderImpl &>(z).lcRunHeader();
  _lcHeader->setRunNumber(zlcrunheader->getRunNumber());
  _lcHeader->setDetectorName(zlcrunheader->getDetectorName());
...
void EUTelRunHeaderImpl::addProcessor(std::string processor) {
  ...
  _lcHeader->parameters().getStringVals(EUTELESCOPE::APPLIEDPROCESSOR,
                                        processorVec);
  processorVec.push_back(processor);
  _lcHeader->parameters().setValues(EUTELESCOPE::APPLIEDPROCESSOR,
                                    processorVec);
}
