...
/**Defaut constructor, we use 1 int to store the sensorID and 12 doubles for the
 * 3 offsets and 3
 * angles and their corresponding uncertainties */
EUTelAlignmentConstant::EUTelAlignmentConstant()
    : IMPL::LCGenericObjectImpl(1, 0, 12) {
  _typeName = "Alignment constant";
  _dataDescription = "sensorID, xOff, yOff, zOff, alpha, beta, gamma, xErr, "
                     "yErr, zErr, alphaErr, betaErr, gammaErr";
  _isFixedSize = true;
}

EUTelAlignmentConstant::EUTelAlignmentConstant(
    int sensorID, double xOff, double yOff, double zOff, double alpha,
    double beta, double gamma, double xOffErr, double yOffErr, double zOffErr,
...
void EUTelAlignmentConstant::setXOffsetError(double err) {
  setDoubleVal(6, err);
}
void EUTelAlignmentConstant::setYOffsetError(double err) {
  setDoubleVal(7, err);
}
void EUTelAlignmentConstant::setZOffsetError(double err) {
  setDoubleVal(8, err);
}
...
void EUTelAlignmentConstant::setGammaError(double err) {
  setDoubleVal(11, err);
}
...
double EUTelAlignmentConstant::getXOffsetError() const {
  return getDoubleVal(6);
}
double EUTelAlignmentConstant::getYOffsetError() const {
  return getDoubleVal(7);
}
double EUTelAlignmentConstant::getZOffsetError() const {
...
double EUTelAlignmentConstant::getGammaError() const {
...
void EUTelAlignmentConstant::print(ostream &os) const {
  ...
  os << dashline << endl
     << setw(lineWidth - narrowWidth) << setiosflags(ios::left) << "Sensor ID "
     << resetiosflags(ios::left) << setw(narrowWidth) << setiosflags(ios::right)
     << getSensorID() << resetiosflags(ios::right) << endl
     << dashline << endl
     << setw(largeWidth) << " X off [mm] " << setw(largeWidth) << " Y off [mm] "
     << setw(largeWidth) << " Z off [mm] " << setw(largeWidth) << " ZY angle "
     << setw(largeWidth) << " ZX angle " << setw(largeWidth) << " XY angle "
     << endl
     << setw(largeWidth) << getXOffset() << setw(largeWidth) << getYOffset()
     << setw(largeWidth) << getZOffset() << setw(largeWidth) << getAlpha()
     << setw(largeWidth) << getBeta() << setw(largeWidth) << getGamma() << endl
     << setw(largeWidth) << getXOffsetError() << setw(largeWidth)
     << getYOffsetError() << setw(largeWidth) << getZOffsetError()
     << setw(largeWidth) << getAlphaError() << setw(largeWidth)
     << getBetaError() << setw(largeWidth) << getGammaError() << endl
     << dashline << endl;
  return;
}
