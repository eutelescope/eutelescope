...
bool AnyOption::alloc() {
  ...
  optionchars =
      static_cast<char *>(malloc((max_char_options + 1) * sizeof(char)));
  optchartype =
      static_cast<int *>(malloc((max_char_options + 1) * sizeof(int)));
  optcharindex =
      static_cast<int *>(malloc((max_char_options + 1) * sizeof(int)));
...
bool AnyOption::doubleOptStorage() {
  ...
  tmp_options = static_cast<const char **>(
      realloc(options, ((2 * max_options) + 1) * sizeof(const char *)));
  tmp_optiontype = static_cast<int *>(
      realloc(optiontype, ((2 * max_options) + 1) * sizeof(int)));
  tmp_optionindex = static_cast<int *>(
      realloc(optionindex, ((2 * max_options) + 1) * sizeof(int)));
  if (tmp_options == nullptr || tmp_optiontype == nullptr || tmp_optionindex == nullptr)
    return false;
...
bool AnyOption::doubleCharStorage() {
  ...
  tmp_optionchars = static_cast<char *>(
      realloc(optionchars, ((2 * max_char_options) + 1) * sizeof(char)));
  tmp_optchartype = static_cast<int *>(
      realloc(optchartype, ((2 * max_char_options) + 1) * sizeof(int)));
  tmp_optcharindex = static_cast<int *>(
      realloc(optcharindex, ((2 * max_char_options) + 1) * sizeof(int)));
  if (tmp_optionchars == nullptr || tmp_optchartype == nullptr ||
      tmp_optcharindex == nullptr)
    return false;
  // realloc worked, reassign pointers
...
bool AnyOption::doubleUsageStorage() {
  ...
  tmp_usage = static_cast<const char **>(
      realloc(usage, ((2 * max_usage_lines) + 1) * sizeof(const char *)));
  if (tmp_usage == nullptr)
    return false;
  else
    usage = tmp_usage;
  ...
  for (int i = 0; i < option_counter; i++) {
    if (strcmp(options[i], option) == 0) {
      values[optionindex[i]] =
          static_cast<char *>(malloc((strlen(value) + 1) * sizeof(char)));
...
bool AnyOption::setFlagOn(const char *option) {
  ...
  for (int i = 0; i < option_counter; i++) {
    if (strcmp(options[i], option) == 0) {
      values[optionindex[i]] =
          static_cast<char *>(malloc((strlen(TRUE_FLAG) + 1) * sizeof(char)));
