...
namespace eutelescope {
  ...
  EUTelTrackFitter::EUTelTrackFitter()
    ...
    std::cout << "default EUTelTrackFitter::EUTelTrackFitter "
              << " with " << _LCIO_fittrackvec->size() << " tracks "
              << std::endl;
  }
  /*
  ...
  */
  EUTelTrackFitter::EUTelTrackFitter(std::string name)
    ...
    std::cout << " normal EUTelTrackFitter::EUTelTrackFitter " << name
              << " with " << _LCIO_fittrackvec->size() << " tracks "
              << std::endl;
  }
  ...
  void EUTelTrackFitter::SetTrackCandidates(const EVENT::TrackVec &) { return; }

  void
  EUTelTrackFitter::SetTrackCandidates(std::vector<const IMPL::TrackImpl *> &) {
    return;
  }
  ...
  void EUTelTrackFitter::Clear() {
    ...
    if (getFitHitVec() == nullptr)
      try {
        ...
        streamlog_out(WARNING2)
            << "Can't allocate output _LCIO_fittracksvec collection"
            << std::endl;
      }
    else
    ...
    if (getFitTrackVec() == nullptr)
      try {
        ...
        streamlog_out(WARNING2)
            << "Can't allocate output _LCIO_fithitvec collection" << std::endl;
      }
    else
  ...
  void EUTelTrackFitter::FitTracks() { return; }

  void EUTelTrackFitter::TrackCandidatesToGBLTrajectories() { return; }

  bool EUTelTrackFitter::PerformMille() { return false; }

  void EUTelTrackFitter::PerformFitGBLTrajectories() { return; }

  void EUTelTrackFitter::FitSingleTrackCandidate() { return; }
}
