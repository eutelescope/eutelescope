...
/** Helper classes to represent the geometry reflected in the GEAR file in EUTelescope
 *	The description is based on layers and planes. A plane can either be an active or passive 
 *	one - where active derives from passive. Even for the case of a one plane telescope, you will
 *	have one layer and one active (plane). The layer holds the alignment information for all
 *	planes in it. Planes (i.e. active & passive) can further hold corrections to this.
 *
 *	The alignment of a layer is stored in two objects: the rotation matrix R and a position
 *	vector V. A local coordinate x is transformed into a global X one by:
 *		
 *			X = V + R*x
 *
 *	If additional alignment constants are stored for a plane, denoted by V' and R' the 
 *	transformation is as follows:
 *
 *			X = (V+V') + (R'*R)x
 *
 *	Notice that the individual angles are NOT simply added, but the rotation are carried 
 *	out subsequently! 
 */
...
class EUTelMaterial {
  public:
	EUTelMaterial(double A, double Z, double rho): _A(A), _Z(Z), _density(rho) {}
...
class EUTelLayer {
  ...
  public:
	...
	gear::TrackerPlanesLayerImpl* getGearPtr() const {
		return _gearLayerPtr;
	}
	...
	void addActivePlane(Par&&... par) {
		_activeVec.push_back(std::forward<Par>(par)...);
	}

	template <typename ...Par>
	void addPassivePlane(Par&&... par) {
	...
	std::vector<std::unique_ptr<EUTelPassive>> const & getPassivePlanes() const {
		return _passiveVec;
	}
	...
	void setPosition(Eigen::Vector3d const & posVec) {
		_posVector = posVec;
	}
	...
	Eigen::Vector3d const & getPosVec() const {
		return _posVector;
	}

	Eigen::Vector3d const & getPosUncVec() const {
		return _posUncVector;
	}
	Eigen::Matrix3d const & getRotMat() const {
		return _rotMatrix;
	}

	Eigen::Vector3d const & getAngleVec() const {
		return _angleVector;
	}
};
...
class EUTelPassive {
  ...
  public:	
	...
	void setDeltaRotation(Eigen::Matrix3d const & deltaRotMat) {
		_deltaRotAnglesVector = Utility::getRotationAnglesFromMatrix( deltaRotMat );
	...
	Eigen::Vector3d const & getPosition() const {
		return _totalPosVector;
	}
 
	Eigen::Vector3d const & getOffset() const {
		return _offVector;
	}
	...
	EUTelLayer const * getParent() const {
		return _parentLayer;
	}
...
class EUTelActive: public EUTelPassive {
  ...
  public:
	...
	void setDeltaRotationUnc(double alphaUnc, double betaUnc, double gammaUnc) {
	...
	void setGeometry(std::string const & g) {
		_geometry = g;
	}

	Eigen::Matrix2i const & getFlipMatrix() const {
		return _flipMatrix;
	}
