/*
  ...
  server hosting CTest results. See
  http://public.kitware.com/Dart/HTML/Tests.shtml

  The variable can either be of type integer, double or string.
  If the string ends in ".png" it will be considered to be the path to a png
  image
  which would be uploaded by CTest. Other formats are known to CDash but not yet
  implemented here.

  All measurements will be accessible on the CDash website for the respective
  test.
  ...
  type="image/png">/home/test/Testing/Temporary/TestTexturedSphere.png</DartMeasurementFile>
...
public:
  CDashMeasurement(std::string name, int value)
      : mymeasurement(), mtype(), mname(), isImage() {
    mname << name;
    setInteger(value);
  }
  CDashMeasurement(std::string name, double value)
      : mymeasurement(), mtype(), mname(), isImage() {
    mname << name;
    setDouble(value);
  }
  CDashMeasurement(std::string name, std::string value)
      : mymeasurement(), mtype(), mname(), isImage() {
    mname << name;
    setString(value);
  }
...
// only show output when precompiler flag is set
  ...
  friend std::ostream &operator<<(std::ostream &os,
    ...
    if (cdm.isImage)
      os << "File";
    os << " name=\"" << cdm.mname.str() << "\" type=\"" << cdm.mtype.str()
       << "\">" << cdm.mymeasurement.str() << "</DartMeasurement";
    if (cdm.isImage)
      os << "File";
    os << ">" << std::endl;
