...
/** @class EUTelGenericPixGeoDescr
      ...
      * used in the pixel geometry framework. It contains three essential
 * virtual
      * methods which have to be implemented by the user information.
 * Additionally
      * it stores information on the pixel count as well as the dimensions of
 * the
...
namespace eutelescope {
  namespace geo {
    ...
    * @param are the dimensions of the sensor (size) as well as the minimum and
    * maximum pixel count (min&max) as well as the radiation length
            * Since every sensor has this information, this constructor has to
    * be called whenever creating an instance of a derived class
            */
      EUTelGenericPixGeoDescr(double sizeX, double sizeY, double sizeZ,
      ...
      /** Takes references to four ints and returns the max and min pixel index
       * in X and Y */
